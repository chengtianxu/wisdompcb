...
var
  AFreeAfterDone: Boolean;
begin
  AFreeAfterDone := FreeAfterDone;
  try
...
begin
  ...
  FAction := reqGet;
  FFreeAfterDone := not AIsSyncMode;
  if AIsSyncMode then
...
begin
  ...
  try
    ...
    begin
      ...
      SaveTextU(AReq.RequestStream, AParams,False);
...
begin
  ...
  try
    ...
    begin
      ...
      SaveTextU(AReq.RequestStream, AContent.Text,False);
      AReq.RequestStream.Position := 0;
    end;
    if Assigned(AReplyStream) then
      AReq.ResponseStream := AReplyStream;
...
begin
  ...
  try
    ...
    begin
      ...
      AReq.RequestStream := TMemoryStream.Create;
      AReq.RequestHeaders.Values['Content-Type'] := AContentType;
      SaveTextU(AReq.RequestStream, AContent,False);
      AReq.RequestStream.Position := 0;
    end;
    Push(AReq);
    AReq.WaitFor(INFINITE);
