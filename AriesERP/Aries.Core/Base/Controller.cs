...
namespace Aries.Core
{
    ...
    public abstract partial class Controller : IHttpHandler, IRequiresSessionState, IBase
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                ...
                {
                    context.Response.AddHeader("P3P", "CP=CAO PSA OUR");//跨域时,解除IE对写Cookie的阻止
                }
        ...
        {
            MDataTable dt;
            using (MAction action = new MAction(CrossObjName, CrossDb.GetConn(ObjName)))
            {
        ...
        public virtual void Add()
        {
            ...
            using (MAction action = new MAction(CrossTableName, CrossDb.GetConn(ObjName)))
            {
        ...
        public void GetCombobox()
        {
            ...
            if (!string.IsNullOrEmpty(itemJson))
            {
                ...
                if (jsonResult.IndexOf("Uninitialized") >= 0 && json.ToString().Length>1)
                {
    ...
    public abstract partial class Controller
    {
        ...
        public string Cur_Conn
        {
            get
            {
                string conn = "Conn";
                HttpCookie cookie = HttpContext.Current.Request.Cookies["aries_company"];
                if (cookie != null)
                {
                    conn = cookie.Value + conn;
                }
                return conn;
            }
        }
        ...
        {
            using (MAction action = new MAction("SELECT GETDATE() AS serverDate"))
            {
                MDataTable dt;
                dt = action.Select();
                return Convert.ToDateTime(dt.Rows[0][0].Value).ToString("yyyy-MM-dd HH:mm:ss.fff");
            }
        }
        /// <summary>
        /// 一般存储过程(可带参数)
        ...
        public void Exec_SP()
        {
            ...
            NameValueCollection param = HttpContext.Current.Request.Form;
            using (MProc proc = new MProc(ObjName, conn))
            {
                for (int i = 0; i < param.Count - 1; i++)
                {
                    proc.Set("@" + param.GetKey(i), param[i]);
                }
                ...
                if (proc.DebugInfo != "")
                {
                    returnInfo = proc.DebugInfo.Substring(proc.DebugInfo.IndexOf(":") + 1);
                    if (returnInfo.IndexOf("<br />") > 0)
                    {
                        returnInfo = returnInfo.Substring(0, returnInfo.IndexOf("<br />"));
                    }
                }
            }
            jsonResult = dt.ToJson();
            jsonResult = jsonResult.Insert(jsonResult.IndexOf("errorMsg") + 11, returnInfo.Replace("\"","'"));
        }
        /// <summary>
        /// 一般明细查询调用存储过程
        /// </summary>
        ...
        /// <returns></returns>
        protected MDataTable SelectList_SP(string spName, string conn)
        {
            MDataTable dt;
            string WhereStr = GetWhere();
            string OrderBy = Query<string>("sort") + " " + Query<string>("Order");
            string SelCompany = Query<string>("SelCompany");
            ...
            using (MProc proc = new MProc(spName, conn))
            {
                ...
                if (!string.IsNullOrEmpty(SelCompany)) {
                    proc.Set("@SelCompany", SelCompany);
                    proc.Set("@TableView", ObjName);
                }
                proc.SetCustom("ReturnValue", ParaType.ReturnValue);//返回值
                dt = proc.ExeMDataTable();
                if (proc.ReturnValue > 0)
        ...
        protected MDataTable Select_SP(string spName, string conn)
        {
            ...
            using (MProc proc = new MProc(spName, conn))
            {
                ...
                proc.Set("@PageSize", PageSize);
                proc.Set("@WhereStr", WhereStr);
                if (!string.IsNullOrEmpty(ShowGroup)){ proc.Set("@ShowGroup", ShowGroup); }
                if (!string.IsNullOrEmpty(ShowStr)){ proc.Set("@ShowStr", ShowStr); }
                if (!string.IsNullOrEmpty(GroupBy)){ proc.Set("@GroupBy", GroupBy); }
                if (!string.IsNullOrEmpty(OrderBy)){ proc.Set("@OrderBy", OrderBy); }
                if (!string.IsNullOrEmpty(SelCompany)) { proc.Set("@SelCompany", SelCompany); }
                proc.SetCustom("ReturnValue", ParaType.ReturnValue);//返回值
                dt = proc.ExeMDataTable();
                if (proc.ReturnValue > 0)
                {
        ...
        public string UploadFile()
        {
            try
            {
                ...
                if (file != null && file.ContentLength > 0)
                {
                    ...
                    string folder = Context.Request.PhysicalApplicationPath + "Upload\\";
                    ...
                    string whereStr = "";
                    NameValueCollection param = HttpContext.Current.Request.Form;
                    using (MAction action = new MAction(TableName, CrossDb.GetConn(TableName)))
                    {
                        action.BeginTransation();
                        for (int i = 0; i < param.Count; i++)
                        {
                            if (param.GetKey(i).ToLower() == "savefield")
                            {
                                action.Set(param[i], file.FileName);
                            }
                            else
                            {
                                whereStr += " and" + param.GetKey(i) + "='" + param[i] + "'";
                            }
                        }
                        if (whereStr != "") { whereStr = whereStr.Substring(4); }
                        result = action.Update(whereStr, false);
                        if (!result)
                        {
                            action.RollBack();
                        }
        ...
        /// </summary>
        /// <returns></returns>
        public virtual string UploadFtpFile()
        {
            try
            {
                HttpPostedFile file = Context.Request.Files[0];
                ...
                {
                    //取FTP连接信息
                    string Rkey = "", FtpIP = "", FtpUserName = "", FtpPassWord = "", FDir = "", user_ptr = "";
                    string connTable = "tier3_ftplist";
                    string connWhere = string.Format("FtpID ={0} order by rkey desc", Query<string>("para_FtpID"));
                    string saveFtpTable = "Tier3_CustomerFiles";
                    using (MAction action = new MAction(connTable, CrossDb.GetConn(connTable)))
                    {
                        if (action.Fill(connWhere))
                        {
                            Rkey = "" + action.Data["Rkey"];
                            FtpIP = "" + action.Data["FtpIP"];
                            FtpUserName = "" + action.Data["FtpUserName"];
                            FtpPassWord = "" + action.Data["FtpPassWord"];
                            FDir = "" + action.Data["FtpDir"] + action.Data["SubDir_1"] + action.Data["SubDir_2"];
                        }
                        object ChildTable = "data0073";
                    ...
                    //上传到FTP服务器
                    string reValue=FtpHelper.Ftp_Upload(file, FtpIP, FDir, FtpUserName, FtpPassWord);
                    if (reValue != "")
                    {
                        //保存相关信息到数据库
                        bool result = false;
                        ...
                        using (MAction action = new MAction(TableName, CrossDb.GetConn(TableName)))
                        {
                            ...
                            for (int i = 0; i < param.Count; i++)
                            {
                                if (param.GetKey(i).ToLower() == "ftpfilefield")
                                {
                                    //action.Set(param[i], file.FileName);
                                    action.Set(param[i], reValue);
                                }
                                else if (param.GetKey(i).Substring(0, 5).ToLower() != "para_")
                                {
                                    action.Set(param.GetKey(i), param[i]);
                                }
                            }
                            result = action.Insert();
                            if (result && !string.IsNullOrEmpty(saveFtpTable)) 
                            {
                                object ChildTable = saveFtpTable;
                                action.ResetTable(ChildTable);
                                action.Data.Clear();
                                action.Set("IDKey", reValue);
                                action.Set("FileName", file.FileName);
                                action.Set("Ftp_Ptr", Rkey);
                                action.Set("Ftp_FileName", reValue);
                                action.Set("UPLoadTime", "getdate()");
                                action.Set("UPloadUser_d05ptr", user_ptr);
                                action.Set("GroupIDKey", reValue);
                                action.Set("BEnable", "1");
        ...
        public void DownloadFtpFile() 
        {
            ...
            {
                //取FTP连接信息
                string FileName = "", Ftp_FileName = "", FtpIP = "", FtpUserName = "", FtpPassWord = "", FDir = "";
                string connTable = "tier3_ftplist";
                string saveFtpTable = "Tier3_CustomerFiles";
                using (MAction action = new MAction(saveFtpTable, CrossDb.GetConn(saveFtpTable)))
                {
                    string Ftp_Ptr = "";
                    string where=string.Format("IDKey='{0}'",Query<string>("FtpIDKey"));
                    if (action.Fill(where))
                    {
                        Ftp_Ptr = "" + action.Data["Ftp_Ptr"];
                        Ftp_FileName = "" + action.Data["Ftp_fileName"];
                        FileName = "" + action.Data["FileName"];
                    }
                    object ChildTable = connTable;
                    action.ResetTable(ChildTable);
                    action.Data.Clear();
                    where = string.Format("Rkey ={0}", Ftp_Ptr);
                    if (action.Fill(where))
                    {
                        FtpIP = "" + action.Data["FtpIP"];
                        FtpUserName = "" + action.Data["FtpUserName"];
                ...
                jsonResult = null;
                MemoryStream ms = FtpHelper.Ftp_Download(Ftp_FileName, FtpIP, FDir, FtpUserName, FtpPassWord);
                if (ms.Capacity > 0)
                {
                    WebHelper.SendFile(FileName, ms);
                }
            ...
            try
            {
                ...
                if (file != null && file.ContentLength > 0)
                {
                    ...
                    using (MAction action = new MAction(TableName, CrossDb.GetConn(TableName)))
                    {
                        ...
                        for (int i = 0; i < param.Count; i++)
                        {
                            if (param.GetKey(i).ToLower() == "filenamefield")
                            {
                                if (!string.IsNullOrEmpty(param[i])) { action.Set(param[i], file.FileName); }
                            }
                            else if (param.GetKey(i).ToLower() == "filedocfield")
                            {
                                action.Set(param[i], bytes);
                            }
                            else
                            {
                                whereStr += " and" + param.GetKey(i) + "='" + param[i] + "'";
        ...
        public void DownloadDBFile()
        {
            ...
            try
            {
                using (MAction action = new MAction(TableName, CrossDb.GetConn(TableName)))
                {
                    result = action.Fill(GetWhere());
                    if (result)
                    {
                        byte[] bytes = null;
                        string fileDocField = Query<string>("fileDocField");
                        if (!string.IsNullOrEmpty(fileDocField)) {
                            bytes = action.Get<byte[]>(fileDocField);
                            ms = new MemoryStream(bytes, 0, bytes.Length, true, true);
                        }
                        string fileNameField = Query<string>("fileNameField");
                        if (!string.IsNullOrEmpty(fileNameField))
                        {
                            fileName = action.Get<string>(fileNameField);
                            if (fileName.Substring(0, 1) == ".") {
                                fileName = action.Get<string>(action.Data.PrimaryCell.ColumnName) + fileName;
                            }
                        }
                        else {
                            fileName = action.Get<string>(action.Data.PrimaryCell.ColumnName) + Query<string>("fileExtension");
                        }
                        jsonResult = null;
                        WebHelper.SendFile(fileName, ms);
                    }
