...
namespace Aries.Core.Extend
{
    ...
    public static partial class CrossDb
    {
        ...
        {
            string reName = "";
            string defconn = AppConfig.GetApp("DefConn");
            foreach (KeyValuePair<string, Dictionary<string, string>> item in DbTables)
            {
                if (item.Key == defconn && item.Value.ContainsKey(tableName))
                {
                    reName = item.Key;
                    break;
                }
            ...
            if (reName == "")
            {
                if (tableName.ToUpper().IndexOf("_SYS_") > 0)
                {
                    reName = defconn;
                }
                else {
                    ...
                    if (cookie != null)
                    {
                        string connstr=AppConfig.GetConn(cookie.Value + "Conn");
                        connstr=connstr.Substring(connstr.IndexOf("Database=")+9);
                        reName=connstr.Substring(0,connstr.IndexOf(";"));
                    }
                    
                }
            }
            ////找不到时，可能是视图，根据数据库类型匹配第一个可能的数据库
            ////框架逻辑不适用具体业务,修改为取最后一个 20180228 by yjy
            //string reDBname = "";
            //string defconn = AppConfig.GetApp("DefConn");
        ...
        public static string GetConn(string sqlOrTableName)
        {
            ...
            //以上原得到连接的方式不适用20180408 by yjy
            string tableName = GetTableNameFromObjName(sqlOrTableName);
            string dbName = GetDBName(tableName);
            string conn = "Conn";
            string defconn = AppConfig.GetApp("DefConn");
            if (dbName == defconn || tableName.ToUpper().IndexOf("_SYS_") > 0)
            {
        ...
        {
            //获取原始表名
            //做了部分优化 20180310 by yjy
            //string[] items = sql.Replace("\r\n"," ").Replace("\t"," ").Split(' ');
            string pattern = @"\s*\r?\n\s*|\t";
            Regex rgx = new Regex(pattern);
            string[] items = rgx.Replace(sql, " ").Split(' ');
            ...
            if (items.Length > 3) // 总是包含空格的select * from xxx
            {
                ...
                foreach (string item in items)
                {
                    if (!string.IsNullOrEmpty(item))
                    {
                        ...
                        {
                            if (item[0] == '(' || (item.IndexOf('.') > -1 && item.ToLower().IndexOf("dbo.") == -1)) { startFrom = false; }
                            else
                            {
                                return item.ToLower().Replace("dbo.","");
                            }
                        }
                    }
                }
