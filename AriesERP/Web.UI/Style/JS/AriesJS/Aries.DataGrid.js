(function ($Core) {
    ...
    //$Core.Lang.addSuccess = parent.AR.Lang.addSuccess;

    //$Core.Lang.isDel = parent.AR.Lang.isDel;
    //$Core.Lang.delSuccess = parent.AR.Lang.delSuccess;
    //$Core.Lang.selectDelData = parent.AR.Lang.selectDelData;

    //$Core.Lang.delError = parent.AR.Lang.delError;
    //$Core.Lang.errorInfo = parent.AR.Lang.errorInfo;
    //$Core.Lang.msg = parent.AR.Lang.msg;
    //$Core.Lang.importTip = parent.AR.Lang.importTip;
    //$Core.Lang.importError = parent.AR.Lang.importError;
    //$Core.Lang.uploadExtendName = parent.AR.Lang.uploadExtendName;

    //$Core.Lang.beforePageText = parent.AR.Lang.beforePageText;
    //$Core.Lang.afterPageText = parent.AR.Lang.afterPageText;
    //$Core.Lang.displayMsg = parent.AR.Lang.displayMsg;

    //$Core.Lang.loadMsg = parent.AR.Lang.loadMsg;
    //}
        ...
        $Core.Utility.Ajax.post("GetHeader", dg.objName + "," + dg.tableName, dg.options.queryParams,
            function (dg) {
                return function (result) {
                    ...
                    $.each(result, function (index, data) { //多语言替换
                        if (AR.Lang[dg.objName + "_" + data.field]) { data.title = AR.Lang[dg.objName + "_" + data.field]; }
                    })
    ...
    function bindGrid(dg) {
        ...
        opts.onLoadSuccess = function (dg) {
            return function (data) {
                ...
                if (data.msg && data.msg != "") { $.messager.alert("ERROR!", data.msg, "error"); return; }
                dg._onLoadSuccess && dg._onLoadSuccess(data);
            }
        }(dg);
        opts.onLoadError = function (XMLHttpRequest) {
        ...
        if (searchJson.length > 0) {
            //options.queryParams['sys_search'] = JSON.stringify(searchJson); //IE8有问题
            var str = JSON.stringify(searchJson).replace(/\'/g, "!#");
            eval("sys_search = '" + str + "'");
            options.queryParams['sys_search'] = sys_search.replace(/!#/g, "'");//JSON.stringify(searchJson);
        }
                    ...
                    if (dg.PKColumn.Editor.isSaveToBehind == false) {
                            ...
                            //else
                        {
                            for (var i = 0; i < dg.Internal.jointPrimary.length; i++) {
                                ...
                                if (!post_data[primary]) {//如果有值不再重新赋值 by yjy
