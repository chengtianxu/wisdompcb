(function ($Core) {
    ...
    //$Core.Lang.selectFirst = parent.AR.Lang.selectFirst;
...
(function ($, $Core) {
    ...
    function bindConfigKey($input) {
        ...
        if (key && $Core.Global.Config[key]) {
            ...
            $.each(op.data, function (index, data) { //多语言替换
                if (AR.Lang[key + "_" + data.value]) { data.text = AR.Lang[key + "_" + data.value]; }
            })
    ...
    function bindObjName($input) {
        ...
        if (comboxData) {
            ...
            if (comboxData[op.key]) {
                ...
                $.each(op.data, function (index, data) {//多语言替换
                    if (AR.Lang[op.key + "_" + data.value]) { data.text = AR.Lang[op.key + "_" + data.value]; }
                })
            }
            if (!op.tree && op.defaultItem && (op.data.length == 0 || op.data[0][op.textField] != defaultItem.text)) {
                op.data.unshift(defaultItem);
    ...
    //初始化配置Key的下拉框数据。
    initConfigKeyCombobox = function (type) {
        if (!$Core.Global.Variable.isLoadCompleted) {
        ...
        $("[dialog]").each(function () {
            ...
            if (!$events || !$events["click"]) {
                $(this).on("click", function () {
                    if ($Core.Combobox.onBeforeExecute("dialog",$(this)) != false) {
                ...
                handler: function () {
                    ...
                    if ($Core.Combobox.onAfterExecute("dialog", $input, options) != false) {
                        ...
                        if ($input.attr("multiple")) {
                            ...
                            $input.val(options.option.data[0].text);
                        }
                        if ($input.attr("multiple")){
    ...
    onBeforeBind = function (type, $input) { };
    ...
    function bindCombo($box, op) {
        ...
        else if (!op.tree && op.data && op.data.length > 0) { setCombo($box, "select", op.data[0][op.valueField]); }
    }
