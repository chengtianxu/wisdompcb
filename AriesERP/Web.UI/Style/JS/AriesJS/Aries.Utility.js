(function ($Core) {
    ...
    //$Core.Lang.configRulesError = parent.AR.Lang.configRulesError;
    //}
        ...
        */
        ...
        createInputHtml: function ($container, dataArray, dg, fromSearch) {
            ...
            for (var i = 0, len = dataArray.length; i < len; i++) {
                ...
                label.attr("id", dataArray[i].field + "_label");
                div_item.append(label);
                var input = $('<input id="' + dataArray[i].field + "_input" + '" type=\"text\"/>'); //加入ID, 20180305 by yjy
                var input2;//日期时的第二个框
                //如果是下拉框模式执行以下代码
                        ...
                        case "datetime":
                            ...
                            if (fromSearch && dataArray[i].rules) {
                                ...
                                if (input.attr("clone") != "false") {
                                    ...
                                    input2.attr("id", dataArray[i].field + "_input2"); //20180305 by yjy
                                    input2.attr("operator", ">=");
                                    div_item.append(input2).append($("<span>").html("&nbsp;ˉ&nbsp;"));
                                    //处理默认值
                ...
                if (dg && dg.Search) {
                    if (input2) {
                        ...
                        dg.Search.Items.set(dataArray[i].field+"_sinput2", input2);
                    }
                    else {
                        dg.Search.Items.set(dataArray[i].field, input);
                    }
                ...
                for (var name in _rules) {
                    ...
                    switch (name) {
                        ...
                        case "defaultValueF":
                            ...
                            $input.removeAttr("defaultValueF");
                            break;
                        default:
            ...
            $.ajax({
                ...
                error: function (d) {
                    try {
                        ...
                        //$.messager.alert("ERROR!","eval json error : " + d.responseText,"error");
                    }
                },
                beforeSend: function () {
                    if (isShowProgress) {
