(function ($Core) {
    ...
    //$Core.Lang.select = parent.AR.Lang.select;
    //$Core.Lang.config = parent.AR.Lang.config;
    //$Core.Lang.add = parent.AR.Lang.add;
    //$Core.Lang.addSameLevel = parent.AR.Lang.addSameLevel;
    //$Core.Lang.addChild = parent.AR.Lang.addChild;
    //$Core.Lang.cancel = parent.AR.Lang.cancel;
    //$Core.Lang.save = parent.AR.Lang.save;
    //$Core.Lang.edit = parent.AR.Lang.edit;
    //$Core.Lang.del = parent.AR.Lang.del;
    //$Core.Lang.batchDel = parent.AR.Lang.batchDel;
    //$Core.Lang.opration = parent.AR.Lang.opration;
    //$Core.Lang.empty = parent.AR.Lang.empty;
    //$Core.Lang.save = parent.AR.Lang.save;
    //$Core.Lang.exportTemplate = parent.AR.Lang.exportTemplate;
    //$Core.Lang.btexport = parent.AR.Lang.btexport;
    //$Core.Lang.btimport = parent.AR.Lang.btimport;
    //$Core.Lang.btquery = parent.AR.Lang.btquery;
    //$Core.Lang.btreset = parent.AR.Lang.btreset;
    //}
...
(function ($, $Core) {
                    ...
                    function Obj() {
                        ...
                        this.onExecute = function (dg, btn_reset) {
                            ...
                            if (this.onBeforeExecute($form) == false) {
                                ...
                                //input，要清
                                $form.find("input[type!=button][type!='reset'][type!='radio']").each(function () {
                                    $(this).val("");
                                });
                                //下拉框的值有缓存，要清。
                        ...
                        */
                        ...
                        this.onExecute = function (dg) {
                            ...
                            if (checked_ids.length > 0) {
                                ...
                                //jsonString = JSON.stringify(searchJson); //IE8对于中文字符转换有问题 20180307 by yjy
                                var str = JSON.stringify(searchJson).replace(/\'/g, "!#");
                                eval("str = '" + str + "'");
    ...
    function _createSearchAreaHtml(dg) {
        ...
        if (dataArray.length > 0 || dg.Search.$target) {
            ...
            //dg.Search.BtnQuery.$target = $('<input class="query" style="font-size:14px;color:#fff" value="' + $Core.Lang.btquery + '" type="button" />');
            //dg.Search.BtnReset.$target = $('<input class="reset" style="font-size:14px;color:#fff" type="button" value="' + $Core.Lang.btreset + '" />');
            dg.Search.BtnQuery.$target = $('<a href="#"><span class=\"btn-query\">' + $Core.Lang.btquery + '</span></a>');
            dg.Search.BtnReset.$target = $('<a href="#"><span class=\"btn-resert\">' + $Core.Lang.btreset + '</span></a>');;
    ...
    function _createToolBarHtml(dg) {
        ...
        if (actionKeys.indexOf(',add,') > -1 && !dg.ToolBar.BtnAdd.isHidden) {
            //dg.ToolBar.BtnAdd.$target = $('<input class=\"add\" flag=\"btn_add\" type=\"button\" name=\"' + $Core.Lang.add + '\" value=\"' + $Core.Lang.add + '\"/>');
            dg.ToolBar.BtnAdd.$target = $('<a href="#" flag=\"btn_add\" ><span class=\"btn-add\">' + $Core.Lang.add + '</span></a>');
        ...
        if (actionKeys.indexOf(',del,') > -1 && !dg.ToolBar.BtnDelBatch.isHidden && dg.isShowCheckBox) {
            //dg.ToolBar.BtnDelBatch.$target = $('<input  class=\"batch_del\" flag=\"btn_del\" type=\"button\" name=\"' + $Core.Lang.batchDel + '\" value=\"' + $Core.Lang.del + '\"/>').attr("dgID", dg.id);
            dg.ToolBar.BtnDelBatch.$target = $('<a href="#" flag=\"btn_del\" ><span class=\"btn-del\">' + $Core.Lang.del + '</span></a>').attr("dgID", dg.id);
        ...
        if (actionKeys.indexOf(',export,') > -1 && !dg.ToolBar.BtnExport.isHidden) {
            //dg.ToolBar.BtnExport.$target = $('<input class=\"export\" flag=\"btn_export\" type=\"button\"  value=\"' + $Core.Lang.btexport + '\"/>');
            dg.ToolBar.BtnExport.$target = $('<a href="#" flag=\"btn_export\" ><span class=\"btn-export\">' + $Core.Lang.btexport + '</span></a>');
        ...
        if (actionKeys.indexOf(',import,') > -1) {
            if (!dg.ToolBar.BtnImport.isHidden) {
                //dg.ToolBar.BtnImport.$target = $('<input class=\"import\" flag=\"btn_import\" type=\"button\"  value=\"' + $Core.Lang.btimport + '\"/>');
                dg.ToolBar.BtnImport.$target = $('<a href="#" flag=\"btn_import\" ><span class=\"btn-import\">' + $Core.Lang.btimport + '</span></a>');
            ...
            if (!dg.ToolBar.BtnExportTemplate.isHidden) {
                ...
                dg.ToolBar.BtnExportTemplate.$target = $('<a href="#" flag=\"btn_export_template\"><span class=\"btn-sm\">' + $Core.Lang.exportTemplate + '</span></a>');
        ...
        for (var i = 0, len = btnArray.length; i < len; i++) {
            ...
            if (!lv2action || actionKeys.indexOf(',' + lv2action + ",") > -1) {
                ...
                if (btn.html) {
                    ...
                    item = $Core.Utility.stringFormat('<a href="#"><span class=\"{0}\" onClick=\"{1}(event)\"  value=\"\">{2}</span></a>', btnClass, btnClick, title);
                }
                item = $(item);
