object Form4: TForm4
  Left = 483
  Top = 204
  BorderIcons = [biSystemMenu, biMinimize]
  BorderStyle = bsSingle
  Caption = #26448#26009#32452#21035#25628#32034
  ClientHeight = 435
  ClientWidth = 400
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object Label2: TLabel
    Left = 54
    Top = 14
    Width = 63
    Height = 13
    Alignment = taRightJustify
    Caption = '    '#32452#21035#20195#30721':'
  end
  object Edit1: TEdit
    Left = 124
    Top = 10
    Width = 191
    Height = 21
    TabOrder = 0
    OnChange = Edit1Change
    OnKeyDown = DBGrid1KeyDown
  end
  object BitBtn1: TBitBtn
    Left = 324
    Top = 8
    Width = 25
    Height = 25
    Hint = #21047#26032
    ParentShowHint = False
    ShowHint = True
    TabOrder = 1
    TabStop = False
    OnClick = BitBtn1Click
    Glyph.Data = {
      
      440AADADADA000A04440DADADADADADA0440ADADADADADADA00D}
  end
  object DBGrid1: TDBGrid
    Left = 0
    Top = 42
    Width = 401
    Height = 342
    DataSource = DataSource1
    Options = [dgTitles, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit]
    TabOrder = 2
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    OnDblClick = DBGrid1DblClick
    OnKeyDown = DBGrid1KeyDown
    Columns = <
      item
        Expanded = False
        FieldName = 'inv_group_name'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'inv_group_desc'
        Width = 254
        Visible = True
      end>
  end
  object Button1: TButton
    Left = 120
    Top = 399
    Width = 75
    Height = 25
    Caption = #30830#23450
    ModalResult = 1
    TabOrder = 3
  end
  object Button2: TButton
    Left = 200
    Top = 399
    Width = 75
    Height = 25
    Caption = #21462#28040
    ModalResult = 2
    TabOrder = 4
  end
  object DataSource1: TDataSource
    DataSet = aq0019
    Left = 320
    Top = 397
  end
  object aq0019: TADOStoredProc
    Connection = DM.ADOConnection1
    CursorType = ctStatic
    Filtered = True
    LockType = ltReadOnly
    ProcedureName = 'ep179;2'
    Parameters = <
      item
        Name = '@RETURN_VALUE'
        DataType = ftInteger
        Direction = pdReturnValue
        Precision = 10
        Value = 0
      end>
    Left = 352
    Top = 397
    object aq0019rkey: TIntegerField
      FieldName = 'rkey'
      ReadOnly = True
    end
    object aq0019inv_group_name: TStringField
      DisplayLabel = #32452#21035#20195#30721
      FieldName = 'inv_group_name'
    end
    object aq0019inv_group_desc: TStringField
      DisplayLabel = #32452#21035#25551#36848
      FieldName = 'inv_group_desc'
      Size = 40
    end
  end
end
