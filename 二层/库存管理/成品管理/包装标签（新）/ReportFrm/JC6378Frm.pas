...
procedure Tfrm6378.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    Key := #0;
...
procedure Tfrm6378.btnBCBHClick(Sender: TObject);
var
...
procedure Tfrm6378.btnXSDDClick(Sender: TObject);
var
  LFrm: Tsearchso_Form;
  LSql: string;
...
function Tfrm6378.GetBCBHInfo(AManu_Part_Number: string): Boolean;
var
  lsql: string;
begin
...
procedure Tfrm6378.edtBCBHExit(Sender: TObject);
begin
  if edtBCBH.Text <> '' then
  begin
...
function Tfrm6378.GetXSDDInfo(ASOno: string;b: Boolean = False): Boolean;
begin
  Result := False;
//  if b then
//  begin
...
procedure Tfrm6378.edtXSDDExit(Sender: TObject);
begin
  if edtXSDD.Text <> '' then
    GetXSDDInfo(edtXSDD.Text, True);
end;

function Tfrm6378.GetPCSNumber: Boolean;
begin
  Result := True;
  try
...
procedure Tfrm6378.edtSLSetExit(Sender: TObject);
begin
  GetPCSNumber;
end;

procedure Tfrm6378.btnSCClick(Sender: TObject);
var
  I: Integer;
  LBarID: string;
  tmpQSXH: string;
...
function Tfrm6378.CommonCheck: Boolean;
begin
  Result := False;
...
procedure Tfrm6378.btnResetClick(Sender: TObject);
var
  I: Integer;
begin
...
function Tfrm6378.ExtractBarID(PackNo: Integer;
  out outBarID: string): Boolean;
begin
  outBarID := '';
  outBarID := edtKHXH.Text + ';' + ads699.FieldByName('lotno').AsString + ';' + edtZXGG.Text + ';' +  edtKGDM.Text + ';' +
  edtSLPCS.Text + ';' + Format('%.4d',[ads699.RecordCount+1]);
  case rgLX.ItemIndex of
    0://内箱
    begin
...
procedure Tfrm6378.intgrfldads699qtyChange(Sender: TField);
begin
...
function Tfrm6378.CheckSCZQ: Boolean;
begin
  Result := False;
...
//    Exit;
//  end;
  if Length(Format('%.2d',[seMoth.Value]) + Format('%.2d',[seYear.Value])) <> 4 then
  begin
    ShowMessage('生产周期不正确，请重新输入4位有效数字');
    Exit;
  end;
...
procedure Tfrm6378.btnOKClick(Sender: TObject);
var
  LSql: string;
  LPcsCount: Integer;
begin
...

{
    ...
    QuotedStr(Format('%.2d',[seMoth.Value]) + Format('%.2d',[seYear.Value])) + ',getdate(),' + gUser.User_Ptr + ',' + edtBQSL.Text + ',' +  QuotedStr(rgLX.Items[rgLX.ItemIndex]) + ',' +
...
procedure Tfrm6378.ViewShow(Rkey698: Integer);
var
  I: Integer;
begin
  btnResetClick(nil);
...
procedure Tfrm6378.btnCloseClick(Sender: TObject);
begin
  ModalResult := mrCancel;
end;

function Tfrm6378.IncBQDM(AOldDM: string): string;
var
  I: Integer;
  nIndex: Integer;
  tmps: string;
...
procedure Tfrm6378.eh699Columns2UpdateData(Sender: TObject;
...
begin
  ...
  begin
    ...
    if n < 4 then NewBar := NewBar + tmpS[I] else Break;
  end;
  NewBar := NewBar + ';' + VarToStr(Value);
  n := 0;
  for I := 1 to Length(tmpS) do
  begin
    if tmpS[I] = ';' then Inc(n);
    if n >= 5 then NewBar := NewBar + tmpS[I];
  end;

  ads699.FieldByName('barcode_id').AsString := NewBar;
end;
