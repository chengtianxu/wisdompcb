...
procedure TfrmDaHua2390.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    Key := #0;
...
procedure TfrmDaHua2390.btnBCBHClick(Sender: TObject);
var
...
procedure TfrmDaHua2390.btnXSDDClick(Sender: TObject);
var
  LFrm: Tsearchso_Form;
  LSql: string;
...
function TfrmDaHua2390.GetBCBHInfo(AManu_Part_Number: string): Boolean;
var
  LSql: string;
begin
...
procedure TfrmDaHua2390.edtBCBHExit(Sender: TObject);
begin
  if edtBCBH.Text <> '' then
  begin
...
function TfrmDaHua2390.GetXSDDInfo(ASOno: string;b: Boolean = False): Boolean;
begin
  Result := False;
//  if b then
//  begin
...
procedure TfrmDaHua2390.edtXSDDExit(Sender: TObject);
begin
  if edtXSDD.Text <> '' then
    GetXSDDInfo(edtXSDD.Text, True);
end;

function TfrmDaHua2390.GetPCSNumber: Boolean;
begin
  Result := True;
  try
...
procedure TfrmDaHua2390.edtSLSetExit(Sender: TObject);
begin
  GetPCSNumber;
end;

procedure TfrmDaHua2390.btnSCClick(Sender: TObject);
var
  I: Integer;
  LBarID: string;
  tmpQSXH: string;
...
function TfrmDaHua2390.CommonCheck: Boolean;
begin
  Result := False;
...
procedure TfrmDaHua2390.btnResetClick(Sender: TObject);
var
  I: Integer;
begin
...
function TfrmDaHua2390.ExtractBarID(PackNo: Integer;
...
procedure TfrmDaHua2390.intgrfldads699qtyChange(Sender: TField);
begin
...
function TfrmDaHua2390.CheckSCZQ: Boolean;
begin
  Result := False;
...
procedure TfrmDaHua2390.btnOKClick(Sender: TObject);
var
  LSql: string;
  LPcsCount: Integer;
begin
...
procedure TfrmDaHua2390.ViewShow(Rkey698: Integer);
var
  I: Integer;
begin
  btnResetClick(nil);
...
procedure TfrmDaHua2390.btnCloseClick(Sender: TObject);
begin
  ModalResult := mrCancel;
end;

function TfrmDaHua2390.IncBQDM(AOldDM: string): string;
var
  I: Integer;
  nIndex: Integer;
  tmps: string;
...
procedure TfrmDaHua2390.eh699Columns2UpdateData(Sender: TObject;
...
procedure TfrmDaHua2390.FormShow(Sender: TObject);
begin
  DM1.SqlOpen('select SEED_VALUE,seed_flag,table_name,datename(week,getdate()) as Nowweek, datename(year,getdate()) as NowYear  from data0004  where rkey = 74');
  edtCPMS.Text:='B'+Format('%.2d',[dm1.qrytmp.FieldByName('Nowweek').AsInteger])+copy(dm1.qrytmp.FieldByName('NowYear').AsString,3,2) + '00001.01504X';
end;
