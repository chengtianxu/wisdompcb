...
type
  TfrmDell7364 = class(TForm)
    ...
    lbl25: TLabel;
    lbl27: TLabel;
    strngfldads699param1: TStringField;
    lblSaleCustName: TLabel;
    lbl123: TLabel;
    strngfldads699Param2: TStringField;
    intgrfldads699wo_ptr: TIntegerField;
    intgrfldads699outpackkey: TIntegerField;
    intgrfldads699IncNO: TIntegerField;
...
procedure TfrmDell7364.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    Key := #0;
...
procedure TfrmDell7364.btnBCBHClick(Sender: TObject);
var
...
procedure TfrmDell7364.btnXSDDClick(Sender: TObject);
var
  LFrm: Tsearchso_Form;
  LSql: string;
...
function TfrmDell7364.GetBCBHInfo(AManu_Part_Number: string): Boolean;
var
  lsql: string;
begin
...
procedure TfrmDell7364.edtBCBHExit(Sender: TObject);
begin
  if edtBCBH.Text <> '' then
  begin
...
function TfrmDell7364.GetXSDDInfo(ASOno: string;b: Boolean = False): Boolean;
begin
  Result := False;
//  if b then
//  begin
...
procedure TfrmDell7364.edtXSDDExit(Sender: TObject);
begin
  if edtXSDD.Text <> '' then
    GetXSDDInfo(edtXSDD.Text, True);
end;

function TfrmDell7364.GetPCSNumber: Boolean;
begin
  Result := True;
  try
...
procedure TfrmDell7364.edtSLSetExit(Sender: TObject);
begin
  GetPCSNumber;
end;

procedure TfrmDell7364.btnSCClick(Sender: TObject);
var
  I: Integer;
  LBarID: string;
  tmpQSXH: string;
...
//  end;
  ...
  begin
    ...
    ads699.FieldByName('lotno').Value := edtZQ.Text;//Format('%.2d',[seMoth.Value]) + Format('%.2d',[seYear.Value]);
    ...
    case rgLX.ItemIndex of
      ...
      begin
        ...
        ads699.FieldByName('param1').AsString := edtLSH.Text + Format('%.8d',[StrToInt(copy(edtQSXH.Text,6,2)) + I]);
      end;
      1://外箱
      begin
        ads699.FieldByName('CaseNo').Value := Format('%.4d',[StrToInt(edtQSXH.Text)+I]);
        tmpQSXH := Format('%.4d',[StrToInt(copy(edtQSXH.Text,1,4))+I + 1]);
        ads699.FieldByName('param1').AsString := edtLSH.Text + Format('%.8d',[ads699.FieldByName('CaseNo').AsInteger]);
      end;
    end;
...
function TfrmDell7364.CommonCheck: Boolean;
begin
  Result := False;
...
procedure TfrmDell7364.btnResetClick(Sender: TObject);
var
  I: Integer;
begin
  ...
  begin
    ...
    begin
      if (Self.Components[I] AS TEdit).Name <> 'edtLSH' then
      begin
  ...
  DM1.SqlOpen('select SEED_VALUE,seed_flag,table_name,datename(week,getdate()) as Nowweek, datename(year,getdate()) as NowYear , datename(day,getdate()) as NowDay, datename(month,getdate()) as NowMonth from data0004  where rkey = 74');
  if DM1.qrytmp.IsEmpty then
  begin
    edtBQDM.Text := '00001';
  end else
...
//  edtZQ.Text := copy(dm1.qrytmp.FieldByName('NowYear').AsString,3,2) +
//                Format('%.2d',[dm1.qrytmp.FieldByName('NowMonth').AsInteger]);
                //Format('%.2d',[dm1.qrytmp.FieldByName('NowDay').AsInteger]);
//                dm1.qrytmp.FieldByName('NowYear').AsString +
//                Format('%.2d',[dm1.qrytmp.FieldByName('NowMonth').AsInteger]) +
//                Format('%.2d',[dm1.qrytmp.FieldByName('NowDay').AsInteger]);

end;

function TfrmDell7364.ExtractBarID(PackNo: Integer;
...
procedure TfrmDell7364.intgrfldads699qtyChange(Sender: TField);
begin
...
function TfrmDell7364.CheckSCZQ: Boolean;
begin
  Result := False;
//  if StrToIntDef(edtZQ.Text,-1) = -1 then
//  begin
//    ShowMessage('生产周期不正确，请重新输入6位有效数字');
//    Exit;
//  end;
//  if Length(edtZQ.Text) <> 6 then
//  begin
//    ShowMessage('生产周期不正确，请重新输入6位有效数字');
//    Exit;
//  end;
...
procedure TfrmDell7364.btnOKClick(Sender: TObject);
var
  LSql: string;
  LPcsCount: Integer;
  LNum: Integer;
  L698IncNum,L699IncNum: Integer;
  ltmp: Integer;
  LNowTime: TDateTime;
begin
...

{
    ...
    QuotedStr(edtZQ.Text) + ',getdate(),' + gUser.User_Ptr + ',' + edtBQSL.Text + ',' +  QuotedStr(rgLX.Items[rgLX.ItemIndex]) + ',' +
    ...
    if rgLX.ItemIndex = 0 then //内箱
    begin
      LSql := 'select max(d699.incno) as incno from data0698 d698 ' +
              ...
              ' where d698.type = ''外箱'' and d10.cust_code = ' + QuotedStr(FCustCode);
      DM1.SqlOpen(LSql);
      L698IncNum := DM1.qrytmp.fieldbyname('incno').AsInteger;

      LSql := 'select max(d699.incno) as incno from data0698 d698 ' +
              ' inner join data0699 d699 on d699.pack_ptr = d698.rkey ' +
              ' inner join data0025 d25 on d25.manu_part_number = d699.manu_part_number ' +
              ' inner join data0010 d10 on d10.ABBR_NAME = d25.ORIG_CUSTOMER ' +
              ' where d698.type = ''内箱'' and d10.cust_code = ' + QuotedStr(FCustCode);
      DM1.SqlOpen(LSql);
      ltmp := DM1.qrytmp.fieldbyname('incno').AsInteger;
    end else
    begin  //外箱

      LSql := 'update data0699 set incno = 0 from data0698 d698 ' +
              ' inner join data0699 d699 on d699.pack_ptr = d698.rkey ' +
              ' inner join data0025 d25 on d25.manu_part_number = d699.manu_part_number ' +
              ' inner join data0010 d10 on d10.ABBR_NAME = d25.ORIG_CUSTOMER ' +
              ' where d698.type = ''内箱'' and d10.cust_code = ' + QuotedStr(FCustCode);
    ...
    begin
      ltmp := ltmp + 1;
      ads699.Edit;
      ads699.FieldByName('incno').AsInteger := ltmp;
      ads699.FieldByName('pack_ptr').Value := DM1.qrytmp.FieldByName('RKey698').AsInteger;
      if rgLX.ItemIndex = 0 then
      begin
        ads699.FieldByName('barcode_id').AsString := 'VCN00224462' + FormatDateTime('YYMM',LNowTime) + Format('%.6d',[L698IncNum]) + Format('%.2d',[ltmp]) ;
      end else
      begin
        ads699.FieldByName('barcode_id').AsString := 'VCN00224462' + FormatDateTime('YYMM',LNowTime) + Format('%.6d',[ltmp]);
      end;
      ads699.Post;
      ads699.Next;
    end;
...
procedure TfrmDell7364.ViewShow(Rkey698: Integer);
var
  I: Integer;
begin
  btnResetClick(nil);
  ...
  edtZQ.Enabled := False;
  edtZQ.Text := DM1.adsFrmMain.FieldByName('lotno').AsString;
...
procedure TfrmDell7364.btnCloseClick(Sender: TObject);
begin
  ModalResult := mrCancel;
end;

function TfrmDell7364.IncBQDM(AOldDM: string): string;
var
  I: Integer;
  nIndex: Integer;
  tmps: string;
...
procedure TfrmDell7364.eh699Columns2UpdateData(Sender: TObject;
