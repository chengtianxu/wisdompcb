...
procedure TfrmDYH7100.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    Key := #0;
...
procedure TfrmDYH7100.btnBCBHClick(Sender: TObject);
var
...
procedure TfrmDYH7100.btnXSDDClick(Sender: TObject);
var
  LFrm: Tsearchso_Form;
  LSql: string;
...
function TfrmDYH7100.GetBCBHInfo(AManu_Part_Number: string): Boolean;
var
  lsql: string;
begin
...
procedure TfrmDYH7100.edtBCBHExit(Sender: TObject);
begin
  if edtBCBH.Text <> '' then
  begin
...
function TfrmDYH7100.GetXSDDInfo(ASOno: string;b: Boolean = False): Boolean;
begin
  Result := False;
//  if b then
//  begin
...
procedure TfrmDYH7100.edtXSDDExit(Sender: TObject);
begin
  if edtXSDD.Text <> '' then
    GetXSDDInfo(edtXSDD.Text, True);
end;

function TfrmDYH7100.GetPCSNumber: Boolean;
begin
  Result := True;
  try
    edtSLPCS.Text := IntToStr(StrToInt(edtSLSet.Text) * StrToInt(lblPcsPerSet.Caption));
    edtJZ.Text := FormatFloat('0.00',StrToInt(edtSLPCS.Text)* FPcsWeight/1000);
  except
    Result := False;
  end;
end;

procedure TfrmDYH7100.edtSLSetExit(Sender: TObject);
begin
  GetPCSNumber;
end;

procedure TfrmDYH7100.btnSCClick(Sender: TObject);
var
  I: Integer;
  LBarID: string;
  tmpQSXH: string;
...
function TfrmDYH7100.CommonCheck: Boolean;
begin
  Result := False;
...
procedure TfrmDYH7100.btnResetClick(Sender: TObject);
var
  I: Integer;
begin
  ...
  edtZQ.Text := copy(dm1.qrytmp.FieldByName('NowYear').AsString,3,2) +
                dm1.qrytmp.FieldByName('Nowweek').asstring +
...
function TfrmDYH7100.ExtractBarID(PackNo: Integer;
...
procedure TfrmDYH7100.intgrfldads699qtyChange(Sender: TField);
begin
...
function TfrmDYH7100.CheckSCZQ: Boolean;
begin
  Result := True;
end;

procedure TfrmDYH7100.btnOKClick(Sender: TObject);
var
  LSql: string;
  LPcsCount: Integer;
begin
...
procedure TfrmDYH7100.ViewShow(Rkey698: Integer);
var
  I: Integer;
begin
  btnResetClick(nil);
...
procedure TfrmDYH7100.btnCloseClick(Sender: TObject);
begin
  ModalResult := mrCancel;
end;

function TfrmDYH7100.IncBQDM(AOldDM: string): string;
var
  I: Integer;
  nIndex: Integer;
  tmps: string;
...
procedure TfrmDYH7100.eh699Columns2UpdateData(Sender: TObject;
