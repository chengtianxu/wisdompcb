...
uses
  ...
  ppProd, ppReport, ppStrtch, ppMemo, ppCtrls, ppVar, ppPrnabl, ppEndUsr;
...
//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
 {if dm.ADO70PAYMENT_TERMS.Value>0 then
  pplabel40.Caption := '付款期'+inttostr(dm.ADO70PAYMENT_TERMS.Value)+'天';
 if dm.ADO70DISCOUNT_DAYS.Value>0 then
  pplabel40.Caption := pplabel40.Caption+','+inttostr(dm.ADO70DISCOUNT_DAYS.Value)+'天内';
 if dm.ADO70CASH_DISCOUNT.Value>0 then
  pplabel40.Caption := pplabel40.Caption+' 折扣'+floattostr(dm.ADO70CASH_DISCOUNT.Value)+'%';

 if dm.ADO70DISCOUNT2_DAYS.Value>0 then
  pplabel40.Caption := pplabel40.Caption+','+inttostr(dm.ADO70DISCOUNT2_DAYS.Value)+'天内';
 if dm.ADO70CASH_DISCOUNT2.Value>0 then
  pplabel40.Caption := pplabel40.Caption+' 折扣'+floattostr(dm.ADO70CASH_DISCOUNT2.Value)+'%';}
