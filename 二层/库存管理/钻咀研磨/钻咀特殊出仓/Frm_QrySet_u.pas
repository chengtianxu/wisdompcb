...
begin
  ...
  begin
    ...
    sgrid1.Cells[2,sgrid1.RowCount-1]:=' and '+v_Field_List_EN[ListBox3.ItemIndex]+' = '+inttostr(ComboBox2.ItemIndex);
  end else if listbox3.itemindex=1 then
  begin
    sgrid1.Cells[1,sgrid1.RowCount-1]:=ComboBox1.Text;
    if ComboBox1.ItemIndex=0 then  sTp:='1' else
    if ComboBox1.ItemIndex=1 then  sTp:='2' else
    if ComboBox1.ItemIndex=2 then  sTp:='3' else
    if ComboBox1.ItemIndex=3 then  sTp:='5' ;
    sgrid1.Cells[2,sgrid1.RowCount-1]:=' and '+v_Field_List_EN[ListBox3.ItemIndex]+' = '+sTp;
  end else 
  begin
...
begin
  ...
  ComboBox1.Visible :=listbox3.ItemIndex =1 ;
  ComboBox2.Visible :=listbox3.ItemIndex =2 ;
  BitBtn7.Visible:=listbox3.ItemIndex =3 ;
  edtValue.Visible := listbox3.ItemIndex in [0,3] ;
  edtValue.Text:='';
...
var i:integer;
begin
  v_Field_List_CN := VarArrayOf(['出仓单号','出仓源','状态','出仓人']);
  v_Field_List_EN := VarArrayOf(['data0221.number','data0221.type','data0221.status','d5_1.employee_name']);
