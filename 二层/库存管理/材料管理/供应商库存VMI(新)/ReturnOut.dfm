object Form_ReturnOut: TForm_ReturnOut
  Left = 207
  Top = 108
  Width = 827
  Height = 448
  Caption = #20379#24212#21830#36864#36135#20986#20179
  Color = clBtnFace
  Font.Charset = ANSI_CHARSET
  Font.Color = clWindowText
  Font.Height = -13
  Font.Name = #23435#20307
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnCreate = FormCreate
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 819
    Height = 41
    Align = alTop
    BevelOuter = bvNone
    TabOrder = 0
    object Label1: TLabel
      Left = 261
      Top = 14
      Width = 100
      Height = 13
      Alignment = taRightJustify
      AutoSize = False
      Caption = #25628#32034#65306#26448#26009#32534#30721
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = #23435#20307
      Font.Style = []
      ParentFont = False
    end
    object BitBtn1: TBitBtn
      Left = 3
      Top = 6
      Width = 55
      Height = 28
      Hint = #36864#20986
      Caption = #36864#20986
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = #23435#20307
      Font.Style = []
      ModalResult = 2
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 2
      TabStop = False
      Glyph.Data = {
        
        0000}
      NumGlyphs = 2
    end
    object BitBtn2: TBitBtn
      Left = 58
      Top = 6
      Width = 55
      Height = 28
      Hint = #36864#20986
      Caption = #21047#26032
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = #23435#20307
      Font.Style = []
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 1
      TabStop = False
      OnClick = BitBtn2Click
      Glyph.Data = {
        
        440AADADADA000A04440DADADADADADA0440ADADADADADADA00D}
    end
    object Edit1: TEdit
      Left = 371
      Top = 10
      Width = 126
      Height = 21
      TabOrder = 0
      OnChange = Edit1Change
    end
    object BitBtn3: TBitBtn
      Left = 114
      Top = 6
      Width = 55
      Height = 28
      Hint = #37325#26032#23450#20041#26597#35810#26465#20214
      Caption = #26597#35810
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = #23435#20307
      Font.Style = []
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 3
      OnClick = BitBtn3Click
      Glyph.Data = {
        
        861D8680775398535355}
    end
    object BitBtn4: TBitBtn
      Left = 171
      Top = 6
      Width = 55
      Height = 28
      Hint = #29992#25143#33258#23450#20041#26174#31034#23383#27573
      Caption = #23383#27573
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = #23435#20307
      Font.Style = []
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 4
      OnClick = BitBtn4Click
      Glyph.Data = {
        
        071B1B1B070D07000000}
    end
  end
  object eh1: TDBGridEh
    Left = 0
    Top = 41
    Width = 819
    Height = 380
    Align = alClient
    DataSource = DataSource1
    FooterColor = clWindow
    FooterFont.Charset = ANSI_CHARSET
    FooterFont.Color = clWindowText
    FooterFont.Height = -13
    FooterFont.Name = #23435#20307
    FooterFont.Style = []
    Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit]
    PopupMenu = PM1
    ReadOnly = True
    TabOrder = 1
    TitleFont.Charset = ANSI_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -13
    TitleFont.Name = #23435#20307
    TitleFont.Style = []
    OnKeyDown = eh1KeyDown
    OnTitleClick = eh1TitleClick
    Columns = <
      item
        EditButtons = <>
        FieldName = 'GRN_NUMBER'
        Footers = <>
        Title.Caption = #20837#24211#21333#21495
      end
      item
        EditButtons = <>
        FieldName = 'stype'
        Footers = <>
        Title.Caption = #31867#22411
      end
      item
        EditButtons = <>
        FieldName = 'CREATE_DATE'
        Footers = <>
        Title.Caption = #21019#24314#26085#26399
        Width = 57
      end
      item
        EditButtons = <>
        FieldName = 'QUANTITY'
        Footers = <>
        Title.Caption = #24050#25509#25910#25968#37327
        Width = 76
      end
      item
        EditButtons = <>
        FieldName = 'QUAN_ON_HAND'
        Footers = <>
        Title.Caption = #29616#25345#26377#25968#37327
        Width = 67
      end
      item
        EditButtons = <>
        FieldName = 'QUAN_RETURNED'
        Footers = <>
        Title.Caption = #36864#22238#30340#25968#37327
        Width = 70
      end
      item
        EditButtons = <>
        FieldName = 'INV_PART_NUMBER'
        Footers = <>
        Title.Caption = #26448#26009#32534#30721
        Width = 61
      end
      item
        EditButtons = <>
        FieldName = 'INV_NAME'
        Footers = <>
        Title.Caption = #26448#26009#21517#31216
        Width = 57
      end
      item
        EditButtons = <>
        FieldName = 'LOCATION'
        Footers = <>
        Title.Caption = #20179#24211#20301#32622
        Width = 54
      end
      item
        EditButtons = <>
        FieldName = 'EMPLOYEE_NAME'
        Footers = <>
        Title.Caption = #21019#24314#20154#21592
        Width = 57
      end
      item
        EditButtons = <>
        FieldName = 'UNIT_NAME'
        Footers = <>
        Title.Caption = #21333#20301
        Width = 28
      end
      item
        EditButtons = <>
        FieldName = 'WAREHOUSE_NAME'
        Footers = <>
        Title.Caption = #24037#21378
        Width = 35
      end
      item
        EditButtons = <>
        FieldName = 'SUPPLIER_NAME'
        Footers = <>
        Title.Caption = #20379#24212#21830
        Width = 45
      end
      item
        EditButtons = <>
        FieldName = 'CODE'
        Footers = <>
        Title.Caption = #20379#24212#21830#20195#30721
      end
      item
        EditButtons = <>
        FieldName = 'tax_price'
        Footers = <>
        Title.Caption = #21547#31246#20215#26684
      end
      item
        EditButtons = <>
        FieldName = 'PRICE'
        Footers = <>
        Title.Caption = #20215#26684
      end
      item
        EditButtons = <>
        FieldName = 'TAX_2'
        Footers = <>
        Title.Caption = #31246#29575
      end>
  end
  object PM2: TPopupMenu
    AutoHotkeys = maManual
    Left = 184
    Top = 152
  end
  object PM1: TPopupMenu
    Left = 96
    Top = 136
    object N1: TMenuItem
      Caption = #36864#36135#22788#29702'(&R)'
      OnClick = N1Click
    end
  end
  object ADS134: TADODataSet
    Connection = dm.ADOConnection1
    CursorType = ctStatic
    Filtered = True
    CommandText = 
      'SELECT         dbo.data0133.*,data0134.*,'#13#10'                     ' +
      '  case data0133.ttype '#13#10'                       when 1 then '#39#25353'PO'#20837 +
      #24211#39#13#10'                       when 3 then '#39#30452#25509#20837#24211#39'  end  as stype,'#13#10' ' +
      '                     dbo.Data0017.INV_PART_NUMBER, dbo.Data0017.' +
      'INV_NAME, dbo.Data0016.LOCATION, '#13#10'                      dbo.Dat' +
      'a0005.EMPLOYEE_NAME, dbo.Data0002.UNIT_NAME, dbo.Data0015.WAREHO' +
      'USE_NAME, dbo.Data0023.SUPPLIER_NAME, '#13#10'                      db' +
      'o.Data0023.CODE'#13#10'FROM         dbo.DATA0133 INNER JOIN'#13#10'         ' +
      '             dbo.DATA0134 ON dbo.DATA0133.RKEY = dbo.DATA0134.GR' +
      'N_PTR INNER JOIN'#13#10'                      dbo.Data0015 ON dbo.DATA' +
      '0133.warehouse_ptr = dbo.Data0015.RKEY INNER JOIN'#13#10'             ' +
      '         dbo.Data0023 ON dbo.DATA0133.SUPP_PTR = dbo.Data0023.RK' +
      'EY INNER JOIN'#13#10'                      dbo.Data0005 ON dbo.DATA013' +
      '3.CREATE_BY = dbo.Data0005.RKEY INNER JOIN'#13#10'                    ' +
      '  dbo.Data0017 ON dbo.DATA0134.INVENTORY_PTR = dbo.Data0017.RKEY' +
      ' INNER JOIN'#13#10'                      dbo.Data0016 ON dbo.DATA0134.' +
      'LOCATION_PTR = dbo.Data0016.RKEY  INNER JOIN'#13#10'                  ' +
      '    dbo.Data0002 ON dbo.DATA0134.UNIT_PTR = dbo.Data0002.RKEY'#13#10'w' +
      'here  data0133.create_date>=:dtpk1 and data0133.create_date<=:dt' +
      'pk2'
    Parameters = <
      item
        Name = 'dtpk1'
        DataType = ftDateTime
        NumericScale = 3
        Precision = 23
        Size = 16
        Value = Null
      end
      item
        Name = 'dtpk2'
        DataType = ftDateTime
        NumericScale = 3
        Precision = 23
        Size = 16
        Value = Null
      end>
    Left = 576
    Top = 8
    object ADS134RKEY: TAutoIncField
      FieldName = 'RKEY'
      ReadOnly = True
    end
    object ADS134GRN_NUMBER: TStringField
      FieldName = 'GRN_NUMBER'
      Size = 10
    end
    object ADS134DELIVER_NUMBER: TStringField
      FieldName = 'DELIVER_NUMBER'
      Size = 15
    end
    object ADS134warehouse_ptr: TIntegerField
      FieldName = 'warehouse_ptr'
    end
    object ADS134SUPP_PTR: TIntegerField
      FieldName = 'SUPP_PTR'
    end
    object ADS134currency_ptr: TIntegerField
      FieldName = 'currency_ptr'
    end
    object ADS134exch_rate: TBCDField
      FieldName = 'exch_rate'
      Precision = 12
      Size = 8
    end
    object ADS134PO_PTR: TIntegerField
      FieldName = 'PO_PTR'
    end
    object ADS134TTYPE: TSmallintField
      FieldName = 'TTYPE'
    end
    object ADS134CREATE_DATE: TDateTimeField
      FieldName = 'CREATE_DATE'
    end
    object ADS134CREATE_BY: TIntegerField
      FieldName = 'CREATE_BY'
    end
    object ADS134ship_DATE: TDateTimeField
      FieldName = 'ship_DATE'
    end
    object ADS134ship_BY: TStringField
      FieldName = 'ship_BY'
    end
    object ADS134REF_NUMBER: TStringField
      FieldName = 'REF_NUMBER'
      Size = 100
    end
    object ADS134RKEY_1: TAutoIncField
      FieldName = 'RKEY_1'
      ReadOnly = True
    end
    object ADS134GRN_PTR: TIntegerField
      FieldName = 'GRN_PTR'
    end
    object ADS134INVENTORY_PTR: TIntegerField
      FieldName = 'INVENTORY_PTR'
    end
    object ADS134SOURCE_PTR: TIntegerField
      FieldName = 'SOURCE_PTR'
    end
    object ADS134LOCATION_PTR: TIntegerField
      FieldName = 'LOCATION_PTR'
    end
    object ADS134UNIT_PTR: TIntegerField
      FieldName = 'UNIT_PTR'
    end
    object ADS134QUANTITY: TBCDField
      FieldName = 'QUANTITY'
      Precision = 19
    end
    object ADS134QUAN_ON_HAND: TBCDField
      FieldName = 'QUAN_ON_HAND'
      Precision = 19
    end
    object ADS134QUAN_RETURNED: TBCDField
      FieldName = 'QUAN_RETURNED'
      Precision = 18
      Size = 6
    end
    object ADS134tax_price: TBCDField
      FieldName = 'tax_price'
      Precision = 12
      Size = 3
    end
    object ADS134PRICE: TBCDField
      FieldName = 'PRICE'
      Precision = 15
      Size = 6
    end
    object ADS134TAX_2: TBCDField
      FieldName = 'TAX_2'
      Precision = 4
      Size = 2
    end
    object ADS134BARCODE_ID: TStringField
      FieldName = 'BARCODE_ID'
      Size = 50
    end
    object ADS134SUPPLIER2: TStringField
      FieldName = 'SUPPLIER2'
      Size = 30
    end
    object ADS134EXPIRE_DATE: TDateTimeField
      FieldName = 'EXPIRE_DATE'
    end
    object ADS134rohs: TStringField
      FieldName = 'rohs'
      Size = 10
    end
    object ADS134INV_PART_NUMBER: TStringField
      FieldName = 'INV_PART_NUMBER'
      Size = 25
    end
    object ADS134INV_NAME: TStringField
      FieldName = 'INV_NAME'
      Size = 30
    end
    object ADS134LOCATION: TStringField
      FieldName = 'LOCATION'
    end
    object ADS134EMPLOYEE_NAME: TStringField
      FieldName = 'EMPLOYEE_NAME'
      Size = 16
    end
    object ADS134UNIT_NAME: TStringField
      FieldName = 'UNIT_NAME'
    end
    object ADS134WAREHOUSE_NAME: TStringField
      FieldName = 'WAREHOUSE_NAME'
      Size = 70
    end
    object ADS134SUPPLIER_NAME: TStringField
      FieldName = 'SUPPLIER_NAME'
      Size = 100
    end
    object ADS134CODE: TStringField
      FieldName = 'CODE'
      Size = 10
    end
    object ADS134stype: TStringField
      FieldName = 'stype'
      ReadOnly = True
      Size = 8
    end
  end
  object DataSource1: TDataSource
    DataSet = ADS134
    Left = 632
  end
  object ado148: TADOQuery
    Connection = dm.ADOConnection1
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'select * from data0724')
    Left = 672
    Top = 320
    object ado148RKEY: TAutoIncField
      FieldName = 'RKEY'
      ReadOnly = True
    end
    object ado148GON_NUMBER: TStringField
      FieldName = 'GON_NUMBER'
      Size = 10
    end
    object ado148warehouse_ptr: TIntegerField
      FieldName = 'warehouse_ptr'
    end
    object ado148TTYPE: TSmallintField
      FieldName = 'TTYPE'
    end
    object ado148CREATE_DATE: TDateTimeField
      FieldName = 'CREATE_DATE'
    end
    object ado148CREATE_BY: TIntegerField
      FieldName = 'CREATE_BY'
    end
    object ado148RECD_BY: TIntegerField
      FieldName = 'RECD_BY'
    end
    object ado148REF_NUMBER: TStringField
      FieldName = 'REF_NUMBER'
      Size = 50
    end
    object ado148supplier_ptr: TIntegerField
      FieldName = 'supplier_ptr'
    end
    object ado148status: TWordField
      FieldName = 'status'
    end
  end
  object ADO149: TADOQuery
    Connection = dm.ADOConnection1
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'select * from data0149 where rkey=null')
    Left = 712
    Top = 320
    object ADO149RKEY: TAutoIncField
      FieldName = 'RKEY'
      ReadOnly = True
    end
    object ADO149GON_PTR: TIntegerField
      FieldName = 'GON_PTR'
    end
    object ADO149D0134_PTR: TIntegerField
      FieldName = 'D0134_PTR'
    end
    object ADO149INVENTORY_PTR: TIntegerField
      FieldName = 'INVENTORY_PTR'
    end
    object ADO149QUANTITY: TBCDField
      FieldName = 'QUANTITY'
      Precision = 14
    end
    object ADO149RTN_QUANTITY: TBCDField
      FieldName = 'RTN_QUANTITY'
      Precision = 14
    end
    object ADO149tax_price: TBCDField
      FieldName = 'tax_price'
      Precision = 12
      Size = 3
    end
    object ADO149TAX_2: TBCDField
      FieldName = 'TAX_2'
      Precision = 4
      Size = 2
    end
  end
end
