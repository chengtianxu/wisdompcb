...
begin
  ...
  begin
    ...
    n3.Enabled := true;
   //提交审批
    n4.Enabled := (dm.ADO268status.AsInteger=0);   //and (((dm.ADO268ismsgsended.AsInteger =0) and (dm.ADO268allow_submit.AsInteger =1 ) )  or  (dm.ADO268allow_submit.AsInteger in [0,2])  )    ;    //提交审批        //and (dm.ADO268allow_submit.AsInteger in [0,1,2])
    ...
    n14.Enabled := (dm.ADO268status.AsInteger in [2,4]) and ((vprev='4') or  DM.ADO268Prted.AsBoolean);   //重印
    n8.Enabled:=true;                            //申请人员只能打印一次,仓库人员可多次打印
    n10.Enabled:=true;
   n9.Enabled:=dm.ADO268status.AsInteger in [1];                   //取消允许
   //允许提交
    ...
    3: // 提交审批
      begin
       ...
       { if dm.ADO268allow_submit.Value = 2 then
        ...
        else if  dm.ADO268allow_submit.Value = 1 then    // 待确认 lisa1024
        begin
         try
          // if not  DM.ADO268.FieldByName('allow_submit').AsInteger in [1,3] then 

           if  DM.ADO268.FieldByName('IsMsgSended').AsInteger=1  then    //已发送确认信息等待确认
           begin
           ShowMessage('该领料单有物料超领已给相关人员发送短信,请等待确认!');
           Exit;
           end;

         if MessageDlg('有物料已超领不能提交，需要发送短信提示给相关人员去确认吗？',mtInformation,[mbyes,mbno],0)=mrno then
           exit;
          ...
          messages:='物料申领单号 '+DM.ADO268.FieldByName('number').asstring+ ' 里的申领物料有超领: '+#13;
           if not DM.qry_temp.IsEmpty then
           begin
           DM.qry_temp.First;
           while not DM.qry_temp.eof do
           begin
           messages:=messages+'控制项目为 '+DM.qry_temp.FieldByName('control_name').asstring +' 的物料:'+DM.qry_temp.FieldByName('remark').asstring + #13  ;
            DM.qry_temp.next;
           end;
           end;
    ...
    exit;
        ...
        begin
            ...
            try
               ...
               // add by lisa 20160706   默认为0计入， 不计为1 IsIn
//             if MessageDlg('该笔领料超标的物料需要计入当月领料累计吗？',mtInformation, [mbYes,mbNo],0)= mryes then
//
//             else
//             begin
...
//
  ...
  try
   ...
   ssql:='select distinct d468.control_ptr'+
    ...
    messages2:=' '+ DM.ADO268.FieldByName('fac_name').asstring+' '+ DM.ADO268.FieldByName('dept_name').asstring+' 领料单号'+ DM.ADO268.FieldByName('number').asstring+': '+#13;
    ...
    begin
     ...
     if not DM.qry_temp.IsEmpty then
      begin
       DM.qry_temp.First;
       messages:='';
       while not DM.qry_temp.eof do    //messages+'物料:'
       begin
         if  messages='' then
       ...
       messages:=messages2+'物料：'+messages+' '+DM.qry_temp.FieldByName('remark').asstring  ;
  ...
  //   if  dm.ADO268allow_submit.Value = 1 then    // 待确认 lisa1024
  //   begin
//      showmessage('有物料已超领不能提交，需要等相关人员确认.');
//       exit;
   //  end
    // else
     if dm.ADO268allow_submit.Value= 2 then    //已确认
      begin
        try
          dm.ADOCon.BeginTrans;
          dm.tmp.Close;
