...
type
  TFrm_detail = class(TForm)
    ...
    procedure SGridMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
    ...
    procedure SGridDrawCell(Sender: TObject; ACol, ARow: Integer; Rect: TRect; State: TGridDrawState);
...
uses
  DM_u, common, Pick_Item_Single, ConstVar, Frm_detailEdt_u, Frm_Out_u,
  Frm_Auth_u, Frm_prod,UBOMsel;
  //UStan;
...
begin
  ...
  SGrid.Cells[11, 0] := 'supplier_ptr';
   SGrid.ColWidths[11] := -1;

     SGrid.Cells[12, 0] := 'cprkey492';
  ...
  begin
    ...
    CBFac.Enabled := True;
    ssql := 'select warehouse_ptr from data0073  where rkey=' + rkey73 + ' ';
    dm.openqry(dm.tmp, ssql);

    if not dm.tmp.IsEmpty then
    begin
      if dm.tmp.fieldbyname('warehouse_ptr').Value <> null then
      begin
        CBFac.Enabled := false;
        WH := 1;
        CBFac.tag := dm.tmp.fieldbyname('warehouse_ptr').asinteger;
        ssql := 'select abbr_name from data0015 where rkey=' + inttostr(CBFac.tag) + ' ';
        dm.openqry(dm.tmp, ssql);
        if not dm.tmp.IsEmpty then
          CBFac.Text := dm.tmp.fieldbyname('abbr_name').asstring;
      end;
    end;
    ...
    DM.tmp.Close;
    DM.tmp.SQL.Text := 'select data0094.rkey,data0094.code, data0094.PURCHASE_APPROV_DESC ' + 'from data0094 inner join data0034 on data0094.dept_ptr=data0034.rkey ' + 'where dbo.Data0094.type=2 and data0034.dept_code=''' + dept_code + ''' ';
    DM.tmp.Open;
...
//              SpeedButton3.Visible := True;
      ...
      begin
        close;
        sql.Text := ' select rkey,control_name from data0136 where rkey = ' + DM.ADO268.FieldByName('control_ptr').AsString + #13;
        Open;
    ...
    begin    //COPY
      ...
      for i := 0 to DM.ADO268_1.FieldCount - 1 do
        if (uppercase(DM.ADO268_1.Fields[i].FullName) <> 'RKEY') and (uppercase(DM.ADO268_1.Fields[i].FullName) <> 'NUMBER') and (uppercase(DM.ADO268_1.Fields[i].FullName) <> 'AUTH_DATE') and (uppercase(DM.ADO268_1.Fields[i].FullName) <> 'AUTH_EMPLPTR') and (uppercase(DM.ADO268_1.Fields[i].FullName) <> 'PRTED') then
      ...
      begin
        ...
        for i := 0 to DM.ADO468_1.FieldCount - 1 do
          if (uppercase(DM.ADO468_1.Fields[i].FullName) <> 'RKEY') and (uppercase(DM.ADO468_1.Fields[i].FullName) <> 'QUAN_ISSUED') and (uppercase(DM.ADO468_1.Fields[i].FullName) <> 'STATUS') then
    ...
    DM.tmp.Close;
    ssql:= ' select rkey from data0492 where cut_no = ''' + dm.ADO268.Fieldbyname('cut_no').AsString+''' ';
    DM.tmp.SQL.Text :=  ssql;
    DM.tmp.Open;
      if DM.tmp.RecordCount>0 then
         Edit1.tag:= DM.tmp.Fields[0].AsInteger;
    ...
    edt_confMan.Text := DM.ADO268.Fieldbyname('confMan').AsString;


    for i := 0 to 4 do
      if CBFac.Tag = Ffac[i] then
      begin
        CBFac.ItemIndex := i;
        break;
      end;
    ...
    begin
      ...
      SGrid.Cells[7, j] := DM.ADO468.fieldbyname('control_name').AsString;
      SGrid.Cells[8, j] := DM.ADO468.fieldbyname('standards').AsString;
      SGrid.Cells[9, j] := DM.ADO468.fieldbyname('remark').AsString;
      SGrid.Cells[10, j] := DM.ADO468.fieldbyname('exreason').AsString;
      SGrid.Cells[11, j] := DM.ADO468.fieldbyname('supplier_ptr').AsString;
      SGrid.Cells[12, j] := DM.ADO468.fieldbyname('wzcp0492_ptr').AsString;
...
var
  ...
  QryTemp: TADOquery;
  sflag: integer;
  Pname, Ptype, smessage, ssql: string;
  C_ptr, CFlag: integer;
  NowQty: Double;
  lst: TStringList;
  Lsql:string;
...
begin
  ...
  if cbbBZ.Text = '' then
  begin
    if MessageBox(Handle, '备注为空，是否继续？', '提示', MB_YESNO + MB_ICONWARNING + MB_DEFBUTTON2) = IDNO then
      Exit;
  end;
  ...
  if SGrid.Cells[12,i]<>''  then 
   for i:=1 to SGrid.RowCount-2 do
    begin
      DM.tmp1.Close;
      DM.tmp1.SQL.Clear;
      DM.tmp1.SQL.Text:='select (STD_QTY-App_QTY) as sl from wzcp0492 where rkey='+SGrid.Cells[12,i];
      dm.tmp1.Open;
  ...
  {   dm.tmp1.Close;
    dm.tmp1.SQL.Text := 'select user_ptr from data0275  where rkey in  (select max(rkey)rkey from data0275 where AUTH_GROUP_PTR=' +
    IntToStr(EdtAud.tag) + ' )';
    DM.tmp1.Open;

   if dm.tmp1.FieldByName('user_ptr').AsString =rkey73 then
   begin
    ShowMessage('申领人不能和最终审核人为同一人， 你不能领料！');
    EdtAud.SetFocus;
    exit;
   end;    }


  if(Trim(Edit1.Text)='') then  // 不按配料单领料
  begin
    rkey136_1 := '';
    ...
    begin
      if (SGrid.Cells[7, j] <> '') then
      begin
        CFlag := 1; //有标准需要检查
        lst.Add(SGrid.Cells[7, j]);
        lst.Sorted := True;
    ...
    begin
      ...
      begin
        for j := 1 to SGrid.RowCount - 2 do
        begin
          if SGrid.Cells[7, j] = lst[k] then
          begin
            nowqty := NowQty + StrToFloat(SGrid.Cells[2, j]);
            Ptype := Copy(SGrid.Cells[8, j], 0, 1);
            Pname := SGrid.Cells[7, j];
          end;
        end;
        ssql := 'select rkey from data0136 where control_name=''' + PName + ''' ';
        dm.OpenQry(qry_CPtr, ssql);

        with dm.sp1 do
        begin
          dm.sp1.close;
          dm.sp1.Parameters.ParamByName('@CType').Value := Ptype;
          dm.sp1.Parameters.ParamByName('@warehouse_ptr').Value := CBFac.tag;
          dm.sp1.Parameters.ParamByName('@control_ptr').Value := qry_CPtr.fieldbyname('rkey').asinteger;
          dm.sp1.Parameters.ParamByName('@cost_ptr').Value := EdtDept.tag;
          dm.sp1.Parameters.ParamByName('@NowQty').Value := 0;//NowQty
          dm.sp1.Parameters.ParamByName('@ReqDate').Value := DateTimePicker1.date;
          ...
          ExecProc;
          smessage := TrimLeft(dm.sp1.Parameters.ParamByName('@smessage').Value);
        end;

  //      showmessage(floattostr(dm.sp1.Parameters.ParamByName('@AllQty').Value));
  //      showmessage(floattostr(dm.sp1.Parameters.ParamByName('@getQty').Value));
  //      showmessage(floattostr(dm.sp1.Parameters.ParamByName('@qcQty').Value));
  //      showmessage(floattostr(dm.sp1.Parameters.ParamByName('@control_ptr').Value));
  //      showmessage((dm.sp1.Parameters.ParamByName('@smessage').Value));

        for j := 1 to SGrid.RowCount - 2 do
        begin
          if (SGrid.Cells[7, j] = lst[k]) then
            SGrid.Cells[9, j] := smessage; //TrimLeft(dm.sp1.Parameters.ParamByName('@smessage').Value);
      // ShowMessage(SGrid.Cells[9,j]) ;
        end;
      ...
      for j := 1 to SGrid.RowCount - 2 do
      begin
        if (copy(SGrid.Cells[9, j], 0, 1) = 'X') and (SGrid.Cells[10, j] = '') then
        begin
      ...
      for i := 1 to SGrid.RowCount - 2 do
        ...
        begin
          DM.ADO468_1.edit;
          DM.ADO468_1.FieldByName('remark').AsString := sGrid.Cells[9, i];
          DM.ADO468_1.post;
        end;
 ...
 //DM.ADO268_1.FieldByName('allow_submit').AsInteger:= 0;
  ...
  try
    ...
    if ((Ftag = 0) or (Ftag = 10)) and not DM.GetNo(MaskEdit1, '36', 1) then
      abort;
    ...
    for i := 1 to SGrid.RowCount - 2 do
      if DM.ADO468_1.Locate('invent_ptr', strtoint(sGrid.Cells[5, i]), []) then
      begin
        if (copy(DM.ADO468_1.FieldByName('remark').AsString, 0, 1) = 'X') then
          sflag := 2

      end;

    if sflag = 0 then   //lisa
      DM.ADO268_1.FieldByName('allow_submit').AsInteger := 0
    else
      DM.ADO268_1.FieldByName('allow_submit').AsInteger := 1;   //有标准，但不符合
    ...
    DM.ADO268_1.UpdateBatch(arAll);
      ...
      for i := 1 to SGrid.RowCount - 2 do
      if DM.ADO468_1.Locate('invent_ptr;supplier_ptr', vararrayof([strtoint(sGrid.Cells[5, i]),strtoint(sGrid.Cells[11, i])]), []) then
      begin
        DM.ADO468_1.Edit;
        ...
        DM.ADO468_1.FieldByName('quan_bom').AsString := SGrid.Cells[2, i];  //lisa20170524
        DM.ADO468_1.FieldByName('vendor').AsString := SGrid.Cells[3, i];

        DM.ADO468_1.FieldByName('Exreason').AsString := SGrid.Cells[10, i]; // 20160815 add
        DM.ADO468_1.FieldByName('standards').AsString := SGrid.Cells[8, i]; // 20160815 add
        DM.ADO468_1.FieldByName('step').value :=0;
        Lsql:='select rkey from data0002  where UNIT_NAME='''+SGrid.Cells[4, i]+'''  ';
        dm.openqry(dm.tmp, Lsql);
        if not dm.tmp.IsEmpty then
        DM.ADO468_1.FieldByName('RUnit').value:=dm.tmp.fieldbyname('rkey').asinteger ;  //SGrid.Cells[4, i]; // 20171027 add 申领单位

        if Trim(SGrid.Cells[6, i]) = '普通库存' then
          DM.ADO468_1.FieldByName('PRINTIT').AsString := 'Y'
        else if Trim(SGrid.Cells[6, i]) = 'VMI库存' then
          DM.ADO468_1.FieldByName('PRINTIT').AsString := 'N'
        else
          DM.ADO468_1.FieldByName('PRINTIT').AsString:='Y';  //按配料单领料时
        ...
        begin
            DM.tmp.Close;
            dm.tmp.SQL.Clear;
            DM.tmp.SQL.Text:='update wzcp0492 set App_QTY=App_QTY +'+(SGrid.Cells[2,i])+
            ' where rkey='+SGrid.Cells[12,i];
...
begin
  ...
  TfrmBOMSel.InitEdt(edtDept.Tag, sGrid, CBFac.ItemIndex,Edit1.Tag);
  if sgrid.RowCount > 2 then
  begin
    CBFac.Enabled := False;
    SpeedButton1.Enabled := false;
...
procedure TFrm_detail.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
procedure TFrm_detail.SGridMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
//var
    ...
    case Tcomponent(Sender).Tag of
      0:
        begin
          ...
          InputVar.Sqlstr := ' select rkey,dept_code,dept_name, ' + ' case when ttype=4 then ''部门''  else ''工序'' end xz ' +  //when ttype=5 then ''班组''
           ' from data0034  ' +
             ' where ((ttype =4)  or ' +
              ' ((is_cost_dept=1) and (COST_DEPT_PTR=rkey))) '+
              ' and (ACTIVE_FLAG=0)    ';
      ...
      case Tcomponent(Sender).Tag of
        0:
          begin
            ...
            DM.tmp.Close;
            DM.tmp.SQL.Text := 'select data0094.rkey,data0094.code, data0094.PURCHASE_APPROV_DESC ' + 'from data0094 inner join data0034 on data0094.dept_ptr=data0034.rkey ' + 'where dbo.Data0094.type=2 and data0034.dept_code=''' + EdtDept.Text + ''' ';
            DM.tmp.Open;
            if DM.tmp.RecordCount = 1 then
            begin
              EdtAud.Text := DM.tmp.FieldValues['Code'];
...
begin
  ...
  DM.tmp.Close;
  DM.tmp.SQL.Text := 'select data0094.rkey,data0094.code, data0094.PURCHASE_APPROV_DESC ' + 'from data0094 inner join data0034 on data0094.dept_ptr=data0034.rkey ' + 'where data0034.dept_code=''' + EdtDept.Text + ''' and data0094.code=''' + EdtAud.Text + '''';
  DM.tmp.Open;
...
begin
  ...
  DM.tmp.Close;
  DM.tmp.SQL.Text := 'select rkey,dept_code,dept_name from data0034 where (ttype in (4,5) or is_cost_dept=1) ' + 'and dept_code=''' + EdtDept.Text + '''';
  DM.tmp.Open;
  if DM.tmp.IsEmpty then
  begin
...
procedure TFrm_detail.SGridSelectCell(Sender: TObject; ACol, ARow: Integer; var CanSelect: Boolean);
begin
  if Ftag = 2 then
    exit;
  if (ARow <> SGrid.RowCount - 1) and ((ACol = 3) or (ACol = 10)) then   //(ACol = 2) or 
...
procedure TFrm_detail.SGridDrawCell(Sender: TObject; ACol, ARow: Integer; Rect: TRect; State: TGridDrawState);
var
  CellString: string;
begin
  CellString := SGrid.Cells[ACol, ARow];
  if ARow = 0 then
    SGrid.Canvas.TextRect(rect, rect.left + (rect.right - rect.left - SGrid.Canvas.TextWidth(CellString)) div 2, rect.top + (rect.bottom - rect.top - SGrid.Canvas.TextHeight(CellString)) div 2, CellString)
  else if (ARow > 0) and (ACol = 2) then    //数字居右
    SGrid.Canvas.TextRect(rect, rect.left + (rect.right - rect.left - SGrid.Canvas.TextWidth(CellString)), rect.top + (rect.bottom - rect.top - SGrid.Canvas.TextHeight(CellString)) div 2, CellString);
end;
  ...
  try
    case Tcomponent(Sender).Tag of
      0:
        begin
          InputVar.Fields := 'Dept_code/部门编号/100,dept_name/部门名称/150,xz/性质/100';
          InputVar.Sqlstr := 'select rkey,dept_code,dept_name,case when ttype=4 then ''部门'' ' + 'when ttype=5 then ''班组'' else ''工序'' end xz from data0034 where (ttype in (4,5) or is_cost_dept=1)' + ' and (ACTIVE_FLAG=0)';
          inputvar.KeyField := 'Dept_code';
        end;
...
begin
  ...
  begin
    if (inttostr(EdtDept.tag) <> '') and (trim(SGrid.Cells[5, i]) <> '') and (CBFac.Tag <> 0) then
    begin
      with QueryTmp do
      begin
        try
          close;
          sql.Text := ' ( ' + ' SELECT     dbo.DATA0136.rkey ' + ' FROM         dbo.DATA0136 LEFT JOIN ' + '                      dbo.DATA0154 ON dbo.DATA0136.rkey = dbo.DATA0154.control_ptr ' + ' WHERE (dbo.DATA0154.dept_ptr = ' + inttostr(EdtDept.tag) + ' ) AND ( dbo.DATA0136.warehouse_ptr = ' + inttostr(CBFac.Tag) + '  ))  ' + ' INTERSECT ' + ' ( ' + ' SELECT     dbo.DATA0136.rkey ' + ' FROM         dbo.DATA0136 LEFT OUTER JOIN ' + '                       dbo.DATA0155 ON dbo.DATA0136.rkey = dbo.DATA0155.control_ptr ' + ' WHERE (dbo.DATA0155.invt_ptr = ' + SGrid.Cells[5, i] + ' ) AND ( dbo.DATA0136.warehouse_ptr = ' + inttostr(CBFac.Tag) + '  ))  ' + #13;
...
begin
  ...
  begin
    DM.tmp.Close;
    DM.tmp.SQL.Text := 'select rkey,CUT_NO,SO_NO from data0492 where TSTATUS in (0,2)' + 'and ttype<>3 and CUT_NO =''' + Trim(Edit1.Text) + '''';
    DM.tmp.Open;
    if not DM.tmp.IsEmpty then
    begin
...
begin
  ...
  try
    ...
    begin
      ...
      Edit1.Tag:= Prod_Frm.ADO492.fieldbyname('rkey').AsInteger;
...
procedure TFrm_detail.Edit1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  if Trim(EdtDept.Text) = '' then
  begin
    MessageDlg('请首先确认领料部门!', mtError, [mbCancel], 0);
...
begin
  ...
  begin
    Close;
    sql.Text := 'SELECT dbo.Data0017.INV_PART_NUMBER, dbo.Data0017.INV_PART_DESCRIPTION,' + 'dbo.DATA0403.VENDOR, dbo.Data0002.UNIT_NAME, dbo.DATA0403.rkey17,' + 'dbo.DATA0403.QTY_BOM - dbo.DATA0403.QUAN_ISSUED AS req_qty ' + 'FROM   dbo.DATA0403 INNER JOIN ' + ' dbo.Data0017 ON dbo.DATA0403.rkey17 = dbo.Data0017.RKEY INNER JOIN ' + ' dbo.Data0002 ON dbo.Data0017.STOCK_UNIT_PTR = dbo.Data0002.RKEY ' + 'INNER JOIN  dbo.Data0060 ON dbo.DATA0403.rkey60 = dbo.Data0060.RKEY ' + 'where (Data0060.SALES_ORDER=' + QuotedStr(so) + ')' + ' and (DATA0403.rkey34=' + inttostr(rkey34) + ')' + ' and (data0403.STATUS=0) and (DATA0403.QTY_BOM - DATA0403.QUAN_ISSUED >0) ';
    Open;
    while not Eof do
    begin
