...
uses damo, purout_invt,putoutinvt_dept,  putoutinvt_inspect,common;
...
begin
  ...
  begin
    ...
    begin            //   如果在复制的数据中有有效期小于 aq22的记录 ，给予提示
      if (FieldByName('EXPIRE_DATE').AsDateTime<aq22.FieldByName('EXPIRE_DATE').AsDateTime) and  (aq22.FieldByName('EXPIRE_DATE').AsDateTime>0)
...
//         exit;
    ...
    if aq22.FieldByName('是否可发料').AsString='否'  then
      begin
  ...
  try
    ...
    begin
      ...
      if self.parent_flag=0 then
       begin
        if (aq22EXPIRE_DATE.Value<common.getsystem_date(dm.ADOQuery1,1) ) and  (aq22EXPIRE_DATE.AsDateTime>0) then
