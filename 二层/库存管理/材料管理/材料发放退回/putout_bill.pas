...
uses damo, main, return_invtstock, putourinvt_report,
 putoutmiscreport,common, putoutinvt_faliao;
...
begin
  ...
  if messagedlg('全部退回货仓您确定?',mtconfirmation,[mbyes,mbno],0)=mryes then
  begin
    try
      ...
      with dm.ADOQuery1 do   //更新cp492的已发放数
      begin
        close;
        sql.Text:='update wzcp0492 set Con_QTY=Con_QTY-d207.qty '+
        'FROM   dbo.wzcp0492 INNER JOIN '+
        '(SELECT   wzcp0492.rkey, SUM(QUANTITY-RTN_QUANTITY) AS qty '+
        'FROM   dbo.Data0468 '+
        ...
        'WHERE QUANTITY-RTN_QUANTITY>0 and  (dbo.Data0207.GON_PTR = '+aq457rkey.AsString+') '+
        'GROUP BY wzcp0492.rkey ) AS d207 ON '+
        'dbo.wzcp0492.rkey = d207.rkey  ';
        execsql;
      end;

        if (aq457TTYPE.Value=1) or (aq457TTYPE.Value=3) then   //配料单发放
        begin
         with dm.ADOQuery1 do
         begin
          close;
...
//             end;
            ...
            ado207RTN_QUANTITY.Value:=ado207QUANTITY.Value;                     //207
            self.ado207.Post;
          end;
          self.ado207.Next;
        end;
...
BEGIN
 ...
 begin
  ...
  Form_returninvt.Edit3.Text:=formatfloat('0.0000',self.ado207QUANTITY.Value-
  ...
  if Form_returninvt.ShowModal=mrok then
   ...
   if (aq457TTYPE.Value=3) and (not  ado207Con_QTY.IsNull) then   //更新492：
...
BEGIN
 ...
 begin
  ...
  Form_returninvt.Edit3.Text:=formatfloat('0.0000',self.ado208QUANTITY.Value-
  ...
  if Form_returninvt.ShowModal=mrok then
   ...
   try
     ...
     ado208QUAN_ON_HAND.Value:=ado208QUAN_ON_HAND.Value+   //修改235增加当前库存
                            strtocurr(Form_returninvt.Edit4.Text);
     ado208RTN_QUANTITY.Value:=ado208RTN_QUANTITY.Value+strtocurr(Form_returninvt.Edit4.Text);  //207
     ado208.Post;
