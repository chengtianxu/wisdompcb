...
//                            strtoint(sgrid2.Cells[3,i])]);  //468.rkey
  begin
  Lsql:='insert into data0207(TTYPE,INVENTORY_PTR,DEPT_PTR,GON_PTR,D0022_PTR,D0468_PTR,TDATE,QUANTITY,STANDARD_COST)'+
        'values(1,'+sgrid2.Cells[0,i]+','+dm.ado457dept_ptr.asstring+','+dm.ado457rkey.asstring+','+sgrid2.Cells[1,i]+','+
        ''+sgrid2.Cells[3,i]+','''+formatdatetime('yyyy-MM-dd hh:mm:ss',dm.ado457CREATE_DATE.asdatetime)+''','+sgrid1.Cells[5,i]+','+sgrid2.Cells[2,i]+' )';
       // ShowMessage(Lsql);
  dm.qrytemp.close;
  ...
  dm.qrytemp.ExecSQL;
    ...
    with dm.ADOQuery1 do
     ...
     ' dbo.wzcp0492 ON  wzcp0492.rkey =  Data0468.wzcp0492_ptr'+
      ...
      ' dbo.DATA0475.cp492_ptr = d207.rkey AND '+
      ' dbo.DATA0475.rkey22 = d207.D0022_PTR';
      execsql;     //减去475已分配的库存(明细不需要汇总)
     end;
    ...
    begin
      ...
      sql.Text:='update Data0022'+#13+
           ...
           '(SELECT data0207.D0022_PTR, (data0207.QUANTITY) AS qty'+#13+
           'FROM Data0207'+#13+
           ' inner join data0475 on data0475.rkey22=data0207.D0022_PTR  ' +#13+
           ' inner join  data0468 on  data0468.rkey=data0207.d0468_ptr'+#13+
           ' inner join  wzcp0492  on wzcp0492.rkey=data0468.wzcp0492_ptr and wzcp0492.rkey=data0475.cp492_ptr '+#13+
  ...
  //20180919 modi
    ...
    begin
      ...
      'from data0017 inner join (select INVENTORY_PTR,sum(QUANTITY) as total_QUANTITY'+#13+
           ...
           ' inner join  data0468 on  data0468.rkey=data0207.d0468_ptr'+#13+
           ' inner join  wzcp0492 on  wzcp0492.rkey=data0468.wzcp0492_ptr and wzcp0492.rkey=data0475.cp492_ptr '+#13+
           ' WHERE data0475.rkey468 is null and data0207.GON_PTR = '+dm.ado457rkey.AsString+#13+
      'group by INVENTORY_PTR ) as d1 on' +#13+
      'data0017.rkey=d1.INVENTORY_PTR';
      execsql;     //减去库存17中已分配的数量 ---data0475.rkey468 is null
    end;
    ...
    begin
      close;
      sql.Text:='update w492 set  QUAN_ASSIGN=QUAN_ASSIGN-isnull(a.QUANTITY,0)  '+
            ' from (select cp492.rkey ,SUM(d207.QUANTITY)QUANTITY '+
            ' from  data0207 d207 '+
            ' inner join data0475 d475 on   d475.rkey22=d207.D0022_PTR '+
            ' inner join  data0468 d468 on  d468.rkey=d207.d0468_ptr'+#13+
            ' inner join  wzcp0492 cp492 on cp492.rkey=d468.wzcp0492_ptr and cp492.rkey=d475.cp492_ptr '+#13+
            ' where d207.GON_PTR='+dm.ado457rkey.AsString+
            ' group by  cp492.rkey ) a '+
            ' inner join wzcp0492  w492  on w492.rkey=a.rkey ';
      execsql;     //更新wzcp0492已分配数量
    end;
    ...
    //20180919 add
      ...
      begin
        ...
        'FROM   dbo.Data0468 '+
        'inner join dbo.wzcp0492 ON Data0468.wzcp0492_ptr=wzcp0492.rkey '+
        'inner join  dbo.Data0207 ON dbo.Data0468.RKEY = dbo.Data0207.D0468_PTR '+
        'WHERE  (dbo.Data0207.GON_PTR = '+dm.ado457rkey.AsString+') '+
        'GROUP BY dbo.wzcp0492.rkey ) AS d207 ON '+
        'dbo.wzcp0492.rkey = d207.rkey  ';//+   AND
        execsql;     //更新cp492的已发放数
      end;
    ...
    begin
      ...
      sql.Text:='delete data0475'+#13+
            ' from  data0475   d475 '+
            ' inner join data0207 d207 on   d475.rkey22=d207.D0022_PTR '+
            ' inner join  data0468 d468 on  d468.rkey=d207.d0468_ptr'+#13+
            ' inner join  wzcp0492 cp492 on cp492.rkey=d468.wzcp0492_ptr and cp492.rkey=d475.cp492_ptr '+#13+
            ' where d475.quan_alloc=0 and d207.GON_PTR='+dm.ado457rkey.AsString;
      execsql;     //删除分配记录
    end;
   ...
   if (dm.ads468_268alloc.Value = 0) and
    ...
    begin
     ...
     while not dm.aq22.Eof do
      begin
       if  // ((dm.aq22EXPIRE_DATE.Value > getsystem_date(dm.ADOQuery1,1)) or (dm.aq22EXPIRE_DATE.Value=0) ) and  //过期也可以发20170912
...
with dm.ADOQuery1 do
 begin
  ...
  'FROM    dbo.DATA0475 INNER JOIN'+#13+
   ...
   // ' inner join  wzcp0492 cp492 on cp492.rkey= data0475.cp492_ptr'+#13+
   // ' inner join  data0492 d492 on d492.rkey= cp492.D492_ptr'+#13+
  'where    DATA0475.cp492_ptr='+dm.ads468_268.fieldbyname('rkeycp492').AsString +          // 过期 的也可以发20170912 (data0022.EXPIRE_DATE>getdate() or  data0022.EXPIRE_DATE is null ) and
 ...
 begin
  ...
  // dm.ads468_268quan_alloc.Value:=0;   //20180919
   if dm.ads468_268QUAN_ISSUED.Value >= dm.ads468_268QUAN_BOM.Value then
   dm.ads468_268STATUS.Value:=1;      //468发放完毕状态
   dm.ads468_268.Post;
 end
...
begin
 ...
 DM.aq22.Filter:='';
  ...
  dm.ads468_268.Edit;
  //dm.ads468_268quan_alloc.Value:=dm.ads468_268alloc.Value;   //wzcp0492里的已分配数
  dm.ads468_268QUAN_ISSUED.Value:=dm.ads468_268QUAN_ISSUED.Value-
