...
uses
  ...
  Dialogs, Menus, Grids, DBGrids, StdCtrls, Buttons, ExtCtrls,Excel2000,ClipBrd, ComObj,DB, ADODB,common;
...
, remain_form2,report2_form,selin_form,remaout_form2;
...
begin
 ...
 form_remain:=tform_remain.Create(self);
  ...
  begin
   close;
   sql.Clear;
   sql.Add('select abbr_name from data0015 where  active_flag=''y''');
   open;
   while  not eof do
   begin
 ...
 with dm.ADOQ do
  ...
  dm.ADOD556.Locate('RKEY',rkey556,[]);
finally
form_remain.Free;
end;
end;
...
begin
 ...
 try
  ...
  edit5.Text:=dm.ADOD556ABBR_NAME.AsString;        //供应商
  edit8.Text:=dm.ADOD556REMARK.Value;          //备注
 ...
 begin
  ...
  dm.ADOD556ABBR_NAME.AsString:=form_numcat.Edit5.Text;

  if form_numcat.Edit6.Modified then
  dm.ADOD556QUAN_ON_HAND.AsString:=form_numcat.Edit6.Text;
  if form_numcat.Edit8.Modified then
  dm.ADOD556REMARK.AsString:=form_numcat.Edit8.Text;
...
try
 form_remaout:=tform_remaout.Create(self);
  ...
  begin
   ...
   //form_remain.ComboBox1.ItemIndex:=form_remain.ComboBox1.Items.IndexOf('深圳双面'); //表示深圳双面
   //form_remain.key15.Caption:='1';//表示深圳双面的主键值
   form_remaout.ComboBox1.Text:=dm.TemADOQuery1.fieldbyname('abbr_name').AsString;
   form_remaout.key15.Caption:=dm.TemADOQuery1.fieldbyname('RKEY15').AsString;
  end;
 with dm.ADOQ do
  begin
   close;
   ...
   //form_remain.ComboBox2.ItemIndex:=form_remain.ComboBox2.Items.IndexOf('铜板仓');
   //form_remain.key16.Caption:='1';    //表示铜板仓的位置主键值
  form_remaout.ComboBox2.Text:=dm.TemADOQuery1.fieldbyname('location').AsString;
  form_remaout.key16.Caption:=dm.TemADOQuery1.fieldbyname('RKEY16').AsString;
  end;
  with dm.ADOQ do
  begin
   close;
...
// rkey73:='5';
 ...
 DbGrid1.Options:=[dgEditing,dgTitles,dgIndicator,dgColumnResize,dgColLines,dgRowLines,dgTabs,dgAlwaysShowSelection,dgConfirmDelete,dgCancelOnExit];
...
begin
  ...
  if  RadioGroup1.ItemIndex=1 then
   dm.ADOD556.Filter:=dm.ADOD556.filter+' and QUAN_ON_HAND>0'
   else
   dm.ADOD556.Filter:=dm.ADOD556.filter+' and QUAN_ON_HAND=0'
  end
 else
...
form_remain2.Free;
//dm.ADOD556.Locate('rkey',form1.rkey556,[]);
end;
end;
end;
...
begin
 ...
 form_report2.aq457.Parameters[0].Value:= null;
 form_report2.aq457.Open;
...
begin
 ...
 try
 Form_remaout2:=tForm_remaout2.Create(self);
  with dm.ADOQ do
  begin
   close;
   sql.Clear;
   ...
   while  not eof do
   begin
    Form_remaout2.ComboBox1.Items.Add(fieldbyname('abbr_name').Value);
    next;
   end;
   dm.TemADOQuery1.close;
   dm.TemADOQuery1.sql.clear;
   ...
   //form_remain.ComboBox1.ItemIndex:=form_remain.ComboBox1.Items.IndexOf('深圳双面'); //表示深圳双面
   //form_remain.key15.Caption:='1';//表示深圳双面的主键值
   Form_remaout2.ComboBox1.Text:=dm.TemADOQuery1.fieldbyname('abbr_name').AsString;
   Form_remaout2.key15.Caption:=dm.TemADOQuery1.fieldbyname('RKEY15').AsString;
  end;
 with dm.ADOQ do
  begin
   close;
   ...
   while not eof do
   begin
    Form_remaout2.ComboBox2.Items.Add(fieldbyname('location').Value);
    next;
   end;
   ...
   //form_remain.ComboBox2.ItemIndex:=form_remain.ComboBox2.Items.IndexOf('铜板仓');
   //form_remain.key16.Caption:='1';    //表示铜板仓的位置主键值
  Form_remaout2.ComboBox2.Text:=dm.TemADOQuery1.fieldbyname('location').AsString;
  Form_remaout2.key16.Caption:=dm.TemADOQuery1.fieldbyname('RKEY16').AsString;
  end;
  with dm.ADOQ do
  begin
   close;
   ...
   sql.Add('where rkey= 23');
   open;
   Form_remaout2.Edit1.Text:=fieldbyname('seed_value').Value;
  end;
  Form_remaout2.ShowModal;
finally
Form_remaout2.Free;
end;
end;
