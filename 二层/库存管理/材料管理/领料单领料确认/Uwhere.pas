...
var i :Integer;
begin
 v_Field_List_CN:=VarArrayOf(['领料单号','申请部门']);
 v_Field_List_EN:=VarArrayOf(['dbo.DATA0268.number','dbo.Data0034.DEPT_NAME']);
...
begin
  ...
  try
    ...
    begin
     InputVar.Fields := 'number/领料单号/100,chinesename/领料人姓名/150,DEPT_NAME/部门名称/150';
     InputVar.Sqlstr :='SELECT  dbo.DATA0268.rkey, dbo.DATA0268.number, dbo.DATA0268.DATE,  employeemsg.chinesename,'+
                      'employeemsg.employeecode, dbo.Data0034.DEPT_NAME, dbo.DATA0268.EMPL_PTR,'+
                      'dbo.DATA0268.ConfMan FROM  dbo.DATA0268 INNER JOIN '+
                      'dbo.Data0034 ON dbo.DATA0268.dept_ptr = dbo.Data0034.RKEY LEFT OUTER JOIN '+
                      'employeemsg ON dbo.DATA0268.ConfMan = employeemsg.employeecode ORDER BY rkey DESC' ;
     inputvar.KeyField := 'rkey';
     end;
    if ListBox1.ItemIndex=1 then
    begin
     InputVar.Fields := 'DEPT_NAME/部门名称/200,DEPT_CODE/部门代码/250';
     InputVar.Sqlstr :='SELECT     DEPT_NAME, DEPT_CODE, RKEY FROM  dbo.Data0034' ;
     inputvar.KeyField := 'rkey';
     end;
    ...
    begin
     case ListBox1.ItemIndex of
      0:
      begin
       Edit1.Text:= frmPick_Item_Single.adsPick.fieldbyname('number').AsString;
      end;
      1:
      begin
       Edit1.Text:= frmPick_Item_Single.adsPick.fieldbyname('DEPT_NAME').AsString;
      end;
      2:
      begin
