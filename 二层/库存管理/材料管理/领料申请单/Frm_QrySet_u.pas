...
begin
  ...
  begin
    s:='';
    if chk1.Checked then s:='0';
    if chk2.Checked then begin if s='' then s:='1' else s:=s+',1'; end;
    if chk3.Checked then begin if s='' then s:='2' else s:=s+',2'; end;
    if s='' then exit;
    sgrid1.Cells[1,sgrid1.RowCount-1]:=s;
    sgrid1.Cells[2,sgrid1.RowCount-1]:=' and '+v_Field_List_EN[ListBox3.ItemIndex]+' in ('+s+') ';
  end else
...
begin
  ...
  Button3.Enabled:= listbox3.ItemIndex in [2,5];
  edtValue.Visible := not listbox3.ItemIndex in [2,5];
  GroupBox1.Visible:= listbox3.ItemIndex = 2;
  grp1.Visible:= listbox3.ItemIndex = 5;
  edtValue.Visible := listbox3.ItemIndex in [0,1,3,4];
  edtValue.Text:='';
...
var i:integer;
begin
  v_Field_List_CN := VarArrayOf(['申领单号','部门代码','审核状态','申领员工','工厂代码','确认状态']);
  v_Field_List_EN := VarArrayOf(['data0268.number','data0034.dept_code','data0268.status','data0005.employee_name','data0015.warehouse_code','data0268.allow_submit']);
