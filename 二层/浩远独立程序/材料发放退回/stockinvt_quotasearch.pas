...
uses damo, main, purout_invt, putoutinvt_quota, putoutinvt_linliao, DB;
...
begin
  ...
  begin
    ...
    begin            //   如果在复制的数据中有有效期小于 aq22的记录 ，给予提示
      if (FieldByName('EXPIRE_DATE').AsDateTime<DM.aq22.FieldByName('EXPIRE_DATE').AsDateTime) and (DM.aq22.FieldByName('EXPIRE_DATE').AsDateTime>0)  then
      begin
  ...
  if dm.aq22.IsEmpty=False then
  if (dm.aq22EXPIRE_DATE.Value >= form1.sys_sortdate) or (dm.aq22EXPIRE_DATE.Value=0) then
  begin
