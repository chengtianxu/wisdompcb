...
//    user_ptr:string;//05指针
    ...
    normal_margin,return_margin,margin,part_priceFlag:integer; //正常欠数,退货欠数,最终欠数的变量
    orig_unit_weight , part_priceTmp :Double ;//原始单重 , 价格
...
implementation
uses constvar,pick_item_single, damo,common, note, DB, stock_search;
...
begin
 dm.aqtmp.Close;
 dm.aqtmp.SQL.Text:='select rkey from data0064 where so_ptr='+inttostr(rkey60);
 dm.aqtmp.Open;
 edit1.Text:=inttostr(dm.aqtmp.RecordCount+1);
end;
...
begin
 ...
 Edit30.Text := dm.ADS64RMA_NUMBER.value;
 Edit30.Tag := dm.ADS64Rkey98.value;
 Label35.Caption := dm.ADS64NOTEPAD.value;
 Label35.Tag :=  dm.ADS64NOTE_PAD_PTR.Value;
...
//   old_ship:=old_ship+fieldvalues['quan_shp'];
   next;
  end;   //插入指派明细到表格里
 ...
 old_ship:=dm.ADS64quan_shipped.Value;
...
//margin:=margin+dm.ADS64quan_shipped.Value;
end;
...
begin
 ...
 try
      case (sender as Tbitbtn).Tag of
          ...
          3:
          begin
           inputvar.Fields:='sales_order/销售订单/90,sch_date/回复交期/80,cust_code/客户代码/50,manu_part_number/本厂编号/80,'+
             'manu_part_desc/客户型号/100,set_qty/交货单元数/20,parts_ordered/订单数量/60,total_shipped/装运数量/60,parts_margin/订单欠数/60,'+
             'po_number/客户订单号/80,v_type/加工形式/40,abbrname23/供应商/50,reference_number/订单参考/60,ABBR_NAME/工厂/50,STATUS60/订单状态/60';
           ...
           inputvar.Sqlstr:='select d25.rkey,d25.manu_part_number,d25.manu_part_desc,d25.report_unit_value1,'+
             ...
             'd60.parts_ordered,d60.parts_shipped,d60.parts_returned,d60.returned_ship,'+
             'D60.parts_shipped-D60.parts_returned+D60.returned_ship as total_shipped,'+
             ...
             'when 2 then ''暂缓'' when 3 then ''关闭'' end as STATUS60 '+
  ...
  with frmpick_item_single.adsPick do
   case (sender as Tbitbtn).Tag of
        ...
        begin
             ...
             edit5.Text:=FieldValues['sales_order'];
             edit6.Text:=FieldValues['po_number'];
             edit7.Text:=FieldValues['ent_date'];
             edit9.Text:=FieldValues['sch_date'];
             edit10.Text:=FieldValues['parts_ordered'];
             edit11.Text:=FieldValues['total_shipped']; //总共装运数
             edit12.Text:=FieldValues['parts_margin'];  //总欠数
             edit13.Text:=Fieldvalues['qty_on_hand'];
             edit15.Text:=FieldValues['report_unit_value1'];
             orig_unit_weight:=FieldValues['report_unit_value1'];
             edit17.Text:=FieldValues['po_date'];
             edit19.Text:=FieldValues['shipping_method'];
             edit18.Text:=FieldValues['location'];
             edit23.Text:=fieldvalues['v_kind'];        //内销,转厂
             edit8.Text:= fieldvalues['v_type'];        //加工形式
             Edit28.Text:=FieldValues['reference_number'];
             if fieldbyname('abbrname23').AsString<>'' then
              edit8.Text:=edit8.Text+':'+fieldvalues['abbrname23'];

             edit26.Text:=fieldvalues['abbr_name'];  //工厂
             edit27.Text:=fieldbyname('set_qty').AsString;
             rkey60:=FieldValues['rkey60'];
             //dm.AQ64part_price.Value:=FieldValues['part_price'];      //价格
             part_priceTmp :=  FieldValues['part_price'];  //价格全局变量
             dm.AQ64prod_tax_flag.Value :=FieldValues['state_prod_tax_flag']; //产品征税
             dm.AQ64tool_tax_flag.Value :=FieldValues['state_tool_tax_flag']; //工具征税
             dm.AQ64discount.Value :=FieldValues['discount'];          //折扣
             dm.AQ64TAX_2.Value:=FieldValues['rush_charge'];           //增值税
             ...
             get_ship_no;
             get_qty_cust_order(rkey25,FieldValues['purchase_order_ptr']);
...
// if (assign_type.ItemIndex=1) and (margin<strtoint(edit14.Text)) then
  ...
  if (assign_type.ItemIndex = 1) and (Edit30.Text = '')  then
  begin
    showmessage('您还未选择相应的退货编号，请选择');
    result:=false;
    exit;
  end;

  if (assign_type.ItemIndex = 0) and (Edit30.Text <> '')  then
  begin
    showmessage('您选择的是正常指派，无需选择退货编号，系统将自动删除');
    Edit30.Text := '';
    Edit30.Tag := 0;
    Label35.Caption:='';
    result:=false;
...
begin
  ...
  begin
    ...
    begin
      ...
      dm.AQ64RMA_PTR.AsInteger := Edit30.Tag;
      if (assign_type.ItemIndex = 1) and (Edit30.Text <> '') and ( Label35.Tag = 1 ) then
       begin
         dm.AQ64part_price.Value:=0;
         part_priceFlag := 1;
       end
      else
       begin
           dm.AQ64part_price.Value:= part_priceTmp ;
       end;
...
procedure Tfm_detail.BitBtn4Click(Sender: TObject);
var inputvar: PDlgInput; //搜索
begin
    frmpick_item_single:=Tfrmpick_item_single.Create(application);
    try
    inputvar.Fields:='RMA_NUMBER/RMA编号/80,RMA_DATE/退货日期/80,NOTEPAD/是否扣款/60,CUST_CODE/客户代码/40,'+
             'MANU_PART_NUMBER/本厂编号/80,MANU_PART_DESC/客户型号/120,SALES_ORDER/订单编号/90,QTY_RECD/退货数量/50,'+
             'ABBR_NAME/客户名称/80,warehouse_name/工厂/60,rmatype/退货类型/70,';
              if rkey10=0 then
              begin
                   inputvar.Sqlstr:='  SELECT    dbo.Data0098.Rkey,   dbo.Data0098.RMA_NUMBER, dbo.Data0098.RMA_DATE,                                             '+#13+
                              '          CASE dbo.Data0098.rma_type WHEN 0 THEN '''+' 客户退货'+''' WHEN 1 THEN '''+'主动召回'+''' END AS rmatype,       '+#13+
                              '          dbo.Data0025.MANU_PART_NUMBER,                                                                  '+#13+
                              '          dbo.Data0025.MANU_PART_DESC, dbo.Data0060.SALES_ORDER, dbo.Data0098.QTY_RECD,                   '+#13+
                              '          dbo.Data0010.CUST_CODE, dbo.Data0010.ABBR_NAME,                                                 '+#13+
                              '          CASE WHEN data0098.so_ptr IS NULL THEN data0015.abbr_name ELSE data0015_1.abbr_name END AS      '+#13+
                              '          warehouse_name, dbo.Data0098.NOTE_PAD_PTR,                                                       '+#13+
                              '          CASE dbo.Data0098.NOTE_PAD_PTR WHEN 0 THEN '''+'需扣款'+''' WHEN 1 THEN '''+'无需扣款'+''' END AS NOTEPAD       '+#13+
                              '   FROM         dbo.Data0098 LEFT OUTER JOIN                                                              '+#13+
                              '          dbo.Data0060 ON dbo.Data0098.SO_PTR = dbo.Data0060.RKEY INNER JOIN                              '+#13+
                              '          dbo.Data0010 ON dbo.Data0098.CUSTOMER_PTR = dbo.Data0010.RKEY INNER JOIN                        '+#13+
                              '          dbo.Data0025 ON dbo.Data0098.CUSTOMER_PART_PTR = dbo.Data0025.RKEY INNER JOIN                   '+#13+
                              '          dbo.Data0015 ON dbo.Data0025.PROD_ROUTE_PTR = dbo.Data0015.RKEY LEFT OUTER JOIN                 '+#13+
                              '          dbo.Data0015 AS Data0015_1 ON dbo.Data0060.SHIP_REG_TAX_ID = Data0015_1.RKEY                    '+#13+
                              '          where dbo.Data0025.parent_ptr is null order by dbo.Data0098.RMA_NUMBER DESC                  '+#13;
              end else begin
                    inputvar.Sqlstr:='  SELECT    dbo.Data0098.Rkey,  dbo.Data0098.RMA_NUMBER, dbo.Data0098.RMA_DATE,                                             '+#13+
                              '          CASE dbo.Data0098.rma_type WHEN 0 THEN '''+'客户退货'+''' WHEN 1 THEN '''+'主动召回'+''' END AS rmatype,       '+#13+
                              '          dbo.Data0025.MANU_PART_NUMBER,                                                                  '+#13+
                              ...
                              '          dbo.Data0015 ON dbo.Data0025.PROD_ROUTE_PTR = dbo.Data0015.RKEY LEFT OUTER JOIN                 '+#13+
                              '          dbo.Data0015 AS Data0015_1 ON dbo.Data0060.SHIP_REG_TAX_ID = Data0015_1.RKEY                    '+#13+
                              '   WHERE dbo.Data0025.parent_ptr is null  AND    dbo.Data0098.CUSTOMER_PTR =  '+inttostr(rkey10)+'        '+#13+
                              '   ORDER BY dbo.Data0098.RMA_NUMBER DESC                  '+#13;
              end;
              inputvar.KeyField:='RMA_NUMBER';
              inputvar.InPut_value:=edit30.Text;
      inputvar.AdoConn:=dm.ADOConnection1;
      frmpick_item_single.InitForm_New(inputvar);
      frmpick_item_single.ShowModal;
      if (frmpick_item_single.ModalResult=mrok) and (not frmpick_item_single.adsPick.IsEmpty) then
      with frmpick_item_single.adsPick do
      begin
          edit30.Text:=trim(FieldValues['RMA_NUMBER']);
          edit30.Tag:=fieldbyname('Rkey').AsInteger;
          Label35.Caption:= trim(FieldValues['NOTEPAD']);
          Label35.Tag := fieldbyname('NOTE_PAD_PTR').AsInteger;
      end;
