...
type
  Tdm = class(TDataModule)
    ...
    ADS64RMA_NUMBER: TStringField;
    ADS64Rkey98: TAutoIncField;
    ADS64NOTEPAD: TStringField;
    ADS64NOTE_PAD_PTR: TIntegerField;
    procedure AQ64BeforePost(DataSet: TDataSet);
  private
...
procedure Tdm.AQ64BeforePost(DataSet: TDataSet);
begin
  if fm_detail.part_priceFlag <> 1 then
  begin
    with aqtmp do
    begin
      Close;
      SQL.Text:= 'select part_price from data0060 where RKEY = ' + AQ64.FieldByName('SO_PTR').AsString;
      Open;
    end;
    if aqtmp.FieldByName('part_price').AsFloat <> AQ64.FieldByName('part_price').AsFloat then
    begin
