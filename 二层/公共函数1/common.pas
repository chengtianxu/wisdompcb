...
function getmaxnokp_date(Query: TAdoQuery): tdatetime; //返回财务未开票截数的最大日期
...
function GetVerInfo(const FileName: string; var info: TFileInfo):boolean;
...
function Update_Seed(rKey: integer; v_Seed: string): boolean; overload;//更新自动增长的字符
function Update_Seed(rKey: integer): boolean;overload; //更新自动增长的字符
...
function GetServerRegion(ACon: TADOConnection): string;
  ...
  begin
    ...
    for I := pos('Data Source=',ACon.ConnectionString) to Length(ACon.ConnectionString) do
    begin
...
function getmaxnokp_date(Query: TAdoQuery): tdatetime;
begin      //返回财务未开票截数的最大日期
    Query.Close;
    query.SQL.Text := 'SELECT isnull(MAX(cut_date),0) AS MATL_DATE from data0162';
    Query.Open;
    result := query.FieldValues['MATL_DATE']
end;
...
begin
  try
    ...
    try
      Lqry.Connection := AdoConn;
      Lqry.SQL.Clear;
      Lqry.SQL.Add('select EMPLOYEE_PTR from data0073 where rkey = ' + rkey73);
      Lqry.Open;
      user_ptr := Lqry.FieldByName('EMPLOYEE_PTR').AsString;
      Lqry.Close;
    finally
      Lqry.Free;
    end;
...
function Update_Seed(rKey: integer; v_Seed: string): boolean;overload; //更新
var
  i, v_length: integer;
...
function Update_Seed(rKey: integer): boolean;overload; //更新
var
  i, v_length: integer;
  v_Seed: string;
  v_last, new_seed, sqlstr: string;
...
begin
  sqlstr := 'select Seed_Value from data0004 where  rKey=' + IntToStr(rKey);
  v_Seed := MyDataClass.Get_Sql_Value(sqlstr);
