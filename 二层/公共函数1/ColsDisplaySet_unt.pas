...
begin
  ...
  begin
    ...
    begin
      ...
      if  AGridEh.Columns[i].Field<>nil then   //20141118添加，当某列没有对应的字段的时候，程序出错。
      begin
        cdsLabel.Append;
    ...
    cdsColsSetM.Filtered:=True;
    i_heg:=( AGridEh.Columns.Count+1 )*17+100;
    if  i_heg < 300 then
    i_heg:=300;
    if  i_heg > 600 then
    i_heg:=600;
    
    ColsDisplaySet_frm.Height:= i_heg;
    ColsDisplaySet_frm.Top :=round( ( SetForm.Height-i_heg)/2)+SetForm.Top  ;
    ColsDisplaySet_frm.Left:=round( ( SetForm.Width -ColsDisplaySet_frm.Width)/2)+SetForm.Left;
...
begin
  ...
  begin
    ...
    begin
      ...
      with   cds_Grid do
      begin
        FieldDefs.Add('FormName',ftstring,50);
        FieldDefs.Add('GridName',ftstring,50);
        FieldDefs.Add('FrozenField',ftstring,50);
        CreateDataSet;                     //建立数据集
        Open;
      end;
      with   cds_ColsOriginal do
      begin
        FieldDefs.Add('FormName',ftstring,50);
        FieldDefs.Add('GridName',ftstring,50);
        FieldDefs.Add('FieldName',ftstring,50);
        FieldDefs.Add('DisplayOrder',ftInteger);
        FieldDefs.Add('DisplayWidth',ftInteger);
        FieldDefs.Add('IsDisplay',ftBoolean);
        IndexFieldNames:='FormName;GridName';
        CreateDataSet;                     //建立数据集
        Open;
      end;
    end;
...
procedure TColsDisplaySet_frm.Button1Click(Sender: TObject);
begin
  ShowMessage(IntToStr(DBGridEh1.RowHeight ));
end;
