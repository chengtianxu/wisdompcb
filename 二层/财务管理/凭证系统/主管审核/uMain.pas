...
//审核更新
function Tmain.SHUpdate():boolean;
begin
 try
  ...
  dm.ADOQ105.Edit;
  dm.ADOQ105status.Value:=2;
  dm.ADOQ105supervisor_ptr.Value:=strtoint(user_ptr);
  dm.ADOQ105.Post;
 ...
 try
  for i:=1 to DBGrid1.SelectedRows.Count do
    begin
      ...
      if dm.ADOQ105.FieldByName('STATUS').asinteger<>2 then
        begin
         Continue;
        end;
      dm.ADOQ105.Edit;
      dm.ADOQ105status.Value:=1;
      dm.ADOQ105supervisor_ptr.AsVariant:=null;
      dm.ADOQ105.Post;
    end;
 except
   messagedlg('数据提交失败，请稍后重新尝试！',mtinformation,[mbok],0);
...
begin
  ...
  begin
    ...
    FrmAcctNote.stringgrid1.Cells[1,FrmAcctNote.stringgrid1.RowCount-2]:=trim(Fieldbyname('gl_acc_number').asstring)+'-'+trim(Fieldbyname('Cgl_Acc').Value);
...
begin
 ...
 with dm.ADOQ105 do
    case FieldByName('STATUS').asinteger of
      ...
      1:n2.Enabled :=true;//'会计已审';
      2:n2.Enabled :=false;//'主管已审';
      3:n2.Enabled :=false;//'已登帐';
      4:n2.Enabled :=false;//'出纳已审';
    end;
 //取消审核
  with dm.ADOQ105 do
    case FieldByName('STATUS').asinteger of
