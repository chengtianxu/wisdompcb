...
begin
 try
  ...
  dm.ADOQ105casher_ptr.Value:=strtoint(user_ptr);
  dm.ADOQ105.Post;
  Result :=true;
 except
...
//  DM.ADOQ105.Open ;
  ...
  SpinEdit1.OnChange:= CheckBox2Click;
  SpinEdit2.OnChange:= CheckBox2Click;
 ...
 try
  ...
  for i:=1 to DBGrid1.SelectedRows.Count do
   begin
    ...
    if dm.ADOQ105.FieldByName('STATUS').asinteger<>4 then
      begin
       Continue;
      end;
      dm.ADOQ105.Edit;
      dm.ADOQ105status.Value:=0;
      dm.ADOQ105casher_ptr.AsVariant:=null;
      dm.ADOQ105.Post;
   end;
 except
   messagedlg('数据提交失败，请稍后重新尝试！',mtinformation,[mbok],0);
...
begin
  ...
  begin
    ...
    FrmAcctNote.stringgrid1.Cells[3,FrmAcctNote.stringgrid1.RowCount-2]:=formatfloat('0.0000',1/Fieldbyname('EXCH_RATE').Value);
 ...
 //取消审核
  with dm.ADOQ105 do
    case FieldByName('STATUS').asinteger of
      ...
      1:n3.Enabled :=false;//'会计已审';
      2:n3.Enabled :=false;//'主管已审';
      3:n3.Enabled :=false;//'已登帐';
      4:n3.Enabled :=true;//'出纳已审';
    end;
...
for i:=low(v_colwidth) to high(v_colwidth) do
  dbgrid1.Columns[i].Width:=v_colwidth[i]+round((dbgrid1.Width-vWidth)*v_colwidth[i]/(vWidth-30));
for i:=low(v_colwidth1) to high(v_colwidth1) do
  dbgrid2.Columns[i].Width:=v_colwidth1[i]+round((dbgrid2.Width-vWidth)*v_colwidth1[i]/(vWidth-50));
 ...
 with DM.ADOQ105 do
  begin
   ...
   if RadioGroup1.ItemIndex=0 then
     sql.Insert(sql.Count-1,'and data0105.status=0')
   else
     sql.Insert(sql.Count-1,'and data0105.status=4');

   //业务类型
  if combobox1.ItemIndex<>combobox1.Items.Count-1 then  //不是最后一行
   begin
    sql.Delete(sql.Count-3);
    sql.Insert(sql.Count-2,'and data0105.TRANS_TYPE='+inttostr(combobox1.ItemIndex+1));
   end
  else
   begin
    sql.Delete(sql.Count-3);
