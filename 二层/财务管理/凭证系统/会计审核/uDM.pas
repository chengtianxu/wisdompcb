...
type
  TDM = class(TDataModule)
    ...
    ADOQ105ENTERED_BY_EMPL_PTR: TIntegerField;
 ...
 with ADOQ105 do
  begin
    case FieldByName('vou_type').asinteger of
      ...
      4:FieldByName('ctype').asstring:='特殊转帐账证';
    end;
...
if dm.ADODS106.FieldByName('pGL_DESCRIPTION').AsVariant<>null then
  dm.ADODS106Cgl_Acc.AsString :=trim(dm.ADODS106.FieldByName('pGL_DESCRIPTION').AsString) +'-'+trim(dm.ADODS106.FieldByName('GL_DESCRIPTION').AsString)
ELSE
 ...
 if dm.ADODS106.FieldByName('tGL_DESCRIPTION').AsVariant <>null then
  dm.ADODS106Cgl_Acc.AsString :=trim(dm.ADODS106.FieldByName('tGL_DESCRIPTION').AsString) +'-'+trim(dm.ADODS106Cgl_Acc.AsString);
