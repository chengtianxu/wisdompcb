...
if self.find_error(trim(edit1.Text)) then
 ...
 try
   ...
   dm.ADO132MATL_TOT.Value:=strtofloat(edit9.Text);   //货款净值
   dm.ADO132FED_TAX.Value:=strtofloat(edit10.Text);    //税金
   dm.ADO132MISC_TOT.Value:=strtofloat(edit7.Text);   //杂项费用
...
begin
 ...
 Edit_employee.Text:=form1.empl_name.Caption;
  ...
  mt_error:=true;
  if strtocurr(edit9.Text)>0 then    //贷方不含税金额
   with dm.ADOQuery1 do
   begin
    close;
    sql.Clear;
  ...
  mt_error:=true;
  if strtocurr(edit10.Text)>0 then    //贷方增值税金
   with dm.ADOQuery1 do
   begin
    close;
    sql.Clear;
  ...
  mt_error:=true;
  if strtocurr(edit7.Text)>0 then    //贷方杂项费用
   with dm.ADOQuery1 do
   begin
    close;
    sql.Clear;
