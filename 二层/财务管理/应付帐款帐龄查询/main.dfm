object Form1: TForm1
  Left = 201
  Top = 139
  Width = 996
  Height = 603
  Caption = #24212#20184#24080#27454#24080#40836#26597#35810
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -13
  Font.Name = #23435#20307
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  WindowState = wsMaximized
  OnCreate = FormCreate
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object StringGrid1: TStringGrid
    Left = 0
    Top = 52
    Width = 980
    Height = 497
    Align = alClient
    ColCount = 15
    DefaultRowHeight = 20
    FixedCols = 6
    RowCount = 2
    Options = [goFixedVertLine, goFixedHorzLine, goVertLine, goHorzLine, goColSizing]
    TabOrder = 1
    OnDrawCell = StringGrid1DrawCell
    OnSelectCell = StringGrid1SelectCell
    ColWidths = (
      64
      64
      36
      64
      65
      68
      64
      64
      64
      64
      64
      64
      64
      64
      64)
  end
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 980
    Height = 52
    Align = alTop
    BevelOuter = bvNone
    TabOrder = 0
    object Label4: TLabel
      Left = 175
      Top = 9
      Width = 46
      Height = 13
      Caption = #26399#38388#20174':'
    end
    object Label3: TLabel
      Left = 201
      Top = 34
      Width = 20
      Height = 13
      Caption = #21040':'
    end
    object Label1: TLabel
      Left = 321
      Top = 8
      Width = 46
      Height = 13
      Caption = #20379#24212#21830':'
    end
    object Label2: TLabel
      Left = 334
      Top = 34
      Width = 33
      Height = 13
      Caption = #36135#24065':'
    end
    object Label5: TLabel
      Left = 474
      Top = 8
      Width = 35
      Height = 13
      Caption = '     '
    end
    object Label6: TLabel
      Left = 474
      Top = 32
      Width = 35
      Height = 13
      Caption = '     '
    end
    object abbr_name: TLabel
      Left = 664
      Top = 1
      Width = 63
      Height = 13
      Caption = 'abbr_name'
      Visible = False
    end
    object exch_rate: TLabel
      Left = 473
      Top = 36
      Width = 63
      Height = 13
      Caption = 'exch_rate'
      Visible = False
    end
    object tax_id_no: TLabel
      Left = 723
      Top = 5
      Width = 63
      Height = 13
      Caption = 'tax_id_no'
      Visible = False
    end
    object Label7: TLabel
      Left = 792
      Top = 33
      Width = 33
      Height = 13
      Caption = #24037#21378':'
    end
    object ANALYSIS_CODE1: TLabel
      Left = 553
      Top = 5
      Width = 98
      Height = 13
      Caption = 'ANALYSIS_CODE1'
      Visible = False
    end
    object Label8: TLabel
      Left = 790
      Top = 9
      Width = 33
      Height = 13
      Caption = #31246#31080':'
    end
    object Edit1: TEdit
      Left = 369
      Top = 5
      Width = 68
      Height = 21
      MaxLength = 10
      TabOrder = 4
      OnExit = Edit1Exit
    end
    object dtpk1: TDateTimePicker
      Left = 223
      Top = 6
      Width = 96
      Height = 21
      Date = 38847.000000000000000000
      Time = 38847.000000000000000000
      TabOrder = 0
      OnCloseUp = dtpk1Exit
      OnExit = dtpk1Exit
    end
    object dtpk2: TDateTimePicker
      Left = 223
      Top = 30
      Width = 96
      Height = 21
      Date = 38727.000000000000000000
      Time = 38727.000000000000000000
      TabOrder = 1
    end
    object BitBtn1: TBitBtn
      Left = 2
      Top = 2
      Width = 57
      Height = 30
      Hint = #36864#20986#31995#32479
      Caption = #36864#20986
      ParentShowHint = False
      ShowHint = True
      TabOrder = 8
      OnClick = BitBtn1Click
      Glyph.Data = {
        
        0000}
      NumGlyphs = 2
    end
    object Edit2: TEdit
      Left = 369
      Top = 30
      Width = 68
      Height = 21
      MaxLength = 5
      TabOrder = 5
      OnExit = Edit2Exit
    end
    object BitBtn6: TBitBtn
      Left = 443
      Top = 3
      Width = 25
      Height = 25
      TabOrder = 2
      OnClick = BitBtn6Click
      Glyph.Data = {
        
        3333333333373733333333333333303333333333333373333333}
      NumGlyphs = 2
    end
    object BitBtn2: TBitBtn
      Left = 443
      Top = 27
      Width = 25
      Height = 25
      TabOrder = 3
      OnClick = BitBtn2Click
      Glyph.Data = {
        
        3333333333373733333333333333303333333333333373333333}
      NumGlyphs = 2
    end
    object BitBtn4: TBitBtn
      Left = 116
      Top = 2
      Width = 57
      Height = 30
      Hint = #24320#22987#25353#26465#20214#26597#35810
      Caption = #26597#35810
      ParentShowHint = False
      ShowHint = True
      TabOrder = 6
      OnClick = BitBtn4Click
      Glyph.Data = {
        
        861D8680775398535355}
    end
    object BitBtn3: TBitBtn
      Left = 59
      Top = 2
      Width = 57
      Height = 30
      Hint = #23548#20986#25968#25454#21040'Excel'
      Caption = #23548#20986
      ParentShowHint = False
      ShowHint = True
      TabOrder = 7
      OnClick = BitBtn3Click
      Glyph.Data = {
        
        70DA0FFFF0ADA0EEEE0D00000ADADA00000AADADADADADADADAD}
    end
    object CheckBox1: TCheckBox
      Left = 543
      Top = 32
      Width = 97
      Height = 17
      Caption = #26174#31034#20998#26399#24080#40836
      TabOrder = 9
      OnClick = CheckBox1Click
    end
    object RadioGroup1: TRadioGroup
      Left = 642
      Top = 21
      Width = 148
      Height = 31
      Columns = 2
      ItemIndex = 0
      Items.Strings = (
        #35760#24080#27719#29575
        #28014#21160#27719#29575)
      TabOrder = 10
    end
    object ComboBox1: TComboBox
      Left = 829
      Top = 28
      Width = 121
      Height = 21
      Style = csDropDownList
      ItemHeight = 13
      TabOrder = 11
    end
    object ComboBox2: TComboBox
      Left = 830
      Top = 5
      Width = 120
      Height = 21
      Style = csDropDownList
      ItemHeight = 13
      ItemIndex = 0
      TabOrder = 12
      Text = #20840#37096
      Items.Strings = (
        #20840#37096
        #26410#24320#31080
        #24050#24320#31080)
    end
  end
  object StringGrid2: TStringGrid
    Left = 776
    Top = 160
    Width = 177
    Height = 345
    ColCount = 2
    FixedCols = 0
    RowCount = 2
    TabOrder = 2
    Visible = False
    ColWidths = (
      43
      41)
  end
  object ProgressBar1: TProgressBar
    Left = 0
    Top = 549
    Width = 980
    Height = 16
    Align = alBottom
    TabOrder = 3
  end
  object ADOConnection1: TADOConnection
    LoginPrompt = False
    Provider = 'SQLOLEDB.1'
    Left = 280
    Top = 128
  end
  object ADOQuery1: TADOQuery
    Connection = ADOConnection1
    Parameters = <>
    Left = 408
    Top = 128
  end
  object AQ_balance: TADOQuery
    AutoCalcFields = False
    Connection = ADOConnection1
    CursorType = ctStatic
    Filtered = True
    LockType = ltReadOnly
    Parameters = <>
    Prepared = True
    SQL.Strings = (
      'SELECT SUPP_PTR, CURRENCY_PTR,'
      'SUM(INV_TOTAL - AMT_PAID - CASH_DISC) AS '#24403#21069#20313#39069','
      
        'ROUND(SUM((INV_TOTAL - AMT_PAID - CASH_DISC) * EX_RATE), 2) AS '#24403 +
        #21069#20313#39069#26412#24065','
      
        'ROUND(SUM((INV_TOTAL - AMT_PAID - CASH_DISC) * BASE_TO_OTHER), 2' +
        ') AS '#24403#21069#20313#39069#35760#24080#26412#24065
      'FROM Data0107 inner join data0001'
      'on data0107.CURRENCY_PTR = data0001.rkey'
      'where INV_TOTAL - AMT_PAID - CASH_DISC > 0  and STATUS=1'
      ''
      ''
      'GROUP BY SUPP_PTR, CURRENCY_PTR'
      'UNION'
      'SELECT SUPP_PTR, CURRENCY_PTR,'
      '      -SUM(AVL_AMT) AS '#24403#21069#20511#39033','
      '      -ROUND(SUM(AVL_AMT * EX_RATE), 2) AS '#24403#21069#20511#39033#26412#24065','
      '      -ROUND(SUM(AVL_AMT * BASE_TO_OTHER), 2) AS '#24403#21069#20511#39033#35760#24080#26412#24065
      'FROM Data0132 inner join data0001'
      'on data0132.CURRENCY_PTR = data0001.rkey'
      'where avl_amt > 0 and (memo_status <> 3)'
      ''
      ''
      'GROUP BY SUPP_PTR, CURRENCY_PTR')
    Left = 592
    Top = 208
    object AQ_balanceSUPP_PTR: TIntegerField
      FieldName = 'SUPP_PTR'
    end
    object AQ_balanceCURRENCY_PTR: TIntegerField
      FieldName = 'CURRENCY_PTR'
    end
    object AQ_balanceDSDesigner: TBCDField
      FieldName = #24403#21069#20313#39069
      ReadOnly = True
      Precision = 19
    end
    object AQ_balanceDSDesigner2: TFloatField
      FieldName = #24403#21069#20313#39069#35760#24080#26412#24065
      ReadOnly = True
    end
    object AQ_balanceDSDesigner3: TFloatField
      FieldName = #24403#21069#20313#39069#26412#24065
      ReadOnly = True
    end
  end
  object aq_lengrece: TADOQuery
    AutoCalcFields = False
    Connection = ADOConnection1
    CursorType = ctStatic
    Filtered = True
    LockType = ltReadOnly
    Parameters = <
      item
        Name = 'dtpk1'
        DataType = ftDateTime
        Precision = 16
        Size = 16
        Value = Null
      end
      item
        Name = 'dtpk2'
        DataType = ftDateTime
        Precision = 16
        Size = 16
        Value = Null
      end>
    Prepared = True
    SQL.Strings = (
      'SELECT SUPP_PTR, CURRENCY_PTR, SUM(INV_TOTAL) AS '#26412#26399#24212#20184','
      '      ROUND(SUM(INV_TOTAL * EX_RATE), 2) AS '#26412#26399#24212#20184#26412#24065','
      '      ROUND(SUM(INV_TOTAL * base_to_other), 2) AS '#26412#26399#24212#20184#35760#24080#26412#24065
      'FROM Data0107 inner join data0001'
      'on data0107.CURRENCY_PTR = data0001.rkey'
      'WHERE (INV_DATE >= :dtpk1) AND'
      '      (INV_DATE <= :dtpk2) and data0107.status<>3'
      ''
      'GROUP BY SUPP_PTR, CURRENCY_PTR')
    Left = 152
    Top = 208
    object aq_lengreceSUPP_PTR: TIntegerField
      FieldName = 'SUPP_PTR'
    end
    object aq_lengreceCURRENCY_PTR: TIntegerField
      FieldName = 'CURRENCY_PTR'
    end
    object aq_lengreceDSDesigner: TBCDField
      FieldName = #26412#26399#24212#20184
      Precision = 19
    end
    object aq_lengreceDSDesigner2: TFloatField
      FieldName = #26412#26399#24212#20184#35760#24080#26412#24065
      ReadOnly = True
    end
    object aq_lengreceDSDesigner3: TFloatField
      FieldName = #26412#26399#24212#20184#26412#24065
      ReadOnly = True
    end
  end
  object aq_lengyirece: TADOQuery
    AutoCalcFields = False
    Connection = ADOConnection1
    CursorType = ctStatic
    Filtered = True
    LockType = ltReadOnly
    Parameters = <
      item
        Name = 'dtpk1'
        DataType = ftDateTime
        Precision = 16
        Size = 16
        Value = Null
      end
      item
        Name = 'dtpk2'
        DataType = ftDateTime
        Precision = 16
        Size = 16
        Value = Null
      end>
    Prepared = True
    SQL.Strings = (
      'SELECT SRCE_PTR, CURR_PTR, SUM(AMOUNT) AS '#26412#26399#20184#27454','
      '      ROUND(SUM(AMOUNT * EX_RATE), 2) AS '#26412#26399#20184#27454#26412#24065','
      '      ROUND(SUM(AMOUNT * BASE_TO_OTHER), 2) AS '#26412#26399#20184#27454#35760#24080#26412#24065
      'FROM Data0129 inner join data0001'
      'on data0129.curr_ptr=data0001.rkey'
      
        'WHERE (TTYPE <> 3) AND (TTYPE <> 4) AND (TTYPE <> 2) AND (TTYPE ' +
        '<> 6)'
      
        'and   (Data0129.TDATE >= :dtpk1) AND (Data0129.TDATE <= :dtpk2) ' +
        'and'
      '      (status<>3) and (AMOUNT>0)'
      ''
      'GROUP BY SRCE_PTR, CURR_PTR')
    Left = 232
    Top = 208
    object aq_lengyireceSRCE_PTR: TIntegerField
      FieldName = 'SRCE_PTR'
    end
    object aq_lengyireceCURR_PTR: TIntegerField
      FieldName = 'CURR_PTR'
    end
    object aq_lengyireceDSDesigner: TBCDField
      FieldName = #26412#26399#20184#27454
      Precision = 19
    end
    object aq_lengyireceDSDesigner2: TFloatField
      FieldName = #26412#26399#20184#27454#35760#24080#26412#24065
      ReadOnly = True
    end
    object aq_lengyireceDSDesigner3: TFloatField
      FieldName = #26412#26399#20184#27454#26412#24065
      ReadOnly = True
    end
  end
  object AQ_lengcredit: TADOQuery
    AutoCalcFields = False
    Connection = ADOConnection1
    CursorType = ctStatic
    Filtered = True
    LockType = ltReadOnly
    Parameters = <
      item
        Name = 'dtpk1'
        DataType = ftDateTime
        Precision = 16
        Size = 16
        Value = Null
      end
      item
        Name = 'dtpk2'
        DataType = ftDateTime
        Precision = 16
        Size = 16
        Value = Null
      end>
    Prepared = True
    SQL.Strings = (
      'SELECT SUPP_PTR, CURRENCY_PTR, SUM(AMOUNT) AS '#26412#26399#25187#27454','
      '      ROUND(SUM(AMOUNT * EX_RATE), 2) AS '#26412#26399#25187#27454#26412#24065','
      '      ROUND(SUM(AMOUNT * BASE_TO_OTHER), 2) AS '#26412#26399#25187#27454#35760#24080#26412#24065'      '
      'FROM Data0132 inner join data0001'
      'on data0132.CURRENCY_PTR = data0001.rkey'
      'WHERE (memo_status <> 3) and'
      '(MEMO_DATE >= :dtpk1) AND'
      '(MEMO_DATE <= :dtpk2)'
      ''
      'GROUP BY SUPP_PTR, CURRENCY_PTR')
    Left = 304
    Top = 208
    object AQ_lengcreditSUPP_PTR: TIntegerField
      FieldName = 'SUPP_PTR'
    end
    object AQ_lengcreditCURRENCY_PTR: TIntegerField
      FieldName = 'CURRENCY_PTR'
    end
    object AQ_lengcreditDSDesigner: TBCDField
      FieldName = #26412#26399#25187#27454
      Precision = 19
    end
    object AQ_lengcreditDSDesigner2: TFloatField
      FieldName = #26412#26399#25187#27454#35760#24080#26412#24065
      ReadOnly = True
    end
    object AQ_lengcreditDSDesigner3: TFloatField
      FieldName = #26412#26399#25187#27454#26412#24065
      ReadOnly = True
    end
  end
  object aq_finarece: TADOQuery
    AutoCalcFields = False
    Connection = ADOConnection1
    CursorType = ctStatic
    Filtered = True
    LockType = ltReadOnly
    Parameters = <
      item
        Name = 'dtpk2'
        DataType = ftDateTime
        Precision = 16
        Size = 16
        Value = Null
      end>
    Prepared = True
    SQL.Strings = (
      'SELECT SUPP_PTR, CURRENCY_PTR, SUM(INV_TOTAL) AS '#26399#21518#24212#20184','
      '      ROUND(SUM(INV_TOTAL * EX_RATE), 2) AS '#26399#21518#24212#20184#26412#24065','
      '      ROUND(SUM(INV_TOTAL * BASE_TO_OTHER), 2) AS '#26399#21518#24212#20184#35760#24080#26412#24065
      'FROM Data0107 inner join data0001'
      'on data0107.CURRENCY_PTR = data0001.rkey'
      'WHERE (INV_DATE > :dtpk2) and data0107.status<>3'
      ''
      'GROUP BY SUPP_PTR, CURRENCY_PTR')
    Left = 376
    Top = 208
    object aq_finareceSUPP_PTR: TIntegerField
      FieldName = 'SUPP_PTR'
    end
    object aq_finareceCURRENCY_PTR: TIntegerField
      FieldName = 'CURRENCY_PTR'
    end
    object aq_finareceDSDesigner: TBCDField
      FieldName = #26399#21518#24212#20184
      Precision = 19
    end
    object aq_finareceDSDesigner2: TFloatField
      FieldName = #26399#21518#24212#20184#35760#24080#26412#24065
      ReadOnly = True
    end
    object aq_finareceDSDesigner3: TFloatField
      FieldName = #26399#21518#24212#20184#26412#24065
      ReadOnly = True
    end
  end
  object aq_finayirece: TADOQuery
    AutoCalcFields = False
    Connection = ADOConnection1
    CursorType = ctStatic
    Filtered = True
    LockType = ltReadOnly
    Parameters = <
      item
        Name = 'dtpk2'
        DataType = ftDateTime
        Precision = 16
        Size = 16
        Value = Null
      end>
    Prepared = True
    SQL.Strings = (
      'SELECT SRCE_PTR, CURR_PTR, SUM(AMOUNT) AS '#26399#21518#20184#27454','
      '      ROUND(SUM(AMOUNT * EX_RATE), 2) AS '#26399#21518#20184#27454#26412#24065','
      '      ROUND(SUM(AMOUNT * BASE_TO_OTHER), 2) AS '#26399#21518#20184#27454#35760#24080#26412#24065
      'FROM Data0129 inner join data0001'
      'on data0129.CURR_PTR = data0001.rkey'
      
        'WHERE (TTYPE <> 3) AND (TTYPE <> 4) AND (TTYPE <> 2) AND (TTYPE ' +
        '<> 6)'
      'and   (data0129.TDATE > :dtpk2) and (status<>3)'
      '       and (AMOUNT>0)'
      ''
      'GROUP BY SRCE_PTR, CURR_PTR')
    Left = 448
    Top = 208
    object aq_finayireceSRCE_PTR: TIntegerField
      FieldName = 'SRCE_PTR'
    end
    object aq_finayireceCURR_PTR: TIntegerField
      FieldName = 'CURR_PTR'
    end
    object aq_finayireceDSDesigner: TBCDField
      FieldName = #26399#21518#20184#27454
      Precision = 19
    end
    object aq_finayireceDSDesigner2: TFloatField
      FieldName = #26399#21518#20184#27454#35760#24080#26412#24065
      ReadOnly = True
    end
    object aq_finayireceDSDesigner3: TFloatField
      FieldName = #26399#21518#20184#27454#26412#24065
      ReadOnly = True
    end
  end
  object AQ_finacredit: TADOQuery
    AutoCalcFields = False
    Connection = ADOConnection1
    CursorType = ctStatic
    Filtered = True
    LockType = ltReadOnly
    Parameters = <
      item
        Name = 'dtpk2'
        DataType = ftDateTime
        Precision = 16
        Size = 16
        Value = Null
      end>
    Prepared = True
    SQL.Strings = (
      'SELECT SUPP_PTR, CURRENCY_PTR, SUM(AMOUNT) AS '#26399#21518#25187#27454','
      '      ROUND(SUM(AMOUNT * EX_RATE), 2) AS '#26399#21518#25187#27454#26412#24065','
      '      ROUND(SUM(AMOUNT * BASE_TO_OTHER), 2) AS '#26399#21518#25187#27454#35760#24080#26412#24065
      'FROM Data0132 inner join data0001'
      'on data0132.CURRENCY_PTR = data0001.rkey'
      'WHERE (memo_status <> 3) and'
      '(MEMO_DATE > :dtpk2)'
      ''
      'GROUP BY SUPP_PTR, CURRENCY_PTR')
    Left = 520
    Top = 208
    object AQ_finacreditSUPP_PTR: TIntegerField
      FieldName = 'SUPP_PTR'
    end
    object AQ_finacreditcurrency_ptr: TIntegerField
      FieldName = 'CURRENCY_PTR'
    end
    object AQ_finacreditDSDesigner: TBCDField
      FieldName = #26399#21518#25187#27454
      Precision = 19
    end
    object AQ_finacreditDSDesigner2: TFloatField
      FieldName = #26399#21518#25187#27454#35760#24080#26412#24065
      ReadOnly = True
    end
    object AQ_finacreditDSDesigner3: TFloatField
      FieldName = #26399#21518#25187#27454#26412#24065
      ReadOnly = True
    end
  end
  object aq01: TADOQuery
    AutoCalcFields = False
    Connection = ADOConnection1
    CursorType = ctStatic
    LockType = ltReadOnly
    Parameters = <>
    SQL.Strings = (
      'select rkey,curr_code,base_to_other '
      'from data0001'
      'order by rkey')
    Left = 344
    Top = 128
    object aq01rkey: TIntegerField
      FieldName = 'rkey'
      ReadOnly = True
    end
    object aq01curr_code: TStringField
      FieldName = 'curr_code'
      Size = 5
    end
    object aq01base_to_other: TFloatField
      FieldName = 'base_to_other'
    end
  end
end
