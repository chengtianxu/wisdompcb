...
if strtoint(user_ptr)<>dm.ADS105ENTERED_BY_EMPL_PTR.Value then
 begin
  messagedlg('对不起作废操作必须由创建人自己完成!',mtinformation,[mbok],0);
  exit;
 end;
 ...
 if messagedlg('您确定要作废该凭证吗??'+dm.ADS105VOUCHER.Value,mtconfirmation,[mbyes,mbno],0)=mryes then
  begin
   try
    ...
    with dm.ADOQuery1 do      //更新其它费用帐龄为未开票
    begin
     close;
...
//============================================
    ...
    begin
     ...
     sql.Text:=
     'update data0107'+#13+
     'set glptr_status=0'+#13+      //更新标准帐龄为未开票
     'where rkey in (SELECT  data0456.dealiv_ptr'+#13+
     'FROM   Data0022 INNER JOIN'+#13+
     '  data0456 ON Data0022.grn_ptr = data0456.rkey'+#13+
...
//==========================================================
    ...
    begin
     ...
     '  data0456 ON Data0235.grn_ptr = data0456.rkey'+#13+
...
//==========================================================
    ...
    begin
     ...
     'FROM data0210 WHERE (data0210.GL_HEADER_PTR = '+dm.ADO105RKEY.AsString+')'+#13+
