object Form_rma: TForm_rma
  Left = 239
  Top = 185
  Width = 863
  Height = 518
  Caption = #36864#36135#25628#32034
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -13
  Font.Name = #23435#20307
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnActivate = FormActivate
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 855
    Height = 41
    Align = alTop
    TabOrder = 0
    object Label3: TLabel
      Left = 281
      Top = 13
      Width = 52
      Height = 13
      Alignment = taRightJustify
      Caption = #38144#21806#35746#21333
    end
    object Edit1: TEdit
      Left = 338
      Top = 8
      Width = 121
      Height = 21
      TabOrder = 0
      OnChange = Edit1Change
    end
    object BitBtn4: TBitBtn
      Left = 17
      Top = 5
      Width = 57
      Height = 30
      Hint = #23548#20986#25968#25454'Excel'
      Caption = #23548#20986
      ParentShowHint = False
      ShowHint = True
      TabOrder = 1
      OnClick = BitBtn4Click
      Glyph.Data = {
        
        70DA0FFFF0ADA0EEEE0D00000ADADA00000AADADADADADADADAD}
    end
    object BtQry: TBitBtn
      Left = 74
      Top = 5
      Width = 57
      Height = 30
      Hint = #37325#26032#23450#20041#26597#35810#26465#20214
      Caption = #26597#35810
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = #23435#20307
      Font.Style = []
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 2
      OnClick = BtQryClick
      Glyph.Data = {
        
        861D8680775398535355}
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 445
    Width = 855
    Height = 39
    Align = alBottom
    BevelOuter = bvNone
    TabOrder = 1
    DesignSize = (
      855
      39)
    object Button1: TButton
      Left = 323
      Top = 8
      Width = 75
      Height = 25
      Anchors = [akTop]
      Caption = #30830#23450
      TabOrder = 0
      OnClick = Button1Click
    end
    object Button2: TButton
      Left = 423
      Top = 8
      Width = 75
      Height = 25
      Anchors = [akTop]
      Cancel = True
      Caption = #21462#28040
      ModalResult = 2
      TabOrder = 1
    end
    object CheckBox1: TCheckBox
      Left = 35
      Top = 12
      Width = 57
      Height = 17
      Caption = #20840#36873
      TabOrder = 2
      OnClick = CheckBox1Click
    end
  end
  object DBGrid1: TDBGrid
    Left = 0
    Top = 41
    Width = 855
    Height = 404
    Align = alClient
    DataSource = DataSource1
    Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit, dgMultiSelect]
    TabOrder = 2
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -13
    TitleFont.Name = #23435#20307
    TitleFont.Style = []
    OnKeyDown = DBGrid1KeyDown
    OnTitleClick = DBGrid1TitleClick
    Columns = <
      item
        Expanded = False
        FieldName = 'SALES_ORDER'
        Title.Color = clRed
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'MANU_PART_NUMBER'
        Width = 93
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'MANU_PART_DESC'
        Width = 123
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'PO_NUMBER'
        Width = 85
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'RMA_NUMBER'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'RMA_DATE'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'QTY_RECD'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'recon_qty'
        Title.Caption = #24050#23545#24080#25968#37327
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'alloc_qty'
        Title.Caption = #26410#23545#24080#25968#37327
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'PARTS_ALLOC'
        Width = 46
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'RUSH_CHARGE'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'MEMO_NUMBER'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'RKEY'
        Width = 70
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'abbr_name'
        Visible = True
      end>
  end
  object DataSource1: TDataSource
    DataSet = ads98
    OnDataChange = DataSource1DataChange
    Left = 56
    Top = 264
  end
  object ads98: TADODataSet
    Connection = DM.ADOConnection1
    CursorType = ctStatic
    Filtered = True
    LockType = ltReadOnly
    CommandText = 
      'SELECT     dbo.Data0060.SALES_ORDER, dbo.Data0025.MANU_PART_NUMB' +
      'ER, dbo.Data0025.MANU_PART_DESC, dbo.Data0097.PO_NUMBER, dbo.Dat' +
      'a0098.RMA_NUMBER, '#13#10'dbo.Data0098.RMA_DATE, Data0098.QTY_RECD,Dat' +
      'a0098.recon_qty,'#13#10'dbo.Data0098.QTY_RECD - dbo.Data0098.recon_qty' +
      ' AS alloc_qty, dbo.Data0060.PARTS_ALLOC,'#13#10' dbo.Data0060.RUSH_CHA' +
      'RGE, data0015.abbr_name,'#13#10' dbo.Data0098.RKEY, dbo.Data0116.MEMO_' +
      'NUMBER'#13#10'FROM         dbo.Data0098 INNER JOIN'#13#10'                  ' +
      '    dbo.Data0060 ON dbo.Data0098.SO_PTR = dbo.Data0060.RKEY INNE' +
      'R JOIN'#13#10'                      dbo.Data0116 ON dbo.Data0098.srce_' +
      'ptr = dbo.Data0116.RKEY INNER JOIN'#13#10'                      dbo.Da' +
      'ta0025 ON dbo.Data0098.CUSTOMER_PART_PTR = dbo.Data0025.RKEY INN' +
      'ER JOIN'#13#10'                      dbo.Data0097 ON dbo.Data0060.PURC' +
      'HASE_ORDER_PTR = dbo.Data0097.RKEY left outer join '#13#10'data0015 on' +
      ' data0116.warehouse_ptr=data0015.rkey'#13#10'where Data0098.QTY_RECD -' +
      ' Data0098.recon_qty>0'#13#10
    IndexFieldNames = 'SALES_ORDER'
    Parameters = <>
    Prepared = True
    Left = 104
    Top = 264
    object ads98SALES_ORDER: TStringField
      DisplayLabel = #38144#21806#35746#21333
      FieldName = 'SALES_ORDER'
      FixedChar = True
      Size = 12
    end
    object ads98MANU_PART_NUMBER: TStringField
      DisplayLabel = #26412#21378#32534#21495
      FieldName = 'MANU_PART_NUMBER'
    end
    object ads98MANU_PART_DESC: TStringField
      DisplayLabel = #23458#25143#22411#21495
      FieldName = 'MANU_PART_DESC'
      Size = 80
    end
    object ads98PO_NUMBER: TStringField
      DisplayLabel = #23458#25143#35746#21333#21495
      FieldName = 'PO_NUMBER'
      Size = 25
    end
    object ads98RMA_NUMBER: TStringField
      DisplayLabel = #36864#36135#21333#21495
      FieldName = 'RMA_NUMBER'
      FixedChar = True
      Size = 10
    end
    object ads98RMA_DATE: TDateTimeField
      DisplayLabel = #36864#36135#26085#26399
      FieldName = 'RMA_DATE'
    end
    object ads98alloc_qty: TIntegerField
      DisplayLabel = #21487#24320#31080#25968#37327
      FieldName = 'alloc_qty'
      ReadOnly = True
    end
    object ads98PARTS_ALLOC: TFloatField
      DisplayLabel = #20215#26684
      FieldName = 'PARTS_ALLOC'
    end
    object ads98RUSH_CHARGE: TBCDField
      DisplayLabel = #31246#29575
      FieldName = 'RUSH_CHARGE'
      Precision = 5
      Size = 2
    end
    object ads98RKEY: TIntegerField
      DisplayLabel = #26631#35782#21015
      FieldName = 'RKEY'
      ReadOnly = True
    end
    object ads98MEMO_NUMBER: TStringField
      DisplayLabel = #36151#39033#22791#24536
      FieldName = 'MEMO_NUMBER'
      Size = 10
    end
    object ads98QTY_RECD: TIntegerField
      DisplayLabel = #36864#36135#25968#37327
      FieldName = 'QTY_RECD'
    end
    object ads98abbr_name: TStringField
      DisplayLabel = #24037#21378#31616#31216
      FieldName = 'abbr_name'
      Size = 10
    end
    object ads98recon_qty: TIntegerField
      FieldName = 'recon_qty'
    end
  end
end
