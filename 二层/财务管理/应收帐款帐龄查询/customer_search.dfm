object Form_customer: TForm_customer
  Left = 334
  Top = 219
  BorderIcons = [biSystemMenu, biMinimize]
  BorderStyle = bsSingle
  Caption = #23458#25143#25628#32034
  ClientHeight = 414
  ClientWidth = 403
  Color = clBtnFace
  Font.Charset = GB2312_CHARSET
  Font.Color = clWindowText
  Font.Height = -13
  Font.Name = #23435#20307
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object Splitter1: TSplitter
    Left = 0
    Top = 0
    Width = 403
    Height = 33
    Cursor = crArrow
    Align = alTop
  end
  object Label1: TLabel
    Left = 56
    Top = 9
    Width = 52
    Height = 13
    Alignment = taRightJustify
    Caption = #23458#25143#20195#30721
    Font.Charset = GB2312_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = #23435#20307
    Font.Style = []
    ParentFont = False
  end
  object DBGrid1: TDBGrid
    Left = 0
    Top = 33
    Width = 403
    Height = 340
    Align = alClient
    DataSource = DataSource1
    Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit]
    ReadOnly = True
    TabOrder = 1
    TitleFont.Charset = GB2312_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -13
    TitleFont.Name = #23435#20307
    TitleFont.Style = []
    OnDblClick = DBGrid1DblClick
    OnKeyDown = DBGrid1KeyDown
    OnTitleClick = DBGrid1TitleClick
    Columns = <
      item
        Expanded = False
        FieldName = 'CUST_CODE'
        Width = 89
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'CUSTOMER_NAME'
        Width = 195
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'CUSTOMER_TYPE'
        Title.Caption = #29366#24577
        Width = 54
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'SALES_REP_NAME'
        Title.Caption = #19994#21153#21592
        Width = 55
        Visible = True
      end>
  end
  object MaskEdit1: TMaskEdit
    Left = 117
    Top = 4
    Width = 153
    Height = 21
    TabOrder = 0
    OnChange = MaskEdit1Change
    OnKeyDown = DBGrid1KeyDown
  end
  object BitBtn2: TBitBtn
    Left = 275
    Top = 4
    Width = 25
    Height = 25
    Hint = #21047#26032
    ParentShowHint = False
    ShowHint = True
    TabOrder = 2
    TabStop = False
    OnClick = BitBtn2Click
    Glyph.Data = {
      
      440AADADADA000A04440DADADADADADA0440ADADADADADADA00D}
  end
  object Panel1: TPanel
    Left = 0
    Top = 373
    Width = 403
    Height = 41
    Align = alBottom
    BevelOuter = bvNone
    TabOrder = 3
    object Button1: TButton
      Left = 131
      Top = 10
      Width = 75
      Height = 25
      Caption = #30830#23450
      ModalResult = 1
      TabOrder = 0
    end
    object Button2: TButton
      Left = 211
      Top = 10
      Width = 75
      Height = 25
      Caption = #21462#28040
      ModalResult = 2
      TabOrder = 1
    end
  end
  object DataSource1: TDataSource
    DataSet = ADOQuery1
    Left = 304
    Top = 379
  end
  object ADOQuery1: TADODataSet
    Connection = Form1.ADOConnection1
    CursorType = ctStatic
    Filtered = True
    LockType = ltReadOnly
    CommandText = 
      'SELECT     dbo.Data0010.RKEY, dbo.Data0010.CUST_CODE, dbo.Data00' +
      '10.CUSTOMER_NAME, dbo.Data0001.CURR_CODE, dbo.Data0001.CURR_NAME' +
      ', '#13#10'                      dbo.Data0001.BASE_TO_OTHER, dbo.Data00' +
      '10.NET_PAY, dbo.Data0010.ACC_REC_PTR, dbo.Data0010.CURRENCY_PTR,' +
      ' dbo.Data0010.ABBR_NAME, '#13#10'                      dbo.Data0010.FE' +
      'D_TAX_ID_NO, dbo.Data0005.EMPLOYEE_NAME as SALES_REP_NAME, '#13#10'   ' +
      '                   CASE dbo.Data0010.CUSTOMER_TYPE WHEN 1 THEN '#39 +
      #27491#24120#39' WHEN 2 THEN '#39#26032#21512#20316#39' WHEN 3 THEN '#39#25511#21046#39' WHEN 4 THEN '#39#20572#27490#39' END AS C' +
      'USTOMER_TYPE'#13#10'FROM         dbo.Data0010 INNER JOIN'#13#10'            ' +
      '          dbo.Data0001 ON dbo.Data0010.CURRENCY_PTR = dbo.Data00' +
      '01.RKEY INNER JOIN'#13#10'                      dbo.Data0005 ON dbo.Da' +
      'ta0010.SALES_REP_PTR = dbo.Data0005.RKEY'#13#10
    Parameters = <>
    Left = 334
    Top = 380
    object ADOQuery1rkey: TIntegerField
      FieldName = 'rkey'
      ReadOnly = True
    end
    object ADOQuery1CUST_CODE: TStringField
      DisplayLabel = #23458#25143#20195#30721
      FieldName = 'CUST_CODE'
      Size = 10
    end
    object ADOQuery1CUSTOMER_NAME: TStringField
      DisplayLabel = #23458#25143#21517#31216
      FieldName = 'CUSTOMER_NAME'
      Size = 100
    end
    object ADOQuery1ABBR_NAME: TStringField
      FieldName = 'ABBR_NAME'
      Size = 10
    end
    object ADOQuery1ACC_REC_PTR: TIntegerField
      FieldName = 'ACC_REC_PTR'
    end
    object ADOQuery1CURR_CODE: TStringField
      FieldName = 'CURR_CODE'
      Size = 5
    end
    object ADOQuery1CURR_NAME: TStringField
      FieldName = 'CURR_NAME'
    end
    object ADOQuery1NET_PAY: TSmallintField
      FieldName = 'NET_PAY'
    end
    object ADOQuery1CURRENCY_PTR: TIntegerField
      FieldName = 'CURRENCY_PTR'
    end
    object ADOQuery1FED_TAX_ID_NO: TStringField
      FieldName = 'FED_TAX_ID_NO'
      FixedChar = True
    end
    object ADOQuery1BASE_TO_OTHER: TFloatField
      FieldName = 'BASE_TO_OTHER'
    end
    object ADOQuery1SALES_REP_NAME: TStringField
      FieldName = 'SALES_REP_NAME'
    end
    object ADOQuery1CUSTOMER_TYPE: TStringField
      FieldName = 'CUSTOMER_TYPE'
      ReadOnly = True
      Size = 6
    end
  end
end
