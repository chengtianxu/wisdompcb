...
uses DAMO, main, customer_search, curr_search, AcctSearch, invoice_search,
  voucher_report, voucher_dictionary;
...
stringgrid1.Cells[3,0]:='产品编号';
stringgrid1.Cells[4,0]:='产品名称';
//stringgrid1.Cells[5,0]:='产品规格';
 ...
 begin
  ...
  with dm.ADOQuery1 do
   begin
    close;
    sql.Clear;
    sql.Add('SELECT t98.RMA_NUMBER, t98.RMA_DATE,');
    sql.Add('t70.sales_order, t8.PROD_CODE,');
    sql.Add('t8.PRODUCT_NAME,t8.PRODUCT_DESC,');
    sql.Add('t71.tax_rate,t98.Putaway_date as sys_date,');
    sql.Add('t98.QTY_RECD,t71.price, ''Y'' as tax_in_price');
    sql.Add('FROM dbo.Data0098 t98');
    sql.Add('INNER JOIN WZCP0063 t63 ON t98.SO_SHIP_PTR = t63.rkey');
    sql.Add('INNER JOIN Data0008 t8  ON t63.prod_ptr = t8.RKEY');
    sql.Add('INNER JOIN WZCP0070 t70 ON t98.SO_PTR = t70.rkey');
    sql.Add('INNER JOIN WZCP0071 t71 ON t70.rkey = t71.so_ptr And t63.cp71_ptr = t71.rkey');
    sql.Add('WHERE t98.srce_ptr='+dm.ado116rkey.AsString);
    sql.Add('AND t98.RMA_STATUS = 4');
    open;
   end;
  ...
  for i:=1 to dm.ADOQuery1.RecordCount do
   begin
    ...
    stringgrid1.Cells[3,i]:=dm.ADOQuery1.fieldbyname('PROD_CODE').AsString;//'本厂编号';
    stringgrid1.Cells[4,i]:=dm.ADOQuery1.fieldbyname('PRODUCT_NAME').AsString;//'客户型号';
    stringgrid1.Cells[5,i]:=dm.ADOQuery1.fieldbyname('PRODUCT_DESC').AsString;
    ...
    stringgrid1.Cells[7,i]:=dm.ADOQuery1.fieldbyname('price').AsString;//'单价';
    stringgrid1.Cells[8,i]:=dm.ADOQuery1.fieldbyname('tax_rate').AsString;//'税率%';
 ...
 begin
  with dm.ADOQuery1 do
   begin
    ...
    sql.Add('INNER JOIN WZCP0070 t70 ON t98.SO_PTR = t70.rkey');
    sql.Add('INNER JOIN WZCP0071 t71 ON t70.rkey = t71.so_ptr And t63.cp71_ptr = t71.rkey');
    sql.Add('WHERE t98.RMA_STATUS = 3');  //已入仓
    sql.Add('And t98.CUSTOMER_PTR= '+dm.ado116CUST_PTR.AsString);
    sql.Add('And t70.currency_ptr= '+dm.ado116CURRENCY_PTR.AsString);
  ...
  for i:=1 to dm.ADOQuery1.RecordCount do
   begin
    ...
    stringgrid1.Cells[1,i]:=dm.ADOQuery1.fieldbyname('RMA_DATE').AsString; //'退货日期';
    stringgrid1.Cells[2,i]:=dm.ADOQuery1.fieldbyname('sales_order').AsString;//'销售订单';
    stringgrid1.Cells[3,i]:=dm.ADOQuery1.fieldbyname('PROD_CODE').AsString;//'产品编号';
    stringgrid1.Cells[4,i]:=dm.ADOQuery1.fieldbyname('PRODUCT_NAME').AsString;//'产品名称';
    //stringgrid1.Cells[5,i]:=dm.ADOQuery1.fieldbyname('PRODUCT_DESC').AsString;
    stringgrid1.Cells[5,i]:=dm.ADOQuery1.fieldbyname('sys_date').AsString;//'入库日期';
    stringgrid1.Cells[6,i]:=dm.ADOQuery1.fieldbyname('QTY_RECD').AsString;//'数量';
    stringgrid1.Cells[7,i]:=dm.ADOQuery1.fieldbyname('price').AsString;//'单价';
...
begin
  ...
  begin
    ...
    open;
     ...
     begin
       ShowMessage('退货入仓数据可能已发生变化,请检查!');
       Exit;
     end;
  end;
 if (StringGrid1.RowCount <= 1) then
 begin
   messagedlg('未选择退货明细，不允许保存',mtinformation,[mbok],0);
   exit;
 end
 else
 begin
   ...
   begin
     if strtoint(stringgrid1.Cells[10,stringgrid1.row])=0 then
       Continue
     else
     begin
       flag:= 1;
 ...
 begin
  ...
  if find_rmanumber(trim(form_rma.ADODataSet1RMA_NUMBER.Value)) then
   ...
   begin
    ...
    stringgrid1.Cells[3,stringgrid1.RowCount-1]:=form_rma.ADODataSet1PROD_CODE.AsString;//'产品编号';
    stringgrid1.Cells[4,stringgrid1.RowCount-1]:=form_rma.ADODataSet1PRODUCT_NAME.AsString;//'产品名称';
    //stringgrid1.Cells[5,stringgrid1.RowCount-1]:=form_rma.ADODataSet1PRODUCT_DESC.AsString;
