...
for i:= 1 to stringgrid1.RowCount-2 do
 with dm.ADOQuery1 do
  begin
   ...
   sql.Clear;
   sql.Add('update WZCP0060');
   sql.Add('set status=4, invoice_ptr='+inttostr(rkey112));  //4:已入账
...
stringgrid1.Cells[0,0]:='出仓单号';
stringgrid1.Cells[1,0]:='日期';
stringgrid1.Cells[2,0]:='装运方法';
stringgrid1.Cells[3,0]:='合计金额';
stringgrid1.Cells[4,0]:='rkey60';

stringgrid2.Cells[0,0]:='销售订单';
stringgrid2.Cells[1,0]:='产品编码';
...
stringgrid2.Cells[5,0]:='出仓数量';
stringgrid2.Cells[6,0]:='价格';
stringgrid2.Cells[7,0]:='税率%';
stringgrid2.Cells[8,0]:='价格含税';
stringgrid2.Cells[9,0]:='rkey60';
stringgrid2.Cells[10,0]:='rkey63';
//stringgrid2.Cells[11,0]:='discount';
stringgrid2.ColWidths[9]:=0;
stringgrid2.ColWidths[10]:=0;
stringgrid2.ColWidths[11]:=-1;
if (dm.ADO112.State=dsedit) or
   (dm.ADO112.State=dsBrowse) then  //编辑或者检查
 begin
  with dm.ADOQuery1 do
   begin
    close;
    sql.Text:='SELECT t60.rkey, t60.sales_order, t60.sales_date,t60.ship_method,'+#13+
    'ROUND(SUM(t63.quantity * t63.price),2) AS sum_amount'+#13+
    'FROM dbo.WZCP0060 t60 INNER JOIN'+#13+
    'dbo.WZCP0063 t63 ON t60.rkey = t63.so_ptr'+#13+
    'WHERE t60.invoice_ptr = '+dm.ADO112RKEY.AsString+#13+
    'GROUP BY t60.rkey, t60.sales_order, t60.sales_date,t60.ship_method';
    open;
   end;
  stringgrid1.RowCount:=dm.ADOQuery1.RecordCount+2;
  for i:=1 to dm.ADOQuery1.RecordCount do
  begin
   stringgrid1.Cells[0,i]:=dm.ADOQuery1.FieldValues['sales_order']; //'出仓单号';
   stringgrid1.Cells[1,i]:=dm.ADOQuery1.fieldbyname('sales_date').AsString; //'日期';
   stringgrid1.Cells[2,i]:=dm.ADOQuery1.fieldbyname('ship_method').AsString; //'装运方法';
   stringgrid1.Cells[3,i]:=dm.ADOQuery1.fieldbyname('sum_amount').AsString; //'合计金额';
   stringgrid1.Cells[4,i]:=dm.ADOQuery1.fieldbyname('rkey').AsString; //'rkey60';
   dm.ADOQuery1.Next;
  end;
  ...
  begin
   ...
   stringgrid2.Cells[1,i]:=dm.ADOD64PROD_CODE.Value; //'产品编码';
   stringgrid2.Cells[2,i]:=dm.ADOD64PRODUCT_NAME.Value; // '产品名称';
//   stringgrid2.Cells[3,i]:=dm.ADOD64CP_REV.Value; // '版本';
   //stringgrid2.Cells[4,i]:=dm.ADOD64PO_NUMBER.Value; //'客户订单号';
   stringgrid2.Cells[5,i]:=dm.ADOD64quantity.AsString; //'出仓数量';
   stringgrid2.Cells[6,i]:=dm.ADOD64price.AsString; //'价格';
   stringgrid2.Cells[7,i]:=dm.ADOD64tax_rate.AsString; //'税率%';
   stringgrid2.Cells[8,i]:=dm.ADOD64tax_in_price.Value; //'价格含税';
   stringgrid2.Cells[9,i]:=dm.ADOD64so_ptr.AsString; //'rkey60';
   stringgrid2.Cells[10,i]:=dm.ADOD64rkey.AsString; //'rkey63';
...
procedure TForm2.DBEdit4Exit(Sender: TObject);
begin
...
begin
 if dm.ADOD64tax_in_price.Value='N' then  //(价格中不含税)
  begin
   ship_price := dm.ADOD64quantity.Value*dm.ADOD64price.Value;  //标准价款不含税
   ovsh_price := 0;//dm.ADOD64ovsh_qty.Value*dm.ADOD64part_price.Value*
                 // (1-dm.ADOD64ovsh_discount.Value*0.01);  //溢装价款不含税
  end
 else                       //(价格中含税)
  begin
   ship_price := dm.ADOD64quantity.Value*dm.ADOD64price.Value/
                 (1+dm.ADOD64tax_rate.Value*0.01);  //标准价款不含税
   ovsh_price := 0;//dm.ADOD64ovsh_qty.Value*dm.ADOD64part_price.Value*
                 //(1-dm.ADOD64ovsh_discount.Value*0.01)/
                 //(1+dm.ADOD64TAX_2.Value*0.01);  //溢装价款不含税
  end;

 disc_single:=0;//(ship_price+ovsh_price)*dm.ADOD64discount.Value*0.01; //折扣
 tax_single:=(ship_price+ovsh_price-disc_single)*
                       dm.ADOD64tax_rate.Value*0.01;                 //增值税
...
var
 i,j:word;
 find_22:boolean;
 ship_price,disc_single,tax_single:Double;//single;
...
while find_22 do
 begin
  if stringgrid2.RowCount>2 then
   begin
    ...
    begin
     if trim(stringgrid2.Cells[8,i])='N' then  //(价格中不含税)
     ship_price := strtocurr(FormatFloat('0.00',strtocurr(stringgrid2.Cells[5,i])*
                   strtocurr(stringgrid2.Cells[6,i])))
     else                                               //(价格中含税)
     ship_price :=strtocurr(FormatFloat('0.00',strtocurr(stringgrid2.Cells[5,i])*
                   strtocurr(stringgrid2.Cells[6,i])/
                   (1+strtocurr(stringgrid2.Cells[7,i])*0.01)));

     //disc_single:=strtocurr(floattostrf(
     //    ship_price*strtocurr(stringgrid2.Cells[11,i])*0.01,ffFixed,10,2)); //折扣

     //tax_single:=strtocurr(FormatFloat('0.00',      //税金
     // (ship_price-disc_single)*strtocurr(stringgrid2.Cells[7,i])*0.01));
     tax_single:= strtocurr(stringgrid2.Cells[5,i])*
                   strtocurr(stringgrid2.Cells[6,i])-ship_price+disc_single;
 ...
 dm.ADO112ship_amount.Value:=dm.ADO112ship_amount.Value-ship_amount;
 dm.ADO112FED_TAX_AMOUNT.Value:=dm.ADO112FED_TAX_AMOUNT.Value-tax_amount;
 dm.ADO112DISCOUNT_AMOUNT.Value:=dm.ADO112DISCOUNT_AMOUNT.Value-disc_amount;
 dm.ADO112INVOICE_TOTAL.Value:=roundto(dm.ADO112SHIPPING_CHARGES.Value+
                               dm.ADO112ship_amount.Value+
                               dm.ADO112FED_TAX_AMOUNT.Value-
                               dm.ADO112DISCOUNT_AMOUNT.Value,-2);
...
if form_selectpk.ShowModal=mrok then
 begin
  ...
  begin
   stringgrid1.Cells[0,stringgrid1.RowCount-1]:=form_selectpk.aqd439sales_order.Value;
   stringgrid1.Cells[1,stringgrid1.RowCount-1]:=form_selectpk.aqd439sales_date.AsString;
   stringgrid1.Cells[2,stringgrid1.RowCount-1]:=form_selectpk.aqd439ship_method.Value;
   ...
   stringgrid2.Cells[1,stringgrid2.RowCount-1]:=form_selectpk.aqd64PROD_CODE.Value;
   stringgrid2.Cells[2,stringgrid2.RowCount-1]:=form_selectpk.aqd64PRODUCT_NAME.Value;
//   stringgrid2.Cells[3,stringgrid2.RowCount-1]:=form_selectpk.aqd64CP_REV.AsString;
   //stringgrid2.Cells[4,stringgrid2.RowCount-1]:=form_selectpk.aqd64PO_NUMBER.AsString;
   stringgrid2.Cells[5,stringgrid2.RowCount-1]:=form_selectpk.aqd64quantity.AsString;
   stringgrid2.Cells[6,stringgrid2.RowCount-1]:=form_selectpk.aqd64price.AsString;
   stringgrid2.Cells[7,stringgrid2.RowCount-1]:=form_selectpk.aqd64tax_rate.AsString;
   stringgrid2.Cells[8,stringgrid2.RowCount-1]:=form_selectpk.aqd64tax_in_price.Value;
   stringgrid2.Cells[9,stringgrid2.RowCount-1]:=form_selectpk.aqd64so_ptr.AsString;
   ...
   if form_selectpk.aqd64tax_in_price.Value='N' then  //(价格中不含税)
     ship_price :=
            strtocurr(formatfloat('0.00',form_selectpk.aqd64quantity.Value*
            form_selectpk.aqd64price.Value))
   else                                               //(价格中含税)
     ship_price :=
           strtocurr(formatfloat('0.00',form_selectpk.aqd64quantity.Value*
           form_selectpk.aqd64price.Value/
           (1+form_selectpk.aqd64tax_rate.Value*0.01)));

   //disc_single:=strtocurr(formatfloat('0.00',
   //     ship_price*form_selectpk.aqd64discount.Value*0.01)); //折扣

   //tax_single:=strtocurr(formatfloat('0.00',
   //    (ship_price-disc_single)*form_selectpk.aqd64tax_rate.Value*0.01)); //增值税
   tax_single:= form_selectpk.aqd64quantity.Value*form_selectpk.aqd64price.Value
                -ship_price; //增值税
