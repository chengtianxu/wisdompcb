...
uses damo, term, packlist_search, addedit_invoice, form_tool,
  sotools_search,misc_invoice, voucher_report,common,chargedetail;
   ...
   try
    ...
    with form4 do
     begin
      ...
      REG_TAX_FIXED_UNUSED:=Form_sotool.Aqd64REG_TAX_FIXED_UNUSED.AsInteger;
     end;
     dm.ADO112.Close;
...
var
 ship_charge:single;
 i:Integer;//v_year,v_month,v_day:integer;
 rkey439_list:string;
begin
  ...
  dm.ADO112warehouse_ptr.Value:=form_packlist.aqd439warehouse_ptr.Value; //工厂指针
 ...
 for i:=1 to form_packlist.DBGrid1.SelectedRows.Count do
  begin
   ...
   form2.stringgrid1.Cells[0,i]:=Form_packlist.aqd439delivery_no.Value;
   form2.stringgrid1.Cells[1,i]:=Form_packlist.aqd439date_sailing.AsString;
   form2.stringgrid1.Cells[2,i]:=Form_packlist.aqd439shipping_method.Value;
   form2.stringgrid1.Cells[3,i]:=Form_packlist.aqd439sum_amount.AsString;  //不含税金额
   form2.stringgrid1.Cells[4,i]:=Form_packlist.aqd439rkey.AsString;

   if i= form_packlist.DBGrid1.SelectedRows.Count then
    rkey439_list:=rkey439_list+Form_packlist.aqd439rkey.AsString+')'
   else
    rkey439_list:=rkey439_list+Form_packlist.aqd439rkey.AsString+',';
  ...
  while not Form_packlist.aqd64.Eof do
   begin
     ...
     form2.stringgrid2.Cells[1,form2.stringgrid2.RowCount-1]:=Form_packlist.aqd64MANU_PART_NUMBER.Value;
     form2.stringgrid2.Cells[2,form2.stringgrid2.RowCount-1]:=Form_packlist.aqd64MANU_PART_DESC.Value;
    ...
    strtocurr(floattostrf(form_packlist.aqd439charge.Value,ffFixed,10,2));  //装运费用
  end; //结束439_FOR主表循环
   ...
   ///chenyang  2014-06-05-------------
    ...
    begin
      ...
      sql.Text:= '    SELECT  round(sum((dbo.Data0064.QUAN_SHIPPED * dbo.Data0064.part_price)                           '+#13+
                 ...
                 '           WHERE     (Data0064.packing_list_ptr in '+rkey439_list+')';
            Open;
    end;
  dm.ado112ship_amount.Value:=DM.ADOQuery1.fieldbyname('notax_amount').AsCurrency; //廷伸成本 11
  dm.ado112FED_TAX_AMOUNT.Value:=DM.ADOQuery1.fieldbyname('tax').AsCurrency;//增值税   15
...
begin
 ...
 tax_single:=0;   //增值税合计
  ...
  if form_sotool.Aqd64REG_TAX_FIXED_UNUSED.AsInteger=1 then    //发票到期计算方法为货到
    dm.ADO112DUE_DATE.Value := dm.ADO112INVOICE_DATE.Value+dm.ADO112net_pay.Value //到期日  8
  else                                             //发票到期计算方法为月结
  ...
  try
   ...
   with form2 do
    begin
     ...
     REG_TAX_FIXED_UNUSED:=Form_packlist.aqd439REG_TAX_FIXED_UNUSED.AsInteger;
  ...
  try
   ...
   with form2 do
    begin
     ...
     REG_TAX_FIXED_UNUSED:=dm.AqD112REG_TAX_FIXED_UNUSED.AsInteger;
...
procedure TForm1.DBGrid1DrawColumnCell(Sender: TObject; const Rect: TRect;
  DataCol: Integer; Column: TColumn; State: TGridDrawState);
begin
 if dm.AqD112.FieldByName('INVOICE_STATUS').AsInteger=3 then
...
begin
 with TFrm_chargedetail.Create(nil) do
   begin
     ShowModal;
     Free;
   end;
