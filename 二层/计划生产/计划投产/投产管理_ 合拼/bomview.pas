...
type
  ...
  private
    ...
    PPC_CONTROL18,d25_type:byte;  //是否允许调高报废率,量产/样板
    rkey34:Tstringlist;
...
begin
 ...
 self.EST_SCRAP:=strtofloat(edit2.text);  //保存初始报废率
 a_pcs:=0;
with dm.ADOQuery1 do
 begin
  close;
...
if self.rkey0015>0 then
 ...
 with dm.ADOQuery1 do
  begin
   close;
   sql.Text:=
    'SELECT PPC_CONTROL18,BALA_DAYS '+#13+  //是否允许调高报废率%
    'FROM dbo.Data0192';
   open;
   PPC_CONTROL18:=fieldbyname('PPC_CONTROL18').AsInteger;
 ...
 begin
  if combobox1.ItemIndex=3 then    //按mrb返修
   ...
   begin
    ...
    if combobox1.ItemIndex=4 then    //按板号投产
 ...
 begin
  ...
  with dm.ADOQuery1 do
   begin
    ...
    sql.Add('where rkey='+inttostr(PTreeValue(Node.data)^.rkey25));
    ExecSQL;
   end;
