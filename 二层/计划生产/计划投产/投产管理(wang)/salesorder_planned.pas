...
uses
  ...
  ADODB, Menus,math, ImgList, ComCtrls;
...
//     (strtoint(edit9.Text) + StrToFloat(edtref.Text) -strtoint(edit2.Text))*dm.ADS60spell_sq.Value,ffFixed,8,3);
    edit13.Text:=Floattostrf(                          //超投面积
     (strtoint(edit9.Text) + StrToFloat(edtref.Text) -strtoint(edit2.Text))* pcs_sq,ffFixed,8,3);
  end
  else
  begin
    edit12.Text:='0';
   ...
   if (Form_editplanned.host_flag) and        //主订单
    ...
    begin
     if self.d25_type=0 then  //如果批量
      begin
       if (StrToFloat(edit2.Text)* pcs_sq <= self.ppc25) and //小订单
...
//     or (StrToInt(Trim(Edit9.Text)) = 0)   then
  ...
  begin
   ...
   with Form_putprod do
    begin
     ...
     d25type_ptr:= Self.d25_type;  //样品量产
     //out_sq:= Trim(Edit13.Text);
      if d25type_ptr = 0 then
       out_sq:= Floattostrf(                          //量产超投面积
            ((strtoint(edit9.Text)+ StrToFloat(edtref.Text))/(1 + EST_SCRAP * 0.01) -strtoint(Edit2.Text))*pcs_sq,ffFixed,8,3)
      else
       out_sq:= Floattostrf(                          //样品超投面积
            ((strtoint(edit9.Text)+ StrToFloat(edtref.Text))/(1 + self.ct_perenct * 0.01) -strtoint(Edit2.Text))*pcs_sq,ffFixed,8,3);
       ...
       for i_j:=1 to ceil(a_pan_qty/PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE) do
        begin
         ...
         stg3.Cells[2,stg3.RowCount-1]:=wo_number(stg1.Cells[0,i],0,      //作业单号
            ceil(a_pan_qty/PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE),i_j,b_pan_qty);
         stg3.Cells[5,stg3.RowCount-1]:=stg1.Cells[12,i];  //A板PCS数
         if i_j < ceil(a_pan_qty/PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE) then
          begin
          ...
          begin
            if a_pan_qty mod PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE = 0 then
             stg3.Cells[4,stg3.RowCount-1]:= inttostr(PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE)
            else
             stg3.Cells[4,stg3.RowCount-1]:=
               inttostr(a_pan_qty mod PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE);
             stg3.Cells[3,stg3.RowCount-1]:= inttostr(strtoint(stg3.Cells[4,stg3.RowCount-1])*
                                                      strtoint(stg3.Cells[5,stg3.RowCount-1]));
          end;
  //       stg3.Cells[6,stg3.RowCount-1] := stg1.Cells[2,i];  //完工日期
       ...
       for i_j:=1 to  ceil(b_pan_qty/PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE) do
        begin
         ...
         stg3.Cells[2,stg3.RowCount-1]:=self.wo_number(stg1.Cells[0,i],1, //作业单号
                 ceil(b_pan_qty/PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE),i_j,b_pan_qty);
         stg3.Cells[5,stg3.RowCount-1]:=stg1.Cells[13,i];  //pcs/bpnl

         if i_j < ceil(b_pan_qty/PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE) then
          begin
           stg3.Cells[4,stg3.RowCount-1]:= inttostr(PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE);
           stg3.Cells[3,stg3.RowCount-1]:= inttostr(strtoint(stg3.Cells[4,stg3.RowCount-1])*
                                           strtoint(stg3.Cells[5,stg3.RowCount-1]));
          ...
          begin
           if b_pan_qty mod PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE=0 then
            stg3.Cells[4,stg3.RowCount-1]:= inttostr(PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE)
           else
            stg3.Cells[4,stg3.RowCount-1]:=
             inttostr(b_pan_qty mod PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE);
