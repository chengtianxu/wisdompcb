...
type
  TForm_salesorder = class(TForm)
    ...
    procedure edtPNLAExit(Sender: TObject);
    procedure edtPNLAChange(Sender: TObject);
    procedure edtPNLAChanging(Sender: TObject; NewValue: Extended;
      var AllowChange: Boolean);
    procedure edtPNLBChanging(Sender: TObject; NewValue: Extended;
      var AllowChange: Boolean);
  private
   ...
   function isFA(Rkey60:string; var FAQty:Integer):Boolean;
...
begin
  ...
  if (strtoint(edit2.Text)>0)                                          //20170615 mark by zsp
      and (strtoint(edit9.Text) + StrToFloat(edtref.Text) -strtoint(edit2.Text) > 0)  then     
  begin                                                            
    edit12.Text:=Floattostrf(                          //超投率%
     (strtoint(edit9.Text) + StrToFloat(edtref.Text) -strtoint(edit2.Text))/
                      strtoint(edit2.Text)*100,ffFixed,8,2);
    edit13.Text:=Floattostrf(                          //超投面积
     (strtoint(edit9.Text) + StrToFloat(edtref.Text) -strtoint(edit2.Text))*dm.ADS60spell_sq.Value,ffFixed,8,3);
  end
  else
  begin
    edit12.Text:='0';
...
//  edit12.Text:=Floattostrf(                          //超投率%
//   (strtoint(edit9.Text) -strtoint(edit2.Text))/
//                    strtoint(edit2.Text)*100,ffFixed,8,2);

//  edit13.Text:=Floattostrf(                          //超投面积
//   (strtoint(edit9.Text)-strtoint(edit2.Text))*dm.ADS60spell_sq.Value,ffFixed,8,3);

//  edit10.Text:=inttostr(a_pan);
//  edit11.Text:=inttostr(b_pan);

  LEven := edtPNLAChange;
  edtPNLA.OnChange := nil;
  try
    edtPNLA.Text:= IntToStr(a_pan);
    edtPNLB.Text:= IntToStr(b_pan);
    PNLA:= a_pan;
    pNLB:= b_pan;
  finally
    edtPNLA.OnChange := LEven;
    edtPNLB.OnChange := LEven;
  end;
end;
...
for i:=1 to sg2.RowCount-1 do
 ...
 if Result - strtoint(trim(edtref.Text)) > 0 then
     Result:= Result - strtoint(trim(edtref.Text))
 else Result:= 0;
end;
  ...
  with DM.ADOQuery1 do
   begin
    ...
    'where data0060.prod_rel=''2'' and'+#13+    //已确认
     ...
     'and data0060.customer_ptr='+dm.ADS60CUSTOMER_PTR.AsString+#13+
...
while not dm.ADOQuery1.Eof do
 begin
  ...
  sg2.Cells[2,dm.ADOQuery1.RecNo]:=dm.ADOQuery1.fieldbyname('SCH_DATE').AsString;
...
//  noten:=noten+dm.ADOQuery1.fieldbyname('notenough').AsInteger;
 ...
 if isFA(DM.ADS60rkey.AsString,FAQty) then
 begin
    edit2.Text:= IntToStr(FAQty);   //待计划数合计
    edit5.Text:= inttostr(FAQty);   //本次计划数合计
    edit6.Text:=inttostr(ceil(strtoint(edit5.Text)*(1+self.EST_SCRAP*0.01)));//计划数
    SG2.Cells[7,rec_row]:= inttostr(FAQty);
    N2.Enabled:= False;
    edtPNLA.Enabled:= False;
    edtPNLB.Enabled:= False;
 end
 else
 begin
...
// a_pan_qty,b_pan_qty:integer;
 ...
 pcs,prodcount,up_qty:Integer;
begin
               //20170615  mark by zsp   begin
  if (StrToFloat(Trim(Edit15.Text)) > 0) and
      (DM.ADS60PARTS_ORDERED.AsInteger + DM.ads60PARTS_RETURNED.asinteger - DM.ADS60REPUT_APPR_BY.AsInteger - StrToFloat(Trim(edtref.Text)) > 0) then
  begin
    if MessageDlg('仓库存在未绑定订单库存，是否继续？', mtConfirmation,[mbYes,mbNo],0) <> mryes then  exit;
  end;

//  if (DM.ADS60PARTS_ORDERED.AsInteger + DM.ads60PARTS_RETURNED.asinteger - DM.ADS60REPUT_APPR_BY.AsInteger - StrToFloat(Trim(edtref.Text)) <= 0)
//     or (StrToInt(Trim(Edit9.Text)) = 0)   then
  if  StrToInt(Trim(Edit9.Text)) = 0   then
  begin
    if MessageDlg('本订单已无需投产,是否继续?', mtConfirmation,[mbYes,mbNo],0) = mryes then
    begin
      ...
      try
        ...
        begin
          close;
          sql.Text:='select prod_rel,to_be_planned,qty_plannned,issued_qty,REPUT_APPR_BY'+#13+
                     'from data0060 where sales_order = '+ QuotedStr(dm.ADS60SALES_ORDER.AsString);
          open;
          edit;
          if StrToIntDef(sg2.Cells[7,get_mainso(dm.ADS60SALES_ORDER.AsString)],0)>0 then
           begin
            fieldbyname('to_be_planned').AsInteger:=fieldbyname('to_be_planned').AsInteger-
                  strtoint(sg2.Cells[7,get_mainso(dm.ADS60SALES_ORDER.AsString)]);
            if fieldbyname('to_be_planned').AsInteger<=0 then
             begin
              fieldbyname('to_be_planned').AsInteger:=0;
              fieldbyname('prod_rel').AsString:='3';
             end;
             fieldbyname('REPUT_APPR_BY').AsInteger:=fieldbyname('REPUT_APPR_BY').AsInteger+
                                                      strtoint(edtref.Text);
           end;
          post;
        end;
        if DM.qry53.Active then DM.qry53.UpdateBatch(arAll);
       ...
       on E: Exception do
        begin
         ...
         ShowMessage('成品库存记录发生改变，可能多人同时操作！！');
        end;
      end;
    end;                                                        //end
  end
  ...
  begin
   ...
   with Form_putprod do
    ...
    begin
       ...
       stg1.Cells[2,i]:=dm.ADS60compl_date.AsString;  //完工日期
...

{
       ...
       begin
         stg1.Cells[5,i]:= inttostr(   //投产数在循环中投产每一层都单独计算
         self.cacl_totalpcs(self.ComboBox1.ItemIndex,Ceil(up_qty*cacl_bomqty(treeview1.Items[i-1])),a_pan_qty,b_pan_qty));
         stg1.Cells[6,i]:=inttostr(a_pan_qty);     //投A板
         stg1.Cells[7,i]:=inttostr(b_pan_qty);     //投B板
       end;
       ...
       stg1.Cells[8,i]:= Floattostrf((strtoint(stg1.Cells[5,i]) + StrToInt(Trim(edtref.Text)) -strtoint(stg1.Cells[3,i]))/
                                        strtoint(stg1.Cells[3,i])*100,ffFixed,8,2);   //超投率%
}
       stg1.Cells[5,i]:=Edit9.Text;       //投产数
       stg1.Cells[6,i]:=edtPNLA.Text;    //投A板
       stg1.Cells[7,i]:=edtPNLB.Text;   //投B板
       ...
       stg1.cells[16,i]:=Trim(edtref.Text);  //'引用库存';

       stg1.Cells[17,i] := cbb1.Items[cbb1.itemindex];
       ...
       while not dm.ADOQuery1.Eof do
        begin
         ...
         if (stg2.Cells[14,stg2.RowCount-2]<>
              dm.ADOQuery1.fieldbyname('DEPT_PTR').AsString) or (dm.ADOQuery1.RecNo=1) then
   ...
   //////HCC修改正常投产单张A板工单不够最佳流转批量///////////////////////////////////////////////////////////////////////
     if (trunc(edtPNLA.IntValue/PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE)=0) and (ceil(edtPNLA.IntValue/PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE)=1) then
     begin
      ...
      if stg3.Cells[10,stg3.RowCount-1]<> stg3.Cells[10,stg3.RowCount-2] then
       ...
       ceil(edtPNLA.IntValue/PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE),i_j,edtPNLA.IntValue);
       stg3.Cells[5,stg3.RowCount-1]:=stg1.Cells[12,i];  //A板PCS数
       stg3.Cells[4,stg3.RowCount-1]:=
       inttostr(edtPNLA.IntValue mod PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE);
     ...
     begin
     for i_j:=1 to Trunc(edtPNLA.IntValue/PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE) do
      begin
       stg3.Cells[10,stg3.RowCount-1]:= stg1.Cells[11,i];
       if stg3.Cells[10,stg3.RowCount-1]<> stg3.Cells[10,stg3.RowCount-2] then
        begin
       ...
       stg3.Cells[2,stg3.RowCount-1]:=wo_number(stg1.Cells[0,i],0,      //作业单号
          Trunc(edtPNLA.IntValue/PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE),i_j,edtPNLA.IntValue);
       stg3.Cells[5,stg3.RowCount-1]:=stg1.Cells[12,i];  //A板PCS数
       if i_j < Trunc(edtPNLA.IntValue/PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE) then
        begin
        ...
        begin
          if edtPNLA.IntValue mod PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE = 0 then
           stg3.Cells[4,stg3.RowCount-1]:= inttostr(PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE)
          else
           stg3.Cells[4,stg3.RowCount-1]:=
             inttostr(edtPNLA.IntValue mod PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE+PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE);
           stg3.Cells[3,stg3.RowCount-1]:= inttostr(strtoint(stg3.Cells[4,stg3.RowCount-1])*
                                                    strtoint(stg3.Cells[5,stg3.RowCount-1]));
        end;
//       stg3.Cells[6,stg3.RowCount-1] := stg1.Cells[2,i];  //完工日期
     ...
     if (Ceil(edtPNLB.IntValue/PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE)=1) and (Trunc(edtPNLB.IntValue/PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE)=0) then
      begin
       ...
       stg3.Cells[2,stg3.RowCount-1]:=self.wo_number(stg1.Cells[0,i],1, //作业单号
               ceil(edtPNLB.IntValue/PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE),i_j,edtPNLB.IntValue);
       stg3.Cells[5,stg3.RowCount-1]:=stg1.Cells[13,i];  //pcs/bpnl
                 stg3.Cells[4,stg3.RowCount-1]:=
           inttostr(edtPNLB.IntValue mod PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE);
     ...
     begin
     for i_j:=1 to Trunc(edtPNLB.IntValue/PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE) do
      begin
       stg3.Cells[10,stg3.RowCount-1]:= stg1.Cells[11,i];  //rkey25
       if stg3.Cells[10,stg3.RowCount-1]<> stg3.Cells[10,stg3.RowCount-2] then
        begin
       ...
       stg3.Cells[2,stg3.RowCount-1]:=self.wo_number(stg1.Cells[0,i],1, //作业单号
               Trunc(edtPNLB.IntValue/PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE),i_j,edtPNLB.IntValue);
       stg3.Cells[5,stg3.RowCount-1]:=stg1.Cells[13,i];  //pcs/bpnl

       if i_j < Trunc(edtPNLB.IntValue/PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE) then
        begin
         stg3.Cells[4,stg3.RowCount-1]:= inttostr(PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE);
         stg3.Cells[3,stg3.RowCount-1]:= inttostr(strtoint(stg3.Cells[4,stg3.RowCount-1])*
                                         strtoint(stg3.Cells[5,stg3.RowCount-1]));
        ...
        begin
         if edtPNLB.IntValue mod PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE=0 then
          stg3.Cells[4,stg3.RowCount-1]:= inttostr(PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE)
         else
          stg3.Cells[4,stg3.RowCount-1]:=
           inttostr(edtPNLB.IntValue mod PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE+PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE);
     ...
     for i:=1 to  sg2.RowCount-1 do
      with dm.ADOQuery1 do
       ...
       try                                                     //20170615 mark by zsp
         if dm.qry53.Active then
             DM.qry53.UpdateBatch(arAll);
       except
         dm.ADOConnection1.RollbackTrans;
         ShowMessage('成品库存记录发生改变，可能多人同时操作，请退出程序后重新投产！！');
...
// if frmQnty <> nil then frmQnty.Free;
end;
...
//  end;
  ...
  try
    ...
    begin
      ...
      begin
       if DM.cdsQnty1IsSelected.AsInteger = 1 then qty:= qty + DM.cdsQnty1QTY_ON_HAND.value;
       DM.cdsQnty1.Next;
      end;
      Edit14.Text:= FloattoStr(qq[0].QTY_ON_HAND + qty);  //已绑定订单数
      Edit15.Text:= FloattoStr(qq[1].QTY_ON_HAND - qty);  //未绑定订单数
      Edit16.Text:= FloattoStr(qq[2].QTY_ON_HAND + qty);  //本订单绑定数
      Edit4.Text:= FloattoStr(dm.ADS60parts_margin.AsFloat - StrToFloat(Trim(Edit16.Text)));  //订单差数
      edtref.Text:= FloatToStr(qty); //订单引用数
  //    if StrToFloat(Trim(SG2.Cells[5,get_mainso(dm.ADS60SALES_ORDER.AsString)])) - qty > 0 then
  //      SG2.Cells[5,get_mainso(dm.ADS60SALES_ORDER.AsString)]:= FloatToStr( StrToFloat(Trim(SG2.Cells[5,get_mainso(dm.ADS60SALES_ORDER.AsString)])) - qty)
  //    else
  //      SG2.Cells[5,get_mainso(dm.ADS60SALES_ORDER.AsString)]:= '0';
  //    Edit5.Text:= SG2.Cells[5,get_mainso(dm.ADS60SALES_ORDER.AsString)];
      edit5.Text:=inttostr(self.total_tobeplanned);   //本次计划数合计  减去了引用库存数
      edit6.Text:=inttostr(ceil(strtoint(edit5.Text)*(1+self.EST_SCRAP*0.01)));//计划数
      self.ComboBox1Change(sender);   //投产数 pnla pnlb
    end;
  finally
    frmQnty.Free;
  end;
...
procedure TForm_salesorder.edtPNLAExit(Sender: TObject);
begin
 if (edtPNLA.IntValue=0) and (edtPNLB.IntValue=0) then
 begin
...
procedure TForm_salesorder.edtPNLAChange(Sender: TObject);
begin
  Edit9.Text := IntToStr(StrToInt(Edit7.Text) * edtPNLA.IntValue
            + StrToInt(Edit8.Text) * edtPNLB.IntValue);
  if strtoint(edit2.Text) > 0 then
    edit12.Text := Floattostrf(                          //超投率%
      (strtoint(edit9.Text) + StrToInt(edtref.Text) - strtoint(edit2.Text))
            / strtoint(edit2.Text) * 100, ffFixed, 8, 2);
  edit13.Text := Floattostrf(                          //超投面积
    (strtoint(edit9.Text) + StrToInt(edtref.Text) - strtoint(edit2.Text))
            * dm.ADS60spell_sq.Value, ffFixed, 8, 3);

   if ((PNLA * StrToInt(Edit7.Text)  + StrToInt(Edit8.Text) * PNLB) < StrToInt(Edit9.Text)) then
     if self.d25_type = 0 then  //如果批量
     begin
       if (StrToFloat(edit2.Text) * dm.ADS60spell_sq.Value <= self.ppc25) and //小订单
         (strtofloat(edit13.Text) < self.ppc26) then   //超投面积小于0.5平米
...
procedure TForm_salesorder.edtPNLAChanging(Sender: TObject;
  NewValue: Extended; var AllowChange: Boolean);
begin
  if StrToInt(Edit7.Text)=0 then AllowChange:=false;
end;

procedure TForm_salesorder.edtPNLBChanging(Sender: TObject;
  NewValue: Extended; var AllowChange: Boolean);
begin
  if StrToInt(Edit8.Text)=0 then AllowChange:=false;
end;

function TForm_salesorder.isFA(Rkey60: string;
  var FAQty: Integer): Boolean;
begin
  with dm.ADOQuery1 do
  begin
   close;
   sql.Text:= 'SELECT top 1 QUAN_ON_HAND FROM dbo.FA0745 WHERE rkey60 = ' + Rkey60 + ' AND check_status =1';
   open;
   if not IsEmpty then
   begin
      FAQty:= FieldByName('QUAN_ON_HAND').AsInteger;
     Result:= true;
   end
   else Result:= False;
  end
