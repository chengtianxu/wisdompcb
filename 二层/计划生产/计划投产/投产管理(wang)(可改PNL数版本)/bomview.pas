...
type
  ...
  private
    ...
    function get_EST_SCRAP_lmt(rkey25:integer):Double;
...
function TForm_bomview.get_EST_SCRAP_lmt(rkey25: integer): Double;
begin
  with dm.ADOQuery1 do
  begin
    close;
    sql.Text:='select case data0025.ttype when 0 then data0008.est_scrap_ltm '+
              'else data0008.est_scrap1_lmt end as est_scrap_top '+
...
begin
 ...
 self.EST_SCRAP:=get_EST_SCRAP_lmt(self.rkey25);  //产品类型最高报废率
...
//     else
     ...
     stg1.Cells[17,i]:= cbb1.Items[cbb1.itemindex];        //    增加于2018-08-10 ZSP
    ...
    /////////////////////////////Hcc修改HZXZG公司投产后，最后一张工卡如果不够最佳流转批量自动合到前一工单///////////////////////////////////////
    if (Ceil(strtoint(stg1.Cells[6,i])/PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE)=1) and (trunc(strtoint(stg1.Cells[6,i])/PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE)=0) then //一个作业单不够最佳流转批量
    begin
    ...
    begin
     for i_j:=1 to Trunc(strtoint(stg1.Cells[6,i])/PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE) do
     begin
     stg3.Cells[10,stg3.RowCount-1]:= stg1.Cells[11,i];     //25.rkey
     if stg3.Cells[10,stg3.RowCount-1]<> stg3.Cells[10,stg3.RowCount-2] then
      begin
     ...
     stg3.Cells[2,stg3.RowCount-1]:=wo_number(stg1.Cells[0,i],0,      //作业单号
         Trunc(strtoint(stg1.Cells[6,i])/PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE),i_j,b_pan_qty);

     stg3.Cells[5,stg3.RowCount-1]:=stg1.Cells[12,i];  //A板PCS数

     if i_j < Trunc(strtoint(stg1.Cells[6,i])/PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE) then
      begin
      ...
      begin
       if strtoint(stg1.Cells[6,i]) mod PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE=0 then
        ...
        stg3.Cells[4,stg3.RowCount-1]:=
             inttostr(strtoint(stg1.Cells[6,i]) mod PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE+PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE);
...
//      ShowMessage(cbb1.Items[cbb1.itemindex]);
    ...
    //////修改hcc单张B板工卡不够最佳流转批量//////////
    if (Ceil(strtoint(stg1.Cells[7,i])/PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE)=1) and (trunc(strtoint(stg1.Cells[7,i])/PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE)=0) then
    begin
     ...
     begin
      ...
      stg3.Cells[3,stg3.RowCount-1]:= inttostr(strtoint(stg3.Cells[4,stg3.RowCount-1])*strtoint(stg3.Cells[5,stg3.RowCount-1]));
    ...
    begin
     for i_j:=1 to Trunc(strtoint(stg1.Cells[7,i])/PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE) do
      begin
       stg3.Cells[10,stg3.RowCount-1]:= stg1.Cells[11,i];  //rkey25
       if stg3.Cells[10,stg3.RowCount-1]<> stg3.Cells[10,stg3.RowCount-2] then
        begin
       ...
       stg3.Cells[2,stg3.RowCount-1]:=self.wo_number(stg1.Cells[0,i],1, //作业单号
            Trunc(strtoint(stg1.Cells[7,i])/PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE),i_j,b_pan_qty);

       stg3.Cells[5,stg3.RowCount-1]:=stg1.Cells[13,i];  //pcs/bpnl

       if i_j < Trunc(strtoint(stg1.Cells[7,i])/PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE) then
        begin
         stg3.Cells[4,stg3.RowCount-1]:= inttostr(PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE);
         stg3.Cells[3,stg3.RowCount-1]:= inttostr(strtoint(stg3.Cells[4,stg3.RowCount-1])*
                                                  strtoint(stg3.Cells[5,stg3.RowCount-1]));
        ...
        begin
         if strtoint(stg1.Cells[7,i]) mod PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE=0 then
          ...
          stg3.Cells[4,stg3.RowCount-1]:=
           inttostr(strtoint(stg1.Cells[7,i]) mod PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE+PTreeValue(self.TreeView1.Items[i-1].data)^.OPT_LOT_SIZE);
 ...
 with Form_putprod do
  begin
   ...
   begin
    Close;
    sql.Clear;
    sql.Add('select rkey from data0034 where dept_code='+quotedstr('RFG'));
    Open;
   end;
   if not dm.ADOQuery1.IsEmpty then
    deptptr:=DM.ADOQuery1.fieldbyname('rkey').Value
   else
   ...
   stg1.Cells[17,1]:= cbb1.Items[cbb1.itemindex];        //    增加于2018-08-10 ZSP
