...
//3.增加56在线记
    ...
    //数据更新启动事务保护
      ...
      sqlstr:= 'Insert into data0056(wo_ptr,dept_ptr,intime,qty_backlog,panels,step,employee_ptr,loc_ptr,wtype,put_type,TDate)'+
               ...
               cdsWOReleaseList.fieldbyname('wtype').AsString+','+cdsWOReleaseList.fieldbyname('put_type').AsString+',getdate())';
  ...
  //6.如果第一个需要过数的工序步骤大于1,那么,前面几个需要过数的工序需要在48中增加之前步骤的流转记录
      ...
      begin
        ...
        begin
          ...
          ado0048.FieldByName('put_type').AsInteger:= cdsWOReleaseList.fieldbyname('put_type').AsInteger  ;
...
var
  sqlstr:string;
begin
  sqlstr:=' select D492.TStatus,D06.WIPMAT_STATUS,D06.PROD_STATUS FROM Data0006 D06 '+
