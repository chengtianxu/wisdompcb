...
type
  ...
  private
    ...
    procedure Single_Print(rKey:integer;subPrint,ipdPrint:boolean);
...
2）	记录暂缓日志：记录暂缓的操作人，操作时间等信息，Data0017新增记录，Source_Type=2，Action类型为1
  ...
  try
    ...
    sqlstr:='update data0006 set prod_status =103 where prod_status in (3,4) and rKey='+IntToStr(rKey);
...
2）	记录暂缓日志：记录暂缓的操作人，操作时间等信息，Data0017新增记录，Source_Type=2，Action类型为2
  ...
  try
    if not dmcon.ADOConnection1.InTransaction then
      dmcon.ADOConnection1.BeginTrans;
    sqlstr:='update data0006 set prod_status =3 where prod_status=103 and rKey='+IntToStr(rKey);
    if not MyDataClass.ExecSql(sqlstr) then
      abort;
...
procedure TfrmWOCtrl.MenuItem2Click(Sender: TObject);
begin
  if (strtoint(vprev)=1) or (strtoint(vprev)=3) then
  begin
    ShowMsg('对不起!您没有该程序的可写权限',1);
                  ...
                  +' and (Data0025.PARENT_PTR is null)'
                   +' AND (Data0056.STEP IN'
                    ...
                    +' WHERE data0038.source_ptr = data0025.rkey AND'
                          ...
                          +' data0038.flow_no = data0056.flow_no)) AND (Data0006.RKEY ='+ dmcon.adsWOCtrlList.FieldByName('rKey').Asstring+')'
...
procedure TfrmWOCtrl.MenuItem4Click(Sender: TObject);
var
  prod_status:integer;
//  sqlstr:string;
begin
...
var
  b_OK,b_print_Layers,b_print_impedance:boolean;
begin
...
procedure TfrmWOCtrl.Single_Print(rKey: integer;subPrint,ipdPrint:boolean);
begin
  with tform_report.Create(application) do
  try
    ads06.Close;
...
//只有状态为：生产中，外发生产，待入仓，待分配的作业单才能打印。
  ...
  try
    ...
    begin
      ...
      begin
        Single_Print(StrToInt(StG2.cells[2,i]),CheckBox1.Checked,CheckBox2.Checked);
        if subsql='' then
