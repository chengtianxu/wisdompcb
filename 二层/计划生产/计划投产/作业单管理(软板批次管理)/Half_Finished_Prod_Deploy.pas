...
begin
  ...
  s:='SELECT barcode_flag,D06.RKEY,D06.WORK_ORDER_NUMBER,D06.BOM_PTR,D06.QUAN_SCH,'+
     'D06.WIPMAT_STATUS,D06.PROD_STATUS,D06.ENT_DATETIME,D06.SCH_COMPL_DATE,D06.PARTS_PER_PANEL,'+
...
procedure TfrmHalf_Finished_Prod_Deploy.CancelDeploy(Isall: boolean);
  ...
  begin
    ...
    try
      ...
      if ((Dmcon.adsProdList.FieldByName('PROD_STATUS').AsInteger<>2) or
          (Dmcon.adsProdList.FieldByName('WIPMAT_STATUS').AsInteger<>1)) then
      begin
        ShowMessage('只有待发放且已分配的作业单才能取消分配');
        Abort;
      end
      else
        Result := true;
