...
uses damo,Pick_Item_Single2 ,ConstVar,common,Math;
  ...
  try
    ...
    if frmPick_Item_Single2.ShowModal=mrok then
     begin
      ...
      if  (RadioGroup1.ItemIndex=0) then
      begin
      edit23.Text:=frmPick_Item_Single2.adsPick1.Fieldbyname('QUAN_ON_HAND2').AsString;
      end
      else
 ...
 for i_j:=1 to FrmEditBom.stg2.RowCount-2 do
  ...
  { dm.ADOQuery7.Close;
   ...
   begin
    ...
    begin
    if (strtofloat(Edit6.Text)>strtofloat(Edit23.Text))  then
    begin
...
//     dm.ads494.CommandText:=DM.ads494SQL;                      //制程参数
     dm.ads494.Parameters[0].Value:=rkey25.Caption;
     dm.ads494.Parameters[1].Value:=edit9.Text;
     dm.ads494.Open;
   end else
