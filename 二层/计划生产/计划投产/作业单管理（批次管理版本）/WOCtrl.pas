...
type
  TfrmWOCtrl = class(TForm)
    ...
    procedure Lot2Click(Sender: TObject);
    procedure LOT3Click(Sender: TObject);
  private
    { Private declarations }
    vColumn:TColumnEh;
    StartDate,EndDate:TDatetime;
    b_print_cbx1:Boolean;
    b_print_cbcheck3:Boolean;
    b_print_cbcheck4:Boolean;
    ...
    procedure Single_Print(rKey:integer;subPrint,ipdPrint:boolean;print29: Boolean = False;print30: Boolean = False;print31: Boolean = False;print502: Boolean = False);
    procedure Single_equipmentPrint(rKey25:integer);
    procedure Single_LOT(rKey06:integer);
  public
    { Public declarations }
  end;
...
var
  b_OK,b_print_Layers,b_print_impedance,b_print_29, b_print_502:boolean;
  ckstr:string;
begin
...
//只有状态为：生产中，外发生产，待入仓，待分配的作业单才能打印。
  ...
  TRY
       ...
       if (cbx1.Checked=False) and (CheckBox3.Checked=False)and (CheckBox4.Checked=False) then
       begin
       ...
       begin
        ckstr:='';
        if cbx1.Checked then ckstr:='0,';
        if CheckBox3.Checked then ckstr:=ckstr+'1,';
        if CheckBox4.Checked then ckstr:=ckstr+'2,';
...
procedure TfrmWOCtrl.Single_Print(rKey: integer;subPrint,ipdPrint:boolean;print29: Boolean = False;print30: Boolean = False;print31: Boolean = False;print502: Boolean = False);
var ckstr:string;
begin
  with tform_report.Create(application) do
  try
    ...
    begin
      ...
      ADS25.Open;
       ...
       if (b_print_cbx1=False) and (b_print_cbcheck3=False)and (b_print_cbcheck4=False) then
       begin
        ppSubReport3.Visible := false;
       end
       else
       begin
        ckstr:='';
        if b_print_cbx1 then ckstr:='0,';
        if b_print_cbcheck3 then ckstr:=ckstr+'1,';
        if b_print_cbcheck4 then ckstr:=ckstr+'2,';
        ads29.CommandText:=ads29.CommandText+'and flow_no IN('+ copy(ckstr,0,Length(ckstr)-1)+') ORDER BY SEQ_NO';
        ppSubReport3.Visible := True;
       end;
...
//只有状态为：生产中，外发生产，待入仓，待分配的作业单才能打印。
  ...
  try
    if Showmodal = mrok then
    begin
      b_print_cbx1:=cbx1.Checked;
      b_print_cbcheck3:=CheckBox3.Checked;
      b_print_cbcheck4:=CheckBox4.Checked;
      subsql:='';
      for i:=1 to StG2.RowCount-2 do
      begin
        Single_Print(StrToInt(StG2.cells[2,i]),CheckBox1.Checked,CheckBox2.Checked,cbx1.checked,CheckBox3.checked,CheckBox4.checked, cbx2.Checked);
        if subsql='' then
...
procedure TfrmWOCtrl.LOT1Click(Sender: TObject);
begin
  with Tfrm_LOTScrapD.Create(Application) do
  try
    ADS06.Close;
    ADS06.Parameters.ParamValues['rkey06'] := Dmcon.adsWOCtrlList.FieldByName('rkey').AsString;
    ADS06.Open;
    ppReport1.Reset;
    ppReport1.Template.FileName := StringReplace(UpperCase(GetCurrentDir),'EXEC','NIERP\REPORT\',[rfReplaceAll])+
                                  'LOTScrap_Report_3.rtm';
    ppReport1.Template.LoadFromFile;
    ppReport1.SaveAsTemplate := False;
    ppReport1.Print;
  finally
...
procedure TfrmWOCtrl.Lot2Click(Sender: TObject);
begin
  with Tfrm_LOTScrapD.Create(Application) do
  try
    ppReport1.Reset;
...
procedure TfrmWOCtrl.Single_LOT(rKey06: integer);
begin
  with Tfrm_LOTScrapD.Create(application) do
  try
    ADS06.Close;
    ADS06.Parameters.ParamValues['rkey06'] := rkey06;
    ADS06.Open;
    ppReport1.Reset;
    ppReport1.Template.FileName := StringReplace(UpperCase(GetCurrentDir),'EXEC','NIERP\REPORT\',[rfReplaceAll])+
                                  'LOTScrap_Report_3.rtm';
...
procedure TfrmWOCtrl.LOT3Click(Sender: TObject);
var
  i:integer;
begin
  if strtoint(vprev)<>4 then
  ...
  try
   ...
   cbx1.Visible:=false;
   cbx2.Visible:=false;
   caption:='连续打印LOT报废明细清单';
   if Showmodal = mrok then
    for i:=1 to StG2.RowCount-2 do
     Single_LOT(StrToInt(StG2.cells[2,i]));
  finally
    free;
  end;
