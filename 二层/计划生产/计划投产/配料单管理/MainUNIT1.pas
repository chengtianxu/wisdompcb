...
type
  ...
  private
    ...
    procedure PrintMoKL(ttype:string);
...
USES
   ...
   ,TReport,   MulPrinUnit1, Ubom, Uallmx;
...
procedure TMainForm1.PrintMoKL(ttype: string);
begin
  With dm.TMPADO1 do //显示CUTTING
  begin
    close ;
...
//        DBGridEh1.DataSource.DataSet.Fieldbyname('cut_no').asstring;
    ...
    FrmReport.ppReport1.Reset;
    FrmReport.ppReport1.Template.FileName :=
                  stringReplace(UpperCase(GetCurrentDir),'EXEC','NIERP\REPORT\',[rfReplaceAll])+'\Newmoreport2.rtm';
    FrmReport.ppReport1.Template.LoadFromFile;
    try
      FrmReport.ppImage1.Picture.Assign(Load_JPG_Pic_From_DB(dm.TMPADO1sheet_BMP));
    except
...
//-----------2013.07.11修改工序参数与材料参数不一致时，查询278表PROTECTION_FLAG是否为1，为1必须一致，否则不通过
   try
     try
      ...
      //dm.ADOQuery12.Close;
      //dm.ADOQuery12.Parameters.ParamByName('CUT_NO').Value:=quotedstr(dm.ADOQALL2CUT_NO.AsString);
      //dm.ADOQuery12.open;

       //IF dm.ADOQuery12.FieldByName('Invent_or')
       ...
       tmpAdo2.SQL.Text:='select dbo.data0468.*,dbo.data0017.group_ptr from dbo.data0468'
                        ...
                        +'  where  dbo.data0468.CUT_NO='+quotedstr(dm.ADOQALL2CUT_NO.AsString);
       ...
       begin
         ...
         begin
           ...
           dm.TemADOQuery1.sql.Text:=
            ...
            'where Data0468.cut_no='+quotedstr(dm.ADOQALL2CUT_NO.AsString);
           dm.TemADOQuery1.open;
          ...
          tmpAdo3.Close;
          tmpAdo3.SQL.Clear;
          tmpAdo3.SQL.Text:='select invent_or from dbo.data0468'
                        +'  where CUT_NO='+quotedstr(dm.ADOQALL2CUT_NO.AsString)
                        +' and invent_or=0';
              ...
              begin
                ...
                begin
                  ...
                  begin
                    ...
                    begin
                       if (DM.TemADOQuery1.IsEmpty) and (not tmpAdo3.IsEmpty) then
                       begin
...
procedure TMainForm1.N9Click(Sender: TObject);
begin
  With dm.TMPADO1 do //显示CUTTING
  begin
       close ;
...
procedure TMainForm1.BOM1Click(Sender: TObject);
begin
 if dm.ADOQALL2.IsEmpty then Exit;
  Application.CreateForm(TForm_BOM, Form_BOM);
  TRY

   Form_BOM.rkey492:=dm.ADOQALL2rkey.Value;
   Form_BOM.rkey25:=DM.ADOQALL2d025_rkey.Value;
   Form_bom.user_ptr:=user_ptr;
   Form_bom.tcsl:=DM.ADOQALL2ISSUED_QTY.AsString;
   Form_BOM.ShowModal;
  FINALLY
   Form_BOM.Free ;
  END ;
