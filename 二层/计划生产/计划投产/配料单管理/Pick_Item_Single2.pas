...
type
  TfrmPick_Item_Single2 = class(TForm)
    Panel1: TPanel;
    Panel2: TPanel;
    OKBtn: TButton;
    CancelBtn: TButton;
...
var
  frmPick_Item_Single2: TfrmPick_Item_Single2;
...
procedure TfrmPick_Item_Single2.NewGridWnd(var Message: TMessage);
var
 IsNeg : Boolean;
begin
...
procedure TfrmPick_Item_Single2.Add_Grid_Fields(Fields:string);
var
  i,j:integer;
...
function TfrmPick_Item_Single2.Get_Rslt: variant;
begin
  result := adsPick.fieldbyName(KeyField).Value;
end;

procedure TfrmPick_Item_Single2.InitForm(InputVar: PDlgInput);
begin
  adsPick:=adsPick1;
...
procedure TfrmPick_Item_Single2.DBGrid1DblClick(Sender: TObject);
begin
if not self.adsPick.IsEmpty then
  ModalResult := mrok;
end;

procedure TfrmPick_Item_Single2.Adjust_Column_Width;
var
  i :integer;
  TotalSize:integer;
begin
...
procedure TfrmPick_Item_Single2.DBGrid1TitleClick(Column: TColumn);
begin
...
procedure TfrmPick_Item_Single2.DBGrid1KeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TfrmPick_Item_Single2.edtKeyValueChange(Sender: TObject);
begin
...
procedure TfrmPick_Item_Single2.Add_Grid_Fields_New(StrFields: string);
var
...
procedure TfrmPick_Item_Single2.InitForm_New(InputVar: PDlgInput;dst:TADODataSet=nil);
var
...
procedure TfrmPick_Item_Single2.DBGrid1KeyDown(Sender: TObject;
...
procedure TfrmPick_Item_Single2.dsPickDataChange(Sender: TObject;
  Field: TField);
begin
...
procedure TfrmPick_Item_Single2.FormCreate(Sender: TObject);
begin
 OldGridWnd := DBGrid1.WindowProc;
  DBGrid1.WindowProc := NewGridWnd;
end;

procedure TfrmPick_Item_Single2.DBGrid1DrawColumnCell(Sender: TObject;
  const Rect: TRect; DataCol: Integer; Column: TColumn;
  State: TGridDrawState);
begin
{if (adsPick.Fieldbyname('XY_1').AsString = '余料库存')  then
dbgrid1.Canvas.Brush.color:=clyellow;
dbgrid1.DefaultDrawColumnCell(Rect,DataCol,Column,State);}


 if (COlumn.FieldName = 'XY_1') and
    (adsPick.Fieldbyname('XY_1').Asstring<>null) and
    (adsPick.Fieldbyname('XY_1').Asstring = '余料库存') then
  begin
    dbgrid1.Canvas.Brush.color:=clyellow;
