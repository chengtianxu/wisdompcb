...
begin
  ...
  dmcon.cdsPickWorkShop.Filter:='';
  dmcon.adsPickWorkShop.Filtered:=false;
  dmcon.adsPickWorkShop.Close;
  dmcon.adsPickWorkShop.Open;
  MyDataClass.UpdateValueToField(dmcon.cdsPickWorkShop,checkbox1.Checked,'IsSelected');
  dmcon.cdsPickWorkShop.Locate('rKey',rKey,[]);
end;
...
begin
  ...
  begin
    try
      dmcon.cdsPickWorkShop.DisableControls;
      MyDataClass.Set_Filter('IsSelected=1',dmcon.cdsPickWorkShop);
      if dmcon.cdsPickWorkShop.IsEmpty then
      begin
        ShowMsg('请选择工序',1);
        canclose:= false;
        dmcon.cdsPickWorkShop.Filter:='';
        dmcon.cdsPickWorkShop.Filtered := false;
        exit;
      end;
    finally
      dmcon.cdsPickWorkShop.EnableControls;
    end;
  end;
end;
...
begin
  edtKey.Enabled:= not Dmcon.cdsPickWorkShop.IsEmpty;
  if Dmcon.cdsPickWorkShop.IsEmpty then abort;
...
begin
  if not dmcon.cdsPickWorkShop.Active then abort;
  DataSet:=dmcon.cdsPickWorkShop;
