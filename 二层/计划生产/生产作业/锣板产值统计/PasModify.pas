...
uses DAMO,Pick_Item_Single,ConstVar,DateUtils, PasParaEdit, PasTable,ADODB,DB;
...
begin
  ...
  begin
    DB571.Columns[3].ReadOnly:=False;
    DB571.Columns[11].ReadOnly:=False;
    DB571.Columns[12].ReadOnly:=False;
    DB571.Columns[15].ReadOnly:=False;
  end;
  if status=1 then
  begin
...
var
  ...
  theory_result,strname:string;
  theory_cur,temp:Currency;
...
//  paraList.Add('板厚钻孔');
  ...
  try
    ...
    begin
      ...
      begin
        if not (TryStrToCurr(DM.cds571PARA_VALUE_0.AsString,temp) and TryStrToCurr(DM.cds571PARA_VALUE_1.AsString,temp)) then
        begin
          dm.cds571.Next;
          Continue;
        end;
...
//        strExpress:=StringReplace(strExpress,'LAYERS',DM.cds571LAYERS.AsString,[rfReplaceAll,rfIgnoreCase]);
        strExpress:=StringReplace(strExpress,'完成板厚mm',DM.cds571PARA_VALUE_0.AsString,[rfReplaceAll,rfIgnoreCase]);
        strExpress:=StringReplace(strExpress,'IRG_LONG',DM.cds571PARA_VALUE_1.AsString,[rfReplaceAll,rfIgnoreCase]);
...
//        strExpress:=StringReplace(strExpress,'DGNUMBER',DM.cds571PARA_VALUE_3.AsString,[rfReplaceAll,rfIgnoreCase]);
          ...
          paraPos:=Pos(strname,strExpress);
          if paraPos<>0 then                                   //pan_qty * IRG_LONG * 锣边板厚( LAM_THK , 1 )
          begin
    ...
    //        ShowMessage(expressLeft+#13+ expressMid +#13+ ExpressRight);
            ...
            begin
              Close;
              SQL.Clear;
              SQL.Text:='select rkey from data0280 where eng_table_name = '+ QuotedStr(strname);
              Open;
              if not IsEmpty then
              begin
...
begin
  ...
  begin
    Close;
    SQL.Clear;
    SQL.Text:='select top 1 data0571.Statistic_ptr,data0571.MANU_PART_NUMBER,data0571.PARA_VALUE_0,data0571.PARA_VALUE_1 '+  #13+
...
//      ShowMessage(SQL.text);
    ...
    begin
      Close;
      SQL.Clear;
      SQL.Text:='SELECT     dbo.DATA0569.PARA_PTR_0, dbo.DATA0569.PARA_PTR_1 '+#13+
...
//        end;
      ...
      begin
        ...
        SQL.Text:='SELECT   dbo.Data0494.PARAMETER_VALUE, dbo.Data0278.PARAMETER_NAME, dbo.Data0494.custpart_ptr '+ #13+
                  '  FROM         dbo.Data0494 INNER JOIN  '+ #13+
                  '                        dbo.Data0278 ON dbo.Data0494.PARAMETER_PTR = dbo.Data0278.RKEY   '+ #13+
                  ' WHERE     (dbo.Data0278.rkey='+ DM.cds571PARA_PTR_1.AsString +') AND (dbo.Data0494.custpart_ptr = '+ IntToStr(custRkey) +')';
//        ShowMessage(SQL.Text);
        Open;
        if not IsEmpty then
        begin
...
//          ShowMessage(SQL.Text);
        ...
        begin
          strSQL:= 'insert data0571(Statistic_ptr, recno_number, ttype, MANU_PART_NUMBER, MANU_PART_DESC,PANEL_A_B, '+
                   ' pan_qty, pcs_qty, pan_size,pcs_size, pan_sqrt, pcs_sqrt, LAYERS, theory_result, PARA_PTR_0, PARA_PTR_1,  '+
                   '  PARA_VALUE_0, PARA_VALUE_1,remark,if_read571) '+ #13+
                   ...
                   ',' + DM.cds571PARA_PTR_1.AsString +
                   ',' + QuotedStr(DM.cds571PARA_VALUE_0.AsString) + ',' + QuotedStr(DM.cds571PARA_VALUE_1.AsString) +
                   ',' + QuotedStr(DM.cds571REMARK.AsString)+',' + QuotedStr(DM.cds571if_read571.AsString) +')'+ #13+ strSql;
           DM.cds571.Next;
        end;
        DM.cds571.EnableControls;
        with dm.adoTmp do
...
begin
  ...
  try
    ...
    InputVar.Sqlstr := 'select data0569.formula_number,data0569.theorycalc_meth,data0034.rkey,data0034.dept_code,data0034.dept_name '+
                      ...
                      ' where data0034.dept_code like ''%RG%''';
    Inputvar.KeyField:='';
...
function TfrmModify.getHFPrice(MENU_NUMBER: string;
  theory: Currency): string;
var
