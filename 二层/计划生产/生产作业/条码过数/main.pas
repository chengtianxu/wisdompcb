...
begin
  ...
  with dmcon.aqTmp do //新增登录日志
  begin
   close;
   SQL.Clear;
   SQL.Add('insert into data0630(compute_name,network_ip,login_date,EMPL_PTR)');
   SQL.Add('Values (:compute_name,:network_ip,:login_date,:empl_ptr)');
   Parameters.ParamByName('compute_name').Value:=GetLocalName;
   Parameters.ParamByName('network_ip').Value:=GetLocalIP;
   Parameters.ParamByName('EMPL_PTR').Value:=dmcon.adsUserInfo.fieldbyname('empl_ptr').Value;
   Parameters.ParamByName('login_date').Value:=getsystem_date(dmcon.tmp,0);
   ExecSQL;
  end;
end;
...
begin
 try
  ...
  InputVar.Fields := 'Work_Order_Number/作业单号/140,MANU_PART_NUMBER/本厂编号/100,'+
   'DEPT_CODE/工序编号/80,CUT_NO/配料单号/100,Qty_BackLog/在线数量/80';
