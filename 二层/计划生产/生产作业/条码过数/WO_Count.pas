...
uses
  ...
  DBGrids,AdoDB, ppComm, ppRelatv, ppProd, ppClass, ppReport, DBGridEh,
  ComCtrls;
...
implementation
uses dm,MyClass,common,Dlg_Wo_Qty,report_data, auto_deploy;
{$R *.dfm}
...
var c:char;
    ...
    Item: TListItem;
begin
  if not dmcon.Check_WO(WoNo) then  abort; //判断作业单是否处于生产中或者外发生产状态(内层未分配时分配数量是否足够),否则不允许过数
  ...
  if (dmcon.adsWOInfo.fieldByname('WIPMAT_STATUS').AsInteger=0) and (dmcon.adsWOInfo.FieldByName('wtype').value=0) then//作业单处于正常过数且未分配状态开始自动分配
  begin
  dmcon.adsProdList.Close;
  dmcon.adsProdList.Parameters[0].Value:=WoNo;
  ...
  try
    ...
    if dmcon.AutoDeployWO(dmcon.adsAuto_WO_List) and dmcon.UpdateWOStatus(dmcon.adsProdList.fieldbyName('rKey').AsInteger,1) then
    begin
      ...
      ShowMsg('自动分配内层数量失败,请手动分配数量!','提示',1);
    end;
  except
    dmcon.ADOConnection1.RollbackTrans;
  end;
...
//显示
 ...
 // while not dmcon.tmp.Eof do
  ...
  s:=trim(DBEdit1.Text);
  c:=s[length(S)];
  if not (c in ['0'..'9']) then   //已经是拆分的子单不能拆分
    b:=false
  else
  begin
    dmcon.tmp.Close;
    ...
    dmcon.tmp.Open;
    b:=not dmcon.tmp.IsEmpty ;
    if b then
    begin
      dmcon.tmp.Close;
      dmcon.tmp.SQL.Text:='select work_order_number  from data0006 where work_order_number like '''+DBEdit1.Text+'%'' order by 1 desc' ;
      dmcon.tmp.Open;
      c:=dmcon.tmp.fields[0].asstring[length(dmcon.tmp.fields[0].asstring)];
      if c in ['0'..'9'] then
        sNWkNo:=s+'A'
      else if c in ['A','B','a','b'] then    //在此最多拆分3次
        sNWkNo:=s+Chr(Ord(c)+1)
      else
        b:=false;
    end ;
  end;
...
begin
 ...
 if (not dmcon.ADOQuery1.IsEmpty) and (dmcon.ADOQuery1.FieldByName('old_step_no').Value>0) then
 begin
...
procedure TfrmWO_Count.SpeedButton7Click(Sender: TObject);
var
  i:integer;
begin
  dmcon.adsProdList.Close;
  dmcon.adsProdList.Parameters[0].Value:=dmcon.adsWOInfo.fieldByname('WORK_ORDER_NUMBER').AsString;
  dmcon.adsProdList.Open;
  if Dmcon.adsSubProds.fieldByname('Qty_Needed').Value <=0 then
  begin
    ShowMsg('该内层产品数量已经发放完成或本工序不需要压合,不需要再发放!',1);
    exit;
  end;

  i:=Dmcon.adsSubProds.fieldByname('rKey').value;
  with TForm_auto_deploy.Create(Application) do
  try
    ...
    begin
      ...
      if CheckSubProdQty then    //当子全部满足parent,则parent 0006.wipmat_status=1
        if not dmcon.UpdateWOStatus(dmcon.adsProdList.fieldbyName('rKey').AsInteger,1) then
        begin
...
function TfrmWO_Count.CheckSubProdQty: boolean;
begin
  result := true;
  with dmcon do
  begin
...
// with dmcon.ADOQuery1 do //合并过数记录48表
//  begin
//   Close;
//   sql.Clear;
//   SQL.Add('select * from data0048 where wo_ptr='+IntToStr(clamp06)+'order by intime');
//   Open;
//  end;
// with dmcon.ADOQuery2 do
//  begin
//   close;
//   sql.Clear;
//   SQL.Add('select * from data0048 where wo_ptr='+IntToStr(rkey06)+'order by intime');
//   Open;
//  end;
