...
type
  TAddEditForm1 = class(TForm)
    ...
    procedure btnPrind2dClick(Sender: TObject);
  private
    { Private declarations }
    FOldManuPartNumber: string;
    vrkey0010,vrkey0008,vrkey0504,vrkey0015:integer;
    ChangType:Boolean;   //编辑时是否修改产品类型
 ...
 //   procedure Abort3;
    ...
    function GetEst_Scrap(const rkey15:Integer;const code08:string;const EstValue:Double):Double;
    function UpdateManuPartNumber: Boolean;
  public
    list:TStringList;
    { Public declarations }
...
begin
  ...
  begin
    ...
    try
      ...
      lqry.SQL.Text := 'select 1 from data0082 where grade_code=' + QuotedStr(cbbgrade.Text);
      lqry.Open;
        ...
        if not QueryTmp1.IsEmpty then
         ...
         begin
               ...
               begin
                DM.TemADOQuery1.Close;
                dm.TemADOQuery1.SQL.Clear;
                dm.TemADOQuery1.SQL.Text:='SELECT data0008.RKEY as rkey08,pr_grp_pointer, layer_pre,data0008.PRECHAR1 as p1,data0008.PRECHAR2 as p2,data0081.*'
                +' FROM Data0008 inner join data0081 on Data0008.layer_pre=data0081.rkey WHERE  data0008.RKEY = '+IntToStr(vrkey0008) ;
                DM.TemADOQuery1.Open;
                if not DM.TemADOQuery1.IsEmpty then
                begin
...
//                v81sufchar :=  trim(dm.ADOQ0081sufchar.AsString);
                Edit2.Text := StringReplace(Edit2.Text,tmpS,dm.TemADOQuery1.fieldByName('sufchar').Value,[rfReplaceAll]);
                v81sufchar :=  trim(dm.TemADOQuery1.fieldBYName('sufchar').AsString);
                Edit2.Tag:=dm.TemADOQuery1.fieldByName('layer_pre').Value;
                end;
               end;
               ...
               DM.ADOConnection1.BeginTrans ;
               TRY
                   if ((VMyEditMode = 0) and (length(v81sufchar)>0)) then
                   if CheckBox1.Checked then// AddOne_1(); //更新81表流水号
                   begin
                     len:=length(v81sufchar);
                     s :=StringofChar('0',len-length(inttostr(strtoint(v81sufchar)+1)))+inttostr(strtoint(v81sufchar)+1);
                     DM.tmp.Close;
                     DM.tmp.SQL.Clear;
                     DM.tmp.SQL.Text:='update data0081 set sufchar='''+s+''' where rkey='+IntToStr(Edit2.Tag);
                     DM.tmp.ExecSQL;
                ...
                if DM.ADOQuery2.FindField('grade_code') <> nil then
                begin
                  if cbbgrade.Text = '' then
                    DM.ADOQuery2.FieldByName('grade_code').Value := null
                  else
                    DM.ADOQuery2.FieldByName('grade_code').AsString := cbbgrade.Text;
                end;
                   ...
                   if chkRoSH.Checked then
                     DM.ADOQuery2RoHS.Value:= True
                   else
                     DM.ADOQuery2RoHS.Value:= False;
...
//
          if cbbgrade.Text = '' then
            ...
            Lvalue := QuotedStr(cbbgrade.Text);
...
//---------注：为什么两个UPDATE语句不写到一起？？？
           ...
           begin
            ...
            sql.Text:='update data0025'+#13+
                'set MANU_PART_DESC=Data0025_1.MANU_PART_DESC,'+#13+
                     ...
                     'RoHS= Data0025_1.RoHS,'+#13+
                     'PROD_CODE_PTR = Data0025_1.PROD_CODE_PTR,'+#13+
                     'ORIG_CUSTOMER= Data0025_1.ORIG_CUSTOMER'+#13+
                     ', grade_code = ' + Lvalue +
            ' FROM Data0025 INNER JOIN'+#13+
            'Data0025 Data0025_1 ON Data0025.PARENT_PTR = Data0025_1.RKEY'+#13+
          ...
          with dm.TemADOQuery1 do      //更新第三层
           begin
            ...
            sql.Text:='update data0025'+#13+
                     ...
                     'RoHS= Data0025_1.RoHS,'+#13+
                     'PROD_CODE_PTR = Data0025_2.PROD_CODE_PTR,'+#13+
                     'ORIG_CUSTOMER= Data0025_2.ORIG_CUSTOMER'+#13+
                     ', grade_code = ' + Lvalue +
              ' FROM data0025 INNER JOIN  '+#13+
              'Data0025 Data0025_1 ON'+#13+
          ...
          with dm.TemADOQuery1 do      //更新第四层
           begin
            ...
            sql.Text:='update data0025'+#13+
                     'set MANU_PART_DESC=Data0025_3.MANU_PART_DESC,'+#13+
                         ...
                         'RoHS= Data0025_1.RoHS,'+#13+
                         'PROD_CODE_PTR = Data0025_3.PROD_CODE_PTR,'+#13+
                         'ORIG_CUSTOMER= Data0025_3.ORIG_CUSTOMER'+#13+
                         ', grade_code = ' + Lvalue +
              ' FROM data0025 INNER JOIN  '+#13+
              'Data0025 Data0025_1 ON'+#13+
           ...
           if FOldManuPartNumber <> Edit2.Text then
           begin
             if not UpdateManuPartNumber then
             begin
      ...
      begin
       ...
       while not dm.TemADOQuery1.Eof do
          begin
            ...
            begin
              if (list.IndexOf(''+IntToStr(dm.TemADOQuery1.FieldValues['PARAMETER_PTR'])+'')<>0) then
              ADOQ0089tvalue.Value :=list.Values[''+IntToStr(dm.TemADOQuery1.FieldValues['PARAMETER_PTR'])+''];
            end else
            //20170904end--;
...
begin
  ...
  dm.TemADOQuery1.SQL.Clear ;
  dm.TemADOQuery1.SQL.Add('select PROD_CODE,LEAD_TIME,OPT_LOT_SIZE,MAX_DAYS_EARLY_BUILD,EST_SCRAP,');
  DM.TemADOQuery1.SQL.Add('EST_SCRAP1,REVIEW_DAYS,PROD_INCREMENTBY from data0008 where rkey = ' +inttostr(vvalue));
  dm.TemADOQuery1.Open ;

  edit4.Text := dm.TemADOQuery1.FieldByName('OPT_LOT_SIZE').AsString ;
  Edit9.Text:=FloatToStr(GetEst_Scrap(vrkey0015,DM.TemADOQuery1.fieldBYName('PROD_CODE').AsString,dm.TemADOQuery1.FieldByName('EST_SCRAP').AsFloat));
//  if RadioGroup1.ItemIndex = 0 then
...
//   edit9.Text := dm.TemADOQuery1.FieldByName('EST_SCRAP1').AsString ;
 ...
 //20170902tang添由08表带产品层数
  Edit12.Text:= dm.TemADOQuery1.FieldByName('PROD_INCREMENTBY').AsString ;
  result := true;
  dm.TemADOQuery1.Close ;
...
or (trim(edit1.Text)='') then
 begin
  ...
  frmProd_Code:=TfrmProd_Code.Create(Application);
  try
    if (frmProd_Code.ShowModal=mrok) then
    begin
      if ( frmProd_Code.ado08.RecordCount>0) and (vrkey0008 <> frmProd_Code.ado08.FieldValues['RKEY']) then
      begin
        if (MyEditMode = 1) then ChangType:= True;
        vrkey0008 := frmProd_Code.ado08.FieldValues['RKEY'];
        edit5.Text := frmProd_Code.ado08.FieldValues['PROD_CODE'];
        Label5.Caption := frmProd_Code.ado08.FieldValues['PRODUCT_NAME'];
        if frmProd_Code.ado08.FieldByName('IESMODEL_PTR').IsNull then
        vrkey0504 := -9999
        else
        vrkey0504 :=frmProd_Code.ado08.FieldValues['IESMODEL_PTR']  ;
        //20170902tang添由08表带产品层数
        Edit12.Text:= frmProd_Code.ado08.FieldValues['PROD_INCREMENTBY'];
        ADOQ0089.First;
        while not ADOQ0089.Eof do
        begin
         list.Add(''+ADOQ0089.fieldBYName('PARAMETER_PTR').AsString+'='+ADOQ0089.FieldBYName('tvalue').AsString+'');
         ADOQ0089.Next;
        end;
        //20170904end;
        ADOQ0089.First ;
...
//    InputVar.Sqlstr := 'select RKEY,PROD_CODE,PRODUCT_NAME,IESMODEL_PTR,REVIEW_DAYS,PROD_INCREMENTBY '+
//    'from data0008 where active=1 order by prod_code';
//    InputVar.AdoConn := DM.ADOConnection1 ;
//    frmPick_Item_Single.InitForm_New(InputVar) ;
//    if frmPick_Item_Single.ShowModal=mrok then
//     begin
...
//           //20170902tang添由08表带产品层数
//           Edit12.Text:= frmPick_Item_Single.adsPick.FieldValues['PROD_INCREMENTBY'];
//           ADOQ0089.First;
//           while not ADOQ0089.Eof do
//           begin
//             list.Add(''+ADOQ0089.fieldBYName('PARAMETER_PTR').AsString+'='+ADOQ0089.FieldBYName('tvalue').AsString+'');
...
begin
  ...
  dm.TemADOQuery1.Close ;
  dm.TemADOQuery1.SQL.Clear;
  dm.TemADOQuery1.SQL.Add('select RKEY,PROD_CODE,PRODUCT_NAME,IESMODEL_PTR,REVIEW_DAYS,PROD_INCREMENTBY ');
  DM.TemADOQuery1.SQL.Add('from Data0008 where active=1 and PROD_CODE ='''+edit5.Text+'''');
  dm.TemADOQuery1.Prepared ;
  dm.TemADOQuery1.Open ;
  if dm.TemADOQuery1.IsEmpty then
    begin
    ...
    begin
      ...
      if dm.TemADOQuery1.FieldByName('IESMODEL_PTR').IsNull then
       ...
       //20170902tang添由08表带产品层数
       Edit12.Text:= dm.TemADOQuery1.FieldValues['PROD_INCREMENTBY'];
...
begin
  ...
  try
    lqry.Connection := DM.ADOConnection1;
    lqry.SQL.Text := 'select grade_code from data0082';
    lqry.open;
     //加载等级到cbb
     ...
     begin
       cbbgrade.Items.Add(lqry.fieldbyname('grade_code').AsString);
       lqry.Next;
     end;
     //cbbgrade.Items.Add('无等级');

     cbbgrade.Text := dm.ADOQuery2.FieldByName('grade_code').AsString;
//     if dm.ADOQuery2.FieldByName('grade_code').AsString <> '' then
//     begin
//       cbbgrade.ItemIndex := cbbgrade.Items.IndexOf(dm.ADOQuery2.FieldByName('grade_code').AsString)
//     end else
   ...
   if dm.ADOQuery2.Active then
       begin
        ...
        if dm.ADOQuery2green_flag.AsString = 'Y' THEN
         ...
         chkRoSH.Checked:= dm.ADOQuery2RoHS.Value = True;
...
//    v81PRECHAR2:=v81PRECHAR2+self.CODE3; //20180224tang注
  end;
  DM.tmp.Close;
  DM.tmp.SQL.Text:='SELECT RKEY,pr_grp_pointer, layer_pre,PRECHAR1,PRECHAR2 FROM Data0008 WHERE RKEY ='+Inttostr(PROD_CODE_PTR2);
  dm.tmp.Open;
//  dm.ADOQ0008.Close ;
...
//  v81PRECHAR2 := v81PRECHAR2 + trim(dm.ADOQ0008PRECHAR2.AsString);
    v81PRECHAR1 := v81PRECHAR1 + trim(dm.tmp.fieldByName('PRECHAR1').AsString);

  if ((not dm.tmp.IsEmpty) and (not DM.tmp.FieldByName('layer_pre').IsNull))  then
   begin
//    dm.ADOQ0081.Close;
//    dm.ADOQ0081.Parameters.ParamValues['layer_pre'] := dm.ADOQ0008layer_pre.AsInteger  ;
//    dm.ADOQ0081.Prepared ;
//    dm.ADOQ0081.Open ;
    DM.qry1.Close;
    dm.qry1.SQL.Clear;
    DM.qry1.SQL.Text:='SELECT PRECHAR1,PRECHAR2, rkey ,sufchar from data0081 WHERE RKEY ='+dm.tmp.fieldByName('layer_pre').AsString;
    DM.qry1.Open;
    if not dm.qry1.IsEmpty then
      begin
       v81sufchar :=  trim(dm.qry1.fieldBYName('sufchar').AsString);
  ...
  begin
   ...
   sql.Text:=
    ...
    'data0015.abbr_name FROM dbo.Data0015'+#13+
     'where active_flag = ''Y'' and WAREHOUSE_CODE='+quotedstr(trim(edit18.Text));
   open;
   if isempty then
    begin
 ...
 //screen.Cursor:=crappstart;
  ...
  try
          ...
          begin
               ...
               if chkRoSH.Checked then          //ROHS要求
                dm.AQY0025ROHS.Value:=true
               else
                dm.AQY0025ROHS.Value:=false;
...
procedure TAddEditForm1.btnPrind2dClick(Sender: TObject);
var
  InputVar: PDlgInput ;
  d10rkey: Integer;
  lqry: TADOQuery;
...
//    try
//      lqry.Connection := DM.ADOConnection1;
//      lqry.SQL.Text := 'select rkey from data0010 where ABBR_NAME = ' + QuotedStr(Edit23.Text);
//      lqry.Open;
...
function TAddEditForm1.GetEst_Scrap(const rkey15: Integer;
  const code08: string; const EstValue: Double): Double;
begin
  Result:=EstValue;
  DM.tmp.Close;
  dm.tmp.SQL.Text:='select * from data0620 where warehouse_ptr='+inttostr(rkey15)+' and PROD_CODE='''+code08+'''';
  dm.tmp.Open;
  if not dm.tmp.IsEmpty then
    Result:=DM.tmp.fieldByName('EST_SCRAP').AsFloat
  else
    Result:=EstValue;
end;

function TAddEditForm1.UpdateManuPartNumber: Boolean;
var
  LSql: string;
  LNewManuPartNumber: string;
  Lqry: TADOQuery;
begin
  LNewManuPartNumber := DM.ADOQuery2.fieldbyname('manu_part_number').AsString;
  LSql := ';WITH CTE(RKey,Lv,MANU_PART_NUMBER,PARENT_PTR) AS ( ' +
' SELECT RKey,0,MANU_PART_NUMBER,PARENT_PTR ' +
' FROM Data0025 WHERE RKey = ' + DM.ADOQuery2.fieldbyname('rkey').AsString + ' UNION ALL ' +
' SELECT D25.RKey,CTE.Lv+1,D25.MANU_PART_NUMBER,d25.PARENT_PTR ' +
' FROM Data0025 D25 INNER JOIN CTE ON D25.PARENT_PTR = CTE.RKey) ' +
' UPDATE Data0025 SET manu_part_number = replace(d25.manu_part_number,'+ QuotedStr(FOldManuPartNumber) + ',' + QuotedStr(LNewManuPartNumber) + ') ' +
' FROM Data0025 D25 ' +
' INNER JOIN CTE ON CTE.RKEY = D25.RKEY ' +
' where d25.rkey <> ' + DM.ADOQuery2.fieldbyname('rkey').AsString;
