...
procedure TFrm_New.SpeedButton5Click(Sender: TObject);
begin
...
try
 ...
 begin
  ...
  begin
  ShowMessage('该本厂编码在MI资料没有工艺流程');
  Exit;
  end;

   // FrmCustPart.InitForm_New(InputVar)  ;
...
procedure TFrm_New.SpeedButton2Click(Sender: TObject);
begin
close;
end;

procedure TFrm_New.update_04(v_seed: string);
var
 i,v_length:integer;
...
procedure TFrm_New.btn1Click(Sender: TObject);
begin
if Edit4.Text='' then
  begin
...
procedure TFrm_New.SpeedButton3Click(Sender: TObject);
var
  InputVar: PDlgInput ;
begin
...
procedure TFrm_New.SpeedButton4Click(Sender: TObject);
var
  InputVar: PDlgInput ;
begin
  frmPick_Item_Single := TfrmPick_Item_Single.Create(application) ;
...
procedure TFrm_New.Edit5KeyPress(Sender: TObject; var Key: Char);
begin
if not (key in ['0'..'9',#8,#13]) then abort;
end;

procedure TFrm_New.SpeedButton6Click(Sender: TObject);
var
  InputVar: PDlgInput ;
begin
 frmPick_Item_Single := TfrmPick_Item_Single.Create(application) ;
  try
    inputvar.Fields:='sales_order/销售订单/70,sch_date/回复交期/80,cust_code/客户代码/50,manu_part_number/本厂编号/80,'+
     'manu_part_desc/客户型号/90,po_n/订单数量/60,po_number/客户订单号/120,set_qty/交货单元数/30,parts_ordered/订单数量/60,total_shipped/装运数量/60,'+
     ...
     'D60.parts_ordered-D60.parts_shipped+D60.parts_returned-D60.returned_ship as ''po_n'','+
     ...
     'where d60.status in (1 , 2) and d60.prod_rel<>3';
      ...
      inputvar.Sqlstr:= inputvar.Sqlstr+(' and MANU_PART_NUMBER LIKE ''%'+Trim(edit2.Text)+'%''');
      //inputvar.Sqlstr:=inputvar.Sqlstr+edit2.Text;
...
try
 if  Edit2.Text='' then
  ...
  begin
   ...
   if  (Edit6.Text='') or (Label60.Caption='Label60')  then
  begin
    showmessage('请选择销售订单号...');
    Edit6.SetFocus;
    exit;
  end;

    if  Edit5.Text=inttostr(0) then
  begin
    showmessage('输入数量必须大于0...');
    Edit5.SetFocus;
    exit;
  end;
  ...
  dm.ADO0745.Open;
  dm.ADO0745.Append;
  dm.ADO0745F_NUM.AsString:=Edit1.Text;
  dm.ADO0745AUDITED_ptr.AsString:=user_ptr;
  dm.ADO0745RKEY25.Value:=StrToInt(Label25.caption);
  dm.ADO0745RKEY10.AsString:=Label10.caption;
  dm.ADO0745RKEY60.AsString:=Label60.caption;
  dm.ADO0745CreateDate.Value:=dt1.DateTime;
  dm.ADO0745check_type.AsString:=Label_741rkey.Caption;  // 审批流程rkey
  dm.ADO0745circle.Value:=1;
  dm.ADO0745QUAN_ON_HAND.AsString:=Edit5.Text;
  dm.ADO0745BEIZHU.AsString:=Memo2.Text;
  dm.ADO0745.Post;

 { with dm.Insert740 do
  begin
  close;
