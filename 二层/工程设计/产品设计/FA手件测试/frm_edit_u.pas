...
procedure TFrm_Edit.SpeedButton5Click(Sender: TObject);
begin
frmcustpart := tfrmcustpart.Create(application);
 ...
 if frmcustpart.ShowModal=mrok then
  if frmcustpart.ADOQuery1.FieldByName('onhold_sales_flag').AsString='1' then
   ...
   Edit3.Text:= frmcustpart.ADOQuery1.FieldByName('CUSTOMER_NAME').AsString;
...
procedure TFrm_Edit.SpeedButton4Click(Sender: TObject);
var
  InputVar: PDlgInput ;
begin
  frmPick_Item_Single := TfrmPick_Item_Single.Create(application) ;
...
procedure TFrm_Edit.Edit5KeyPress(Sender: TObject; var Key: Char);
begin
if not (key in ['0'..'9',#8,#13]) then abort;
end;

procedure TFrm_Edit.SpeedButton6Click(Sender: TObject);
var
  InputVar: PDlgInput ;
begin
  if Edit2.Text='' then
