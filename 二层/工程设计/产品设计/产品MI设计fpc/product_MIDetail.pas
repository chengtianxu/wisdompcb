...
for i:=1 to stringgrid2.RowCount-2 do  stringgrid2.Rows[i].Clear;
 ...
 begin
  ...
  begin
   ...
   //ringgrid2.Cells[2,i]:=trim(fieldbyname('mark').AsString);//标记
   stringgrid2.Cells[2,i]:=trim(fieldbyname('layer').AsString);//层次
   stringgrid2.Cells[3,i]:=trim(fieldbyname('refe_layer').AsString);//参考层
   stringgrid2.Cells[4,i]:=trim(fieldbyname('product_impedance').AsString);//成品阻抗
   stringgrid2.Cells[5,i]:=trim(fieldbyname('adjust_impedance').AsString);//绿油前阻抗
   stringgrid2.Cells[6,i]:=trim(fieldbyname('impedance_scope').AsString);//阻抗控制范围
   stringgrid2.Cells[7,i]:=trim(fieldbyname('line_width').AsString);//客户线宽
   stringgrid2.Cells[8,i]:=trim(fieldbyname('line_ofsight').AsString); //客户线距;
   //ringgrid2.Cells[10,i]:=trim(fieldbyname('impedance').AsString); //客户阻抗;
   stringgrid2.Cells[9,i]:=trim(fieldbyname('orig_juli').AsString); //原始到铜皮距离;
   stringgrid2.Cells[10,i]:=trim(fieldbyname('adjust_linewidth').AsString); //调整线宽;
   stringgrid2.Cells[11,i]:=trim(fieldbyname('adjust_lineofsight').AsString); //调整线距;
   stringgrid2.Cells[12,i]:=trim(fieldbyname('adjust_jiuli').AsString); //调整后到铜皮距离;
   next;
  end;
  impedance:=true;
 end;
   ...
   begin
    while not dm.ADOQuery1.Eof do
     begin
      ...
      dm.ads279PARAMETER_VALUE.Value:=dm.ADOQuery1.fieldbyname('PARAMETER_INFO').AsString; //默认值
      dm.ads279.Post;
      dm.ADOQuery1.Next;
     end;
    dm.ADOQuery1.Close;
...
begin
 ...
 stringgrid2.Cells[4,0]:='成品阻抗';
 stringgrid2.Cells[5,0]:='绿油前阻抗';
 stringgrid2.Cells[6,0]:='阻抗控制范围';
 stringgrid2.Cells[7,0]:='客户线宽';
 stringgrid2.Cells[8,0]:='客户线距';
 stringgrid2.Cells[9,0]:='原始到铜皮距离';
 stringgrid2.Cells[10,0]:='调整线宽';
 stringgrid2.Cells[11,0]:='调整线距';
 stringgrid2.Cells[12,0]:='调整后到铜距离';
...
//                begin
//                  if messagedlg('节点： ' + NodeName + '， 压合步骤为0，有误！, 是否自动跳转到错误节点?', mtconfirmation, [mbyes,mbno], 0) = mryes then
//                  begin
...
//            end;
            ...
            begin
              ...
              begin
                ...
                begin
                  if messagedlg('节点： ' + NodeName + '， 压合步骤为1, 是否继续?', mtconfirmation, [mbyes,mbno], 0) = mryes then
                  begin
                    SaveData();
                    Break;
                  end
...
begin
  ...
  SaveData();
//   for i := 1 to TreeView1.Items.Count-1 do      //2017-2-7注释，由于软板需要先做内层，还没有压合步骤
//  begin
//    NodeName := TreeView1.Items.Item[I].Text;
//    with DM.AQtmp do
//    begin
