...
uses
  ...
  Grids, DBGrids, Menus, DB, ADODB,math, DBGridEh, IdFTP;
...
type
  ...
  private
   ...
   file_name:Tstringlist; //用于交换的字符串,用于存储上传的工程文档
...
uses damo, pcb_cutting,common, route_flow, flowpara_edit, deptcode_search,
  ...
  product_search, jpeg, Excel2000,PubFunc,ShellAPI;
...
begin
if dm.ads564.IsEmpty then N33.Enabled:= False;
end;

procedure TForm2.PopupMenu6Popup(Sender: TObject);
begin
  n11.Enabled:= (not DM.qryFile.IsEmpty);
end;

procedure TForm2.PopupMenu7Popup(Sender: TObject);
begin
  N12.Enabled:= (not DM.qryFile2.IsEmpty);
end;
