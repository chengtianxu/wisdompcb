...
uses
  ...
  Dialogs, StdCtrls, StrUtils, StdConvs,ShellAPI,Grids;
...
begin
     ...
     if FileExists(progdir+'features.z') then
     DeleteFile(progdir+'features.z');
     gd:=GetEnvironmentVariable(pchar('GENESIS_DIR'));
     if length(gd)<1 then
     begin
   ...
   if not un7zip() then
    ...
    begin
     if analysis(progdir+'tmp') then
      with form2.StringGrid1 do for i := 1 to RowCount - 1 do Rows[i].Clear;
      form2.StringGrid1.RowCount:=2;
      listdrl(form2.StringGrid1);
