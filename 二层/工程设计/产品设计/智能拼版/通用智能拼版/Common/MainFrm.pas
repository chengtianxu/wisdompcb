...
type
  TfrmMain1 = class(TForm)
    ...
    procedure cbxBBClick(Sender: TObject);
  private
...
begin
  ...
  begin
    ...
    begin
      LPaintData.DWK := LPaintData.DWK + [kBB];
      if rgBBWZ.ItemIndex = 0 then
        LPaintData.DWK := LPaintData.DWK + [kBBShuiPing]
      else
        LPaintData.DWK := LPaintData.DWK + [kBBChuiZhi];
    end;
...
//      LPaintDate.BlockArr[I].ArrowCB := LRet.PnlA.PaintData.BlockArr[I].ArrowCB;
    LPaintDate := LRet.PnlA.PPaintData;
    ...
    if LRet.CutParam.PnlASetVertSpace < g_PaintSpaceMin then LSetVerSpace := g_PaintSpaceMin else LSetVerSpace := LRet.CutParam.PnlASetVertSpace;
    if LRet.CutParam.PnlASetHorzSpace < g_PaintSpaceMin then LSetHorSpace := g_PaintSpaceMin else LSetHorSpace := LRet.CutParam.PnlASetHorzSpace;
...
begin
  ...
  begin
    ...
    bA := False;
    //取消这个if 注释是自动判断水平还是垂直阻抗,函数参数为I
    //for I := 0 to 1 do
    //begin
      if AddZKPnl(FSelectRet.PnlA,rgZKPos.ItemIndex,1) then
      begin
        bA := True;
        //Break;
      end else
      begin
        if rgZKPos.ItemIndex = 0 then
          AddZKPnl(FSelectRet.PnlA,1,1)
        else
          AddZKPnl(FSelectRet.PnlA,0,1);
        bA := True;
      end;
    //end;
    ...
    begin
      bB := False;
      //取消这个if 注释是自动判断水平还是垂直阻抗
//      for I := 0 to 1 do
//      begin
        if AddZKPnl(FSelectRet.PnlB,rgZKPos.ItemIndex,2) then
        begin
          bB := True;
          //Break;
        end else
        begin
          if rgZKPos.ItemIndex = 0 then
            AddZKPnl(FSelectRet.PnlA,1,2)
          else
            AddZKPnl(FSelectRet.PnlA,0,2);
          bB := True;
        end;
//      end;
//      if not bB then
...
procedure ZoomZKReg(var outZKReg: TFloatRect; var outZKArr: TFloatRectArr; Zoom: double; Hor_Ver: Integer);
var
  I: Integer;
begin
  if Hor_Ver = 0 then//水平
  begin
    outZKReg.W := outZKReg.W * Zoom;
    for I := 0 to Length(outZKArr) - 1 do
    begin
      outZKArr[I].X := outzkArr[I].X * Zoom;
      outZKArr[I].W := outzkarr[I].W * Zoom;
    end;
  end else
  begin
    outZKReg.H := outZKReg.H * Zoom;
    for I := 0 to Length(outZKArr) - 1 do
    begin
      outZKArr[I].Y := outzkArr[I].Y * Zoom;
      outZKArr[I].H := outzkarr[I].H * Zoom;
    end;
  end;
end;
...
//              LRet.CutParam.PnlASetVertSpace, LRet.CutParam.PnlASetHorzSpace,
//              APnl.PBorderSize^.OldTopBottomW,
//              APnl.PBorderSize^.OldLeftRightW, APnl.PaintData.Zoom);
//    end else
//    begin
    LPaintDate := Calc4RegToPaintData(APnl.Calc4Reg,
            LRet.CutParam.PnlASetHorzSpace, LRet.CutParam.PnlASetVertSpace,
            APnl.PBorderSize^.OldLeftRightW,
            APnl.PBorderSize^.OldTopBottomW, APnl.PaintData.Zoom);
//    end;
    for I := 0 to Length(LPaintDate.BlockArr) - 1 do
      LPaintDate.BlockArr[I].ArrowCB := APnl.PaintData.BlockArr[I].ArrowCB;

    //LPaintDate := APnl.PPaintData;
    ...
    case APos of
      ...
      begin
        ...
        begin
          if MakeHorZKReg(StrToFloat(edtZK6mmLength.Text),StrToFloat(edtZK3mmLength.Text),se6mm.Value,se3mm.Value,LZKReg,LZKBlockArr,LMaxW,LMaxH) then
          begin
            ZoomZKReg(LZKReg,LZKBlockArr,APnl.PaintData.Zoom,APos);
...
begin
  ...
  //Sheet
  LZoom := LRet.SheetA.PaintData.Zoom;
  ...
  //LZoom := g_PaintMapSize / MaxValue([LRet.SheetA.W, LRet.SheetA.H]);
  LPaintData := Calc4RegToPaintData(LSheetCalc4Reg, 0, 0, 0, 0, LZoom,False);
  ...
  * 100);
  ...
  //Pnl_A
  LZoom := LRet.PnlA.PaintData.Zoom;
  ...
  LRet.PnlA.PaintData := Calc4RegToPaintData(LPnlCalc4Reg,
    LRet.CutParam.PnlASetHorzSpace, LRet.CutParam.PnlASetVertSpace,
    LRet.PnlA.PBorderSize^.NowLeftRightW,
    LRet.PnlA.PBorderSize^.NowTopBottomW, LZoom);
...
//    LRetA.SheetA.H := Ltmp;
    ...
    LRetB.PCutParam^.SheetH := ARet.CutParam.SheetH - ARet.SheetA.H;
    AutoBorder(LRetB);
  ...
  begin
    ...
    LRetB.PnlA := ARet.PnlB;
    LRetB.CutParam := ARet.CutParam;
    LRetB.PCutParam^.SheetW := ARet.CutParam.SheetW - ARet.SheetA.W;
    AutoBorder(LRetB);
    ARet.SheetB := LRetB.SheetA;
    ARet.PnlB := LRetB.PnlA;
    for I := 0 to Length(ARet.SheetB.PaintData.BlockArr) - 1 do
...
begin
  ...
  try
    LFrm.edtSetW_A.Text := edtSetW_A.Text;
    LFrm.edtSetH_A.Text := edtSetH_A.Text;
    LFrm.edtSetSpaceX_A.Text := edtSetSpaceX_A.Text;
    LFrm.edtSetSpaceY_A.Text := edtSetSpaceY_A.Text;
    LFrm.edtPnlLeftRightW_A.Text := edtPnlLeftRightW_A.Text;
    LFrm.edtPnlTopBottomW_A.Text := edtPnlTopBottomW_A.Text;
...
begin
  ...
  if (FSelectRet.SheetB <> nil) and (FSelectRet.PnlB <> nil) and (FSelectRet.SheetB.GetCellCount <> 0) then
  begin
  ...
  LRet.PCutParam^.SheetW := StrToFloat(edtSheetW.Text);
  LRet.PCutParam^.SheetH := StrToFloat(edtSheetH.Text);
  ...
  LRet.SheetA.Calc4Reg := LSheetCalc4Reg;
  //LZoom := g_PaintMapSize / MaxValue([LRet.SheetA.W, LRet.SheetA.H]);
  LRet.SheetA.PaintData := Calc4RegToPaintData(LSheetCalc4Reg, 0, 0, 0, 0,LZoom, False);
...
begin
  ...
  begin
    if (StrToFloat(frmSheetList.lvSheetList.Items[I].SubItems[0]) = StrToFloat(edtSheetW.Text)) and (StrToFloat(frmSheetList.lvSheetList.Items[I].SubItems[2]) = StrToFloat(edtSheetH.Text)) then
    begin
      LSeet_Size := frmSheetList.lvSheetList.Items[I].SubItems[3];
    end;
  end;
  ...
  try
    ...
    LIni.WriteString('CutRet','Seet_Size',LSeet_Size);
...
procedure TfrmMain1.cbxBBClick(Sender: TObject);
begin
  rgBBWZ.Enabled := cbxBB.Checked;
  lvRetClick(lvRet);
end;
...
//  end;
  ...
  begin
    FSelectRet := FRetLst.Items[Integer(lvRet.Items[0].Data)];
    FHotRetIndex := lvRet.Items.IndexOf(lvRet.Items[0]);
    lvRetClick(lvRet);
    lvRet.Items[0].Selected := True;
  end;
end;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        if Copy(LS[I],posc+1,1) = 'Y' then Checked := True;
        if Pos('@',LS[I]) > 0 then
        begin
          SubItems.Add(Copy(LS[I],Pos('@',LS[I])+1,Length(LS[I])-POS('@',LS[I])));
        end else
          SubItems.Add('');
      end;
    end;
...
var
  LFrm: TfrmOption;
begin
  LFrm := TfrmOption.Create(Self);
  try
    LFrm.LoadFromDefParam;
    if LFrm.ShowModal = mrOk then
    begin
      cbxSetRotate_A.Checked := LFrm.cbxSetRotate.Checked;
      cbxPnlRotate_A.Checked := LFrm.cbxPnlRotate.Checked;
      cbxDoubleCut.Checked := LFrm.cbxDoubleCut.Checked;
      cbxBB.Checked := LFrm.cbxbb.Checked;
      cbxPXGG.Checked := LFrm.cbxPXGG.Checked;
      cbxXD.Checked := LFrm.cbxXDK.Checked;
      cbxJDK.Checked := LFrm.cbxJDK.Checked;
      edtZK3mmLength.Text := LFrm.edtZK3mmLength.Text;
      edtZK6mmLength.Text := LFrm.edtZK6mmLength.Text;
      se3mm.Value := LFrm.se3mm.Value;
      se6mm.Value := LFrm.se6mm.Value;
      rgZKPos.ItemIndex := LFrm.rgZKPos.ItemIndex;
      LFrm.SaveToDefParam;
    end
  finally
    LFrm.Free;
