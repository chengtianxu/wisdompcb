...
function Calc4RegToPaintData(ACalcRet: TCalc4Reg;ASpace_X,ASpace_Y,ABorder_X,ABorder_Y: Double;AZoom: double; MakeCenter: Boolean = True; offsetX: double = 0;offsetY: Double = 0): TPaintData;
...
var
  ...
  ltmpArr: array of TCalcReg;
begin
  LOldMapW := ACalcRet.W;
  lOldMapH := ACalcRet.H;
  //AZoom := g_MapSize / MaxValue([LOldMapW,lOldMapH]);
  LNewSpaceX := ASpace_X * AZoom;
  LNewSpaceY := ASpace_Y * AZoom;
  LNewBorderX := ABorder_X * AZoom;
  LNewBorderY := ABorder_Y * AZoom;
  LNewBlockW := ACalcRet.RegArr[0].Block.W * AZoom;
  LNewBlockH := ACalcRet.RegArr[0].Block.H * AZoom;
  lCalcRet := ACalcRet;
  for I := 0 to Length(ACalcRet.RegArr) - 1 do
  begin
    SetLength(ltmpArr,Length(ltmpArr)+1);
    ltmpArr[High(ltmpArr)] := ACalcRet.RegArr[I];
  end;
  ...
  if (LNewSpaceX < g_PaintSpaceMin) and (LNewSpaceX > 0) then LNewSpaceX := g_PaintSpaceMin;
  if (LNewSpaceY < g_PaintSpaceMin) and (LNewSpaceY > 0) then LNewSpaceY := g_PaintSpaceMin;
  if (LNewBorderX < g_PaintBorderMin) and (LNewBorderX > 0) then LNewBorderX := g_PaintBorderMin;
  if (LNewBorderY < g_PaintBorderMin) and (LNewBorderY > 0) then LNewBorderY := g_PaintBorderMin;
