...
implementation
uses  uDM,uNew,u_frmbusinessprocess, uEdit,uCheck,uEdit2,report2_form,Unit_remser,so_report;
...
if (strtoint(vprev)=1) or (strtoint(vprev)=3)  then
  ...
  Edit9.text:=IntToStr(dm.ADO0738Quality.Value);
  ...
  RadioGroup1.ItemIndex:=dm.ADO0738FAStatus.Value;
  ...
  with  dm.ADOFaTest do
  begin
  close;
  Parameters.ParamByName('MANU_PART_NUMBER').Value:=Edit4.Text;
  Parameters.ParamByName('RKEY738').Value:=dm.ADO0738Rkey.Value;
  Open;
  end;

  dm.ADO0740Edit.Close();
  dm.ADO0740Edit.Parameters.ParamByName('RKEY738').Value:=dm.ADO0738Rkey.Value;
  dm.ADO0740Edit.Open();
  dm.ADO0740Edit.Edit;
  ...
  begin
  dm.ADO0738.Edit;
  //if FormEdit.Label25.Modified then

  dm.ADO0738BasicTechnology.Value:=FormEdit2.Memo2.Text;
  dm.ADO0738RKEY25.Value:=StrToInt(FormEdit2.Label25.caption);

  //if FormEdit.Label10.Modified then
  dm.ADO0738RKEY10.AsString:=FormEdit2.Label10.caption;

  //if FormEdit.dt1.Modified then
  dm.ADO0738CreateDate.Value:=FormEdit2.dt1.DateTime;

  if FormEdit2.Edit9.Modified then
  dm.ADO0738Quality.Value:=strtoInt(FormEdit2.Edit9.text);
  dm.ADO0738.Post;


  dm.ADO0740EditTStatus.Value:=FormEdit2.RadioGroup2.ItemIndex;

  if FormEdit2.Edit7.Modified then
  dm.ADO0740Editfinished.Value:=FormEdit2.Edit7.text;

   if FormEdit2.Memo1.Modified then
  dm.ADO0740Editsuggestion.Value:=FormEdit2.Memo1.text;
   dm.ADO0740Edit.Post;

  //dm.ADO0740.Edit;
  //dm.ADO0740TestResult.Value:=DateTimeToStr(FormEdit2.DateTimePicker1.datetime);
  ...
  if (Tmenuitem(Sender).Tag in [1]) then
    begin
      try
        ...
        if dm.ADO0743check_suggestion.AsString =''  then
        frm_check.RichEdit1.Text:=inttostr(dm.ADO0743d34_index.Value)+'审:'+#13
        else
         frm_check.RichEdit1.Text:= dm.ADO0743check_suggestion.AsString;
...
begin
  ...
  begin
    dm.ADOTMP1.SQL.Clear;
    dm.ADOTMP1.SQL.Text := 'select top 1 rkey,d738_ptr,d34_index,d34_ptr,check_person,check_time,check_suggestion,check_status  from FA0743 where d738_ptr='+inttostr(dm.ADO0738Rkey.Value)+' order by d34_index desc ';
    dm.ADOTMP1.Open;
