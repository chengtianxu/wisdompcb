...
uses damo, prod_search, iesmodel_search, DB, main,common, ADODB;
...
if Pos('.',Edit12.Text)>0 then        //返回字符'.' 在编号中的位置，没有代表顶层
 edit1.Text:=Copy(edit12.Text,1,Pos('.',Edit12.Text)-1)
else
 edit1.Text:=edit12.Text;
 
Edit13.Text:='.';

edit13.SelStart:=Edit13.GetTextLen; //length(edit1.Text);
...
try
 ...
 dm.AQY0025MANU_PART_NUMBER.Value := edit1.Text+EDIT13.Text;
 ...
 with dm.ADOQuery1 do
  begin
   ...
   sql.Text:='update  data0025 '+
             ...
             'where rkey='+dm.AQY0025PARENT_PTR.AsString;
   ExecSQL;
  end;
