...
type
  TForm2 = class(TForm)
    ...
    procedure ehBOMColumns5EditButtonClick(Sender: TObject;
      var Handled: Boolean);
  ...
  private
    ...
    dataname:string;
    FStepHasChange: Boolean;
    FHintFrm: THintWindow;
   est_scrap : single; 
   OldGridWnd : TWndMethod;
   dbgrid2wnd : TWndMethod;
   ecnopen,if_post,impedance,target,ftp:boolean; //是否需要重新打开ecn清单
   strow,file_name:Tstringlist; //用于交换的字符串,用于存储上传的工程文档
   rev:string; //产品版本
   FIDKey:string;
   procedure DirllPth(ASg: TStringGrid;AEdt:TEdit);
   procedure SetParentPREStepZero(AParentRkey: Integer);
 ...
 //  function find_difficulty(rkey25:integer) :boolean;
   ...
   procedure UpdateStatus(Arkey25:string);

   function update_BomQty(rkey25:integer):Boolean ;
   ...
   procedure append26(rkey25:Integer);
//   function GetDept(cds:TADODataSet):string;
...
uses damo, common, route_flow, flowpara_edit, deptcode_search,
  ...
  IniFiles, SameQryFrm, FindStemParamFrm, PasType, DeptFrm, pasPress,Pick_Item_Single,ConstVar;
...
for i:=1 to stringgrid2.RowCount-2 do  stringgrid2.Rows[i].Clear;
 ...
 begin
  ...
  sql.Text:='select  MARK, layer, refe_layer, line_width, line_ofsight,'+
    ...
    'impedance_scope,orig_juli,adjust_jiuli,coupon,impedance_first,product_control,adjust_first,adjust_control,'+
...
with dm.ADOQuery1 do
 begin
  ...
  sql.Text:='select seq_no,source_ptr, MARK, layer, refe_layer, line_width, line_ofsight,'+
    ...
    'type,impedance_scope,orig_juli,adjust_jiuli,impedance_first,product_control,adjust_first,adjust_control '+
    ' from data0030 where source_ptr='+inttostr(rkey25)+' order by seq_no ';
  open;
 end;
  ...
  begin
    ...
    dm.ADOQuery1.fieldbyname('impedance_first').AsString := stringgrid2.Cells[14,i]; //成品阻抗首件控制;
    dm.ADOQuery1.fieldbyname('product_control').AsString := stringgrid2.Cells[15,i]; //成品阻抗控制;
    dm.ADOQuery1.fieldbyname('adjust_first').AsString := stringgrid2.Cells[16,i]; //绿油前阻抗首件控制;
    dm.ADOQuery1.fieldbyname('adjust_control').AsString := stringgrid2.Cells[17,i]; //绿油前阻抗控制;
    dm.ADOQuery1.Post;
  end;
  ...
  begin
   while pos(' '+trim(dm.ads279PARAMETER_NAME.Value)+' ',vstring)>0 do //如果一个参数出现了多次就通过这个循环逐一代换
    begin
      ...
      begin
//        ShowMessage(dm.ads279PARAMETER_NAME.AsString);
...
begin
 ...
 DM.qryFtp.Close;
 dm.qryFile.Close;
 DM.qryFileList.Close;
 DM.qryFtp2.Close;
 dm.qryFile2.Close;
 DM.qryFileList2.Close;
...
begin
 if row <= 1 then
  ...
  for i:=row-1 downto 1 do
   if trim(fsg.Cells[1,i])<>'' then
    begin
     t0:=copy(trim(fsg.Cells[1,i]),2,length(trim(fsg.Cells[1,i]))-1);
     try
      t1:=strtoint(t0)+1;
     except
      t1:=row;
...
with dm.ADOQuery1 do
 begin
  ...
  ' 	 , d494.TaskTopLimt, d494.TaskLowLimt, D494.InnerTopLimt, d494.InnerLowLimt, D494.SpecialNote ' +
  'FROM dbo.Data0494 d494'+#13+
...
begin
   dm.ADOQuery1.AppendRecord([dm.ads494PARAMETER_PTR.Value,  //参数指针278
                              ...
                              DM.ads494.FieldByName('TaskTopLimt').Value,
                              DM.ads494.FieldByName('TaskLowLimt').Value,
                              DM.ads494.FieldByName('InnerTopLimt').Value,
                              DM.ads494.FieldByName('InnerLowLimt').Value,
                              DM.ads494.FieldByName('SpecialNote').Value
...
with dm.ADOQuery1 do
 begin
  ...
  'UNIT, SET_UNMBER,PANEL_A, PANEL_B, SEQ_NO,REMARK,Flow_no,mo_number,drill_number '+#13+
  ...
  for i:=1 to stringgrid1.RowCount-2 do
   dm.ADOQuery1.AppendRecord([rkey25,
                              ...
                              0,
                              strtointdef(stringgrid1.Cells[12,i],0),
                              strtointdef(stringgrid1.Cells[13,i],0)
                              ]);
  ...
  for i:=1 to stringgrid4.RowCount-2 do
   dm.ADOQuery1.AppendRecord([rkey25,
                              ...
                              stringgrid4.Cells[9,i],
                              stringgrid4.Cells[10,i],
                              n+i,
                              stringgrid4.Cells[11,i],
                              1,
                              strtointdef(stringgrid4.Cells[12,i],0),
                              strtointdef(stringgrid4.Cells[13,i],0)
                              ]);
  ...
  for i:=1 to stringgrid5.RowCount-2 do
   dm.ADOQuery1.AppendRecord([rkey25,
                              ...
                              stringgrid5.Cells[8,i],
                              stringgrid5.Cells[9,i],
                              stringgrid5.Cells[10,i],
                              n+i,
                              stringgrid5.Cells[11,i],
                              2,
                              strtointdef(stringgrid5.Cells[12,i],0),
                              strtointdef(stringgrid5.Cells[13,i],0)
                              ]);                              
...
with dm.ADOQuery1 do
 begin
  ...
  'UNIT, PANEL_A, PANEL_B, REMARK,SET_UNMBER,flow_no,mo_number,drill_number '+#13+
  'FROM dbo.Data0029'+#13+
  'where source_ptr='+inttostr(rkey25)+#13+
  ...
  for i:=1 to RecordCount do
   begin
    ...
    asg.Cells[12,i]:=trim(fieldbyname('mo_number').AsString);//'磨次';
    asg.Cells[13,i]:=trim(fieldbyname('drill_number').AsString);//'数量;
    next;
   end;
 end;
...
//  begin
//  dbedt1.Enabled:= ((strtoint(vprev) = 2) or (strtoint(vprev) = 4)) and dm.AQY0025MinRing.IsNull;
//  btn7.Visible:= ((strtoint(vprev) = 2) or (strtoint(vprev) = 4)) and dm.AQY0025MinRing.IsNull;
//  end;
    cbb1.Enabled:= False;
    cbb2.Enabled:= False;
 end;
 ...
 begin
  ...
  cbbgrade.Text := dm.AQY0025.FieldByName('grade_code').AsString;
...
//   cbbgrade.ItemIndex := cbbgrade.Items.IndexOf('无等级');
 ...
 checkbox3.Checked:= trim(dm.AQY0025amp_mb.Value) = 'Y';  //无卤数
 ...
 if (not DM.qryPress.Active) then
 begin
    dm.qryPress.Close;                //打开层压结构介质厚度表
    DM.qryPress.Parameters[0].Value:=  dm.AQY0025RKEY.Value;
    DM.qryPress.Open;
 end;
...
if treeview1.Selected.Level=0 then       //顶层产品时需打开
 ...
 if treeview1.Selected.Level=0 then       //非顶层产品时
 begin
   notebook1.PageIndex := listbox1.ItemIndex+1;
 end
 else
 begin
   if ListBox1.ItemIndex = 8 then  Notebook1.PageIndex:= 16
   else Notebook1.PageIndex:= listbox1.ItemIndex+1;
 end;

 if Notebook1.PageIndex = 16 then DM.ads26.Filtered := False;
...
stringgrid1.ColWidths[12] := 0;  //磨次
stringgrid1.ColWidths[13] := 0;  //数量

stringgrid4.ColWidths[12] := 0;
stringgrid4.ColWidths[13] := 0;

stringgrid5.ColWidths[12] := 0;
stringgrid5.ColWidths[13] := 0;
...
begin
  ...
  if not DM.CheckGrade_Code then Exit;
   if (DM.AQY0025barcode_flag.Value = 1) and ((Trim(cbb1.Text)='') or (Trim(cbb2.Text)='')) then
  begin
    ShowMessage('高难度产品，请选择级别与难度！！');
    Exit;
  end;
  if not DM.ADS38.IsEmpty then
    if Self.update_BomQty(dm.AQY0025RKEY.Value) then Exit;  //检查BOM材料是否存在
...
//  ShowMessage('502' +DM.ads502TOTAL_PNLS_1.AsString);

 update_quanju;            //交货面积,交货单元数
// ShowMessage('503' +DM.ads502TOTAL_PNLS_1.AsString);
 ...
 if DM.qryFtp2.Active then DM.qryFtp2.UpdateBatch(arAll);
 if dm.qryFileList2.Active then dm.qryFileList2.UpdateBatch(arAll);
 if dm.qryPress.Active then DM.qryPress.UpdateBatch(arAll);

 self.update_deptcode(dm.AQY0025RKEY.Value);  //38
 self.update_tecparameter(dm.AQY0025RKEY.Value); //494
 Self.update_IESparameter(dm.AQY0025RKEY.Value);   //497

 Self.append26(dm.AQY0025RKEY.Value);
...
//  dm.AQY0025barcode_flag.Value:=1;          //自动标识高难度
 ...
 if checkbox2.Checked then
  ...
  dm.AQY0025amp_mb.Value:='Y'
 else
  dm.AQY0025amp_mb.Value:='N';
 dm.AQY0025.Post;
 if_post:=true;
 ...
 Self.UpdateStatus(DM.ADS25rkey.AsString);   //更新所有层状态为0
...
if not (dataname = 'wisdompcb_rb') then
...
try
 ...
 dm.ads26.Filter:='FLOW_NO = 1 and route_step_no='+dm.ADS38STEP_NUMBER.AsString;
 DM.ads26.Filtered:= True;

 DM.adsBOMParams.Close;
 DM.adsBOMParams.Parameters[0].Value := DM.ADS38rkey.Value;
 dm.adsBOMParams.Open;
  with DM.ads498 do
  begin
    Close;
    Parameters[0].Value:= DM.ADS38DEPT_PTR.Value;
    Open;
  end ;
...
var
 ...
 _rev:string;
  ...
  LGradeNote: string;
begin
  dm.SqlOpen('select 1 from data0025 where parent_ptr = ' + dm.AQY0025.fieldbyname('rkey').AsString);
  if not dm.qryTmp.IsEmpty then
  begin
    ShowMessage('变动流程请 注意同步更新内层压合步骤');
  end;
  ...
  begin
   ...
   if dm.ADS38.IsEmpty then
    ...
    begin
     ...
     dm.ads26.IndexFieldNames:='';
     dm.ads26.Filter:='ROUTE_STEP_NO >= '+inttostr(step_number);
     dm.ads26.First;
     while not dm.ads26.Eof do
     begin
      dm.ads26.Edit;
      dm.ads26ROUTE_STEP_NO.Value := dm.ads26ROUTE_STEP_NO.Value+1;
      dm.ads26.Post;
      dm.ads26.Next;
     end;
     dm.ads26.IndexFieldNames := 'ROUTE_STEP_NO';
     dm.ads26.Filter:='';
    end;
...
'SELECT TOP 100 PERCENT PARAMETER_PTR, TValue, Doc_Flag, after_flag, formula,'+#13+
   ...
   if not bitbtn11.Enabled then bitbtn11.Enabled:=true;
   FStepHasChange := True;
   SetParentPREStepZero(DM.AQY0025.fieldbyname('rkey').AsInteger);
  end;
finally
 form_deptcode.Free;
end;
...
begin
  ...
  if (DM.AQY0025barcode_flag.Value = 1) and ((Trim(cbb1.Text)='') or (Trim(cbb2.Text)='')) then
  begin
    ShowMessage('高难度产品，请选择难度级别！！');
    Exit;
  end;
  if not DM.ADS38.IsEmpty then
    if Self.update_BomQty(dm.AQY0025RKEY.Value) then Exit;

dm.ADOConnection1.BeginTrans;
try
 if  (cbb1.ItemIndex = -1) then dm.AQY0025DiffMark.AsVariant:= null;
 if  (cbb2.ItemIndex = -1) then dm.AQY0025ProdGrade.AsVariant:= null;
...
begin
 ...
 if not DM.ads26.Filtered then dm.ads26.Filtered := true;
 dm.ads26.First;
 while not dm.ads26.Eof do dm.ads26.Delete;
 dm.ads26.Filter:='';
...
if button=mbright then
 begin
  (sender as tstringgrid).MouseToCell(x,y,column,row);
   if (row>0) AND (row<(sender as tstringgrid).RowCount) and
     (column>0) AND (column<(sender as tstringgrid).ColCount) then
   begin
...
var i:word;
begin
 for i:=fsg.Row to fsg.RowCount-2 do
  begin
   fsg.Rows[i].Text:=fsg.Rows[i+1].Text;
   fsg.Cells[0,i]:=inttostr(i);
  end;
...
begin
 if fsg.Row=fsg.RowCount-1 then
  begin
   n6.Enabled:=false;
   n7.Enabled:=false;
   n16.Enabled:=false;
 ...
 if fsg.Row=1 then
  ...
  n15.Enabled:=true;
 n18.Enabled:=fsg.RowCount>2;
end;

procedure TForm2.N6Click(Sender: TObject);
var i:word;
begin
 for i:=fsg.RowCount-2 downto fsg.Row do
  begin
   fsg.Rows[i+1].Text:=fsg.Rows[i].Text;
   fsg.Cells[0,i+1]:=inttostr(i+1);
  end;
  fsg.Rows[fsg.Row].Text:='';
  fsg.Cells[0,fsg.Row]:= inttostr(fsg.Row);
  fsg.RowCount:=fsg.RowCount+1;
if not bitbtn11.Enabled then bitbtn11.Enabled:=true;
end;
...
if (lsg.Row=lsg.RowCount-1) and
   (trim(lsg.Rows[lsg.Row].Text)<>'') then
 begin
  lsg.Cells[0,lsg.Row]:=inttostr(lsg.Row);
  lsg.Cells[1,lsg.Row]:='T'+get_drTrow(lsg.Row);
  lsg.RowCount:=lsg.RowCount+1;
  if not bitbtn11.Enabled then bitbtn11.Enabled:=true;
 end;
end;
...
begin
  Lsg := sender as TStringGrid;
   ...
   IF (KEY=13) and (lsg.row<lsg.RowCount-1) then
   lsg.row:=lsg.row+1;
 if (chr(key)='D') and (ssCtrl in shift) and (lsg.Row>1) then
  begin
   lsg.Cells[lsg.col,lsg.row]:=
   lsg.Cells[lsg.col,lsg.row-1];
  end;
...
begin
 ...
 edit6.Text:=formatfloat('0.00',(get_mindrill(FSg)));
 //edit2.Text:=formatfloat('0.00',(get_mindrill(stringgrid4)));
 //edit18.Text:=formatfloat('0.00',(get_mindrill(stringgrid5)));
...
begin
 err:=false;
 for i:= 1 to fsg.RowCount-2 do
 try
  if (trim(fsg.Cells[4,i])='Y') and
     (roundto(strtofloat(fsg.Cells[6,i]),-2)<>
      roundto(dr_jishuan(strtofloat(fsg.Cells[3,i]))+strtofloat(edit7.Text),-2)) then
   begin
    messagedlg('第'+inttostr(i)+'行钻咀:'+fsg.Cells[3,i]+'不符沉铜孔偏差'
    ,mtinformation,[mbcancel],0);
    err:=true;
   end
  else
   if (trim(fsg.Cells[4,i])='N') and
     (roundto(strtofloat(fsg.Cells[6,i]),-2)<>
      roundto(dr_jishuan(strtofloat(fsg.Cells[3,i]))+strtofloat(edit8.Text),-2)) then
    begin
     messagedlg('第'+inttostr(i)+'行钻咀:'+fsg.Cells[3,i]+'不符非沉铜孔偏差'
     ,mtinformation,[mbcancel],0);
     err:=true;
    end;
 except
...
if dm.AQY0025.State=dsedit then
 begin
  if not (dataname = 'wisdompcb_rb') then    //软件特殊要求，后将改为控制开关
...
begin
 ...
 begin
  if (dm.ads494SPEC_RKEY.Value='AP') then  //找到最小孔径参数
   IF formatfloat('0.00',(get_mindrill(StringGrid1))) <> Trim(DM.ads494PARAMETER_VALUE.Value) then
   begin
...
for i:=1 to fsg.RowCount-2 do
 try
  if pnla_pcs>0 then
   fsg.Cells[9,i]:=
   inttostr(strtoint(trim(fsg.Cells[7,i]))*pnla_pcs+1);
  if pnlb_pcs>0 then
   fsg.Cells[10,i]:=
   inttostr(strtoint(trim(fsg.Cells[7,i]))*pnlb_pcs+1);
  {
  if set_pcs > 0 then
   Stringgrid1.Cells[8,i]:=
...
if find_spec_rkey('JN') then //set单元数
 ...
 try
  if pnla_pcs>0 then
   fsg.Cells[9,fsg.row]:=
   inttostr(strtoint(trim(fsg.Cells[7,fsg.row]))*pnla_pcs+1);
  if pnlb_pcs>0 then
   fsg.Cells[10,fsg.row]:=
   inttostr(strtoint(trim(fsg.Cells[7,fsg.row]))*pnlb_pcs+1);
 { if set_pcs > 0 then
   fsg.Cells[8,fsg.row]:=
   inttostr(strtoint(trim(fsg.Cells[7,fsg.row]))*set_pcs);
  }
  if not bitbtn11.Enabled then bitbtn11.Enabled:=true;
 except
...
begin
 ...
 total_b := 0;
 total_set := 0;
 if pos('TOTAL',UpperCase(fsg.Cells[7,fsg.RowCount-2]))>0 then
 ...
 for i:=1 to fsg.RowCount-2 do
  begin
   if trim(fsg.Cells[8,i])<>'' then
   total_set := total_set+strtoint(fsg.Cells[8,i]);

   if trim(fsg.Cells[9,i])<>'' then
   total_a := total_a+strtoint(fsg.Cells[9,i]);
   if trim(fsg.Cells[10,i])<>'' then
   total_b := total_b+strtoint(fsg.Cells[10,i]);
  end;
 except
...
fsg.Cells[7,fsg.RowCount-1]:='Total:';
if total_set>0 then
fsg.Cells[8,fsg.RowCount-1]:=inttostr(total_set);
if total_a>0 then
begin
fsg.Cells[9,fsg.RowCount-1]:=inttostr(total_a);
fsg.Cells[11,fsg.RowCount-1] := 'A板用时:' + FormatFloat('0.00',total_a/12000);
end;
if total_b>0 then
begin
fsg.Cells[10,fsg.RowCount-1]:=inttostr(total_b);
fsg.Cells[11,fsg.RowCount-1] := fsg.Cells[11,fsg.RowCount-1] + ';' +'B板用时:' + FormatFloat('0.00',total_b/12000);
end;
...
begin
 ...
 if form_drcalc.ShowModal=mrok then
  begin
   ...
   fsg.Cells[7,strtoint(form_drcalc.Edit2.text)+1]:='Total:';
   if total_set>0 then
    fsg.Cells[8,strtoint(form_drcalc.Edit2.text)+1]:=inttostr(total_set);

   if total_a>0 then
   begin
    fsg.Cells[9,strtoint(form_drcalc.Edit2.text)+1]:=inttostr(total_a);
    fsg.Cells[11,fsg.RowCount-1] := 'A板用时:' + FormatFloat('0.00',total_a/12000);
   end;
   if total_b>0 then
   begin
    fsg.Cells[10,strtoint(form_drcalc.Edit2.text)+1]:=inttostr(total_b);
    fsg.Cells[11,fsg.RowCount-1] := fsg.Cells[11,fsg.RowCount-1] + ';' +'B板用时:' + FormatFloat('0.00',total_b/12000);
   end;
  fsg.RowCount:=fsg.RowCount+1;
  if not bitbtn11.Enabled then bitbtn11.Enabled:=true;
...
begin
if fsg.RowCount>2 then
 Export_Grid_to_Excel(fsg,DBtext3.Caption,true);
end;
...
begin
  ...
  if messagedlg('导入新的钻孔表,原数据将丢失继续吗?',mtconfirmation,[mbyes,mbno],0)=mryes then
   begin
    for i:=1 to fsg.RowCount-2 do  fsg.Rows[i].Clear;
     fsg.RowCount:=2;
    load_xls_to_sgrid(OpenDialog1.FileName,fsg);
   end;
end;
...
begin
 ...
 strow.AddStrings(self.fsg.Rows[fsg.row]);
 for i:=2 to fsg.ColCount-1 do
 begin
  fsg.Cells[i,fsg.row]:=fsg.Cells[i,fsg.row-1];
  fsg.Cells[i,fsg.row-1]:=strow.Strings[i];
 end;
 strow.Destroy;
 fsg.row:=fsg.row-1;
 if not bitbtn11.Enabled then bitbtn11.Enabled:=true;
...
var row:string;
    i:integer;
begin
 row:=inttostr(self.fsg.row);
 row:=
 InputBox('信息输入', '请输入对调行数:      ',row);

 try
  if (strtoint(row)<=0) or (strtoint(row)>=fsg.RowCount) then
   begin
    showmessage('输入行号有误');
    exit;
   end;
 ...
 for i:=2 to fsg.ColCount-1 do
 begin
  fsg.Cells[i,fsg.row]:=fsg.Cells[i,strtoint(row)];
  fsg.Cells[i,strtoint(row)]:=strow.Strings[i];
 end;
 strow.Destroy;
 fsg.row:=strtoint(row);
 if not bitbtn11.Enabled then bitbtn11.Enabled:=true;
end;
...
begin
  ...
  try
    ...
    lqry.SQL.Text := 'select file_content from data0020 where rkey = ' + dm.ADO20.fieldbyname('rkey').AsString;
    lqry.Open;

     if lqry.FieldByName('file_content').IsNull then
     begin
       showmessage('此文档为空...');
       exit;
     end;

     TBlobField(lqry.FieldByName('file_content')).SaveToStream(fs);
     UnPackStream(fs);
...
begin
 ...
 try
  ...
  if pnla_pcs>0 then
   fsg.Cells[9,fsg.row]:=
   inttostr(strtoint(trim(fsg.Cells[8,fsg.row]))*trunc(pnla_pcs/set_pcs)+1);

  if pnlb_pcs>0 then
   fsg.Cells[10,fsg.row]:=
   inttostr(strtoint(trim(fsg.Cells[8,fsg.row]))*trunc(pnlb_pcs/set_pcs)+1);
...
for i:=1 to fsg.RowCount-2 do
 try
  ...
  if set_pcs > 0 then
   fsg.Cells[7,i]:=
   inttostr(trunc(strtoint(trim(fsg.Cells[8,i]))/set_pcs));
  }
  if pnla_pcs>0 then
   fsg.Cells[9,i]:=
   inttostr(strtoint(trim(fsg.Cells[8,i]))*trunc(pnla_pcs/set_pcs)+1);

  if pnlb_pcs>0 then
   fsg.Cells[10,i]:=
   inttostr(strtoint(trim(fsg.Cells[8,i]))*trunc(pnlb_pcs/set_pcs)+1);
...
// dm.ads26.IndexFieldNames:='';
 ...
 begin
  dm.ads26.Edit;
  dm.ads26ROUTE_STEP_NO.Value := recd_cound+1;
  dm.ads26.Post;
//  dm.ads26.Next;
 end;
// dm.ads26.IndexFieldNames := 'ROUTE_STEP_NO';
...
// dm.ads26.IndexFieldNames:='';
 dm.ads26.Filter:='ROUTE_STEP_NO= '+inttostr(step_number-1);
 dm.ads26.First;
 while not dm.ads26.Eof do
 begin
  dm.ads26.Edit;
  dm.ads26ROUTE_STEP_NO.Value := step_number;
  dm.ads26.Post;
//  dm.ads26.Next;
 end;
// dm.ads26.IndexFieldNames := 'ROUTE_STEP_NO';
...
// dm.ads26.IndexFieldNames:='';
 dm.ads26.Filter:='ROUTE_STEP_NO = '+inttostr(recd_cound+1);
 dm.ads26.First;
 while not dm.ads26.Eof do
 begin
  dm.ads26.Edit;
  dm.ads26ROUTE_STEP_NO.Value := step_number-1;;
  dm.ads26.Post;
//  dm.ads26.Next;
 end;
// dm.ads26.IndexFieldNames := 'ROUTE_STEP_NO';
    ...
    showmessage('粘贴板内容为空，请复制所需内容。')
              ...
              begin
                  for y:=1 to tmpy+1 do
                        fsg.Cells[y-1,x+1]:=FromCB1[x][y+1];
              end;
        end;
end;
...
var
 i:integer;
begin
 for i:=fsg.Row+1 to fsg.RowCount-1 do
  if trim(fsg.Cells[1,i])<>'' then
   fsg.Cells[1,i] := hz_jiajian(fsg.Cells[1,i],1);//加1
end;
...
begin
 for i:=fsg.Row+1 to fsg.RowCount-1 do
  if trim(fsg.Cells[1,i])<>'' then
   fsg.Cells[1,i] := hz_jiajian(fsg.Cells[1,i],0);  //减1
end;
...
procedure TForm2.DirllPth(ASg: TStringGrid;AEdt:TEdit);
var
 i:word;
begin
...
begin
 ...
 for i:= 1 to ASg.RowCount-2 do
 try
   if trim(ASg.Cells[4,i])='Y' then
    ASg.Cells[6,i]:=formatfloat('0.00',
                            dr_jishuan(strtofloat(ASg.Cells[3,i]))+
                            strtofloat(edit7.Text))

  else
   if trim(ASg.Cells[4,i])='N' then
    ASg.Cells[6,i]:=formatfloat('0.00',
                            dr_jishuan(strtofloat(ASg.Cells[3,i]))+
...
begin
 ...
 for i:= 1 to fsg.RowCount-2 do
 try
   if trim(fsg.Cells[4,i])='Y' then
    fsg.Cells[3,i]:=formatfloat('0.00',
                            StrToCurr(fsg.Cells[6,i])-StrToCurr(edit7.Text))

  else
   if trim(fsg.Cells[4,i])='N' then
    fsg.Cells[3,i]:=formatfloat('0.00',
                            StrToCurr(fsg.Cells[6,i])-StrToCurr(edit8.Text));
...
//      dm.qryTmp.ExecSQL;
      ...
      //梅州黄志辉需求
      //拼版后自动同步更新SPCK-RKEY 为LS的工序参数
      //2017-6-22
      if dm.ads278.Locate('spec_rkey','LS',[loCaseInsensitive]) then
      begin
        try
          dm.FChangeAllParam := False;
        DM.ads494.Filter := ' PARAMETER_PTR = ' + dm.ads278.fieldbyname('rkey').AsString;
        dm.ads494.Filtered := true;
        dm.ads494.First;
        while not dm.ads494.Eof do
        begin
          DM.ads494.Edit;
          dm.ads494.FieldByName('PARAMETER_VALUE').AsString := LIni.ReadString('CutRet','Seet_Size','');
          dm.ads494.Post;
          dm.ads494.Next;
        end;
        dm.ads494.Filter := '';
        finally
          dm.FChangeAllParam := True;
        end;
      end;
      //-----------
      //-----------2017-6-22-----
...
begin
  try
   ...
   Form4.Caption:=IntToStr(fsg.Row);
   Form4.ShowModal;
  finally
   Form4.Free;
  end;
...
begin
  if (Length(Trim(fsg.Cells[1,fsg.Row])) <> 0) and
      (Length(Trim(fsg.Cells[3,fsg.Row])) <> 0) and
      (Length(Trim(fsg.Cells[5,fsg.Row])) <> 0)
  then
  begin
    if (Sender as TMenuItem).Name = 'mniYJDW' then
    begin
    Lstr := '压接定位孔孔径控制: ' + fsg.Cells[1,fsg.Row] + '=' +
      fsg.Cells[3,fsg.Row] + fsg.Cells[5,fsg.Row];
    end else
    if (Sender as TMenuItem).Name = 'mniYJCP' then
    begin
    Lstr := '压接成品孔孔径控制: ' + fsg.Cells[1,fsg.Row] + '=' +
      fsg.Cells[3,fsg.Row] + fsg.Cells[5,fsg.Row];
    end else
    begin
      ShowMessage('无效的事件');
...
var
  ...
  tmpS: string;
begin
  for I := 0 to fsg.RowCount - 1 do
  begin
    total_a := 0;
    total_b := 0;
    tmpS := '';
    if LowerCase(fsg.Cells[7,I]) = 'total:' then
    begin
      if StrToIntDef(fsg.Cells[9,I],-1) <> -1 then
      begin
        total_a := StrToIntDef(fsg.Cells[9,I],-1);
      end;
      if StrToIntDef(fsg.Cells[10,I],-1) <> -1 then
      begin
        total_b := StrToIntDef(fsg.Cells[10,I],-1);
      end;
      if total_a <> 0 then
      begin
        tmpS := 'A板用时:' + FormatFloat('0.00',total_a/12000) + ';';
      end;
      if total_b <> 0 then
      begin
        tmpS := tmpS + 'B板用时:' + FormatFloat('0.00',total_b/12000);
      end;
      if tmpS <> '' then
      begin
        fsg.Cells[11,I] := fsg.Cells[11,I] + ';' + tmpS
      end;
    end;
  end;
end;
...
begin
  if (cbbgrade.Text = '') and (DM.AQY0025.FieldByName('grade_code').AsString = '') then Exit;
  if DM.AQY0025.FieldByName('grade_code').AsString = cbbgrade.Text then Exit;
  if cbbgrade.Text <> '' then
  begin
  ...
  begin
    ...
    begin
      ...
      if cbbgrade.Text = '' then
        ...
        DM.AQY0025.FieldByName('grade_code').AsString := cbbgrade.text;//cbbgrade.Items[cbbgrade.ItemIndex];
//      if cbbgrade.Items[cbbgrade.ItemIndex] = '无等级' then
      if cbbgrade.Text = '' then
        dm.SetGrade_Code(dm.AQY0025.fieldbyname('rkey').AsString,'')
      else
        dm.SetGrade_Code(dm.AQY0025.fieldbyname('rkey').AsString,cbbgrade.Text);
    end else
    begin
        cbbgrade.Text := DM.AQY0025.FieldByName('grade_code').AsString;
      //cbbgrade.ItemIndex := cbbgrade.Items.IndexOf(DM.AQY0025.FieldByName('grade_code').AsString);
      ...
      begin
        ...
        begin
          ...
          SQL.Text:= 'SELECT top 1 Rkey, FtpIP, FtpUserName, FtpPassWord, FtpDir + isnull(SubDir_1,'''') + isnull(SubDir_2,'''') as FDir ' + #13+
                     ' from tier3_ftplist where FtpID = 2 order by rkey desc';
  ...
  //        begin
            try
              ...
              begin
                ...
                DM.qryFileList.FieldByName('rkey25').Value:= DM.ADS25rkey.Value;
                DM.qryFileList.FieldByName('FtpIDKey').Value:= FIDKey;
                dm.qryFileList.FieldByName('FileType').value:= LType;
                DM.qryFileList.Post;

                DM.qryFile.Append;
                DM.qryFile.FieldByName('FileName').Value:= ExtractFileName(LFilePath);
                dm.qryFile.FieldByName('UploadTime').value:= DM.qryFtp.FieldByName('UPLoadTime').Value;
                dm.qryFile.FieldByName('filetype').value:= LType;
                dm.qryFile.FieldByName('IDKey').value:= FIDKey;
                dm.qryFile.FieldByName('Ftp_fileName').value:= FIDKey;
                DM.qryFile.FieldByName('EMPLOYEE_NAME').AsString:= Get_05Ptr(rkey73,1);
                DM.qryFile.Post;
              end;
            except
...
procedure TForm2.CreateDir2(AFtp: TIdFTP; ADir: string);
var
...
function TForm2.Get_05Ptr(ARkey73: string;AType:Integer): string;
begin
 with dm.ADOQuery1 do
 begin
  close;
...
begin
  N46.Enabled:= strtoint(status.Caption)=0;
  N47.Enabled:= not DM.qryFile.IsEmpty;
  N48.Enabled:= (not DM.qryFile.IsEmpty) and (strtoint(status.Caption)=0);
end;
...
//      Close;
//      SQL.Text:= 'SELECT top 1 Rkey, FtpIP, FtpUserName, FtpPassWord, FtpDir + isnull(SubDir_1,'''') + isnull(SubDir_2,'''') as FDir ' + #13+
//                 ' from tier3_ftplist where rkey = ' + DM.qryFtpFtp_Ptr.AsString;
//      open;
//      if not IsEmpty then
//      begin
//        LFtp.Host := Fields[1].AsString;
...
//        LFtp.Put(LFilePath,LFtpDir + FIDKey,True);
      ...
      if DM.qryFileList.Locate('FtpIDKey',DM.qryFileIDKey.Value,[]) then
        DM.qryFileList.Delete;
      if DM.qryFtp.Locate('IDKey',DM.qryFileIDKey.Value,[]) then
        DM.qryFtp.Delete;
      DM.qryFile.Delete;
...
procedure TForm2.cbbTypeChange(Sender: TObject);
begin
  DM.qryFile.Filtered:= False;
  if cbbType.ItemIndex = 4 then
    DM.qryFile.Filter:=''
  else
    DM.qryFile.Filter:='filetype = ' + IntToStr(cbbType.ItemIndex + 1);
  DM.qryFile.Filtered:= True;
end;

procedure TForm2.UpdateStatus(Arkey25: string);
var
  LTmp: TADOQuery;
begin
  LTmp:= TADOQuery.Create(nil);
  try
    LTmp.Connection:= DM.ADOConnection1;
    LTmp.Close;
    LTmp.SQL.Text:= ';WITH CTE  AS     '+#13+
                    '  (                    '+#13+
                    '    SELECT RKey         '+#13+
                    '    FROM Data0025 WHERE RKey =  ' + Arkey25 +
                    '    UNION ALL   '+#13+
                    '    SELECT D25.RKey  '+#13+
                    '    FROM Data0025 D25   '+#13+
                    '      INNER JOIN CTE ON D25.PARENT_PTR = CTE.RKey  '+#13+
                    '  )      '+#13+
                    '  update dbo.Data0025 SET TSTATUS=0  where RKEY IN ( SELECT rkey FROM CTE )  ';
    LTmp.ExecSQL;
  finally
    LTmp.Free;
  end;
...
procedure TForm2.SetParentPREStepZero(AParentRkey: Integer);
var
  LSql: string;
begin
  ShowMessage('更改流程，内层压合步骤必须重新填写');
  LSql := ';WITH CTE(RKey,Lv,MANU_PART_NUMBER) ' +
          ' AS (SELECT RKey,1,MANU_PART_NUMBER ' +
          ' FROM Data0025 WHERE PARENT_PTR = '+ IntToStr(AParentRkey) +' UNION ALL ' +
          ' SELECT D25.RKey,CTE.Lv+1,D25.MANU_PART_NUMBER ' +
          ' FROM Data0025 D25 INNER JOIN CTE ON D25.PARENT_PTR = CTE.RKey ) ' +
          ' update Data0025 SET BOM_STEP = 0 from Data0025 INNER JOIN cte on cte.rkey = Data0025.rkey where BOM_STEP <> 1 ' ;
  //ShowMessage(LSql);
  DM.SqlExec(LSql);
end;
  ...
  //        begin
            try
              ...
              begin
                ...
                DM.qryFtp2.FieldByName('UPLoadTime').Value:= getsystem_date(LTmp,0);
                ...
                DM.qryFileList2.FieldByName('rkey25').Value:= DM.ADS25rkey.Value;
                DM.qryFileList2.FieldByName('FtpIDKey').Value:= FIDKey;
                dm.qryFileList2.FieldByName('FileType').value:= 0;
                DM.qryFileList2.FieldByName('groups').Value:= 1;
                DM.qryFileList2.FieldByName('dept_ptr').Value:= LDept;
                DM.qryFileList2.Post;

                DM.qryFile2.Append;
                DM.qryFile2.FieldByName('FileName').Value:= ExtractFileName(LFilePath);
                dm.qryFile2.FieldByName('UploadTime').value:= DM.qryFtp2.FieldByName('UPLoadTime').Value;
                dm.qryFile2.FieldByName('filetype').value:= 0;
                dm.qryFile2.FieldByName('IDKey').value:= FIDKey;
                dm.qryFile2.FieldByName('Ftp_fileName').value:= FIDKey;
                DM.qryFile2.FieldByName('EMPLOYEE_NAME').AsString:= Get_05Ptr(rkey73,1);
                DM.qryFile2.FieldByName('dept_code').AsString:= LDeptCode;
                DM.qryFile2.Post;
...
begin
  if DM.qryFileList2.Locate('FtpIDKey',DM.qryFile2IDKey.Value,[]) then
    DM.qryFileList2.Delete;
  if DM.qryFtp2.Locate('IDKey',DM.qryFile2IDKey.Value,[]) then
    DM.qryFtp2.Delete;
  DM.qryFile2.Delete;
end;
...
begin
  if dm.AQY0025MinRing.AsString ='' then exit;
  dm.qryTmp.Close;
  DM.qryTmp.SQL.Text:= 'UPDATE DATA0025 SET MinRing = '+ dm.AQY0025MinRing.AsString + ' where rkey = '+ rkey25.Caption;
  DM.qryTmp.ExecSQL;
...
var
  LFrm: TfrmPress;
begin
  LFrm:= TfrmPress.Create(nil);
  try
    DM.qryPress.Append;
    DM.qryPressrkey25.Value:= dm.AQY0025RKEY.Value;
    if LFrm.ShowModal = mrok then DM.qryPress.Post
    else DM.qryPress.Cancel;
  finally
    LFrm.Free;
  end;
end;
...
procedure TForm2.N54Click(Sender: TObject);
begin
  DM.qryPress.Delete;
end;

procedure TForm2.pm3Popup(Sender: TObject);
begin
  N52.Enabled:= (status.Caption = '0') and ((vprev='2') or (vprev ='4'));
  N53.Enabled:= (not DM.qryPress.IsEmpty) and ((vprev='2') or (vprev ='4'));
  N54.Enabled:= N53.Enabled;
end;

procedure TForm2.DBCheckBox5Click(Sender: TObject);
begin
...
begin
  ListBox1.ItemIndex:=13;
  ListBox1Click(nil);
  N46Click(nil);
end;

function TForm2.CalcBomPartQty(var ExprStr: string;
  AStepNumber: Integer;rkey25:Integer): double;
var
  i,j:integer;
  X,Y:string;
...
function TForm2.Update_Expr_Param_Value(DAtaSet: TCustomADODataSet;
...
begin
  ...
  begin
    DM.adsParamsInfo.Locate('rkey',DAtaSet.fieldbyName('PARAMETER_PTR').AsString,[]);
...
function TForm2.update_BomQty(rkey25:integer):Boolean;
var
  ExprStr,dept:string;
  IsCale:Boolean;
begin
  Result:= False;
//  dept:= GetDept(DM.ADS38);
...
//    IsCale:= False;
    ...
    begin
      Close;
      sql.Text:= 'SELECT DATA0034.RKey,DATA0034.DEPT_CODE, DATA0034.DEPT_NAME, Data0498.INV_GROUP_PTR, Data0498.BOM_NAME, ' +#13+
                 '   Data0498.FORMULA, Data0498.SEQ_NO, Data0496.GROUP_NAME, Data0496.GROUP_DESC, Data0496.SPEC_RKEY,  ' +#13+
                 '   Data0498.DOC_FLAG, Data0498.RKEY AS MGRkey,Data0498.required,Data0496.IsMaster     ' +#13+
                 '   FROM Data0498                 ' +#13+
                 '       INNER JOIN DATA0034 ON Data0034.RKEY = Data0498.DEPT_PTR     ' +#13+
                 '       INNER JOIN Data0496 ON Data0496.RKEY = Data0498.INV_GROUP_PTR      ' +#13+
                  '  WHERE Data0496.IsMaster = 1 AND DATA0034.IS_COST_DEPT = 1    ' +#13+
                  '    AND DATA0034.RKEY = '+ dm.ADS38DEPT_PTR.AsString +#13+
                  '  ORDER BY  Data0498.SEQ_NO' ;
      Open;
//
      if not IsEmpty then
      begin
        while not Eof do
        begin
          DM.ads26.Filter:= 'Route_step_no = ' + DM.ADS38STEP_NUMBER.AsString + ' and GROUP_PTR = '+ FieldByName('INV_GROUP_PTR').AsString;
          DM.ads26.Filtered:= True;
          if DM.ads26.IsEmpty then
          begin
            ShowMessage('步骤 '+ DM.ADS38STEP_NUMBER.AsString + ' 必须增加材料类型 ' + FieldByName('GROUP_DESC').AsString + ' 的材料');
            Result := True;
            Break;
          end;

//          if not DM.ads26.IsEmpty then
//          begin
//            while not DM.ads26.Eof do
//            begin
//              try
//                DM.ads26.Edit;
//                DM.ads26DOC_FLAG.AsString:= FieldByName('DOC_FLAG').AsString;
//                ExprStr:=' '+trim(fieldbyName('FORMULA').Asstring)+' ';
//                try
//                  DM.ads26QTY_BOM.Value:= CalcBomPartQty(ExprStr,DM.ads26ROUTE_STEP_NO.AsInteger,rkey25);
//                except
//                  ShowMessage(DM.ads26ROUTE_STEP_NO.AsString +'步骤用量公式计算错误:'+trim(fieldbyName('FORMULA').Asstring));
//                  DM.ads26.Cancel;
//                  Result:= True;
//                  exit;
//                end;
...
procedure TForm2.append26(rkey25:Integer);
begin
with dm.ADOQuery1 do
 begin
  close;
  sql.Text:= 'DELETE FROM DATA0026 WHERE FLOW_NO=1 AND MANU_BOM_PTR= ' + IntToStr(rkey25);
  ExecSQL;
 end;
...
with dm.qryTmp do
 begin
  close;
  sql.Text:= 'SELECT MANU_BOM_PTR, DEPT_PTR, INVENTORY_PTR, QTY_BOM, FLOW_NO '+#13+
              ' , ROUTE_STEP_NO, DOC_FLAG, VENDOR, supplier_ptr,D498_ptr FROM DATA0026 WHERE 1=0';
  open;
 end;
...
begin
  DM.qryTmp.Append;
  dm.qryTmp.FieldByName('MANU_BOM_PTR').Value :=dm.ads26MANU_BOM_PTR.Value;
  dm.qryTmp.FieldByName('DEPT_PTR').Value :=dm.ads26DEPT_PTR.Value;
  dm.qryTmp.FieldByName('INVENTORY_PTR').Value :=dm.ads26INVENTORY_PTR.Value;
  dm.qryTmp.FieldByName('QTY_BOM').Value:=dm.ads26QTY_BOM.Value;
  dm.qryTmp.FieldByName('FLOW_NO').Value:=dm.ads26FLOW_NO.Value;
  dm.qryTmp.FieldByName('ROUTE_STEP_NO').Value:=dm.ads26ROUTE_STEP_NO.Value;
  dm.qryTmp.FieldByName('DOC_FLAG').Value:=dm.ads26DOC_FLAG.Value;
  dm.qryTmp.FieldByName('VENDOR').Value:=dm.ads26VENDOR.Value;
  if DM.ads26supplier_ptr.AsInteger = 0 then dm.qryTmp.FieldByName('supplier_ptr').AsVariant:= null
  else dm.qryTmp.FieldByName('supplier_ptr').Value:=DM.ads26supplier_ptr.Value ;
  dm.qryTmp.FieldByName('D498_ptr').Value:=dm.ads26D498_ptr.Value;
  dm.qrytmp.Post;
  dm.ads26.Next;
end;
end;

//function TForm2.GetDept(cds: TADODataSet): string;
//VAR
//  strfilter,dept:string;
//begin
//  dept:='';
//  if cds.Filtered then
//    strfilter:= cds.Filter;
//  cds.Filter:= '';
//  cds.First;
//  while not cds.Eof do
//  begin
//    dept:= cds.FieldByName('DEPT_PTR').AsString + ',' + dept;
//    cds.Next;
//  end;
//  cds.Filter:= strfilter;
//  Result:= Copy(dept,1,Length(dept)-1);
//end;

procedure TForm2.ehBOMColumns5EditButtonClick(Sender: TObject;
  var Handled: Boolean);
var
  InputVar:PDlgInput;
begin
  if DM.ads26INVENTORY_PTR.AsInteger = 0 then exit;
...
procedure TForm2.pm2Popup(Sender: TObject);
begin
  N49.Enabled:= strtoint(status.Caption)=0;
  N50.Enabled:= not DM.qryFile2.IsEmpty;
  N51.Enabled:= (not DM.qryFile2.IsEmpty) and (strtoint(status.Caption)=0);
end;
...
begin
  ...
  begin
    ...
    try
      lqry.Connection := DM.ADOConnection1;
      lqry.SQL.Text := 'select rkey from data0010 where ABBR_NAME = ' + QuotedStr(DM.AQY0025.FieldByName('ORIG_CUSTOMER').AsString);
      lqry.Open;
      if not lqry.IsEmpty then
      begin
        d10rkey := lqry.fieldbyname('rkey').AsInteger;
      end;
    finally
      lqry.free;
    end;
  end else
  if DM.AQY0025.FieldByName('CUSTOMER_PTR').AsInteger <> 0 then
  begin
    d10rkey := DM.AQY0025.FieldByName('CUSTOMER_PTR').AsInteger;
  end;

  if (d10rkey = 0)  then
  begin
    ShowMessage('请先选择客户或关联客户');
    Exit;
  end;
...
try
 InputVar.Fields := 'barname/二维码格式/300';
 InputVar.Sqlstr := 'select * from Print2D_BarDef where d10_ptr = ' + IntToStr(d10rkey);
 InputVar.AdoConn := DM.ADOConnection1;
 frmPick_Item_Single.InitForm_New(InputVar);
if frmPick_Item_Single.ShowModal=mrok then
 begin
  edtPrint2D.Text := frmPick_Item_Single.adsPick.FieldValues['barname'];
  edtPrint2D.Tag := frmPick_Item_Single.adsPick.FieldValues['rkey'];
  dm.AQY0025.edit;
  DM.AQY0025.fieldbyname('print2dkey').AsInteger := edtPrint2D.tag;
  dm.AQY0025.Post;
 end;
finally
