...
{$R *.dfm}
function TFrm_YsMain.GetCheckBox():string;
var sTp:string;
    i:Integer;
begin
  sTp:='';
...
procedure TFrm_YsMain.BitBtn5Click(Sender: TObject);
begin
  Close;
end;

procedure TFrm_YsMain.BitBtn4Click(Sender: TObject);
begin
  Export_dbGridEH_to_Excel(DBGridEh1,self.Caption) ;
end;

procedure TFrm_YsMain.BitBtn1Click(Sender: TObject);
var i,j,iNo:integer;
...
begin
  ...
  begin
    ...
    try
      ...
      DM.Ado538.Close;
      DM.Ado538.SQL.Clear;
      DM.Ado538.SQL.Text:=strSQLTxt+' where data0538.ys_user_ptr='+common.user_ptr+Sqlstr+sWhereIN+' and DATA0538.yszp_date>='''+formatdatetime('yyyy-mm-dd ',FrmQuery.DateTimePicker3.Date)+
                           ''' and DATA0538.yszp_date<='''+formatdatetime('yyyy-mm-dd ',FrmQuery.DateTimePicker5.Date+1)+''' order by DATA0538.yszp_date desc';
...
procedure TFrm_YsMain.FormShow(Sender: TObject);
var i:Integer;
begin
  ...
  begin
    ...
    DM.Ado538.Close;
    DM.Ado538.SQL.Clear;
    DM.Ado538.SQL.Text:=strSQLTxt+' where YS_USER_PTR='+common.user_ptr+sWhereIN+' and DATA0538.yszp_date>='''+formatdatetime('yyyy-mm-dd ',StartDate)+
                         ''' and DATA0538.yszp_date<='''+formatdatetime('yyyy-mm-dd ',EndDate+1)+''' order by DATA0538.yszp_date desc';
    DM.Ado538.Prepared;
    DM.Ado538.Open;
     try
        DM.Ado0849.Close;
        ...
        begin
          ...
          begin
            ...
            DM.Ado849s.FieldByName('FormName').Value:='Frm_YsMain';
...
procedure TFrm_YsMain.BitBtn2Click(Sender: TObject);
var strtemp:string;
begin
...
procedure TFrm_YsMain.FormCreate(Sender: TObject);
begin
...
procedure TFrm_YsMain.BitBtn3Click(Sender: TObject);
var i:Integer;
begin
...
procedure TFrm_YsMain.DBGridEh1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrm_YsMain.DBGridEh1DrawColumnCell(Sender: TObject;
...
procedure TFrm_YsMain.CheckBox1Click(Sender: TObject);
var strtemp:string;
begin
  sWhereIN:=GetCheckBox();
  DM.Ado538.Close;
  ...
  DM.Ado538.SQL.Text:=strSQLTxt+
    ' where ys_user_ptr='+common.user_ptr+sWhereIN+
    ' and DATA0538.yszp_date>='''
    +formatdatetime('yyyy-mm-dd ',FrmQuery.DateTimePicker3.Date)+
    ''' and DATA0538.yszp_date<='''
    +formatdatetime('yyyy-mm-dd ',FrmQuery.DateTimePicker5.Date)+
    ''' order by DATA0538.yszp_date desc';
  DM.Ado538.Prepared;
  DM.Ado538.Open;

end;

procedure TFrm_YsMain.CDSFilterRecord(DataSet: TDataSet;
  var Accept: Boolean);
begin
...
procedure TFrm_YsMain.PopupMenu1Popup(Sender: TObject);
begin
 ...
 begin
  N1.Enabled:=((dm.Ado538STATUS.Value=1) or (dm.Ado538STATUS.Value=9)) and (DM.Ado538YS_EQ_STATUS.Value<>1);
  N2.Enabled:=(dm.Ado538STATUS.Value=2) and (DM.Ado538YS_EQ_STATUS.Value<>1);
  N3.Enabled:=(dm.Ado538YS_EQ_STATUS.Value<>1) and ((dm.Ado538STATUS.Value=1) or (dm.Ado538STATUS.Value=2));
  N4.Enabled:=dm.Ado538YS_EQ_STATUS.Value=1;
 end;
end;

procedure TFrm_YsMain.N1Click(Sender: TObject);
begin
  ...
  dm.Ado538.Edit;
  dm.Ado538STATUS.Value :=2;
  dm.Ado538YS_USER_PTR.Value := strtoint(user_ptr);
  dm.tmp.SQL.Clear;
  dm.tmp.SQL.Text := 'select getdate() as t';
  dm.tmp.ExecSQL;
  dm.tmp.Open;
  dm.Ado538YS_STAR_DATE.Value := dm.tmp.fieldbyname('t').AsDateTime;
  dm.tmp.Close;
...
procedure TFrm_YsMain.N2Click(Sender: TObject);
begin
      if (strtoint(vprev) = 1) or (strtoint(vprev)= 3 ) then
  begin
    messagedlg('对不起!您没有该程序的可写权限',mtinformation,[mbok],0);
  ...
  dm.Ado538.Edit;
  dm.Ado538STATUS.Value :=3;
  dm.tmp.SQL.Clear;
  dm.tmp.SQL.Text := 'select getdate() as t';
  dm.tmp.ExecSQL;
  dm.tmp.Open;
  dm.Ado538YS_EDN_DATE.Value := dm.tmp.fieldbyname('t').AsDateTime;
  dm.tmp.Close;
...
procedure TFrm_YsMain.N3Click(Sender: TObject);
begin
  if (strtoint(vprev) = 1) or (strtoint(vprev)= 3 ) then
  begin
    messagedlg('对不起!您没有该程序的可写权限',mtinformation,[mbok],0);
  ...
  dm.Ado538.Edit;
  DM.Ado538YS_EQ_STATUS.Value:=1;
  dm.tmp.SQL.Clear;
  dm.tmp.SQL.Text := 'select getdate() as t';
  dm.tmp.ExecSQL;
  dm.tmp.Open;
  dm.Ado538YS_EQ_STAR.Value := dm.tmp.fieldbyname('t').AsDateTime;
  dm.tmp.Close;
...
procedure TFrm_YsMain.N4Click(Sender: TObject);
begin
  if (strtoint(vprev) = 1) or (strtoint(vprev)= 3 ) then
  begin
    messagedlg('对不起!您没有该程序的可写权限',mtinformation,[mbok],0);
  ...
  dm.Ado538.Edit;
  DM.Ado538YS_EQ_STATUS.Value:=2;
  dm.tmp.SQL.Clear;
  dm.tmp.SQL.Text := 'select getdate() as t';
  dm.tmp.ExecSQL;
  dm.tmp.Open;
  dm.Ado538YS_EQ_END.Value := dm.tmp.fieldbyname('t').AsDateTime;
  dm.tmp.Close;
...
procedure TFrm_YsMain.BitBtn6Click(Sender: TObject);
begin
 if (strtoint(vprev) = 1) or (strtoint(vprev)= 3 ) then
  begin
    messagedlg('对不起!您没有该程序的可写权限',mtinformation,[mbok],0);
  ...
  begin
   ...
   begin
    Close;
    SQL.Clear;
    SQL.Add('update data0538 set status=1,yszp_user_ptr='+common.user_ptr);
    sql.Add(',ys_user_ptr='+common.user_ptr+',yszp_date=getdate()');
...
procedure TFrm_YsMain.DBGridEh1TitleClick(Column: TColumnEh);
begin
...
procedure TFrm_YsMain.Edit1Change(Sender: TObject);
begin
if trim(edit1.Text)<>'' then
