...
begin
 ...
 if form_product.ShowModal=mrok then
  begin
   Edit8.Text:=form_product.adoquery1ANALYSIS_CODE_2.Value;
   Edit9.Text:=form_product.adoquery1ANALYSIS_CODE_2.Value;
   ...
   if CUSTSTASHIP='Y' then           //流水号在前
     if (radiobutton1.Checked) or (radiobutton4.Checked) then
      ...
      edit5.Text:=fieldbyname('sufchar').AsString+
                  ...
                  ANALYSIS_CODE3 +
                  fieldbyname('PRECHAR1').AsString+'A0' //样板初始值
   else                               //流水号在后
   if (radiobutton1.Checked) or (radiobutton4.Checked) then
    ...
    edit5.Text:=dm.adoquery2.fieldbyname('part_templet').AsString+ANALYSIS_CODE3+
      fieldbyname('PRECHAR1').AsString+fieldbyname('sufchar').AsString+'A0'; //样板初始值
...
begin
 ...
 begin
  close;
  sql.Text:='select 1 from data0008 where active = 1 and  rkey = '+ rkey08.caption;
//  ShowMessage(SQL.Text);
  open;
  if IsEmpty then
  if MessageDlg('使用的产品类型已经过期，是否继续？',mtWarning, [mbYes,mbNo],0) <> mryes then Exit;
 end;
 ...
 try
  ...
  begin
   ...
   dm.AQY0025LAST_SO_DATE.AsVariant:=null;
   ...
   DM.AQY0025ANALYSIS_CODE_2.Value:=Edit9.Text;//----------------------------------------------------------------------------
   if (radiobutton1.Checked) or (radiobutton4.Checked) then
   ...
   if i=0 then             //顶层产品:复制评审,ENC,阻抗,检查审核时输入的备注信息
   begin
    ...
    begin
     ...
     'product_impedance, impedance_scope, orig_juli, adjust_jiuli,impedance_first,product_control,adjust_first,adjust_control,coupon) '+
     ...
     'adjust_impedance,product_impedance, impedance_scope, orig_juli, adjust_jiuli,impedance_first, '+
     ' product_control,adjust_first,adjust_control,coupon  '+
     'FROM  DATA0030 AS DATA0030_1 '+
     'WHERE  (SOURCE_PTR = '+inttostr(parent)+')';
     ExecSQL;
    end;

    with dm.ADOQuery1 do      //180907王书鹏恢复
     begin
      close;
      sql.Text:= 'insert into data0011 '+
       '(FILE_POINTER,SOURCE_TYPE,memo_text,NOTE_PAD_LINE_1) '+
       'select '+dm.AQY0025RKEY.AsString+' as FILE_POINTER,SOURCE_TYPE,memo_text,NOTE_PAD_LINE_1'+#13+
