...
uses
  ...
  Grids, DBGrids, Menus, DB, ADODB,math, DBGridEh, jpeg, ValEdit, IdFTP
...
type
  ...
  TForm2 = class(TForm)
    ...
    procedure  get_drill2(rkey25:integer;ASg: TStringGrid;AFlowNo: Integer;Aedt: tedit);
  ...
  private
   ...
   function get_mindrill2(ASg: TStringGrid):single;
...
uses damo, pcb_cutting,common, route_flow, flowpara_edit, deptcode_search,
  ...
  product_search,PubFunc,shellAPI, Excel2000,StrUtils;
...
//    stringgrid1.Cells[11,i]:=trim(IntToStr(fieldbyname('mo_number').AsInteger));//'磨次';
    stringgrid1.Cells[11,i]:=  ifthen(fieldbyname('mo_number').AsInteger = 0,'/',trim(IntToStr(fieldbyname('mo_number').AsInteger)));//'磨次';
...
begin
   self.get_drill2(dm.AQY0025RKEY.Value,StringGrid1,0,edit6); //打开钻孔表
   self.get_drill2(dm.AQY0025RKEY.Value,StringGrid4,1,edit6); //打开钻孔表
   self.get_drill2(dm.AQY0025RKEY.Value,StringGrid5,2,edit6); //打开钻孔表
   edit6.Text:=formatfloat('0.00',(get_mindrill2(StringGrid1)));
end;
 if not target then self.get_target(dm.AQY0025RKEY.Value);    //打开压合靶矩表
 DBEdit36.Text := IntToStr(dm.AQY0025.FieldByName('flod_number').AsInteger);
 cbbZJType.Text := DM.AQY0025.fieldbyname('ZDR_TYPE').AsString;
 edt1.Text:= dm.AQY0025.fieldByName('DrillRemark').AsString;
 FillOldData;
 ...
 begin
   if ListBox1.ItemIndex = 8 then  Notebook1.PageIndex:= 18
...
begin
 ...
 edit6.Text:=formatfloat('0.00',(get_mindrill2(FSg)));
 ...
 try
  if (trim(fsg.Cells[4,i])='Y') and
     (roundto(strtofloat(fsg.Cells[6,i]),-2)<>
      roundto(dr_jishuan(strtofloat(fsg.Cells[3,i]))+strtofloat(edit16.Text),-2)) then
   begin
   ...
   if (trim(fsg.Cells[4,i])='N') and
     (roundto(strtofloat(fsg.Cells[6,i]),-2)<>
      roundto(dr_jishuan(strtofloat(fsg.Cells[3,i]))+strtofloat(edit17.Text),-2)) then
    begin
...
if form_reportselect.ShowModal=mrok then
 begin
  ...
  // if not form_reportselect.CheckBox4.Checked then
   // if not ((form_reportselect.CheckBox4.Checked=False) or (form_reportselect.CheckBox7.Checked=False)or (form_reportselect.CheckBox8.Checked=False))  then
   // form_report.ppSubReport3.Visible:=false;
...
begin
  ...
  //FOldData.DieBan := StrToInt(DBEdit36.Text);
  FOldData.DieBan := StrToInt(DBEdit36.Text);
  FOldData.ZK_Type := cbbZJType.Text;
  FOldData.Remark := edt1.Text;
end;
...
begin
  ...
  if FOldData.DieBan <> StrToInt(DBEdit36.Text) then Result := True;
  if FOldData.ZK_Type <> cbbZJType.Text then Result := True;
  if FOldData.Remark <> edt1.Text then Result := True;
end;
...
begin
  ...
  try
    ...
    tmpS := 'UPDATE Data0025 SET flod_number = ' + DBEdit36.Text + ',' +
    'ZDR_TYPE = ' + QuotedStr(cbbZJType.Text) +
    ',DrillRemark = '+ QuotedStr(edt1.Text) +
...
procedure  TForm2.get_drill2(rkey25:integer;ASg: TStringGrid;AFlowNo: Integer;AEdt: tedit);
var i:integer;
begin
...
begin
 ...
 sql.Text:=' SELECT DG_ADJ_PTH, DG_ADJ_NPTH FROM Data0008'+#13+
            'where rkey='+dm.AQY0025PROD_CODE_PTR.AsString;
 open;
 edit16.Text:=fieldbyname('dg_adj_pth').AsString;
 edit17.Text:=fieldbyname('dg_adj_npth').AsString;
end;
 ...
 begin
  ...
  for i:=1 to RecordCount do
   begin
    ...
    asg.Cells[11,i]:=ifthen(fieldbyname('mo_number').AsInteger = 0,'/',trim(IntToStr(fieldbyname('mo_number').AsInteger)));//'磨次';
    asg.Cells[12,i]:=trim(fieldbyname('drill_number').AsString);//'数量';
    asg.Cells[13,i]:=trim(fieldbyname('REMARK').AsString);//'备注';
    next;
   end;
 end;
 AEdt.Text:=formatfloat('0.00',(get_mindrill2(asg)));
 isopen.Checked:=true;
end;

function TForm2.get_mindrill2(ASg: TStringGrid):single;
var
 min_drill:single;
 i:integer;
begin
