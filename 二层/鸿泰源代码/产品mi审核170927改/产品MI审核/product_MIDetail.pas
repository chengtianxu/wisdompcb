...
uses
  ...
  Grids, DBGrids, Menus, DB, ADODB,math, jpeg;
...
type
  ...
  private
   ...
   ecnopen,impedance,if_post:boolean; //是否需要重新打开ecn清单
...
uses damo, pcb_cutting,common, route_flow, flowpara_edit, deptcode_search,
  ...
  product_search, Excel2000, PCBCUT;
...
begin
 ...
 for i:=2 to stringgrid1.RowCount-3 do
 begin
  try
...
begin
 ...
 begin
  ...
  'UNIT, PANEL_A, PANEL_B,SET_UNMBER, REMARK'+#13+
  'FROM dbo.Data0029'+#13+
  ...
  for i:=1 to RecordCount do
   begin
    ...
    stringgrid1.Cells[7,i]:=trim(fieldbyname('SET_UNMBER').AsString);//'set孔数';
 ...
 begin
  ...
  for i:=1 to RecordCount do
   begin
    strngrd1.Cells[0,i]:=trim(fieldbyname('SEQ_NR').AsString); //'刀具';
    strngrd1.Cells[1,i]:=trim(fieldbyname('MARK').AsString);//'符号';
    strngrd1.Cells[2,i]:=trim(fieldbyname('HOLES_DIA').AsString);//'成品孔径';
    strngrd1.Cells[3,i]:=trim(fieldbyname('PTH').AsString);//'PTH';
    strngrd1.Cells[4,i]:=trim(fieldbyname('TOLERANCE').AsString);//'公差';
    strngrd1.Cells[5,i]:=trim(fieldbyname('DRILL_DIA').AsString);//'钻咀';
    strngrd1.Cells[6,i]:=trim(fieldbyname('UNIT').AsString);//'PCS孔数';
    strngrd1.Cells[7,i]:=trim(fieldbyname('SET_UNMBER').AsString);//'set孔数';
    strngrd1.Cells[8,i]:=trim(fieldbyname('PANEL_A').AsString);//'A板孔数';
    strngrd1.Cells[9,i]:=trim(fieldbyname('PANEL_B').AsString);//'B板孔数';

    strngrd1.Cells[10,i]:=trim(fieldbyname('REMARK').AsString);//'备注';
    next;
   end;
 end;
 ...
 begin
  ...
  for i:=1 to RecordCount do
   begin
    strngrd2.Cells[0,i]:=trim(fieldbyname('SEQ_NR').AsString); //'刀具';
    strngrd2.Cells[1,i]:=trim(fieldbyname('MARK').AsString);//'符号';
    strngrd2.Cells[2,i]:=trim(fieldbyname('HOLES_DIA').AsString);//'成品孔径';
    strngrd2.Cells[3,i]:=trim(fieldbyname('PTH').AsString);//'PTH';
    strngrd2.Cells[4,i]:=trim(fieldbyname('TOLERANCE').AsString);//'公差';
    strngrd2.Cells[5,i]:=trim(fieldbyname('DRILL_DIA').AsString);//'钻咀';
    strngrd2.Cells[6,i]:=trim(fieldbyname('UNIT').AsString);//'PCS孔数';
    strngrd2.Cells[7,i]:=trim(fieldbyname('SET_UNMBER').AsString);//'set孔数';
    strngrd2.Cells[8,i]:=trim(fieldbyname('PANEL_A').AsString);//'A板孔数';
    strngrd2.Cells[9,i]:=trim(fieldbyname('PANEL_B').AsString);//'B板孔数';

    strngrd2.Cells[10,i]:=trim(fieldbyname('REMARK').AsString);//'备注';
    next;
   end;
 end;
...
checkbox2.Checked:=trim(dm.AQY0025current_rev.Value)='Y';
 ...
 begin
 if ecnopen=false then get_ecnlist(dm.AQY0025RKEY.Value); //打开ECN履历表
...
begin
 ...
 stringgrid1.Cells[0,0]:='刀具';
 stringgrid1.Cells[1,0]:='符号';
 stringgrid1.Cells[2,0]:='成品孔径';
 stringgrid1.Cells[3,0]:='PTH';
 stringgrid1.Cells[4,0]:='公差';
 stringgrid1.Cells[5,0]:='钻咀';
 stringgrid1.Cells[6,0]:='PCS孔数';
 stringgrid1.Cells[7,0]:='SET孔数';
 stringgrid1.Cells[8,0]:='A板孔数';
 stringgrid1.Cells[9,0]:='B板孔数';
 stringgrid1.Cells[10,0]:='备注';

 strngrd1.Cells[0,0]:='刀具';
 strngrd1.Cells[1,0]:='符号';
 strngrd1.Cells[2,0]:='成品孔径';
 strngrd1.Cells[3,0]:='PTH';
 strngrd1.Cells[4,0]:='公差';
 strngrd1.Cells[5,0]:='钻咀';
 strngrd1.Cells[6,0]:='PCS孔数';
 strngrd1.Cells[7,0]:='SET孔数';
 strngrd1.Cells[8,0]:='A板孔数';
 strngrd1.Cells[9,0]:='B板孔数';
 strngrd1.Cells[10,0]:='备注';

 strngrd2.Cells[0,0]:='刀具';
 strngrd2.Cells[1,0]:='符号';
 strngrd2.Cells[2,0]:='成品孔径';
 strngrd2.Cells[3,0]:='PTH';
 strngrd2.Cells[4,0]:='公差';
 strngrd2.Cells[5,0]:='钻咀';
 strngrd2.Cells[6,0]:='PCS孔数';
 strngrd2.Cells[7,0]:='SET孔数';
 strngrd2.Cells[8,0]:='A板孔数';
 strngrd2.Cells[9,0]:='B板孔数';
 strngrd2.Cells[10,0]:='备注';
 ...
 begin
  ...
  strngrd2.Options:=strngrd2.Options+[gorowselect];
  ...
  dbedtvcut_B.ReadOnly:= True;
  dbedtvcut_T.ReadOnly:= True;
  dbedtvcut_X.ReadOnly:= True;
  dbedtgf_A.ReadOnly:= True;
  dbedtgf_T.ReadOnly:= True;
  dbedtgf_L.ReadOnly:= True;
 end;
end;
...
try
 ...
 if form_deptcode.ShowModal=mrok then
  begin
   ...
   DM.ADS38.FieldByName('DEPT_CODE').Value := form_deptcode.ADOQuery1DEPT_CODE.Value;
   DM.ADS38.FieldByName('DEPT_NAME').Value := form_deptcode.ADOQuery1DEPT_NAME.Value;
   DM.ADS38.FieldByName('BARCODE_ENTRY_FLAG').Value:= form_deptcode.ADOQuery1BARCODE_ENTRY_FLAG.Value;
     dm.ADS38.Post;
...
begin
 try
  if strngrd1.Cells[6,1] <> '' then
    min_drill:= strtocurrdef(strngrd1.Cells[6,1],0);
 except
   min_drill:=0;
 end;
...
begin
 try
  if strngrd2.Cells[6,1] <> '' then
    min_drill:= StrToCurrDef(strngrd2.Cells[6,1],0);
 except
  min_drill:=0;
 end;
