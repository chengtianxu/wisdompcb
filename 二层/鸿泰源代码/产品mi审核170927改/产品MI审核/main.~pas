 ...
 for i:=1 to dbgrid1.FieldCount do
  begin
   ...
   begin
    ...
    sql.Add('where EMPLOYEE_PTR =' + user_ptr);
    open;
 ...
 if form3.ShowModal=mrok then
  begin
    // MI退回记录
        ...
        dm.qry1.FieldValues['ttype']:='审核退回';
...
begin
 ...
 dm.AQY0025.Open;
 DM.AQY0025.Edit;
 dm.AQY0025AUDITED_DATE.Value := getsystem_date(dm.adoquery1, 0);
 dm.AQY0025AUDITED_BY_PTR.Value := strtoint(user_ptr);
 if DM.ADS25TSTATUS.Value <> DM.AQY0025TSTATUS.Value then
 begin
   ShowMessage('状态发生改变，请刷新后再试！');
   exit;
 end;
 if DM.ADS25TSTATUS.Value <> 4 then
 begin
   ShowMessage('必须待审核状态才可更改！');
   exit;
 end;

   application.CreateForm(tForm2,Form2);
 try
   form2.status.Caption:='0'; //代表编辑
   if form2.ShowModal=mrok then
    begin
        rkey := dm.ADS25rkey.Value;
        dm.ADS25.Close;
