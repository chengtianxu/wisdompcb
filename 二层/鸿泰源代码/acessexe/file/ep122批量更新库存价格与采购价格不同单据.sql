---更新无税价格
--22表和909表
update data0022 set price=a.jsprice
from data0022,
(select data0022.rkey,data0070.po_number,data0071.price,
data0071.tax_price,data0071.tax_2,data0022.price as data0022price, 
((data0071.tax_price/(1+data0071.tax_2*0.01))*(1-data0071.discount/100))/data0071.conversion_factor as jsprice
from data0022 
...
and ((data0071.tax_price/(1+data0071.tax_2*0.01))*(1-data0071.discount/100))/data0071.conversion_factor<>data0022.price) a
...
where 
data0022.source_ptr is not null
--and (data0071.tax_price*(1-data0071.discount/100))/data0071.conversion_factor<>data0022.tax_price
) a
where data0022.rkey=a.rkey
