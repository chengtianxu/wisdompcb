...
type
  ...
  private
   ...
   function Find_Dateerror: boolean;
  public
    { Public declarations }
   v_post:boolean;
   rkey439:Integer;
...
try
 ...
 if form_unitcode.ShowModal=mrok then
  begin
   form5.edit3.Text:=form_unitcode.ADOQuery1.fieldvalues['unit_code'];
   form5.Label8.Caption :=form_unitcode.ADOQuery1.fieldvalues['unit_name'];
   dm.ado0439unit_ptr.value :=form_unitcode.adoquery1.fieldvalues['rkey'];
   dbedit2.SetFocus;
  end
 else
  dbedit2.SetFocus;
...
function TForm5.Find_Dateerror: boolean;
begin
  with DM.ADOQuery1 do
  begin
    Close;
    SQL.Text :='select * from data0064 where rkey in '+self.return_rkey(StringGrid2,0)+
                ' and convert(date,DATE_ASSIGN) > '+ QuotedStr(MaskEdit1.Text);
    
//    ShowMessage(SQL.Text);
    Open;
  end;
  if not DM.ADOQuery1.IsEmpty then    //存在指派日期大于出货日期
    Result := True
  else
    Result := False;
end;
...
begin
  ...
  if Find_Dateerror then
  begin
    MessageDlg('存在大于出货日期的指派日期，请修改',mtinformation,[mbok],0);
    Exit;
  end;
