...
{ TCustomDBEditEh }
  ...
  TCustomDBEditEh = class(TCustomMaskEdit, IComboEditEh, IUnknown)
  private
...
//    function IsCheckedStored: Boolean;
  ...
  protected
    FDataPosting: Boolean;
    FIsPaintControlCreateParams: Boolean;
...
begin
  if ReadOnly
    ...
    else IsReadOnly := not (FDataLink.CanModify or Assigned(OnUpdateData));
...
begin
  ...
  Modified := False;  
  if MRUList.Active and Showing then
    MRUList.Add(Text);
end;
...
begin
  ...
  ResetEditButtonControl(FEditButtonControlList[0], EditButton);
  for i := 1 to EditButtons.Count do
    ResetEditButtonControl(FEditButtonControlList[i], EditButtons[i - 1]);
...
begin
  ...
  begin
    ...
    Invalidate;
    Perform(CM_CANCELMODE, 0, Longint(FInplaceEditHolder));
    with Loc do
...
procedure TCustomDBEditEh.WMKillFocus(var Message: TWMSetFocus);
var i: Integer;
begin
...
begin
  ...
  begin
    C := AnsiUpperCase(EditFormatStr[i])[1];
    if (C in ['D', 'M', 'Y', 'H', 'N', 'S']) and not EscChar then
      PromoteDateTimeChar(C)
    else
    begin
      if CurElement <> #0 then
        PromoteDateTimeChar(#0);
      if (AnsiUpperCase(EditFormatStr[i])[1] = '\') and not EscChar then
      begin
        Dec(EmPos);
        EscChar := True;
      end
      else if (C in ['!','>','<','L','l','A','a','C','c','0','9','#',';']) and
        ( (i = 1) or ((AnsiUpperCase(EditFormatStr[i])[1] <> '\')) )
      then
      begin
...
var
  ...
  ADateTime: TDateTime;
begin
  DecodeDate(0, Y, M, D);  //Y := 1; M := 1; D := 1;
  H := 0; N := 0; S := 0; MS := 0;
  with DateTimeStamp do
  begin
...
begin
  ...
  begin
    C := AnsiUpperCase(EditFormat[i])[1];
    if (C in ['D', 'M', 'Y', 'H', 'N', 'S', '/', ':']) and not EscChar then
    begin
      if InQuote then
      begin
...
begin
  ...
  begin
    ...
    if Value = Null
      then TPopupMonthCalendarEh(DropDownCalendar).Date := Date
      else TPopupMonthCalendarEh(DropDownCalendar).Date := Value;
...
begin
  ...
  begin
    ...
    begin
      AValue := FValue;
      ADate := TPopupMonthCalendarEh(DropDownCalendar).Date;
...
begin
  ...
  inherited KeyPress(Key);
  if IsMasked and (Key in [DateSeparator, TimeSeparator]) then
  begin
    SStart := SelStart + 1;
...
begin
  ...
  for i := Length(Text) downto 1 do
    if Text[i] in [' ',DateSeparator]
      then Dec(p)
      else Break;
  Bot := Copy(Text, 1, p);
  Accept := (AnsiCompareText(Copy(AText, 1, Length(Bot)), Bot) = 0);
end;
...
begin
  ...
  if ((Key = VK_UP) or (Key = VK_DOWN)) and (not WordWrap or (SelLength = Length(Text))) then
    ...
    if EditCanModify and not FListVisible and not ReadOnly then
    begin
  ...
  if (Key = VK_DELETE) and FKeyBased and
    ...
    EditCanModify and not ReadOnly then
    InternalSetValue(Null);
end;
...
begin
  ...
  if S  = ''
    then Result := True
    else Result := IsValidFloat(S, RetValue);
...
begin
  if FCalculatorVisible and (Key in [#13, #27]) then
  begin
    CloseUp(Key = #13);
    Key := #0;
  end;
  ...
  begin
    case Message.Msg of
      wm_KeyDown, wm_SysKeyDown, wm_Char:
        with TWMKey(Message) do
        begin
          if (CharCode = 13) or ((CharCode >= 32) and (CharCode < 127)) then
          begin
...
begin
  ...
  FState := cbUnchecked;
  FDataLink.DataIndependentValue := cbUnchecked;
end;
...
procedure TCustomDBCheckBoxEh.WMKillFocus(var Message: TWMSetFocus);
begin
  inherited;
  Invalidate;
end;
...
procedure TCustomDBCheckBoxEh.WMSize(var Message: TMessage);
begin
  inherited;
  Invalidate;
end;
...
{$IFDEF EH_LIB_7}
  if ThemeServices.ThemesEnabled
    then Message.Result := Perform(CN_CTLCOLORSTATIC, Message.DC,0)
    else inherited;
{$ELSE}
     inherited;
{$ENDIF}
