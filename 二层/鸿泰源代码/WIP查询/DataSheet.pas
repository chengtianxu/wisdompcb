...
begin
  try
    ...
    begin
      ...
      begin
        if SumType = 1 then // 按工序
          sqlstr := ' declare @CalcUnit  int select @CalcUnit =' + IntToStr
            ...
            #13
            + ' case @CalcUnit when 0 then sum((D56.QTY_BACKLOG+D56.TO_BE_STOCKED)) when 1 then sum((D56.PANELS)) when 2 then round(sum((D56.QTY_BACKLOG+D56.TO_BE_STOCKED)*D25.unit_sq),3)  end ' + ' ,sum((D56.QTY_BACKLOG+D56.TO_BE_STOCKED)) as Qty,sum(D56.PANELS) AS PNL_Qty,round(sum((D56.QTY_BACKLOG+D56.TO_BE_STOCKED)*D25.unit_sq),3) as SQM_Qty, ' +
          ...
          sqlstr := ' declare @CalcUnit  int select @CalcUnit =' + IntToStr
            ...
            + #13
            + ' case @CalcUnit when 0 then sum((D56.QTY_BACKLOG+D56.TO_BE_STOCKED)) when 1 then sum((D56.PANELS)) when 2 then round(sum((D56.QTY_BACKLOG+D56.TO_BE_STOCKED)*D25.unit_sq),3)  end '
            +
      ...
      begin
        if SumType = 1 then // 按工序
          sqlstr := ' declare @CalcUnit  int select @CalcUnit =' + IntToStr
            ...
            ' SELECT D34.Seq_Nr,D34.DEPT_CODE,D34.Dept_Name,Qty= ' +
            #13
            + ' case @CalcUnit when 0 then sum((D56.QTY_BACKLOG+D56.TO_BE_STOCKED)) when 1 then sum((D56.PANELS)) when 2 then round(sum((D56.QTY_BACKLOG+D56.TO_BE_STOCKED)*(D06.panel_ln*panel_wd*0.000001/PARTS_PER_PANEL)),3)  end ' + ' ,sum((D56.QTY_BACKLOG+D56.TO_BE_STOCKED)) as Qty,sum(D56.PANELS) AS PNL_Qty,round(sum((D56.QTY_BACKLOG+D56.TO_BE_STOCKED)*(D06.panel_ln*panel_wd*0.000001/PARTS_PER_PANEL)),3) as SQM_Qty, ' +
          ...
          sqlstr := ' declare @CalcUnit  int select @CalcUnit =' + IntToStr
            ...
            ' SELECT D34_Dpt.Seq_Nr, D34_Dpt.Dept_Code,D34_Dpt.Dept_Name,Qty= '
            + #13
            + ' case @CalcUnit when 0 then sum((D56.QTY_BACKLOG+D56.TO_BE_STOCKED)) when 1 then sum((D56.PANELS)) when 2 then round(sum((D56.QTY_BACKLOG+D56.TO_BE_STOCKED)*(D06.panel_ln*panel_wd*0.000001/PARTS_PER_PANEL)),3)  end '
            +
      ...
      if UnitType = 0 then
        DBGridEh1.Columns[2].Title.Caption := '数量(PCS)'
      else if UnitType = 1 then
        DBGridEh1.Columns[2].Title.Caption := '数量(Panel)'
      else if UnitType = 2 then
        DBGridEh1.Columns[2].Title.Caption := '数量(平方米)';
    end;
    result := true;
  except
    on E: Exception do
...
begin
  ...
  begin
    try
      ...
      begin
        FIncludeBorder := rgBB.ItemIndex = 1;
        ...
        begin
          ...
          if ((v_RptType = 0) and (i = 3)) or ((v_RptType = 1) and (i = 7)) then
...
//              .AsString);
      ...
      begin
        ...
        begin
          Qty_ONLine_SQM := Qty_ONLine_SQM + adsDept_Sum.fieldbyName('SQM_Qty')
            .asfloat;
          Qty_Online := Qty_Online + adsDept_Sum.fieldbyName('Qty').asfloat;
          ...
          StringGrid1.Cells[FixedCol + aqDept_List.RecNo - 1, j + 2] := format
            ('%8.1f', [adsDept_Sum.fieldbyName('SQM_Qty').asfloat]);
        end;
        adsDept_Sum.next;
      end;
      ...
      begin
        ...
        StringGrid1.Cells[19 + abbr, j + 2] := format('%.1f', [Qty_ONLine_SQM]);
      ...
      begin
        ...
        StringGrid1.Cells[13, j + 2] := format('%.1f', [Qty_ONLine_SQM]);
      end;
    end;
  end;
  ...
  begin
    ...
    begin
      while not Eof do
      begin
        BarSeries1.Add(dmcon.adsDept_Sum.fieldbyName('Qty').asfloat,
