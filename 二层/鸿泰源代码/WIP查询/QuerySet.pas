...
begin
  ...
  v_Field_List_CN := VarArrayOf(['客户代码','本厂编号','客户型号','批量类型','产品类别','投产工厂代码','加工方式','计划完工期','工序','投产类型','在线状态','当前工厂','关联原客户','测试成型备注','产品阶数','产品类型','产品组别']);
  v_Field_List_EN := VarArrayOf(['D10.Cust_Code','d25.MANU_PART_NUMBER','d25.Manu_Part_Desc','D25.TType','IsNULL(D25.Parent_ptr,0)','d492.whouse_ptr','d60.so_tp','d492.SCH_COMPL_DATE','d34.Seq_Nr','d492.ttype','d56.wtype','d56.loc_ptr','d25.ORIG_CUSTOMER','d25.SAMPLE_NR','d25.CPJS','d25.PROD_CODE_PTR','d08.PR_GRP_POINTER']);
...
begin
  edtValue.text:='';
  Button3.Enabled:=listbox3.ItemIndex in [3,4,7,9,10];
  BitBtn6.visible :=listbox3.ItemIndex in [0,5,8,11,12,14,15,16];
  edtValue.Visible := (listbox3.ItemIndex <3) or (listbox3.ItemIndex = 5) or (listbox3.ItemIndex = 11) or (listbox3.ItemIndex = 8) or (listbox3.ItemIndex = 12) or (ListBox3.ItemIndex = 13) or (ListBox3.ItemIndex in [14,15,16]);
  rgType.Visible := not edtValue.Visible and (listbox3.ItemIndex <> 6) and (listbox3.ItemIndex <> 7);
