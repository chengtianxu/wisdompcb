...
begin
  ...
  begin
    ...
    begin            //   如果在复制的数据中有有效期小于 aq22的记录 ，给予提示
      if (FieldByName('EXPIRE_DATE').AsDateTime<DM.aq22.FieldByName('EXPIRE_DATE').AsDateTime) and  (FieldByName('EXPIRE_DATE').AsDateTime>0) then
      begin
  ...
  if (dm.aq22EXPIRE_DATE.Value >= common.getsystem_date(dm.ADOQuery1,1) )  or (dm.aq22EXPIRE_DATE.AsDateTime=0) then
  begin
