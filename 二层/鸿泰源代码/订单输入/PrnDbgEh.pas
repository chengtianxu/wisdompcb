...
uses PrnDGDlg, RichEdit, ComStrs, EhLibConsts;
...
type TColCellParamsEhCracker = class(TColCellParamsEh) end;
...
{  function ComparePolyBookmark(var APBm1,APBm2:TPolyBookmark):Boolean;
            ...
            begin
              case PrnColumns[i].GetColumnType of
                ...
                begin
                  ...
                  FCol := -1;
                  FState := [];
                  FFont := VPrinter_Canvas.Font;
                  FBackground := clWhite;
                  FAlignment := taLeftJustify;
                  FImageIndex := -1;
                  FText := PrnColumns[i].DisplayText;
                  FCheckboxState := PrnColumns[i].CheckboxState;

                  if Assigned(DBGridEh.OnGetCellParams) then
                    DBGridEh.OnGetCellParams(DBGridEh, PrnColumns[i], Font, FBackground, []);
  ...
  begin
    Result := 0;
    for i := 0 to PrnColWidths.Count - 1 do
    begin
...
{$ENDIF}
  ...
  begin
    ...
    begin // data cell
      ...
      begin
        FFont := VPrinter_Canvas.Font;
        FBackground := NewBackgrnd;
        if Assigned(DBGridEh.OnGetCellParams) then
          DBGridEh.OnGetCellParams(DBGridEh, CurColumn, FFont, FBackground, FState);
      end;
      ...
      begin
        ...
        begin
          Result := WriteTextEh(VPrinter_Canvas, ARect, False, 0, 0, FColCellParamsEh.Text,
            FColCellParamsEh.Alignment, tlTop, Stretch, CurColumn.EndEllipsis, 0, 0,
    ...
    begin // footer cell
      ...
      Result := WriteTextEh(VPrinter_Canvas, ARect, False, 0, 0, Value,
        AAlignment, tlTop, Stretch, Footer.EndEllipsis, 0, 0,
        CalcRect or CalcInfoMode, DBGridEh.UseRightToLeftAlignment, DBGridEh.UseRightToLeftReading) + PSY2 * 2;
    end;
  end;
...
//      PrintPageColontitle(FooterRect,False,False);
      ...
      begin
        ...
        begin
          ...
          DrawCell(i, ARect1, CurPBm, False);
  ...
  begin
    ...
    if (htLast.Column <> nil) and (htLast.Column.Title.Orientation = tohVertical) then
      ...
      th := DrawText(VPrinter_Canvas.Handle, PChar(s),
