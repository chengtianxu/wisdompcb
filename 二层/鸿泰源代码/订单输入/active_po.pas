...
procedure TForm10.StringGrid1MouseDown(Sender: TObject;
...
procedure TForm10.StringGrid2MouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var
  column,row:longint;
begin
...
procedure TForm10.PopupMenu2Popup(Sender: TObject);
begin
  ...
  begin
   ...
   with dm.ADOQuery2 do
    begin
     ...
     sql.Add('and status=2 and PROD_REL=''1''');
 ...
 for i:=1 to stringgrid2.RowCount-2 do
  begin
    ...
    begin
     ...
     edit_note.Caption:='从暂缓中释放:'+stringgrid2.Cells[0,2];
      with dm.AQ9711 do
       begin
        if active=true then active:=false;
        sql.Clear;
        sql.Add('select RemarkSO from data0060 where  Rkey='+dm.ADOQuery2.fieldbyname('RKEY').AsString);
        active:=true;
     ...
     if edit_note.ShowModal=mrok then
      begin
       if (not dm.AQ9711.IsEmpty) and (trim(edit_note.Memo1.Text)<>'') then
        begin             //原记录有记事本而且修改了
         dm.AQ9711.Edit;
         dm.AQ9711.FieldValues['RemarkSO']:=edit_note.Memo1.Text;
         dm.AQ9711.Post;
//        end
//       else
