object frmViewCredit: TfrmViewCredit
  Left = 28
  Top = 152
  Width = 997
  Height = 480
  Caption = #23458#25143#20449#29992#39069#24230#26597#35810
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poMainFormCenter
  WindowState = wsMaximized
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 989
    Height = 453
    Align = alClient
    BevelOuter = bvLowered
    TabOrder = 0
    object Panel2: TPanel
      Left = 1
      Top = 1
      Width = 987
      Height = 42
      Align = alTop
      TabOrder = 0
      object spBtnExport: TSpeedButton
        Left = 270
        Top = 6
        Width = 70
        Height = 27
        Caption = #23548#20986
        Glyph.Data = {
          
          70DA0FFFF0ADA0EEEE0D00000ADADA00000AADADADADADADADAD}
        ParentShowHint = False
        ShowHint = False
        OnClick = spBtnExportClick
      end
      object spBtnQuery: TSpeedButton
        Left = 183
        Top = 6
        Width = 70
        Height = 27
        Caption = #26597#35810
        Glyph.Data = {
          
          861D8680775398535355}
        ParentShowHint = False
        ShowHint = False
        OnClick = spBtnQueryClick
      end
      object Label5: TLabel
        Left = 16
        Top = 14
        Width = 45
        Height = 13
        Caption = #23458'   '#25143#65306
      end
      object SpeedButton4: TSpeedButton
        Left = 148
        Top = 9
        Width = 23
        Height = 22
        Glyph.Data = {
          
          3333333333373733333333333333303333333333333373333333}
        NumGlyphs = 2
        OnClick = SpeedButton4Click
      end
      object edtCustId: TEdit
        Left = 66
        Top = 10
        Width = 76
        Height = 21
        TabOrder = 0
        OnKeyPress = edtCustIdKeyPress
      end
      object Panel4: TPanel
        Left = 914
        Top = 1
        Width = 72
        Height = 40
        Align = alRight
        BevelOuter = bvLowered
        TabOrder = 1
        object spBtnExit: TSpeedButton
          Left = 1
          Top = 1
          Width = 70
          Height = 38
          Caption = #36864#20986
          Glyph.Data = {
            
            0000}
          NumGlyphs = 2
          ParentShowHint = False
          ShowHint = False
          OnClick = spBtnExitClick
        end
      end
    end
    object Panel3: TPanel
      Left = 1
      Top = 43
      Width = 987
      Height = 409
      Align = alClient
      TabOrder = 1
      object DBGridEh1: TDBGridEh
        Left = 1
        Top = 1
        Width = 985
        Height = 407
        Align = alClient
        DataSource = DataSource1
        FooterColor = clWindow
        FooterFont.Charset = DEFAULT_CHARSET
        FooterFont.Color = clWindowText
        FooterFont.Height = -11
        FooterFont.Name = 'MS Sans Serif'
        FooterFont.Style = []
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = []
        OnDrawColumnCell = DBGridEh1DrawColumnCell
        OnKeyDown = DBGridEh1KeyDown
        OnTitleBtnClick = DBGridEh1TitleBtnClick
        Columns = <
          item
            EditButtons = <>
            FieldName = 'CUST_CODE'
            Footers = <>
            Title.Alignment = taCenter
            Title.Caption = #23458#25143#20195#30721
            Title.TitleButton = True
            Width = 55
          end
          item
            EditButtons = <>
            FieldName = 'ABBR_NAME'
            Footers = <>
            Title.Alignment = taCenter
            Title.Caption = #23458#25143#31616#31216
            Title.TitleButton = True
          end
          item
            EditButtons = <>
            FieldName = 'SALES_REP_NAME'
            Footers = <>
            Title.Alignment = taCenter
            Title.Caption = #19994#21153#21592
            Title.TitleButton = True
            Width = 56
          end
          item
            EditButtons = <>
            FieldName = 'FED_TAX_ID_NO'
            Footers = <>
            Title.Alignment = taCenter
            Title.Caption = #20184#27454#26041#24335
            Title.TitleButton = True
            Width = 108
          end
          item
            EditButtons = <>
            FieldName = 'TCREDITRATING'
            Footers = <>
            Title.Alignment = taCenter
            Title.Caption = #20449#29992#31561#32423
            Title.TitleButton = True
            Width = 52
          end
          item
            EditButtons = <>
            FieldName = 'CREDIT_LIMIT'
            Footers = <>
            Title.Alignment = taCenter
            Title.Caption = #20449#29992#39069#24230
            Title.TitleButton = True
            Width = 80
          end
          item
            EditButtons = <>
            FieldName = 'HIGH_BALANCE'
            Footers = <>
            Title.Alignment = taCenter
            Title.Caption = #30003#35831#20445#38505#37329#39069
            Title.TitleButton = True
            Width = 76
          end
          item
            EditButtons = <>
            FieldName = 'BALANCE_DUE'
            Footers = <>
            Title.Alignment = taCenter
            Title.Caption = #25209#20934#20445#38505#37329#39069
            Title.TitleButton = True
            Width = 76
          end
          item
            EditButtons = <>
            FieldName = 'AMOUNT1'
            Footers = <>
            Title.Alignment = taCenter
            Title.Caption = #24050#24320#31080#26410#25910#27454#37329#39069
            Title.TitleButton = True
            Width = 100
          end
          item
            EditButtons = <>
            FieldName = 'AMOUNT2'
            Footers = <>
            Title.Alignment = taCenter
            Title.Caption = #24050#25351#27966#26410#29983#25104#36134#40836#37329#39069
            Title.TitleButton = True
            Width = 120
          end
          item
            EditButtons = <>
            FieldName = 'AMOUNT3'
            Footers = <>
            Title.Alignment = taCenter
            Title.Caption = #26410#25351#27966#37329#39069
            Title.TitleButton = True
            Width = 80
          end
          item
            EditButtons = <>
            FieldName = 'AMOUNT4'
            Footers = <>
            Title.Alignment = taCenter
            Title.Caption = #26410#24320#31080#24037#20855#36153#29992
            Title.TitleButton = True
            Width = 90
          end
          item
            EditButtons = <>
            FieldName = 'AMOUNT5'
            Footers = <>
            Title.Alignment = taCenter
            Title.Caption = #36151#39033#22791#24536
            Title.TitleButton = True
            Width = 80
          end
          item
            EditButtons = <>
            FieldName = 'AMOUNT6'
            Footers = <>
            Title.Alignment = taCenter
            Title.Caption = #24050#36864#36135#26410#29983#25104#36151#39033#22791#24536
            Title.TitleButton = True
            Width = 120
          end
          item
            EditButtons = <>
            FieldName = 'AMOUNT'
            Footers = <>
            Title.Alignment = taCenter
            Title.Caption = #26410#32467#31639#24635#37329#39069
            Title.TitleButton = True
            Width = 100
          end>
      end
    end
  end
  object DataSource1: TDataSource
    DataSet = adsCalcCREDIT
    Left = 537
    Top = 137
  end
  object adsCalcCREDIT: TADODataSet
    Connection = dm.ADOConnection1
    CursorType = ctStatic
    CommandText = 'Exec Calc_CREDIT :CustId , :RKEY73 , :ViewAll'
    Parameters = <
      item
        Name = 'CustId'
        Attributes = [paNullable]
        DataType = ftString
        NumericScale = 255
        Precision = 255
        Size = 10
        Value = Null
      end
      item
        Name = 'RKEY73'
        Attributes = [paSigned, paNullable]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end
      item
        Name = 'ViewAll'
        Attributes = [paNullable]
        DataType = ftWord
        Precision = 3
        Size = 1
        Value = Null
      end>
    Left = 537
    Top = 91
    object adsCalcCREDITRKEY: TIntegerField
      FieldName = 'RKEY'
    end
    object adsCalcCREDITCUST_CODE: TStringField
      FieldName = 'CUST_CODE'
      Size = 10
    end
    object adsCalcCREDITABBR_NAME: TStringField
      FieldName = 'ABBR_NAME'
      Size = 10
    end
    object adsCalcCREDITSALES_REP_NAME: TStringField
      FieldName = 'SALES_REP_NAME'
    end
    object adsCalcCREDITFED_TAX_ID_NO: TStringField
      FieldName = 'FED_TAX_ID_NO'
      Size = 50
    end
    object adsCalcCREDITTCREDITRATING: TStringField
      FieldName = 'TCREDITRATING'
      FixedChar = True
      Size = 2
    end
    object adsCalcCREDITCREDIT_LIMIT: TFloatField
      FieldName = 'CREDIT_LIMIT'
    end
    object adsCalcCREDITHIGH_BALANCE: TFloatField
      FieldName = 'HIGH_BALANCE'
    end
    object adsCalcCREDITBALANCE_DUE: TFloatField
      FieldName = 'BALANCE_DUE'
    end
    object adsCalcCREDITAMOUNT1: TBCDField
      FieldName = 'AMOUNT1'
      Precision = 18
      Size = 3
    end
    object adsCalcCREDITAMOUNT2: TBCDField
      FieldName = 'AMOUNT2'
      Precision = 18
      Size = 3
    end
    object adsCalcCREDITAMOUNT3: TBCDField
      FieldName = 'AMOUNT3'
      Precision = 18
      Size = 3
    end
    object adsCalcCREDITAMOUNT4: TBCDField
      FieldName = 'AMOUNT4'
      Precision = 18
      Size = 3
    end
    object adsCalcCREDITAMOUNT5: TBCDField
      FieldName = 'AMOUNT5'
      Precision = 18
      Size = 3
    end
    object adsCalcCREDITAMOUNT6: TBCDField
      FieldName = 'AMOUNT6'
      Precision = 18
      Size = 3
    end
    object adsCalcCREDITAMOUNT: TBCDField
      FieldName = 'AMOUNT'
      ReadOnly = True
      Precision = 23
      Size = 3
    end
  end
end
