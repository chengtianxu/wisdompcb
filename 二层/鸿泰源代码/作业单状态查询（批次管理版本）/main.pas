...
type
  TfrmMain = class(TForm)
    ...
    Edit11: TEdit;
    Label12: TLabel;
    adsWo_ListABBR_NAME: TStringField;
    adsWo_ListSALES_ORDER: TStringField;
    adsWo_ListPARTS_ORDERED: TFloatField;
    adsWo_ListPARTS_SHIPPED: TFloatField;
    adsWo_Listqianjiao_qty: TFloatField;
    adsWo_ListQTY_REJECT: TIntegerField;
    adsWo_ListISSUED_QTY: TIntegerField;
    N16: TMenuItem;
...
uses QuerySet,common,MyClass,WO_INFO,Pick_Item_Single,ConstVar, Dlg_WO_Split, uSumQuery,
  ManuPart_WIP, PasFieldShow, ComplainFrm;
{$R *.dfm}
...
var tmpstr,sFilter,Status_filter,Lmore_filter:string;
...
//      Calc_Qty;
  ...
  begin
    Lmore_filter := Lmore_filter + '';
  end
  else
  begin
    Lmore_filter := Lmore_filter + ' and WORK_ORDER_NUMBER like ''%' + Trim(Edit5.Text) +'%''';
  end;
  ...
  begin
    Lmore_filter := Lmore_filter + ' and CUST_CODE like ''%' + Trim(Edit6.Text)+'%''';
  end;
  ...
  begin
    Lmore_filter := Lmore_filter + ' and MANU_PART_NUMBER like ''%' + Trim(Edit7.Text)+'%''';
  end;
  ...
  begin
    Lmore_filter := Lmore_filter + ' and ABBR_NAME like ''%' + Trim(Edit8.Text)+'%''';
  end;
  ...
  begin
    Lmore_filter := Lmore_filter + ' and MANU_PART_DESC like ''%' + Trim(Edit9.Text)+'%''';
  end;
  ...
  begin
    Lmore_filter := Lmore_filter + ' and SALES_ORDER like ''%' + Trim(Edit10.Text)+'%''';
  end;
  ...
  begin
    Lmore_filter := Lmore_filter + ' and PRODUCT_NAME like ''%' + Trim(Edit11.Text)+'%''';
  end;
  ...
  begin
    Lmore_filter := Lmore_filter + ' and DEPT_CODE like ''%' + Trim(Edit12.Text)+'%''';
  end;

  if (tmpstr='') and (Status_filter='') then
  begin
    if Lmore_filter<>'' then
      DATASET.Filter := Stringreplace(Lmore_filter,' and ','',[])
    else
      DATASET.Filter := '';
    DATASET.Filtered:= true;
//     DATASET.Filter := Status_filter + Lmore_filter;
  end
  else
  begin
    ...
    begin
      DATASET.Filter := vColumn.Field.FieldName+' Like '+ QuotedStr(sFilter)+Status_filter + Lmore_filter;
    end;
  end;
...
begin
  try
    ...
    while not adsWo_List.eof do begin
      ...
      Qty_pcs:=Qty_pcs+adsWo_List.fieldByName('QTY_BACKLOG').AsInteger;
          ...
          begin
            ...
            begin
              ...
              tmpstr2:= tmpstr2 + 'insert into data0849(D419_ptr, D073_ptr, FieldName, FieldCaption, FieldOrder, FieldWidth, IsDisplay,formname,GridName)' +
                                      ...
                                      QuotedStr(dbGrd.Columns[i].Title.Caption)+','+ IntToStr(dbGrd.Columns.Count - addInt)+','+
...
begin
  ...
  try
    ...
    if frmPick_Item_Single.ShowModal=mrok then
    begin
      Edit12.Text:=frmPick_Item_Single.adsPick.fieldbyname('Dept_Code').AsString;
    end;
...
begin
  Frm_Complain := TFrm_Complain.Create(Self);
  try
    Frm_Complain.ADODataSet1.Close;
    Frm_Complain.ADODataSet1.CommandText := Frm_Complain.ADODataSet1.CommandText +
    ' and Data0025.MANU_PART_NUMBER = '+ QuotedStr(adsWo_ListMANU_PART_NUMBER.AsString);
    Frm_Complain.ADODataSet1.Open;
    if Frm_Complain.ShowModal = mrok then
  finally
    Frm_Complain.Free;
  end;
end;
