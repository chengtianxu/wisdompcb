...
begin
  ...
  try
    ...
    case ListBox3.ItemIndex of
       ...
       8:begin
           if FType=1 then edtValue.Text:=frmPick_Item_Single.adsPick.fieldbyname('Dept_Code').AsString;
         end;
    end;
  finally
...
begin
 ...
 else if ListBox3.ItemIndex=8 then
 begin
   if FType=1 then
     sgrid1.Cells[2,sgrid1.RowCount-1]:=' and '+v_Field_List_EN[ListBox3.ItemIndex]+' LIKE ''%'+edtValue.Text+'%'''
   else
     sgrid1.Cells[2,sgrid1.RowCount-1]:=' AND EXISTS(SELECT 1 FROM Data0489 D489 WHERE D06.RKEY=D489.WO_PTR)';
 end
 else
   sgrid1.Cells[2,sgrid1.RowCount-1]:=' and '+v_Field_List_EN[ListBox3.ItemIndex]+' LIKE ''%'+edtValue.Text+'%''';
...
begin
  if FType=1 then
    ...
    v_Field_List_CN := VarArrayOf(['作业单号','本厂编号','客户型号','产品类型','配料单号','订单编号','客户编号','投产工厂','内层已分配']);
  if FType=1 then
    ...
    v_Field_List_EN := VarArrayOf(['d06.work_order_number','d25.MANU_PART_NUMBER','d25.Manu_Part_Desc','D08.Prod_Code','D06.CUT_NO','D492.SO_NO','D10.CUST_Code','D492.whouse_ptr','']);
...
begin
  ...
  begin
    BitBtn6.visible := listbox3.ItemIndex in [3,6,7];
