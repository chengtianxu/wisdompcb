...
//                     +'join data0006 on data0056.WO_ptr=data0006.rkey '
   ...
   // ShowMessage(SQL.Text);
    ...
    Close;
    SQL.Clear;
    SQL.Text:='select data0056.DEPT_PTR,* from data0056'+#13+
              'inner join data0006 on data0056.WO_PTR=data0006.RKEY'+#13+
              'inner join data0025 on data0006.BOM_PTR=data0025.RKEY'+#13+
              'inner join data0038 on data0025.RKEY=data0038.SOURCE_PTR and data0056.DEPT_PTR=data0038.DEPT_PTR'+#13+
              'where data0006.WORK_ORDER_NUMBER='+QuotedStr(Dmcon.adsWOCtrlList.FieldByName('WORK_ORDER_NUMBER').AsString);
    Open;
    if  RecordCount<=0 then
    begin
      MessageBox(Self.Handle,PChar('该作业单当前工序不在产品工艺流程中,请到作业单状态查询(更新步骤号)中操作！'),'警告',MB_ok+MB_iconStop);
      Exit;
    end;
  end;
  ModalResult:=mrOk;
