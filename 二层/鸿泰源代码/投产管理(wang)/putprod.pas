...
//        if ((DM.ADS60so_oldnew.AsString='新单') and (DM.ADS60ttype.AsInteger=0) and (dm.ads60ANCESTOR_PTR.AsInteger=0)) then //新单且量产且手工创建的自动增加一条记录到工具申请表585
        ...
        begin
          ...
          dm.qrytmp.SQL.Text:='select data0494.PARAMETER_VALUE from data0494 inner join data0278 on data0494.PARAMETER_PTR=data0278.Rkey inner join'
                              ...
                              +' where data0278.CATEGORY_PTR=3 and data0278.PARAMETER_DESC like ''%共用测试架%'' and data0025.Rkey='+dm.aqTmp.FieldBYName('rkey25').AsString+' and data0494.step_number='+dm.aqTmp.FieldBYName('STEP_NUMBER').AsString;
          dm.qrytmp.Open;
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                dm.aqTmp.Parameters.ParamByName('Generate_Date').Value:=getsystem_date(DM.qrytmp,0);//长时间格式
                dm.aqTmp.Parameters.ParamByName('PlusOpen').Value:=0;
                dm.aqTmp.Parameters.ParamByName('OrderDate').Value:=getsystem_date(DM.qrytmp,0);//长时间格式
                dm.aqTmp.Parameters.ParamByName('ClaimOutDate').Value:=IncDay(getsystem_date(DM.qrytmp,0),iday);//长时间格式
                dm.aqTmp.ExecSQL;
              end;
            end;
          end else
