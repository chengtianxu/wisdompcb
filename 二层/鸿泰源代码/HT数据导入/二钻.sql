...
select MAX(SEQ_NO) seq_no,SOURCE_PTR  into #12 from [192.168.1.253].sj_v20_live.dbo.Data0029 
group by SOURCE_PTR 
...
select  s1+1 as s2,SOURCE_PTR,'¶þ×ê' as REmark  into #15 from #14 
...
PANEL_A,PANEL_B,PTH,REMARK,SEQ_NO,SEQ_NR,PANEL_D into #16 from [192.168.1.253].sj_v20_live.dbo.Data0029
 where SOURCE_PTR in (select SOURCE_PTR from #12 ) and flow_no=1 
...
PANEL_A,PANEL_B,PTH,REMARK,SEQ_NO,SEQ_NR,PANEL_D )
select SOURCE_PTR,UNIT,MARK,HOLES_DIA,TOLERANCE,DRILL_DIA,
PANEL_A,PANEL_B,PTH,REMARK,SEQ_NO,SEQ_NR,PANEL_D  from #17 where source_ptr in (select rkey from Data0025 )


select * from Data0025 where RKEY not in (select source_ptr  from #17)

select distnict source_ptr  from #17 where source_ptr not in (select rkey from Data0025 )


select * from Data0025 where RKEY =3443