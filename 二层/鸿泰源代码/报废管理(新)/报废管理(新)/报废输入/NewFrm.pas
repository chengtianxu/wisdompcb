...
uses
  ...
  Dialogs, StdCtrls, DBCtrls, ExtCtrls, DB, DBClient, Buttons,ADODB;
      ...
      begin
        ...
        if FType06 = 0 then
          ...
          LInPut.Sqlstr := 'SELECT D06.RKey,D06.WORK_ORDER_NUMBER, D25.MANU_PART_NUMBER,D25.MANU_PART_DESC, (CASE D56.WTYPE WHEN 0 THEN '''' ELSE ''返工'' END) AS FGZ,' +
                    ...
                    ' INNER JOIN Data0034 D34 ON D56.DEPT_PTR=D34.RKey' +
                    ' INNER JOIN Data0015 D15 ON D56.LOC_PTR=D15.RKey' +
                    ' WHERE D06.PROD_STATUS in (3,5,6,103) and D06.wtype <> 2 and D06.wtype <> 4 and D25.IsSpell<1 and d25.PARENT_PTR is not null' ;
...
begin
     qry1.Close;
    qry1.Parameters[0].Value:= Trim(edtZYDH.Text);
    qry1.Open;
    qty:=StrToInt(edtPCS.Text);
    sp:=     qry1.FieldByName('unit_sq').AsFloat * qty;
...
begin
  ...
  dm1.qrytmp.Close;
  dm1.qrytmp.SQL.Clear;
  dm1.qrytmp.SQL.Add(' SELECT D06.RKey AS RKey06,D15.RKey FROM Data0006 D06 INNER JOIN Data0056 D56 ON D06.RKey=D56.WO_PTR ');
...
begin
  ...
  dm1.qrytmp.Close;
  dm1.qrytmp.SQL.Clear;
  dm1.qrytmp.SQL.Add(' SELECT RKey, DEPT_NAME FROM Data0034 WHERE DEPT_CODE = ' + QuotedStr(edtFSGX.Text));
  dm1.qrytmp.Open;
  if dm1.qrytmp.IsEmpty then
  begin
    Result := False;
