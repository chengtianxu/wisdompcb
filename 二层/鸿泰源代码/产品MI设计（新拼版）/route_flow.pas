    ...
    begin
     ...
     begin
      if fieldbyname('ppc_status').AsInteger=0 then
       ...
       dm.ADS38.Fields[1].Value := fieldvalues['dept_ptr'];
       dm.ADS38.Fields[2].Value := fieldvalues['DEF_ROUT_INST'];
       dm.ADS38.Fields[3].Value := step_number;
       dm.ADS38.Fields[4].Value := rev;
       dm.ADS38.Fields[5].Value := 1;
       dm.ADS38.Fields[0].Value := dm.AQY0025RKEY.Value;
       dm.ADS38.Post;

//
//      dm.ADS38.AppendRecord([,
...
'SELECT TOP 100 PERCENT PARAMETER_PTR, TValue, Doc_Flag, after_flag, formula,'+#13+
      ...
      begin
        DM.ads494.Append;
        dm.ads494.Fields[0].Value :=  dm.ADOQuery1.FieldValues['PARAMETER_PTR'];
        dm.ads494.Fields[1].Value :=                      cacl_flowpara(dm.ADOQuery1.Fieldbyname('TValue').AsString,
                             trim(dm.ADOQuery1.Fieldbyname('formula').AsString),
                                  dm.ADOQuery1.FieldValues['PARAMETER_PTR']);

        dm.ads494.Fields[3].Value :=  dm.ADS38STEP_NUMBER.Value;
        dm.ads494.Fields[4].Value :=  dm.ADOQuery1.FieldValues['SEQ_NO'];
        dm.ads494.Fields[5].Value :=  dm.ADOQuery1.FieldValues['Doc_Flag'];
        dm.ads494.Fields[6].Value :=  dm.ADOQuery1.FieldValues['after_flag'];
        dm.ads494.Fields[7].Value :=  dm.ADOQuery1.FieldValues['flow_spfc_flag'];
        dm.ads494.Fields[8].Value :=  DM.ADOQuery1.FieldValues['outp_spfc_flag'];
        dm.ads494.Fields[2].Value :=  dm.AQY0025RKEY.Value;
        DM.ads494.Post;
...
form_iesmodel:=tform_iesmodel.Create(application);
 Form_iesmodel.MaskEdit1.Text:= Edit1.Text;
...
begin
 ...
 begin
  ...
  begin
   ...
   begin
     DM.ADS279_0.Append;
     DM.ads279_0.Fields[1].AsString := dm.ADOQuery1.fieldbyname('PARAMETER_PTR').AsString;
     DM.ads279_0.Fields[4].AsInteger := dm.ADOQuery1.RecNo;
     DM.ads279_0.Fields[2].AsString := '';
     DM.ads279_0.Fields[3].AsInteger := 0;
     DM.ads279_0.Fields[0].AsString := dm.AQY0025RKEY.AsString;
     DM.ADS279_0.Post;
//    dm.ADS279_0.AppendRecord([,
//                              fieldvalues[''],
//                              '',
//                              0,       //代表流程判断参数1代表全局参数
//                              RecNo]);
