...
begin
  ...
  if Printer.Printers.Count > 0
    then FMetafileCanvas := TMetafileCanvas.Create(
      FMetafileList[FMetafileList.Count - 1], Printer.Handle {0})
  else FMetafileCanvas := TMetafileCanvas.Create(
      FMetafileList[FMetafileList.Count - 1], 0);
...
begin
  ...
  if Printer.Printers.Count > 0 then
    FMetafileCanvas := TMetafileCanvas.Create(FMetafileList[FMetafileList.Count - 1], Printer.Handle {0})
  else
    FMetafileCanvas := TMetafileCanvas.Create(FMetafileList[FMetafileList.Count - 1], 0);
  FontSize := FMetafileCanvas.Font.Size;
  if Printer.Printers.Count > 0 then
  begin
    FMetafileCanvas.Font.PixelsPerInch := GetDeviceCaps(Printer.Handle, LOGPIXELSX);
...
begin
  Canvas.Draw(0, 0, FMetafileList[PageNumber - 1]);
end;
...
procedure TPrinterPreview.GetPrinter(ADevice, ADriver, APort: PChar;
  var ADeviceMode: THandle);
begin
  Printer.GetPrinter(ADevice, ADriver, APort, ADeviceMode);
end;

procedure TPrinterPreview.SetPrinter(ADevice, ADriver, APort: PChar;
  ADeviceMode: THandle);
begin
