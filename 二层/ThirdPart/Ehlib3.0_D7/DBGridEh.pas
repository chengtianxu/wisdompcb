...
{      J := Column.DefaultWidth;
    ...
    if (FActivelist = FDropDownList) then
    begin
        FDropDownList.Color := Color;
        FDropDownList.Font := Font;
        if Assigned(Column.DropDownBox.ListSource) and Assigned(Column.DropDownBox.ListSource.DataSet) then
        begin
          FDropDownList.Items.BeginUpdate;
          FDropDownList.Items.Clear;
          Column.DropDownBox.ListSource.DataSet.First;
          while not Column.DropDownBox.ListSource.DataSet.Eof do
          begin
            FDropDownList.Items.Add(Column.DropDownBox.ListSource.DataSet.fieldbyname(Column.DropDownBox.Columns[0].FieldName).AsString);
            Column.DropDownBox.ListSource.DataSet.Next;
          end;
          FDropDownList.Items.EndUpdate;
        end;
        if FDropDownList.Items.Count >= Integer(Column.DropDownRows) then
          FDropDownList.Height := Integer(Column.DropDownRows) * FDropDownList.ItemHeight + 4
        else
          FDropDownList.Height := FDropDownList.Items.Count * FDropDownList.ItemHeight + 4;
        if Column.Field.IsNull then
          FDropDownList.ItemIndex := -1
        else
          FDropDownList.ItemIndex := FDropDownList.Items.IndexOf({dddColumn.Field.Value} Text);
        J := FDropDownList.ClientWidth;
        for I := 0 to FDropDownList.Items.Count - 1 do
        begin
          Y := FDropDownList.Canvas.TextWidth(FDropDownList.Items[I]);
          if Y > J then J := Y;
        end;
        FDropDownList.ClientWidth := J + 4;
    end else
    begin
...
begin
  ...
  begin
    ...
    if FActiveList = FDropDownList then
    begin
      if FDropDownList.ItemIndex <> - 1 then
        ListValue := FDropDownList.Items[FDropDownList.ItemIndex];
    end 
    else if FPickList = FActiveList then
    begin
    ...
    begin
      ...
      if (not VarIsNull(ListValue)) and Grid.CanEditModifyText and (FActiveList = FDropDownList) then
      begin
        with Grid, CurColumn.Field do
        begin
          if FDropDownList.Items.IndexOf(ListValue) <> - 1 then
          begin
            Self.Text := ListValue;
      ...
      esDropDown:
        begin
          ...
          begin
            ...
            begin
              FDropDownList := TPopupListboxEh.Create(Self);
              FDropDownList.Visible := False;
              FDropDownList.Parent := Self;
              TWinControlCracker(FDropDownList).OnMouseUp := ListMouseUp;
      //            FPickList.IntegralHeight := True;
              FDropDownList.ItemHeight := FDropDownList.GetTextHeight;
              FDropDownList.Ctl3D := True;
            end;
            FActiveList := FDropDownList;
          end;
        end;
