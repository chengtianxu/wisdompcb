...
type
  ...
  private
    _CostPTR:integer;
    function New714():boolean;
    function Edit714():boolean;
    procedure Get714();
...
//新增
function TfrmKFKH.New714():boolean;
var
 vrkey:integer;
begin
  dm.AQery.Close ;
  dm.AQery.SQL.Text:='SELECT * FROM data0714 WHERE (RKEY IS NULL)';
  dm.AQery.Open ;
  try

    dm.AQery.Append ;
    dm.AQery.FieldValues['PTR712']:=dm.AQery712.FieldValues['rkey']; //712表指针
...
//编辑
function TfrmKFKH.Edit714():boolean;
var
 vrkey:integer;
begin
  dm.AQery.Close ;
  dm.AQery.SQL.Text:='SELECT * FROM data0714 WHERE (RKEY ='+dm.ADS714.FieldByName('rkey').AsString+')';
  dm.AQery.Open ;
  try
    dm.AQery.Edit;
    dm.AQery.FieldValues['PTR10']:=_CostPTR;                        //客户10指针
    ...
    dm.AQery.Post;
    messagedlg('保存成功！！！',mtinformation,[mbok],0);
    vrkey:=dm.ADS714.FieldValues['rkey'];
    dm.ADS714.Close;
    dm.ADS714.Open ;
    dm.ADS714.Locate('rkey',vrkey,[]);
    result:=true;
...
begin
 ...
 //客户
 dm.AQery.Close ;
 dm.AQery.SQL.Text:='select rkey,cust_code,CUSTOMER_NAME  from data0010 where rkey='+dm.ADS714.FieldByName('PTR10').AsString;
 dm.AQery.Open ;
...
begin
 ...
 try
  ...
  if not _isedit then
   begin
    if New714() =false then exit;
   end
  else
   begin
    if Edit714()=false then exit;
   end;
  close;
 except
   on E: Exception do
