object Form_StaffList: TForm_StaffList
  Left = 475
  Top = 139
  Width = 459
  Height = 555
  BorderIcons = [biSystemMenu, biMinimize]
  Caption = #21592#24037#25628#32034
  Color = clBtnFace
  Font.Charset = ANSI_CHARSET
  Font.Color = clWindowText
  Font.Height = -13
  Font.Name = #23435#20307
  Font.Style = []
  OldCreateOrder = False
  Position = poMainFormCenter
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 451
    Height = 41
    Align = alTop
    TabOrder = 0
    object Label1: TLabel
      Left = 80
      Top = 16
      Width = 52
      Height = 13
      Alignment = taRightJustify
      Caption = #21592#24037#24037#21495
    end
    object Edit1: TEdit
      Left = 144
      Top = 12
      Width = 121
      Height = 21
      TabOrder = 0
      OnChange = Edit1Change
    end
    object BitBtn1: TBitBtn
      Left = 272
      Top = 8
      Width = 28
      Height = 26
      TabOrder = 1
      OnClick = BitBtn1Click
      Glyph.Data = {
        
        440AADADADA000A04440DADADADADADA0440ADADADADADADA00D}
    end
  end
  object DBGridEh1: TDBGridEh
    Left = 0
    Top = 41
    Width = 451
    Height = 446
    Align = alClient
    DataSource = DataSource1
    FooterColor = clWindow
    FooterFont.Charset = ANSI_CHARSET
    FooterFont.Color = clWindowText
    FooterFont.Height = -13
    FooterFont.Name = #23435#20307
    FooterFont.Style = []
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgCancelOnExit]
    TabOrder = 1
    TitleFont.Charset = ANSI_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -13
    TitleFont.Name = #23435#20307
    TitleFont.Style = []
    OnDblClick = DBGridEh1DblClick
    OnTitleClick = DBGridEh1TitleClick
    Columns = <
      item
        EditButtons = <>
        FieldName = 'employeecode'
        Footers = <>
        Title.Caption = #21592#24037#24037#21495
        Title.Color = clRed
        Title.SortIndex = 1
        Title.SortMarker = smUpEh
        Width = 106
      end
      item
        EditButtons = <>
        FieldName = 'chinesename'
        Footers = <>
        Title.Caption = #21592#24037#22995#21517
      end
      item
        EditButtons = <>
        FieldName = 'ondutytime'
        Footers = <>
        Title.Caption = #20837#32844#26085#26399
        Width = 139
      end>
  end
  object Panel2: TPanel
    Left = 0
    Top = 487
    Width = 451
    Height = 41
    Align = alBottom
    TabOrder = 2
    object Button1: TButton
      Left = 136
      Top = 8
      Width = 75
      Height = 25
      Caption = #30830#23450
      TabOrder = 0
      OnClick = Button1Click
    end
    object Button2: TButton
      Left = 232
      Top = 8
      Width = 75
      Height = 25
      Caption = #21462#28040
      TabOrder = 1
      OnClick = Button2Click
    end
  end
  object ADOEmployeeMsg: TADODataSet
    Connection = DM.ADOConn
    CursorType = ctStatic
    Filtered = True
    CommandText = 
      'select rkey,employeecode,chinesename,ondutytime from employeemsg' +
      ' '#13#10'where employeecode like '#39'%'#39'+:employeecode+'#39'%'#39' and (status=1) ' +
      'and (isnull(cardno,'#39#39')='#39#39')'
    Parameters = <
      item
        Name = 'employeecode+'#39'%'#39
        DataType = ftWideString
        NumericScale = 255
        Precision = 255
        Size = 20
        Value = Null
      end>
    Left = 40
    Top = 439
    object ADOEmployeeMsgrkey: TAutoIncField
      FieldName = 'rkey'
      ReadOnly = True
    end
    object ADOEmployeeMsgemployeecode: TWideStringField
      FieldName = 'employeecode'
    end
    object ADOEmployeeMsgchinesename: TWideStringField
      FieldName = 'chinesename'
    end
    object ADOEmployeeMsgondutytime: TDateTimeField
      FieldName = 'ondutytime'
    end
  end
  object DataSource1: TDataSource
    DataSet = ADOEmployeeMsg
    Left = 128
    Top = 439
  end
end
