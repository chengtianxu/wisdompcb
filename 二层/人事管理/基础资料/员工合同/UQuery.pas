...
begin
 listbox3.Items.Clear;
  v_Field_List_CN := VarArrayOf(['合同编号','员工姓名','结束时间',
  '合同类型', '合同状态','部门','员工入职日期']);
  v_Field_List_EN := VarArrayOf(['employee_contract.code',
  'employee_contract.employeeid','employee_contract.enddate',
  'employee_contract.type' ,'employee_contract.status',
  'v_employeemsg.departmentid','v_employeemsg.ondutytime']);
   ...
   Self.ComboBox1.Items.Clear;
    ...
    begin
          ...
          SQL.Text:='SELECT   rkey, dictid, item '+
           ' FROM  dbo.datadetail WHERE dictid = 5 '+#13;
          Open;
          first;
          while not Eof do
          begin
...
begin
 ...
 0:
  begin
        ...
        0:sgrid1.Cells[2,sgrid_row]:='and employee_contract.code = '''+edtValue.Text+'''';
        1:sgrid1.Cells[2,sgrid_row]:='and employee_contract.code like ''%'+edtValue.Text+'%''';
        end;
   end;
 1:
  begin
         sgrid1.Cells[1,sgrid_row]:=trim(edtValue.Text);
        case radiogroup1.ItemIndex of
        0:sgrid1.Cells[2,sgrid_row]:='and employee_contract.employeeid = '''+inttostr(edtValue.tag)+'''';
  ...
  2: begin
        ...
        0:sgrid1.Cells[2,sgrid_row]:='and  replace(convert(char(10),employee_contract.enddate,111),'+#39+'/'+#39+','+#39+'-'+#39+')  = '''+datetostr(DateTimePicker2.Date)+'''';
        1:sgrid1.Cells[2,sgrid_row]:='and replace(convert(char(10),employee_contract.enddate,111),'+#39+'/'+#39+','+#39+'-'+#39+')  > '''+datetostr(DateTimePicker2.Date)+'''';
        2:sgrid1.Cells[2,sgrid_row]:='and replace(convert(char(10),employee_contract.enddate,111),'+#39+'/'+#39+','+#39+'-'+#39+')  < '''+datetostr(DateTimePicker2.Date)+'''';
        3:sgrid1.Cells[2,sgrid_row]:='and replace(convert(char(10),employee_contract.enddate,111),'+#39+'/'+#39+','+#39+'-'+#39+')  like ''%'+datetostr(DateTimePicker2.Date)+'%''';
        end;
    end;
 3: begin
          sgrid1.Cells[1,sgrid_row]:=trim(ComboBox2.Text);
          sgrid1.Cells[2,sgrid_row]:='and employee_contract.type = '''+Self.ComboBox1.Text+'''';
    end;
 4: begin
          sgrid1.Cells[1,sgrid_row]:= RadioGroup2.Items[RadioGroup2.ItemIndex];
          sgrid1.Cells[2,sgrid_row]:='and employee_contract.status = '''+inttostr(Self.RadioGroup2.ItemIndex+1)+'''';
    end;
 5: begin
        sgrid1.Cells[1,sgrid_row]:=trim(edtValue.Text);
        case radiogroup1.ItemIndex of
 ...
 6: begin
        sgrid1.Cells[1,sgrid_row]:=DateToStr(DateTimePicker2.Date);
        case radiogroup3.ItemIndex of
        0:sgrid1.Cells[2,sgrid_row]:='and  replace(convert(char(10),v_employeemsg.ondutytime,111),'+#39+'/'+#39+','+#39+'-'+#39+')  = '''+datetostr(DateTimePicker2.Date)+'''';
        1:sgrid1.Cells[2,sgrid_row]:='and replace(convert(char(10),v_employeemsg.ondutytime,111),'+#39+'/'+#39+','+#39+'-'+#39+')  > '''+datetostr(DateTimePicker2.Date)+'''';
        2:sgrid1.Cells[2,sgrid_row]:='and replace(convert(char(10),v_employeemsg.ondutytime,111),'+#39+'/'+#39+','+#39+'-'+#39+')  < '''+datetostr(DateTimePicker2.Date)+'''';
        3:sgrid1.Cells[2,sgrid_row]:='and replace(convert(char(10),v_employeemsg.ondutytime,111),'+#39+'/'+#39+','+#39+'-'+#39+')  like ''%'+datetostr(DateTimePicker2.Date)+'%''';
        end;
    end;
 end;  //end case
end;
