...
implementation
uses
  EditFrm, QueFrm, EditFrms, ExcelFrm;
...
begin
  ...
  DM.qry1s.SQL.Text := Format('select distinct employeemsg.rkey,employeecode,chinesename,departmentname , '
    ...
    + 'and  year(effectdate)=''%s'' and month(effectdate)=''%s'' and employee_rewards_punishment.employeeid=employeemsg.rkey) as ''总奖励'', '
    + '(select isnull(sum(money),0) Smoney from employee_rewards_punishment, datadetail '
    + 'where employee_rewards_punishment.type=datadetail.rkey  and flag = 1 '
    + 'and  year(effectdate)=''%s'' and month(effectdate)=''%s'' and employee_rewards_punishment.employeeid=employeemsg.rkey) as ''总惩罚'', '
    + '(select isnull(sum(money),0) Smoney from employee_rewards_punishment, datadetail '
    + 'where employee_rewards_punishment.type=datadetail.rkey and flag= 0 '
    ...
    intToStr(YearOf(dtp4.DateTime)), IntToStr(MonthOf(dtp4.DateTime)), intToStr(YearOf(dtp4.DateTime))]);
  DM.qry1s.Open;
