...
uses
  ...
  DBGridEh, Menus,CommCtrl;

type
  TfrmEdit4 = class(TForm)
    pnl1: TPanel;
    qry_tp: TADOQuery;
    pnl2: TPanel;
    pnl3: TPanel;
...
procedure TfrmEdit4.GetData(ARkey: string);
var Lsql:string;
begin
  Lsql:='select   * from trainPlan where rkey='+ARkey;
  dm.OpenQry(qry_tp,Lsql);

  Lsql :=' select  a.rkey, a.PMonth,Pdate,a.Bdate,a.BTime, a.EDate,a.ETime,'+
  ...
  begin
    edt_THOurs.Text:='0';
    DateTime_SetFormat(pdate.Handle,'''    -  -  ''');
    DateTime_SetFormat(BDate.Handle,'''    -  -  ''');
    DateTime_SetFormat(Edate.Handle,'''    -  -  ''') ;
  end
  ...
  begin
   ...
   if not qrytemp.fieldbyname('PDate').IsNull then
   PDate.Date:=qrytemp.fieldbyname('PDate').AsDateTime
   else
   begin
   //DateTime_SetFormat(PDate.Handle,'''    -  -  ''');
   PDate.Date:=Now;
   ...
   BTime.time:=qrytemp.fieldbyname('Btime').AsDateTime;
   if not qrytemp.fieldbyname('ETime').IsNull then
   ETime.Time:=qrytemp.fieldbyname('Etime').AsDateTime;
...
procedure TfrmEdit4.btn_exitClick(Sender: TObject);
begin
close;
end;

procedure TfrmEdit4.btn_SaveClick(Sender: TObject);
begin
 ...
 // qry_tp.FieldByName('tc_ptr').Value:=edt_tc.Tag;
  ...
  qry_tp.FieldByName('EDate').Value:=StrToDateTime(FormatDateTime('yyyy-MM-dd',EDate.Date));
   ...
   if (FormatDateTime('hh:mm:ss',BTime.Date))>'00:01:01' then
  qry_tp.FieldByName('BTime').Value:=StrToDateTime(FormatDateTime('hh:mm:ss',BTime.Date));

    if (FormatDateTime('hh:mm:ss',ETime.Date))>'00:01:01' then
  qry_tp.FieldByName('ETIme').Value:=StrToDateTime(FormatDateTime('hh:mm:ss',ETime.Date));


 // qry_tp.FieldByName('Pmonth').Value:=trim(edt_pmonth.text);

   if (FormatDateTime('yyyy-MM-dd',PDate.Date))>'2000-01-01' then
  qry_tp.FieldByName('PDate').Value:=StrToDateTime(FormatDateTime('yyyy-MM-dd',PDate.Date));
  qry_tp.FieldByName('place').Value:=Trim(edt_TPlace.text);
  qry_tp.FieldByName('Teacher').Value:=trim(edt_TTeacher.text);
  qry_tp.FieldByName('Thours').Value:=StrToFloat((edt_THOurs.Text));

  if chk_IsFinished.Checked then
     qry_tp.FieldByName('IsFinished').Value:=1
  else
...
procedure TfrmEdit4.btn_selDTCClick(Sender: TObject);
var
  InputVar:PDlgInput;
begin
