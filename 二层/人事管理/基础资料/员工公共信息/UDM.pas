...
Dialogs, DB, ExtDlgs, StdCtrls, ADODB, Grids, DBGrids, ExtCtrls, jpeg, DBCtrls, UMain;
...
type
  TDM = class(TDataModule)
    ...
    employee_msgemployeecode: TWideStringField;
    ...
    procedure employee_msgCalcFields(DataSet: TDataSet);
    procedure employee_msgAfterScroll(DataSet: TDataSet);
...
procedure TDM.employee_msgCalcFields(DataSet: TDataSet);
begin
  //DataSet.fieldbyname('RecNo').value:=DataSet.recno;
end;

procedure TDM.employee_msgAfterScroll(DataSet: TDataSet);
begin
//ShowMessage(IntToStr(DataSet['RecNo']));
  //if (DataSet.RecNo = 1) or (DataSet.RecNo = DataSet.RecordCount) then DataSet.Refresh;
  //try
  {if (DataSet['RecNo']=-1) And (DataSet.RecordCount>0)
