...
begin
  OpenDialog1.Title := '请选择正确的excel文件';
   OpenDialog1.Filter := 'Excel(*.xls)|*.xls|*.xlsx|*.xlsx';
  ...
  try
    ...
    begin
      ...
      begin
        ...
        Cells[10, iRow - 1] := trim(Excel.WorkSheets[1].Cells[iRow,iCol+6].value); //文控编号
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        //验证导入的绩效得分
        if ((Trim(StringGrid1.Cells[3, i]) = '') ) then    // or  (strtoint(Trim(StringGrid1.Cells[3, i]))<0)
        begin
        ...
        begin
          ...
          begin
            ...
            if FieldByName('dictid').AsInteger = 26 then StringGrid1.Cells[9, i] := '2';
      ...
      try
        ...
        begin
          ...
          begin
            Close;
            SQL.Clear;
            SQL.Text := Format('insert into employee_rewards_punishment(employeeid, type, money, effectdate, remark, flag,ModiMan,PNO) '
              + 'values(%d, %d, %d, ''%s'', ''%s'', %d, %d, ''%s'')',
              [StrToInt(stringGrid1.Cells[7, i]), StrToInt(StringGrid1.Cells[8,i]), StrToInt(StringGrid1.Cells[3,i]),
              stringgrid1.Cells[4,i], stringgrid1.Cells[5,i], StrToInt(stringgrid1.Cells[9,i]), StrToInt(rkey73),stringgrid1.Cells[10,i] ]);
            ExecSQL;
            j := j + 1;
          end;
