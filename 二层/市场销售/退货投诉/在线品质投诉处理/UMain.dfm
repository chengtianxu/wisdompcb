object frmComplaintOnline: TfrmComplaintOnline
  Left = 708
  Top = 242
  Width = 878
  Height = 571
  Caption = #22312#32447#21697#36136#25237#35785#22788#29702
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -13
  Font.Name = #23435#20307
  Font.Style = []
  OldCreateOrder = False
  WindowState = wsMaximized
  OnCreate = FormCreate
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object pnl1: TPanel
    Left = 0
    Top = 0
    Width = 870
    Height = 540
    Align = alClient
    TabOrder = 0
    object strngrd362sgrid1: TStringGrid
      Left = 232
      Top = 108
      Width = 248
      Height = 158
      ColCount = 3
      FixedCols = 0
      RowCount = 2
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = #23435#20307
      Font.Style = []
      Options = [goFixedVertLine, goFixedHorzLine, goVertLine, goHorzLine, goRowSelect]
      ParentFont = False
      ScrollBars = ssVertical
      TabOrder = 0
      Visible = False
      ColWidths = (
        83
        160
        79)
    end
    object pnl2: TPanel
      Left = 1
      Top = 1
      Width = 868
      Height = 35
      Align = alTop
      BevelOuter = bvNone
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = #23435#20307
      Font.Style = []
      ParentFont = False
      TabOrder = 1
      object lbl_field: TLabel
        Left = 431
        Top = 9
        Width = 52
        Height = 13
        Alignment = taRightJustify
        Caption = #26412#21378#32534#21495
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = #23435#20307
        Font.Style = []
        ParentFont = False
      end
      object btn_exit: TBitBtn
        Left = 0
        Top = 2
        Width = 57
        Height = 30
        Hint = #36864#20986#31995#32479
        Caption = #36864#20986
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = #23435#20307
        Font.Style = []
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 0
        OnClick = btn_exitClick
        Glyph.Data = {
          
          0000}
        NumGlyphs = 2
      end
      object btn_refresh: TBitBtn
        Left = 62
        Top = 2
        Width = 57
        Height = 30
        Hint = #21047#26032#24403#21069#25968#25454
        Caption = #21047#26032
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = #23435#20307
        Font.Style = []
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 1
        OnClick = btn_refreshClick
        Glyph.Data = {
          
          440AADADADA000A04440DADADADADADA0440ADADADADADADA00D}
      end
      object btn_export: TBitBtn
        Left = 124
        Top = 2
        Width = 57
        Height = 30
        Hint = #23548#20986#25968#25454#21040'Excel'
        Caption = #23548#20986
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = #23435#20307
        Font.Style = []
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 2
        OnClick = btn_exportClick
        Glyph.Data = {
          
          70DA0FFFF0ADA0EEEE0D00000ADADA00000AADADADADADADADAD}
      end
      object btn_qry: TBitBtn
        Left = 187
        Top = 2
        Width = 57
        Height = 30
        Hint = #37325#26032#23450#20041#26597#35810#26465#20214
        Caption = #26597#35810
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = #23435#20307
        Font.Style = []
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 3
        OnClick = btn_qryClick
        Glyph.Data = {
          
          861D8680775398535355}
      end
      object btn5: TBitBtn
        Left = 243
        Top = 2
        Width = 57
        Height = 30
        Hint = #29992#25143#33258#23450#20041#26174#31034#23383#27573
        Caption = #23383#27573
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = #23435#20307
        Font.Style = []
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 4
        OnClick = btn5Click
        Glyph.Data = {
          
          0707170F2A03070707070707070707070000}
      end
      object edt_value: TEdit
        Left = 492
        Top = 5
        Width = 121
        Height = 21
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = #23435#20307
        Font.Style = []
        ImeName = #20013#25991' ('#31616#20307') - '#25628#29399#20116#31508#36755#20837#27861
        ParentFont = False
        TabOrder = 5
        OnChange = edt_valueChange
      end
      object btn6: TBitBtn
        Left = 303
        Top = 4
        Width = 60
        Height = 27
        Hint = #30424#28857#28165#21333#25253#34920#23450#20041
        Caption = #23450#20041
        ParentShowHint = False
        ShowHint = True
        TabOrder = 6
        OnClick = btn6Click
        Glyph.Data = {
          
          FF00}
      end
    end
    object eh1: TDBGridEh
      Left = 1
      Top = 36
      Width = 868
      Height = 503
      Align = alClient
      DataSource = ds1
      DrawMemoText = True
      FooterColor = clWindow
      FooterFont.Charset = DEFAULT_CHARSET
      FooterFont.Color = clWindowText
      FooterFont.Height = -13
      FooterFont.Name = #23435#20307
      FooterFont.Style = []
      ImeName = #20013#25991' ('#31616#20307') - '#25628#29399#20116#31508#36755#20837#27861
      Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
      PopupMenu = pm1
      TabOrder = 2
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -13
      TitleFont.Name = #23435#20307
      TitleFont.Style = []
      OnTitleClick = eh1TitleClick
      Columns = <
        item
          EditButtons = <>
          FieldName = 'LOG_NUMBER'
          Footers = <>
          Title.Caption = #25237#35785#32534#21495
        end
        item
          EditButtons = <>
          FieldName = 'MANU_PART_NUMBER'
          Footers = <>
          Title.Caption = #26412#21378#32534#21495
        end
        item
          EditButtons = <>
          FieldName = 'MANU_PART_DESC'
          Footers = <>
          Title.Caption = #23458#25143#22411#21495
          Width = 180
        end
        item
          EditButtons = <>
          FieldName = 'CUST_CODE'
          Footers = <>
          Title.Caption = #23458#25143#20195#30721
        end
        item
          EditButtons = <>
          FieldName = 'QTY'
          Footers = <>
          Title.Caption = #25968#37327
        end
        item
          EditButtons = <>
          FieldName = 'Unit_Name'
          Footers = <>
          Title.Caption = #21333#20301
          Width = 36
        end
        item
          EditButtons = <>
          FieldName = 'DEFECT_MAIN'
          Footers = <>
          Title.Caption = #20027#35201#32570#38519
          Width = 73
        end
        item
          EditButtons = <>
          FieldName = 'CYCLES'
          Footers = <>
          Title.Caption = #21608#26399
          Width = 59
        end
        item
          EditButtons = <>
          FieldName = 'dept_names'
          Footers = <>
          Title.Caption = #36131#20219#37096#38376
          Width = 110
        end
        item
          EditButtons = <>
          FieldName = 'EFFECT_TP'
          Footers = <>
          Title.Caption = #25237#35785#32423#21035
          Width = 59
        end
        item
          EditButtons = <>
          FieldName = 'Mystatus'
          Footers = <>
          Title.Caption = #29366#24577
        end
        item
          EditButtons = <>
          FieldName = 'SHIP_DATE'
          Footers = <>
          Title.Caption = #25237#35785#26085#26399
        end
        item
          EditButtons = <>
          FieldName = 'LOG_DATE'
          Footers = <>
          Title.Caption = #24314#26723#26085#26399
        end
        item
          EditButtons = <>
          FieldName = 'Ent_EMPLOYEE_NAME'
          Footers = <>
          Title.Caption = #24314#26723#20154
        end
        item
          EditButtons = <>
          FieldName = 'CLOSED_DATE'
          Footers = <>
          Title.Caption = #32467#26696#26085#26399
        end
        item
          EditButtons = <>
          FieldName = 'Close_EMPLOYEE_NAME'
          Footers = <>
          Title.Caption = #32467#26696#20154
          Width = 88
        end
        item
          EditButtons = <>
          FieldName = 'Ssend'
          Footers = <>
          Title.Caption = #26159#21542#21457#36865
          Visible = False
          Width = 67
        end
        item
          EditButtons = <>
          FieldName = 'DEALT_DATE'
          Footers = <>
          Title.Caption = #21463#29702#20154
          Visible = False
        end
        item
          EditButtons = <>
          FieldName = 'Dealt_EMPLOYEE_NAME'
          Footers = <>
          Visible = False
        end
        item
          EditButtons = <>
          FieldName = 'CUSTOMER_NAME'
          Footers = <>
          Visible = False
        end>
    end
  end
  object pm2: TPopupMenu
    AutoHotkeys = maManual
    Left = 384
    Top = 3
  end
  object ds1: TDataSource
    DataSet = qry1
    Left = 176
    Top = 288
  end
  object pm1: TPopupMenu
    OnPopup = pm1Popup
    Left = 392
    Top = 104
    object mni_add: TMenuItem
      Caption = #26032#22686
      OnClick = mni_addClick
    end
    object mni_edit: TMenuItem
      Caption = #20462#25913
      OnClick = mni_editClick
    end
    object mni_view: TMenuItem
      Caption = #26816#26597
      OnClick = mni_viewClick
    end
    object mni_del: TMenuItem
      Caption = #21024#38500
      OnClick = mni_delClick
    end
    object mni_close: TMenuItem
      Caption = #32467#26696
      OnClick = mni_closeClick
    end
    object mni_UnClose: TMenuItem
      Caption = #21462#28040#32467#26696
      OnClick = mni_UnCloseClick
    end
    object mni_print: TMenuItem
      Caption = #25171#21360
      OnClick = mni_printClick
    end
  end
  object qry1: TADOQuery
    Connection = DM.con1
    CursorType = ctStatic
    OnCalcFields = qry1CalcFields
    Parameters = <
      item
        Name = 'vdate1'
        DataType = ftDateTime
        Precision = 16
        Size = 16
        Value = Null
      end
      item
        Name = 'vdate2'
        DataType = ftDateTime
        Precision = 16
        Size = 16
        Value = Null
      end>
    SQL.Strings = (
      
        'SELECT Data0101.*, Data0010.CUST_CODE, Data0010.CUSTOMER_NAME, c' +
        'ase data0101.if_send when 0 then  '#39#26410#21457#36865#39'  when 1 then '#39#24050#21457#36865#39' end  ' +
        'as Ssend,'
      
        '      Data0005.EMPLOYEE_NAME AS Close_EMPLOYEE_NAME, Data0025.MA' +
        'NU_PART_NUMBER, '
      '      Data0025.MANU_PART_DESC, '
      '      Data0005_1.EMPLOYEE_NAME AS Dealt_EMPLOYEE_NAME, '
      '      Data0005_2.EMPLOYEE_NAME AS Ent_EMPLOYEE_NAME,'
      '      Data0002.Unit_Name,derivedtb.dept_names '
      'FROM Data0101 INNER JOIN'
      
        '      Data0010 ON Data0101.CUSTOMER_PTR = Data0010.RKEY INNER JO' +
        'IN'
      '      Data0025 ON '
      '      Data0101.CUSTOMER_PART_PTR = Data0025.RKEY LEFT OUTER JOIN'
      '      Data0002 ON'
      '      Data0101.unit_ptr=Data0002.rkey  LEFT OUTER JOIN'
      '      Data0005 Data0005_2 ON '
      '      Data0101.CSI_USER_PTR = Data0005_2.RKEY LEFT OUTER JOIN'
      '      Data0005 Data0005_1 ON '
      '      Data0101.DEALT_BY_PTR = Data0005_1.RKEY LEFT OUTER JOIN'
      
        '      Data0005 ON Data0101.CLOSED_BY_PTR = Data0005.RKEY LEFT OU' +
        'TER JOIN'
      '      (select SOURCE_PTR,[dept_names]=STUFF'
      '                      ((SELECT '#39','#39' + convert(varchar,dept_name)'
      '                      FROM  dbo.Data0034  INNER JOIN'
      
        '                      Data0118 ON Data0118.resp_dept_ptr=Data003' +
        '4.rkey  '
      
        '                      where  SOURCE_PTR=tb.source_ptr FOR XML PA' +
        'TH('#39#39')), 1, 1, '#39#39')'
      'from data0118 as tb'
      
        'GROUP BY source_PTR) AS derivedtb on data0101.rkey=derivedtb.sou' +
        'rce_ptr   '
      'where (data0101.log_date >= :vdate1)'
      '      and (data0101.log_date <= :vdate2)')
    Left = 288
    Top = 296
    object qry1RKEY: TAutoIncField
      FieldName = 'RKEY'
      ReadOnly = True
    end
    object qry1LOG_NUMBER: TStringField
      FieldName = 'LOG_NUMBER'
      FixedChar = True
      Size = 10
    end
    object qry1CUSTOMER_PART_PTR: TIntegerField
      FieldName = 'CUSTOMER_PART_PTR'
    end
    object qry1CSI_USER_PTR: TIntegerField
      FieldName = 'CSI_USER_PTR'
    end
    object qry1CUSTOMER_PTR: TIntegerField
      FieldName = 'CUSTOMER_PTR'
    end
    object qry1LOG_DATE: TDateTimeField
      FieldName = 'LOG_DATE'
    end
    object qry1SHIP_DATE: TDateTimeField
      FieldName = 'SHIP_DATE'
    end
    object qry1QTY: TFloatField
      FieldName = 'QTY'
    end
    object qry1EFFECT_TP: TStringField
      FieldName = 'EFFECT_TP'
      FixedChar = True
      Size = 4
    end
    object qry1ANALYSIS: TStringField
      FieldName = 'ANALYSIS'
      Size = 600
    end
    object qry1AFTER_ACTION: TStringField
      FieldName = 'AFTER_ACTION'
      Size = 600
    end
    object qry1STATUS: TSmallintField
      FieldName = 'STATUS'
    end
    object qry1CLOSED_BY_PTR: TIntegerField
      FieldName = 'CLOSED_BY_PTR'
    end
    object qry1CLOSED_DATE: TDateTimeField
      FieldName = 'CLOSED_DATE'
    end
    object qry1DEALT_BY_PTR: TIntegerField
      FieldName = 'DEALT_BY_PTR'
    end
    object qry1DEALT_DATE: TDateTimeField
      FieldName = 'DEALT_DATE'
    end
    object qry1LOSS_AMT: TIntegerField
      FieldName = 'LOSS_AMT'
    end
    object qry1RESULT: TStringField
      FieldName = 'RESULT'
      Size = 600
    end
    object qry1CONTENT: TStringField
      FieldName = 'CONTENT'
      Size = 600
    end
    object qry1DEFECT_MAIN: TWideStringField
      FieldName = 'DEFECT_MAIN'
      Size = 50
    end
    object qry1CYCLES: TWideStringField
      FieldName = 'CYCLES'
      Size = 50
    end
    object qry1UNIT_PTR: TIntegerField
      FieldName = 'UNIT_PTR'
    end
    object qry1if_send: TBooleanField
      FieldName = 'if_send'
    end
    object qry1CType: TIntegerField
      FieldName = 'CType'
    end
    object qry1CUST_CODE: TStringField
      FieldName = 'CUST_CODE'
      Size = 10
    end
    object qry1CUSTOMER_NAME: TStringField
      FieldName = 'CUSTOMER_NAME'
      Size = 100
    end
    object qry1Ssend: TStringField
      FieldName = 'Ssend'
      ReadOnly = True
      Size = 6
    end
    object qry1Close_EMPLOYEE_NAME: TStringField
      FieldName = 'Close_EMPLOYEE_NAME'
      Size = 16
    end
    object qry1MANU_PART_NUMBER: TStringField
      FieldName = 'MANU_PART_NUMBER'
      Size = 30
    end
    object qry1MANU_PART_DESC: TStringField
      FieldName = 'MANU_PART_DESC'
      Size = 80
    end
    object qry1Dealt_EMPLOYEE_NAME: TStringField
      FieldName = 'Dealt_EMPLOYEE_NAME'
      Size = 16
    end
    object qry1Ent_EMPLOYEE_NAME: TStringField
      FieldName = 'Ent_EMPLOYEE_NAME'
      Size = 16
    end
    object qry1Unit_Name: TStringField
      FieldName = 'Unit_Name'
    end
    object qry1dept_names: TMemoField
      FieldName = 'dept_names'
      ReadOnly = True
      BlobType = ftMemo
    end
    object qry1Mystatus: TStringField
      FieldKind = fkCalculated
      FieldName = 'Mystatus'
      Size = 10
      Calculated = True
    end
  end
end
