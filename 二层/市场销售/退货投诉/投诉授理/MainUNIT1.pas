...
{  hMapFile := CreateFileMapping ($FFFFFFFF,nil,page_ReadWrite,0,100,'xyz2055coimasp20');
  ...
  user_ptrCaption := trim(copy(S,3,pos(' ',s)-3)); //雇员指针 data0005
  //权限 (1:只读不帶銀碼 2:可写不帶銀碼;3:只读+银码;4:可写+银码)
  vprevCaption := trim(copy(S,pos(' ',s)+1,pos(',',s)-pos(' ',s)-1));

  db_ptrCaption := trim(copy(S,pos(',',s)+1,length(s)-pos(',',s)));  //连接串
...
begin
 if not dm.ADOConnection1.Connected then
  ...
  if not dm.ADOConnection1.Connected then
      begin
          if trim(vprevCaption) = '' then  application.Terminate;
              dm.ADOConnection1.ConnectionString := db_ptrCaption;
          try
  ...
  {try
      ...
      dm.TemADOQuery.SQL.Add('select top 1 flag from data0118');
      dm.TemADOQuery.Prepared ;
      dm.TemADOQuery.Open ;
      if  dm.TemADOQuery.FieldByName('flag').DataType <> ftBoolean then
          begin
              if dm.TemADOQuery.Active then dm.TemADOQuery.Close ;
              dm.TemADOQuery.SQL.Clear ;
              dm.TemADOQuery.SQL.Add('ALTER TABLE data0118 ALTER COLUMN flag [bit] NULL ');
              dm.TemADOQuery.Prepared ;
...
begin
  ...
  if not dm.ADOQALL2.IsEmpty then
      begin
          ...
          if dm.ADOQ0101.fieldbyname('status').AsInteger = 1 then
              begin
...
begin
  ...
  if dm.adoq0101.fieldbyname('status').AsInteger = 1 then
      begin
          ...
          dm.ADOQ0101DEALT_BY_PTR.AsString := user_ptrCaption ;
...
begin
  ...
  if dm.adoq0101.fieldbyname('status').AsInteger = 2 then
      begin
          dm.adoq0101.Edit ;
          dm.adoq0101status.AsInteger := 1 ;
          dm.ADOQ0101DEALT_BY_PTR.AsVariant := null ;
          dm.ADOQ0101DEALT_DATE.AsVariant := null ;
          dm.ADOConnection1.BeginTrans ;
          try
              dm.adoq0101.UpdateBatch();
              dm.ADOConnection1.CommitTrans ;
