...
//      begin
//          vqty := vqty + DM.ADOQ0118Qty.AsInteger ;
//          DM.ADOQ0118.Next ;
//      end;

//  if strtoint(edit15.Text) <> vqty then
//      begin
//          showmessage('分板数量与投诉的总数量不相等,请修改!');
//          exit;
//      end;
  ...
  if VEditMode = 0 then
      ...
      begin
          dm.ADOQ0101.Edit;
          dm.ADOQ0101DEALT_BY_PTR.AsString := vuser_ptrCaption ;
          dm.ADOQ0101DEALT_DATE.AsDateTime := valymd ;
      end;
...
var
  LBMFrm: TfrmBMList;
begin
//              s1 := 'select rkey,dept_code,dept_name,case ttype when 1 then'+'''工序'''+'when 4 then'+'''部门'''+'when 5 then'+'''班组'''+'end as sttype';
//              s1 :=s1 + ' from data0034  where ttype in (4,5)';
//              s1 :=s1 + ' or ((ttype=1) and (BARCODE_ENTRY_FLAG='+'''Y'''+')) ';
//              s1 :=s1 + ' order by sttype desc';
//                frmPick_Item_Single3 := TfrmPick_Item_Single.Create(application) ;
//                try
//                    InputVar3.Fields :=  'dept_code/部门代码/200,dept_name/部门名称/200,sttype/部门类型/80';
//                    //InputVar.KeyField
//                    InputVar3.Sqlstr := s1 ;
//                    //InputVar.InPut_value
//                    InputVar3.AdoConn := DM.ADOConnection1 ;
//                    frmPick_Item_Single3.InitForm_New(InputVar3)  ;
//                    if frmPick_Item_Single3.ShowModal = mrok then
//                        begin
//                            if frmPick_Item_Single3.adsPick.IsEmpty then exit;
//
//                            dm.ADOQ0118.Append ;
//                            if not dm.ADOQ0101rkey.IsNull then
//                                dm.ADOQ0118SOURCE_PTR.AsInteger := dm.ADOQ0101rkey.AsInteger ;
//                            //dm.ADOQ0118defect_ptr.AsInteger := frmPick_Item_Single2.adsPick.fieldbyname('rkey').AsInteger ;
//                            dm.ADOQ0118RESP_DEPT_PTR.AsInteger := frmPick_Item_Single3.adsPick.fieldbyname('rkey').AsInteger ;
//                            dm.ADOQ0118FLAG.AsBoolean := true;
//                            dm.ADOQ0118qty.AsInteger := 0 ;
//                        end ;
//
//                finally
//                    frmPick_Item_Single3.adsPick.Close;
//                    frmPick_Item_Single3.Free ;
//                end;
        if trim(Edit12.Text)='' then
        begin
...
begin
  ...
  s := 'select rkey,rej_code,reject_description from data0039 where reject_defect_flag = ''D'' order by reject_defect_flag,rej_code';
  frmPick_Item_Single2 := TfrmPick_Item_Single.Create(application) ;
  try
      //frmPick_Item_Single.Height := 600;
      //frmPick_Item_Single.Width := 800;
...
procedure TEditForm1.N5Click(Sender: TObject);
var
  InputVar3: PDlgInput ;
  s1:string;
  frmPick_Item_Single3 :TfrmPick_Item_Single;
begin
  if DM.ADOQ0118.IsEmpty then exit;

  s1 := 'select rkey,dept_code,dept_name,ttype from data0034 where ttype = 4 ';
  s1 :=s1 + ' union select rkey,dept_code,dept_name,ttype ';
  s1 :=s1 + ' from data0034 where ttype = 1 ORDER by ttype,dept_code';
  frmPick_Item_Single3 := TfrmPick_Item_Single.Create(application) ;
  try
      //frmPick_Item_Single.Height := 600;
      //frmPick_Item_Single.Width := 800;
      InputVar3.Fields := 'dept_code/资源代码/200,dept_name/资源名称/200';
      //InputVar.KeyField
      InputVar3.Sqlstr := s1 ;
      //InputVar.InPut_value
      InputVar3.AdoConn := DM.ADOConnection1 ;
...
procedure TEditForm1.N2Click(Sender: TObject);
begin
  if DM.ADOQ0118.IsEmpty then exit;
  DM.ADOQ0118.Delete ;
end;
