...
type
  TForm2 = class(TForm)
    ...
    ADOQuery1srce_ptr: TIntegerField;
    ADOQuery1GLPTR_STATUS: TBooleanField;
    ADOQuery1Putaway_date: TDateTimeField;
...
if adoquery1.State=dsedit then       //编辑
 begin
  Edit6.Text:=dm.ADOQuery1warehouse_name.AsString;
   ...
   if not adoquery1.FieldByName('remark').IsNull  then
   quote.Text:=adoquery1.FieldByName('remark').AsString;
 ...
 begin
  self.ADOQuery1NOTE_PAD_PTR.Value:=0;
...
begin
  ...
  try
    ...
    form_salesorder.adoquery1.Close;
    form_salesorder.adoquery1.Parameters.ParamByName('dtpk1').Value:= common.getsystem_date(ADOQuery2,1)-30;
    form_salesorder.adoquery1.Parameters.ParamByName('dtpk2').Value:= common.getsystem_date(ADOQuery2,1)+1;
    form_salesorder.adoquery1.Parameters.ParamByName('rkey10').Value:=strtoint(Label15.Caption);
    form_salesorder.adoquery1.Open;
    ...
    begin
      ...
      ADOQuery1TAX_2.Value:=form_salesorder.ADOquery1RUSH_CHARGE.Value;
...
if adoquery1.State=dsedit then  //编辑
 begin
  try
//   save_note;     //2018-3-27取消11备注到98

   ADOQuery1.FieldByName('remark').AsString := quote.Text;
...
//   save_note;   //保存记事本 //2018-3-27取消11备注到98
 ...
 begin
  if if_rkey64(dm.ADO64rkey.Value) then
   ...
   begin
    ...
    Self.ADOQuery1TAX_2.Value:=dm.ado64tax_2.Value;
