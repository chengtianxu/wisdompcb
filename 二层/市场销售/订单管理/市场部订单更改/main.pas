     ...
     begin
      ...
      begin
        ...
        if dm.ADO60tax_in_price.Value='N' then
          ...
          dm.ADO60parts_alloc.Value:=dm.ADO60part_price.Value/
                                (1+dm.ADO60RUSH_CHARGE.Value*0.01);
          dm.ADO60parts_alloc1.Value:=dm.ADO60parts_alloc.Value;
      end;
       ...
       if (dm.ADO60PART_price.Value<>parts_price)  then  //and (dm.ADO60so_tp.Value=1)
       begin
        with DM.ADOQuery1 do
        begin
          close;
...
//end 20161219tang
  ...
  Form10.edt_OCustName.Text:= dm.ADO60.fieldbyname('ORIG_CUSTOMER').AsString;   //tao20171219  ORIG_CUSTOMER
  Form10.edt_indate.text:= FormatDateTime('yyyy-MM-dd hh:mm:ss',dm.ADO60ENT_DATE.asdatetime); //tao20171219
  
  old_ponumber:=dm.ADO60PO_NUMBER.Value;
  old_fob:=dm.ADO60FOB.Value;
  old_cust_code:=dm.ADO60CUST_CODE.Value;

  oInDate:= FormatDateTime('yyyy-MM-dd hh:mm:ss',dm.ADO60ENT_DATE.asdatetime);  //tao20171219
  OORIG_CUSTOMER:=dm.ADO60.fieldbyname('ORIG_CUSTOMER').AsString;;
  //  ORIG_CUSTOMER
  ...
  //20150709客户交期修改-------
     ...
     //tao20171219 下单日期修改：
      if (Trim(Form10.edt_inDate.Text)<>OInDate) then
      begin
        with DM.ADOQuery1 do
        begin
          Close;
          SQL.Clear;
          SQL.Add('update data0060 set ENT_DATE='''+trim(Form10.edt_inDate.Text)+'''');
          SQL.Add('where rkey='+dm.ADO60rkey.AsString);
          ExecSQL;
        end;
        ...
        begin
          ...
          sql.Add('insert into data0318 (SALES_ORDER_PTR,TRANS_TYPE,TRANS_DESCRIPTION,');
          sql.Add('FROM_STRING,TO_STRING,USER_PTR,TRANS_DATE,PROGRAM_SOURCE,file_number)');
          sql.Add('values ('+dm.ADO60rkey.AsString+',1,''更改下单日期'',');
          sql.Add(''''+OInDate+''','''+form10.edt_inDate.Text+''','+rkey73+',');
          sql.add('getdate(),305,'''+form10.Edit1.Text+''')');
          ExecSQL;
        end;
      end;
       ...
       //tao20171219 原关联客户修改：
      if (Trim(Form10.edt_OCustName.Text)<>OORIG_CUSTOMER) then
      begin
        with DM.ADOQuery1 do
        begin
          Close;
          SQL.Clear;
          SQL.Add('update data0060 set ORIG_CUSTOMER='''+trim(Form10.edt_OCustName.Text)+'''');
          SQL.Add('where rkey='+dm.ADO60rkey.AsString);
          ExecSQL;
        end;
        ...
        begin
          ...
          sql.Add('insert into data0318 (SALES_ORDER_PTR,TRANS_TYPE,TRANS_DESCRIPTION,');
          sql.Add('FROM_STRING,TO_STRING,USER_PTR,TRANS_DATE,PROGRAM_SOURCE,file_number)');
          sql.Add('values ('+dm.ADO60rkey.AsString+',1,''更改原关系客户'',');
          sql.Add(''''+OORIG_CUSTOMER+''','''+form10.edt_OCustName.Text+''','+rkey73+',');
          sql.add('getdate(),305,'''+form10.Edit1.Text+''')');
          ExecSQL;
        end;
      end;
