...
uses
  ...
  Dialogs, Buttons, StdCtrls,ConstVar, DB, ADODB;
...
begin
  frmPick_Item_Single := TfrmPick_Item_Single.Create(application) ;
  try
    InputVar.Fields := 'CUST_CODE/客户代码/100,CUSTOMER_NAME/客户名称/100,ABBR_NAME/客户简称/100';
    InputVar.Sqlstr := 'select rkey,CUST_CODE,CUSTOMER_NAME,ABBR_NAME from data0010';
    InputVar.AdoConn := DM.ADOConnection1 ;
    frmPick_Item_Single.InitForm_New(InputVar)  ;
    if frmPick_Item_Single.ShowModal=mrok then
    begin
      edt_OCustName.Text:=frmPick_Item_Single.adsPick.Fieldbyname('ABBR_NAME').AsString;
    end;
  finally
    frmPick_Item_Single.adsPick.Close;
    frmPick_Item_Single.Free ;
...
procedure TForm10.edt_OCustNameExit(Sender: TObject);
var
  Lsql:string;
begin
...
procedure TForm10.edt_inDateExit(Sender: TObject);
begin
TRY
