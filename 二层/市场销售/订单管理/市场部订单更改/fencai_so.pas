...
type
  TForm6 = class(TForm)
    ...
    function get_sonumber( bef:string):string;
...
function TForm6.get_sonumber(bef:string): string;
var
 v_so:string;
 v_row:integer;
begin
  with dm.ADOQuery2 do
   begin
     ...
     sql.Clear;
     sql.Add('select top 1 sales_order from data0060');
     sql.Add('where sales_order like '''+bef+'%''');
...
begin
  ...
  form7.Edit1.Text :=
  get_sonumber(Copy(dbEdit1.Text,1,Pos('-',dbEdit1.Text)));
...
begin
  ...
  if strtoint(edit1.Text)<dm.ADOQuery3PARTS_SHIPPED.AsInteger then
   ...
   if checkQty(StrToInt(Edit1.Text)) then
   begin
