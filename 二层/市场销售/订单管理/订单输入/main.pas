...
uses
  ...
  DBCtrls, DB, ADODB, ToolWin, ExtCtrls, Excel2000,  ComObj, DBGridEh;
...
1:
 begin//有效
  ...
  n13.Enabled := true;
  n14.Enabled:=false;
  if dm.ADOQuery1D60PROD_REL.Value='1' then //PROD_REL生产标记
   n23.Enabled :=true
  else
   n23.Enabled :=false;
  if (dm.ADOQuery1QUOTE_PRICE.Value=2) then
//  if (dm.ADOQuery1QUOTE_PRICE.Value=2) or (dm.ADOQuery1QUOTE_PRICE.Value=0) or ((dm.ADOQuery1QUOTE_PRICE.Value=3) and (dm.ADOQuery1D60PROD_REL.Value='1')) then
    N6.Enabled:=True
  else
    N6.Enabled := false;  //编辑
  if (N16.Enabled=False) and (iset=0) and ((dm.ADOQuery1QUOTE_PRICE.Value=0) or ((dm.ADOQuery1QUOTE_PRICE.Value=3) and (dm.ADOQuery1D60PROD_REL.Value='1'))) then
    N28.Enabled:=True
  else
    N28.Enabled:=False;
 end;
...
//     dm.ADOQuery4.Parameters[1].Value:=dm.ADOQuery1D60RKEY.Value;
   ...
   //2017-10-25tang改不用存储过程
     dm.qrytmp.Close;
     dm.qrytmp.SQL.Text:='SELECT dbo.data0492.CUT_NO, dbo.Data0006.WORK_ORDER_NUMBER, '
                        ...
                        +'  WHERE (data0492.so_no='''+Trim(dm.ADOQuery1D60SALES_ORDER.AsString)+''') and '
                        +'  dbo.Data0006.PROD_STATUS in (2,3,4,5,6) '
                        +'  order by work_order_number';
     dm.qrytmp.Open;
     if dm.qrytmp.RecordCount>0 then
     begin
       dm.qrytmp.First;
       while not dm.qrytmp.Eof  do
       begin
         if (dm.qrytmp.FieldByName('PROD_STATUS').AsInteger=4) then
         begin
            with dm.ADOQuery5 do
            begin
              close;
              ...
              sql.Text:='insert into data0117(Source_ptr,Source_Type,Empl_ptr,Tdate,Action,Remark) '+
               ' values ('+dm.qrytmp.fieldBYName('Rkey').AsString+',2,'+User_ptr+',getdate(),1,'+QuotedStr('市场订单暂缓导致外发工单暂缓')+')';
              ExecSQL;
            end
         end else
         begin
           ...
           begin
             ...
             sql.Clear;
             sql.Text:='insert into data0117(Source_ptr,Source_Type,Empl_ptr,Tdate,Action,Remark) '+
              ' values ('+dm.qrytmp.fieldBYName('Rkey').AsString+',2,'+User_ptr+',getdate(),1,'+QuotedStr('市场订单暂缓导致工单暂缓')+')';
             ExecSQL;
           end;
         end;
         if dm.qrytmp.FieldByName('PROD_STATUS').Value=2 then
         begin
           with dm.ADOQuery5 do
           begin
             close;
             sql.Clear;
             sql.Text:='update data0006 set ENGG_STATUS=PROD_STATUS,PROD_STATUS=102 where rkey='+dm.qrytmp.FieldBYName('Rkey').AsString;
             ExecSQL;
           end;
         end else
         begin
           ...
           begin
             close;
             sql.Clear;
             sql.Text:='update data0006 set ENGG_STATUS=PROD_STATUS,PROD_STATUS=103 where rkey='+dm.qrytmp.FieldBYName('Rkey').AsString;
             ExecSQL;
           end;
         end;
         dm.qrytmp.Next;
...
//     dm.ADOQuery4.Parameters[1].Value:=dm.ADOQuery1D60RKEY.Value;
   ...
   //2017-10-25tang改不用存储过程
     dm.qrytmp.Close;
     dm.qrytmp.SQL.Text:='SELECT dbo.data0492.CUT_NO, Data0006.WORK_ORDER_NUMBER,'
                        +'         dbo.Data0006.PROD_STATUS, dbo.data0492.PLANNED_QTY,'
                        +'          dbo.data0492.ISSUED_QTY, dbo.Data0006.QUAN_SCH,'
                        +'        data0006.rkey'
                        +'    FROM dbo.Data0006 INNER JOIN'
                        +'          dbo.data0492 ON dbo.Data0006.CUT_NO = dbo.data0492.CUT_NO '
                        +'    WHERE (data0492.so_no='''+dm.ADOQuery1D60SALES_ORDER.AsString+''') and'
     ...
     begin
       ...
       while not dm.qrytmp.Eof do
       begin
         if (dm.qrytmp.FieldByName('PROD_STATUS').Value=102) then
         begin
           with dm.ADOQuery5 do
           begin
             close;
             sql.Clear;
             sql.Text:='update data0006 set PROD_STATUS=2 where Rkey='+dm.qrytmp.fieldBYName('Rkey').AsString;
             ExecSQL;
           end;
         end else
         begin
           ...
           begin
             close;
             sql.Clear;
             sql.Text:='update data0006 set PROD_STATUS=ENGG_STATUS where Rkey='+dm.qrytmp.fieldBYName('Rkey').AsString;
             ExecSQL;
           end;
         end;
         with dm.ADOQuery5 do
         begin
           ...
           sql.Clear;
           sql.Text:='insert into data0117(Source_ptr,Source_Type,Empl_ptr,Tdate,Action,Remark) '+
            ' values ('+dm.qrytmp.fieldBYName('Rkey').AsString+',2,'+User_ptr+',getdate(),2,'+QuotedStr('市场订恢复导致工单恢复')+')';
           ExecSQL;
         end;
         dm.qrytmp.Next;
       end;
...
//   form8.DBEdit17.ReadOnly := true;
   ...
   Form8.BitBtn15.Enabled:=False;
   Form8.Edit18.ReadOnly:=True;
  //20161019tang添
    if (dm.ADOQuery1.fieldbyname('BPNO').AsString<>'') then
    Form8.Label49.Caption:='Bom报价编号：'+dm.ADOQuery1.fieldbyname('BPNO').AsString;
    form8.Edit14.Text:=dm.adoquery1CPJS.AsString;
...
if form_condition.ShowModal=mrok then
 begin
  ...
  begin
   dm.adoquery1.CommandText:=dm.adoquery1.CommandText+#13+
...
//   end;
  enter_empl_ptr.Caption := user_ptr;

  v_dateNew:=getsystem_date(DM.ADOQuery5,1);
...
if (strtoint(vprev)=1) or (strtoint(vprev)=3) then
 begin
  ...
  checkvar;
  dm.ADOQuery1.Close;
  dm.ADOQuery1.Parameters[6].Value:=v_dateNew;
  dm.ADOQuery1.Parameters[7].Value:=v_dateNew+1;
  dm.ADOQuery1.Prepared;
  dm.ADOQuery1.Open;
...
//20171128tang改单价取不含税PARTS_ALLOC
  sql.Add('ROUND(SUM(PARTS_ORDERED * (PARTS_ALLOC * (1 - DISCOUNT * 0.01))');
...
begin
 with dm.ADOQuery5 do
  ...
  if not dm.ADOQuery5.IsEmpty then
  begin
    //20171025tang去掉ADOQuery1的Edit改为ltReadOnly
//    dm.ADOQuery1.Edit;
//    dm.ADOQuery1D60STATUS.Value := 1;
    dm.qrytmp.Close;
    dm.qrytmp.SQL.Text:='update data0060 set STATUS=1 where Rkey='+dm.ADOQuery1D60RKEY.AsString;
    dm.qrytmp.ExecSQL;
    ...
    begin
      ...
      begin
        ...
        begin
          dm.qrytmp.Close;
          dm.qrytmp.SQL.Text:='update data0060 set quote_price=3,SCH_DATE='''+dm.ADOQuery1ORIG_SCHED_SHIP_DATE.AsString+''' where Rkey='+dm.ADOQuery1D60RKEY.AsString;
          dm.qrytmp.ExecSQL;
        end;
      end
      else
...
//        dm.AQ9711.post;
    ...
    //20171025tang去掉ADOQuery1的Edit改为ltReadOnly
      dm.qrytmp.Close;
      dm.qrytmp.SQL.Text:='update data0060 set STATUS=3 where Rkey='+dm.ADOQuery1D60RKEY.AsString;
      dm.qrytmp.ExecSQL;
//     dm.ADOQuery1.Edit;
...
//     dm.ADOQuery1.Post;
     with dm.ADOQuery5 do
      begin
       ...
       sql.Add('insert into data0318 (SALES_ORDER_PTR,TRANS_TYPE,TRANS_DESCRIPTION,');
       sql.Add('FROM_string,TO_string,USER_PTR,TRANS_DATE,PROGRAM_SOURCE,file_number)');
       sql.Add('values ('+dm.ADOQuery1D60RKEY.AsString+',11,''手工将销售订单关闭'',');
...
//       open;
//      end;
    if (not dm.qrytmp.IsEmpty)  and
      (messagedlg('关闭订单的同时暂缓其在线作业单吗?',mtConfirmation,[mbYes,mbNo],0)= mrYes) then
       while not dm.qrytmp.Eof do
        begin
         if dm.qrytmp.FieldByName('PROD_STATUS').Value=4 then
          with dm.ADOQuery5 do
           begin
           close;
           sql.Clear;
       ...
       //2017-10-26tang 判断06状态是否暂缓，否记录并操作
         if (dm.qrytmp.FieldByName('PROD_STATUS').Value<>102) or (dm.qrytmp.FieldByName('PROD_STATUS').Value<>103) then
         begin
           if dm.qrytmp.FieldByName('PROD_STATUS').Value=2 then
           begin
             dm.ADOQuery5.Close;
             dm.ADOQuery5.SQL.Text:='update data0006 set PROD_STATUS=102,ENGG_STATUS=PROD_STATUS where rkey='+dm.qrytmp.FieldBYName('rkey').AsString;
             dm.ADOQuery5.ExecSQL;
           end else
           begin
             dm.ADOQuery5.Close;
             dm.ADOQuery5.SQL.Text:='update data0006 set PROD_STATUS=103,ENGG_STATUS=PROD_STATUS where rkey='+dm.qrytmp.FieldBYName('rkey').AsString;
             dm.ADOQuery5.ExecSQL;
           end;
         end;
//         dm.ado06.Edit;
...
//   dm.ADOQuery1.Post;
   ...
   begin
    ...
    sql.Add('values ('+dm.ADOQuery1D60RKEY.AsString+',11,''手工将销售订单从关闭中重新起动'',');
    sql.Add(''''+status+''','''+dm.ADOQuery1status.AsString+''','+rkey73+',');
    sql.add('getdate(),32)');
    ExecSQL;
   end;
   //20171026tang更新06状态回原值--
     dm.qrytmp.Close;
     dm.qrytmp.SQL.Text:='SELECT dbo.data0492.CUT_NO, Data0006.WORK_ORDER_NUMBER,'
                        +'         dbo.Data0006.PROD_STATUS, dbo.data0492.PLANNED_QTY,'
                        +'          dbo.data0492.ISSUED_QTY, dbo.Data0006.QUAN_SCH,'
...
var
  fileName:String;
  iCount, jCount,i,PO_NUM97: Integer;
  XLApp: variant;
  Sheet: Variant;
...
begin
  ...
  dDate2:=getsystem_date(dm.aqTmp,1);
  ...
  AdoTmp.Connection:=dm.ADOConnection1;
   try
     ...
     begin
       ...
       dm.qrytmp.Close;
       dm.qrytmp.SQL.Clear;
       dm.qrytmp.SQL.Text:='select Rkey from data0097 where PO_NUMBER ='''+trim(XLApp.workbooks[1].sheets[1].cells[iCount,2])+''' and CUSTOMER_PTR='+AdoTmp.FieldByName('Rkey').AsString;
       dm.qrytmp.Open;
       if dm.qrytmp.IsEmpty then
       begin
         PO_NUM97:=1;
       end else Rkey97:=dm.qrytmp.FieldByName('Rkey').AsString;
...
//       dm.qrytmp.SQL.Text:='select Rkey,TAX_2 from data0023 where CODE='''+trim(XLApp.workbooks[1].sheets[1].cells[iCount,9])+''' and ttype<>0 and status=0';
     ...
     //*****************************************开始更新data0060及它的子表
       ...
       begin
         ...
         begin
          dm.aqTmp.Close;
          dm.aqTmp.SQL.Text:=' insert into data0097 (PO_NUMBER,CUSTOMER_PTR,PO_DATE,OPEN_SOS) values (:PO_NUMBER,:CUSTOMER_PTR,:PO_DATE,:OPEN_SOS)';
          dm.aqTmp.SQL.Add('select scope_identity() as fRet');
          dm.aqTmp.Parameters.ParamByName('PO_NUMBER').Value:=trim(XLApp.workbooks[1].sheets[1].cells[iCount,2]);
          dm.aqTmp.Parameters.ParamByName('CUSTOMER_PTR').Value:=AdoTmp.FieldByName('Rkey').AsString;
          dm.aqTmp.Parameters.ParamByName('PO_DATE').Value:=dDate2;
          dm.aqTmp.Parameters.ParamByName('OPEN_SOS').Value:=0;
          dm.aqTmp.Open;
          Rkey97:=dm.aqTmp.FieldByName('fRet').AsString;
         end;
        if active=true then active:=false;
        sql.Clear;
        ...
        active:=true;
         ...
         Rkey04:=dm.qrytmp.FieldByName('SEED_VALUE').AsString+'-01';
          ...
          fieldbyname('SCH_DATE').AsString :=AdoTmps.fieldbyname('DUE_DATE').AsString;         //客户交期
...
//          dm.AQ62.post;
          ...
          dm.ADO360quantity.Value:=AdoTmps.fieldbyname('PARTS_ORDERED').AsInteger;
          dm.ADO360sch_date.Value:=AdoTmps.fieldbyname('DUE_DATE').AsDateTime;
          dm.ADO360.Post;
          try
...
procedure TForm1.DBGridEh1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
 if key=13 then DBGridEh1dblclick(sender);
 if (chr(key)='S') and (ssalt in shift) then
  showmessage(dm.adoquery1.CommandText);
end;
...
begin
 if (strtoint(vprev)=1) or (strtoint(vprev)=3) then
  ...
  dm.qrytmp.Close;
  dm.qrytmp.SQL.Text:='update data0060 set STATUS=6 where Rkey='+dm.ADOQuery1D60RKEY.AsString;
  dm.qrytmp.ExecSQL;
  rkey60 := dm.ADOQuery1D60RKEY.Value;
  dm.ADOQuery1.Close;
  dm.ADOQuery1.Open;
