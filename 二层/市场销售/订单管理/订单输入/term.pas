...
begin
 ...
 0:
  if RadioGroup2.ItemIndex=0 then
    sgrid1.Cells[2,sgrid_row]:='and Data0060.sales_order= '''+edit3.Text+''''
  else
    sgrid1.Cells[2,sgrid_row]:='and Data0060.sales_order LIKE ''%'+edit3.Text+'%''';
 1:
  if RadioGroup2.ItemIndex=0 then
    sgrid1.Cells[2,sgrid_row]:='and data0025.MANU_PART_NUMBER = '''+edit3.Text+''''
  else
    sgrid1.Cells[2,sgrid_row]:='and data0025.MANU_PART_NUMBER LIKE ''%'+edit3.Text+'%''';
 2:
  if RadioGroup2.ItemIndex=0 then
    sgrid1.Cells[2,sgrid_row]:='and Data0025.MANU_PART_DESC = '''+edit3.Text+''''
  else
 ...
 4:
  if RadioGroup2.ItemIndex=0 then
    sgrid1.Cells[2,sgrid_row]:='and data0097.PO_NUMBER = '''+edit3.Text+''''
  else
 ...
 13:
  if RadioGroup2.ItemIndex=0 then
    ...
    sgrid1.Cells[2,sgrid_row]:='and data0060.ORIG_CUSTOMER like ''%'+edit3.Text+'%''';
 14:
 begin
  ...
  case listbox3.ItemIndex of
   ...
   14:
    begin
      ...
      dm.qrytmp.Close;
      dm.qrytmp.SQL.Text:='select data0338.tvalue from data0278 inner join data0338 on data0278.Rkey=data0338.parameter_ptr where  data0278.SPEC_RKEY=''js''';
      dm.qrytmp.Open;
      while not dm.qrytmp.Eof do
      begin
        combobox2.Items.Add(dm.qrytmp.FieldBYName('tvalue').AsString);
       dm.qrytmp.Next;
      end;
      edit3.Visible:=false;
      button3.Enabled:=true;
...
begin
  ...
  3:
    try
     ...
     InputVar.Sqlstr := 'select RKEY,CUST_CODE,ABBR_NAME,CUSTOMER_NAME,ANALYSIS_CODE3 from data0010 where CUSTOMER_TYPE<>4 order by CUST_CODE';
...
//20161202tang改公用窗体
    try
     frmPick_Item_Single := TfrmPick_Item_Single.Create(application) ;
     InputVar.Fields := 'EMPL_CODE/员工代码/160,EMPLOYEE_NAME/员工姓名/250';
     InputVar.Sqlstr := 'select rkey, EMPL_CODE, EMPLOYEE_NAME, ABBR_NAME from data0005 where ACTIVE_FLAG=''Y'' order by EMPL_CODE';
     InputVar.AdoConn := DM.ADOConnection1 ;
     frmPick_Item_Single.InitForm_New(InputVar)  ;
     if frmPick_Item_Single.ShowModal=mrok then
     begin
