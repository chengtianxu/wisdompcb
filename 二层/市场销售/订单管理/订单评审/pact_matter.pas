...
begin
  ...
  begin
//2018-3-12tang添检测完工日期当天是否超出工厂定义的最高入仓数面积
    if (DM.ADO97.FieldByName('date_area').AsInteger<>0) and (iShow=0) then
    begin
      DM.ADO360.First;
      while not dm.ADO360.Eof do
      begin
        DM.Adoarea.close;
        dm.Adoarea.Parameters.ParamByName('data1').Value:=DM.ADO360sch_date.Value;
        dm.Adoarea.Parameters.ParamByName('rkey15').Value:=DM.ADO97.fieldByName('SHIP_REG_TAX_ID').Value;
        DM.Adoarea.Open;
        if not dm.Adoarea.IsEmpty then
        begin
          ishow:=1;
          if messagedlg('完工日期'+DM.ADO360sch_date.AsString+'已超出工厂每日最多入库面积，您确定要通过评审吗？',mtconfirmation,[mbYes,mbNo],0)=mrno then
          begin
            pagecontrol1.ActivePageIndex:=4;
            dbgrid6.SetFocus;
            Exit;
        ...
        if (ishow=1) then Break;
        dm.ADO360.Next;
      end;
    end;
    if (DM.ADO97.FieldByName('date_quantity').AsInteger<>0) and (iShow2=0) then   //最高入仓款数
    begin
      DM.ADO360.First;
      while not dm.ADO360.Eof do
      begin
        dm.Adoquantity.Close;
        dm.Adoquantity.Parameters.ParamByName('data1').Value:=DM.ADO360sch_date.Value;
        dm.Adoquantity.Parameters.ParamByName('rkey15').Value:=DM.ADO97.fieldByName('SHIP_REG_TAX_ID').Value;
        DM.Adoquantity.Open;
        if not dm.Adoquantity.IsEmpty then
        begin
          iShow2:=1;
          if messagedlg('完工日期'+DM.ADO360sch_date.AsString+'已超出工厂每日最高入库款数，您确定要通过评审吗？',mtconfirmation,[mbYes,mbNo],0)=mrno then
          begin
            pagecontrol1.ActivePageIndex:=4;
            dbgrid6.SetFocus;
            Exit;
...
begin
  ...
  begin
//2018-3-12tang添检测完工日期当天是否超出工厂定义的最高入仓数面积
  if (DM.ADO97.FieldByName('date_area').AsInteger<>0)  then
  begin
    DM.ADO360.First;
    while not dm.ADO360.Eof do
    begin
  ...
  if (DM.ADO97.FieldByName('date_quantity').AsInteger<>0) then   //最高入仓款数
  begin
    DM.ADO360.First;
    while not dm.ADO360.Eof do
    begin
