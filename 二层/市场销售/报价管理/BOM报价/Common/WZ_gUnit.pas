...
function StrInArr(str: string; strarr: array of string;IngroCase: Boolean = True): Boolean;
...
begin
  ...
  outRet.DBConnectString := 'Provider=SQLOLEDB.1;Password=62ddf8f60269678;Persist Security Info=True;' +
    'User ID=WZPCBConnect;Initial Catalog=wisdompcb;Data Source=127.0.0.1;Use Procedure for Prepare=1;Auto Translate=True;Packet Size=4096;Workstation ID=CHENGTX-PC;Use Encryption for Data=False;Tag with column collation when possible=False';
  outRet.DBName := '开发备份';
end;
...
begin
  ...
  begin
    if IngroCase then
    begin
      if UpperCase(str) = UpperCase(strarr[I]) then
      begin
        Result := True;
        Exit;
      end;
