object frmContract: TfrmContract
  Left = 580
  Top = 124
  Width = 1135
  Height = 746
  Caption = #21512#21516#35780#23457#31649#29702
  Color = clBtnFace
  Font.Charset = ANSI_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = #23435#20307
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnCreate = FormCreate
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 12
  object pnlAllClient: TPanel
    Left = 0
    Top = 0
    Width = 1119
    Height = 708
    Align = alClient
    BevelOuter = bvNone
    TabOrder = 0
    object pnlTop: TPanel
      Left = 0
      Top = 0
      Width = 1119
      Height = 38
      Align = alTop
      BevelOuter = bvNone
      TabOrder = 0
      object lblFilter: TLabel
        Left = 462
        Top = 11
        Width = 36
        Height = 12
        Alignment = taRightJustify
        Caption = #21512#21516#21495
      end
      object dtpDate: TDateTimePicker
        Left = 502
        Top = 7
        Width = 109
        Height = 20
        Date = 41058.000000000000000000
        Time = 41058.000000000000000000
        TabOrder = 6
        OnChange = dtpDateChange
      end
      object edtFilter: TEdit
        Left = 502
        Top = 7
        Width = 109
        Height = 20
        TabOrder = 5
        OnChange = edtFilterChange
      end
      object btnExit: TBitBtn
        Left = 0
        Top = 2
        Width = 57
        Height = 30
        Hint = #36864#20986#31995#32479
        Caption = #36864#20986
        ParentShowHint = False
        ShowHint = True
        TabOrder = 0
        OnClick = btnExitClick
        Glyph.Data = {
          
          0000}
        NumGlyphs = 2
      end
      object btnRefresh: TBitBtn
        Left = 57
        Top = 2
        Width = 57
        Height = 30
        Hint = #21047#26032#24403#21069#25968#25454
        Caption = #21047#26032
        ParentShowHint = False
        ShowHint = True
        TabOrder = 1
        OnClick = btnRefreshClick
        Glyph.Data = {
          
          440AADADADA000A04440DADADADADADA0440ADADADADADADA00D}
      end
      object btnExport: TBitBtn
        Left = 114
        Top = 2
        Width = 57
        Height = 30
        Hint = #23548#20986#25968#25454#21040'Excel'
        Caption = #23548#20986
        ParentShowHint = False
        ShowHint = True
        TabOrder = 2
        OnClick = btnExportClick
        Glyph.Data = {
          
          70DA0FFFF0ADA0EEEE0D00000ADADA00000AADADADADADADADAD}
      end
      object btnQuery: TBitBtn
        Left = 171
        Top = 2
        Width = 57
        Height = 30
        Hint = #37325#26032#23450#20041#26597#35810#26465#20214
        Caption = #26597#35810
        ParentShowHint = False
        ShowHint = True
        TabOrder = 3
        OnClick = btnQueryClick
        Glyph.Data = {
          
          861D8680775398535355}
      end
      object btnColumn: TBitBtn
        Left = 228
        Top = 2
        Width = 57
        Height = 30
        Hint = #29992#25143#33258#23450#20041#26174#31034#23383#27573
        Caption = #23383#27573
        ParentShowHint = False
        ShowHint = True
        TabOrder = 4
        OnClick = btnColumnClick
        Glyph.Data = {
          
          0707170F2A03070707070707070707070000}
      end
      object benDefined: TBitBtn
        Left = 285
        Top = 2
        Width = 57
        Height = 30
        Hint = #29992#25143#33258#23450#20041#25253#34920#26684#24335
        Caption = #23450#20041
        ParentShowHint = False
        ShowHint = True
        TabOrder = 7
        OnClick = benDefinedClick
        Glyph.Data = {
          
          FF00}
      end
      object btnSet: TBitBtn
        Left = 342
        Top = 2
        Width = 72
        Height = 30
        Hint = #23548#20986#25968#25454#21040'Excel'
        Caption = #21442#25968#35774#32622
        ParentShowHint = False
        ShowHint = True
        TabOrder = 8
        OnClick = btnSetClick
        Glyph.Data = {
          
          A090D0B0A0C0B0A0B0A090000000000000000000000000000000}
      end
    end
    object pnlClient: TPanel
      Left = 0
      Top = 38
      Width = 1119
      Height = 670
      Align = alClient
      BevelOuter = bvNone
      TabOrder = 1
      object dbgBrowse: TDBGridEh
        Left = 0
        Top = 0
        Width = 1119
        Height = 629
        Align = alClient
        DataSource = dm.dsBrowse
        FooterColor = clWindow
        FooterFont.Charset = ANSI_CHARSET
        FooterFont.Color = clWindowText
        FooterFont.Height = -12
        FooterFont.Name = #23435#20307
        FooterFont.Style = []
        OptionsEh = [dghFixed3D, dghHighlightFocus, dghClearSelection, dghIncSearch, dghPreferIncSearch]
        PopupMenu = pmOp
        TabOrder = 0
        TitleFont.Charset = ANSI_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -12
        TitleFont.Name = #23435#20307
        TitleFont.Style = []
        UseMultiTitle = True
        OnDblClick = N3Click
        OnDrawColumnCell = dbgBrowseDrawColumnCell
        OnKeyDown = dbgBrowseKeyDown
        OnTitleClick = dbgBrowseTitleClick
        Columns = <
          item
            EditButtons = <>
            FieldName = 'ContractNo'
            Footers = <>
          end
          item
            EditButtons = <>
            FieldName = 'Cust_Id'
            Footers = <>
          end
          item
            EditButtons = <>
            FieldName = 'Cust_Name'
            Footers = <>
          end
          item
            EditButtons = <>
            FieldName = 'TStatus'
            Footers = <>
          end
          item
            EditButtons = <>
            FieldName = 'CreateName'
            Footers = <>
          end
          item
            EditButtons = <>
            FieldName = 'CreateDate'
            Footers = <>
          end
          item
            EditButtons = <>
            FieldName = 'LastModifyName'
            Footers = <>
          end
          item
            EditButtons = <>
            FieldName = 'LastModifyDate'
            Footers = <>
          end
          item
            EditButtons = <>
            FieldName = 'AuditName'
            Footers = <>
          end
          item
            EditButtons = <>
            FieldName = 'AuditDate'
            Footers = <>
          end>
      end
      object pnlBottom: TPanel
        Left = 0
        Top = 629
        Width = 1119
        Height = 41
        Align = alBottom
        BevelOuter = bvNone
        TabOrder = 1
        object rgStatus: TRadioGroup
          Left = 0
          Top = 0
          Width = 1119
          Height = 41
          Align = alClient
          Caption = #29366#24577
          Columns = 5
          ItemIndex = 0
          Items.Strings = (
            #26410#23457#26680
            #24050#25552#20132
            #24050#23457#26680
            #24050#36864#22238
            #20840#37096)
          TabOrder = 0
          OnClick = rgStatusClick
        end
      end
    end
  end
  object pmColumn: TPopupMenu
    AutoHotkeys = maManual
    Left = 240
    Top = 40
  end
  object dmDesignRpt: TPopupMenu
    Left = 304
    Top = 40
    object MI1: TMenuItem
      Caption = #35774#35745'PCB'#21152#24037#21512#21516
      OnClick = MI1Click
    end
    object N15: TMenuItem
      Tag = 1
      Caption = #35774#35745#20135#21697#25253#20215#20998#26512#26723#26696#34920
      OnClick = MI1Click
    end
    object mniN18: TMenuItem
      Tag = 2
      Caption = #20891#24037
      OnClick = mniN18Click
    end
    object PCB21: TMenuItem
      Tag = 3
      Caption = #35774#35745'PCB'#21152#24037#21512#21516'2'
      OnClick = PCB21Click
    end
  end
  object pmOp: TPopupMenu
    OnPopup = pmOpPopup
    Left = 256
    Top = 288
    object N1: TMenuItem
      Caption = #26032#22686
      OnClick = N1Click
    end
    object N2: TMenuItem
      Caption = #32534#36753
      OnClick = N2Click
    end
    object N3: TMenuItem
      Caption = #26816#26597
      OnClick = N3Click
    end
    object N4: TMenuItem
      Caption = #22797#21046
      Visible = False
    end
    object N9: TMenuItem
      Caption = '-'
    end
    object N13: TMenuItem
      Caption = #25552#20132#23457#26680
      OnClick = N13Click
    end
    object N14: TMenuItem
      Caption = '-'
    end
    object N5: TMenuItem
      Caption = #23457#26680
      OnClick = N5Click
    end
    object N6: TMenuItem
      Caption = #36864#23457
      OnClick = N6Click
    end
    object N10: TMenuItem
      Caption = '-'
    end
    object N7: TMenuItem
      Caption = #21024#38500
      OnClick = N7Click
    end
    object N11: TMenuItem
      Caption = '-'
    end
    object N8: TMenuItem
      Caption = #25171#21360#25253#34920
      object mniN16: TMenuItem
        Caption = #25171#21360'(set)'
        OnClick = N8Click
      end
      object mniN17: TMenuItem
        Caption = #20891#24037
        OnClick = mniN17Click
      end
      object pcs1: TMenuItem
        Caption = #25171#21360'(pcs)'
        OnClick = pcs1Click
      end
    end
  end
  object pmToExcel: TPopupMenu
    Left = 144
    Top = 40
    object MenuItem1: TMenuItem
      Bitmap.Data = {
        
        70DA0FFFF0ADA0EEEE0D00000ADADA00000AADADADADADADADAD}
      Caption = #23548#20986#22522#26412#36164#26009
      OnClick = MenuItem1Click
    end
    object N12: TMenuItem
      Bitmap.Data = {
        
        70DA0FFFF0ADA0EEEE0D00000ADADA00000AADADADADADADADAD}
      Caption = #23548#20986#35814#32454#36164#26009
      OnClick = N12Click
    end
  end
  object aspExcel: TADOStoredProc
    Connection = dm.ADOConn
    CursorType = ctStatic
    LockType = ltBatchOptimistic
    ProcedureName = 'Rpt0167;2'
    Parameters = <
      item
        Name = '@RETURN_VALUE'
        DataType = ftInteger
        Direction = pdReturnValue
        Precision = 10
        Value = 0
      end
      item
        Name = '@Condition'
        DataType = ftString
        Size = 255
        Value = Null
      end>
    Left = 144
    Top = 88
    object aspExcelTFlag: TStringField
      DisplayLabel = #36873#25321
      FieldName = 'TFlag'
      Size = 2
    end
    object aspExcelContractNo: TStringField
      DisplayLabel = #21512#21516#32534#21495
      FieldName = 'ContractNo'
      Size = 15
    end
    object aspExcelCust_Id: TStringField
      DisplayLabel = #23458#25143#20195#30721
      FieldName = 'Cust_Id'
      Size = 10
    end
    object aspExcelABBR_NAME: TStringField
      DisplayLabel = #23458#25143#31616#31216
      FieldName = 'ABBR_NAME'
      Size = 10
    end
    object aspExcelSalesMan: TStringField
      DisplayLabel = #38144#21806#20154#21592
      FieldName = 'SalesMan'
      Size = 10
    end
    object aspExcelRFQNo: TStringField
      DisplayLabel = 'RFQ'
      FieldName = 'RFQNo'
      Size = 15
    end
    object aspExcelTPOType: TStringField
      DisplayLabel = #35746#21333#31867#22411
      FieldName = 'TPOType'
      Size = 10
    end
    object aspExcelFileName: TStringField
      DisplayLabel = #23458#25143#20135#21697#22411#21495
      FieldName = 'FileName'
      Size = 30
    end
    object aspExcelPartsId: TStringField
      DisplayLabel = #23458#25143#29289#26009#21495
      FieldName = 'PartsId'
      Size = 30
    end
    object aspExcelLayers: TWordField
      DisplayLabel = #23618#25968
      FieldName = 'Layers'
    end
    object aspExcelSetSizeX: TStringField
      DisplayLabel = #25104#21697#23610#23544'(mm)'
      FieldName = 'SetSizeX'
      Size = 16
    end
    object aspExcelSetSizeY: TStringField
      DisplayLabel = #25104#21697#23610#23544'(mm)'
      FieldName = 'SetSizeY'
      Size = 16
    end
    object aspExcelPCS_SET: TStringField
      DisplayLabel = 'unit/set'
      FieldName = 'PCS_SET'
      Size = 50
    end
    object aspExcelLAM2: TStringField
      DisplayLabel = #26495#26448
      FieldName = 'LAM2'
      Size = 30
    end
    object aspExcelBoardThick: TStringField
      DisplayLabel = #26495#21402'(mm)'
      FieldName = 'BoardThick'
      Size = 10
    end
    object aspExcelSurfaceDeal: TStringField
      DisplayLabel = #34920#38754#24037#33402
      FieldName = 'SurfaceDeal'
      Size = 24
    end
    object aspExcelNCCPTH: TStringField
      DisplayLabel = #20869#22806#38108#21402'(OZ)'
      FieldName = 'NCCPTH'
      Size = 12
    end
    object aspExcelWCCPTH: TStringField
      DisplayLabel = #20869#22806#38108#21402'(OZ)'
      FieldName = 'WCCPTH'
      Size = 12
    end
    object aspExcelQnty: TIntegerField
      DisplayLabel = #25968#37327'(set)'
      FieldName = 'Qnty'
    end
    object aspExcelDueArea: TBCDField
      DisplayLabel = #24635#38754#31215'('#13217')'
      FieldName = 'DueArea'
      Precision = 32
      Size = 12
    end
    object aspExcelPrice: TBCDField
      DisplayLabel = #21333#20215'('#20803'/set)'
      FieldName = 'Price'
      Precision = 18
      Size = 3
    end
    object aspExcelEngAmount: TBCDField
      DisplayLabel = #24037#31243#36153'('#20803')'
      FieldName = 'EngAmount'
      Precision = 18
      Size = 3
    end
    object aspExcelTestAmount: TBCDField
      DisplayLabel = #27979#35797#36153'('#20803')'
      FieldName = 'TestAmount'
      Precision = 18
      Size = 3
    end
    object aspExcelOtherAmount: TBCDField
      DisplayLabel = #20854#20182#36153#29992'('#20803')'
      FieldName = 'OtherAmount'
      Precision = 18
      Size = 3
    end
    object aspExcelDiscount: TBCDField
      DisplayLabel = #25240#25187#37329#39069
      FieldName = 'Discount'
      Precision = 18
      Size = 3
    end
    object aspExcelAftDiscountAmount: TBCDField
      DisplayLabel = #21512#21516#37329#39069
      FieldName = 'AftDiscountAmount'
      Precision = 18
      Size = 3
    end
    object aspExcelContractAmt: TBCDField
      DisplayLabel = #21512#21516#24635#37329#39069
      FieldName = 'ContractAmt'
      Precision = 18
      Size = 3
    end
    object aspExcelCURR_CODE: TStringField
      DisplayLabel = #32467#31639#24065#31181
      FieldName = 'CURR_CODE'
      Size = 5
    end
    object aspExcelDueDays: TWordField
      DisplayLabel = #21512#21516#20132#36135#22825#25968
      FieldName = 'DueDays'
    end
    object aspExcelNotes: TStringField
      DisplayLabel = #22791#27880#20869#23481
      FieldName = 'Notes'
      Size = 255
    end
  end
end
