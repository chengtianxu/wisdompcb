...
uses
  ...
  ppModule, DB, ADODB,Spell,Spell2,ShellCtrls,shellapi;
...
type
  ...
  TFrm_PCBCUT = class(TForm)
    ...
    procedure tbinportClick(Sender: TObject);
...
begin
  ...
  dm.aqtmp.Close;
  dm.aqtmp.sql.Text:='select selected,Length,Width from data0503 order by selected desc';
  dm.aqtmp.Open;
...
begin
  ...
  begin
    ...
    bmp:=TBitMap.Create;
    if not dm.ads511sheet_BMP.IsNull then
    begin
      dm.ads511sheet_BMP.SaveToStream(Stream);
      Stream.Position:=0;
    ...
    if not dm.ads511pnl_bmp.IsNull then
    begin
      dm.ads511pnl_bmp.SaveToStream(Stream);
      Stream.Position:=0;
      jpg.LoadFromStream(Stream);
      jpg.DIBNeeded;
      bmp.Assign(jpg);
    ...
    Edit15.Text :=dm.ads511pnl_lenspace.AsString;
    Edit16.Text :=dm.ads511pnl_wthspace.AsString;
    Edit1.Text :=dm.ads511pnl_lenspace.AsString;
    Edit38.Text :=dm.ads511pnl_wthspace.AsString;
    ...
    if not dm.ads511pnl2_bmp.IsNull then      //双拼板
    begin
      dm.ads511pnl2_bmp.SaveToStream(Stream);
      Stream.Position:=0;
      jpg.LoadFromStream(Stream);
      jpg.DIBNeeded;
      bmp.Assign(jpg);
  ...
  begin
    close;
    sql.Text:='Select cut_unit_tp,cut_l1_max,cut_l1_min,cut_w1_max,cut_w1_min,'
        +'cut_l2_max,cut_l2_min,cut_w2_max,cut_w2_min,SPACE_L_Min,SPACE_w_Min,'
        +'border_l_min,border_w_min from data0192';
    open;
...
begin
 ...
 if not svValid then exit;
  ...
  if dm.ads511.RecordCount>0 then dm.ads511.Edit
  else  dm.ads511.Append;
  ...
  Stream.Position :=0;
  dm.ads511sheet_BMP.LoadFromStream(stream);
  stream.Clear;
  ...
  jpegimage.SaveToStream(stream);
  Stream.Position :=0;
  dm.ads511pnl_bmp.LoadFromStream(stream);
  stream.Clear;
  ...
  begin
     ...
     jpegimage.SaveToStream(stream);
     Stream.Position :=0;
     dm.ads511pnl2_bmp.LoadFromStream(stream); // 读取保存的内存图
     stream.Clear;
  end
  else dm.ads511pnl2_bmp.Clear;

  dm.ads511.Fieldbyname('unit_unit').asinteger:=0;                 //单位为mm
  dm.ads511.Fieldbyname('minspace1').asString:=Edit13.Text ;
  dm.ads511.Fieldbyname('minspace2').asString:=Edit14.Text ;
  dm.ads511pnl_lenspace.AsString := Edit15.Text;
  dm.ads511pnl_wthspace.AsString := Edit16.Text;
  dm.ads511.Fieldbyname('upanel').AsString := Edit2.Text;       //A板每PAN的unit数
  dm.ads511.Fieldbyname('total_pnls_1').AsString:=Edit3.Text;   //大料总A板数

  dm.ads511pnl_lenspace.AsString := Edit1.Text;
  dm.ads511pnl_wthspace.AsString := Edit38.Text;

  if ttype='0' then  
    dm.ads511.Fieldbyname('unit_num').AsString:=Edit19.text        //拼版单元个数
  else dm.ads511.Fieldbyname('unit_num').AsString:=Edit8.text ;
  ...
  begin
    ...
    begin
      ...
      Fieldbyname('total_pnls_2').AsInteger:=strtoint('0'+Edit5.Text);                    //大料总B板数
      ...
      Fieldbyname('unit_len').AsString:=Edit6.Text;   //拼版长
      Fieldbyname('unit_wth').AsString:=Edit7.Text;; //拼版宽
      Fieldbyname('unit_num').AsString:=Edit8.text;
    ...
    begin
      ...
      begin
        Result_MakeUp2[iSel].PNLB_Long:=0;
        Result_MakeUp2[iSel].PNLB_Width:=0;
      end;
//修改:结果集未清除B排版长宽，只清空数量，在这里处理，如数量为0则清空B长宽--lihao 08-10-14
        
      Fieldbyname('unit_len').AsFloat:=Result_MakeUp2[iSel].PNLA_Unit_Long;   //拼版长
      Fieldbyname('unit_wth').AsFloat:=Result_MakeUp2[iSel].PNLA_Unit_Width; //拼版宽
      ...
      begin
        ...
        Fieldbyname('upanel_2').AsString:=Edit4.Text;                        //B板每PAN的unit数
        Fieldbyname('total_pnls_2').AsString:=Edit5.Text;                   //大料总B板数
      end
      else begin     //调换A/B板
        jpegimage.Assign(image2.Picture.Bitmap);
        jpegimage.JPEGNeeded;
        jpegimage.SaveToStream(stream);
        Stream.Position :=0;
        dm.ads511pnl2_bmp.LoadFromStream(stream);
        stream.Clear;
        ...
        dm.ads511.Fieldbyname('upanel').AsString := Edit4.Text;       //A板每PAN的unit数
        dm.ads511.Fieldbyname('total_pnls_1').AsString:=Edit5.Text;   //大料总A板数
        Fieldbyname('upanel_2').AsString:=Edit2.Text;                 //B板每PAN的unit数
        Fieldbyname('total_pnls_2').AsString:=Edit3.Text;
      end;
    end else if (ttype='0') or (ttype='1') then begin   //ttype=0,1  单拼板或手动拼版
      ...
      if  ttype='0' then
      begin
        dm.ads511.Fieldbyname('minspace1').asString:=Edit24.Text ;
        dm.ads511.Fieldbyname('minspace2').asString:=Edit25.Text ;

        dm.ads511pnl_lenspace.AsString := Edit26.Text;
        dm.ads511pnl_wthspace.AsString := Edit27.Text;
      end;
      dm.ads511pnl2_bmp.AsVariant:=null;
      dm.ads511PNL_LEN_2.Value:=0;
      dm.ads511PNL_WTH_2.Value:=0;
      dm.ads511UPANEL_2.Value:=0;
      dm.ads511TOTAL_PNLS_2.Value:=0;
    end;
...
begin
  ...
  begin
    ...
    MicroAdapt2(strtocurr(Edit42.text),strtocurr(Edit43.text),strtocurr(Edit44.text),
      strtocurr(Edit45.text),Result_MakeUp2[iSel]);
    DrawPNL2(iSel)
  end;
end;
...
var
  ...
  tempal3,tempbl3,tempaw3,tempbw3:currency;  //用于记录三区临时PNL长与宽
...
begin
  ...
  Draw_Picture1.Free;
   ...
   if  Result_MakeUp[i].Sheet_Long>Result_MakeUp[0].Sheet_Width then
   begin
...
procedure TFrm_PCBCUT.tbinportClick(Sender: TObject);
...
begin
  ...
  begin
    ...
    Edit33.Text:=Dm.ads511.Fieldbyname('Tusage').AsString;
    Edit31.Text:=Dm.ads511.Fieldbyname('sht_len').AsString;
    Edit32.Text:=Dm.ads511.Fieldbyname('sht_wth').AsString;
  end;
...
procedure TFrm_PCBCUT.tboutputClick(Sender: TObject);
var myjpg:Tjpegimage;
...
procedure TFrm_PCBCUT.tbprintClick(Sender: TObject);
begin
...
procedure TFrm_PCBCUT.tbclearClick(Sender: TObject);
begin
...
procedure TFrm_PCBCUT.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
  dm.AQ17.Filter:='';
end;
