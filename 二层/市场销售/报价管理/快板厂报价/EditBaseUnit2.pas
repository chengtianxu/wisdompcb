...
type
  ...
  Tgenesisreport = RECORD
      Padtocircuit:STRING;    //[最小线宽/线距]
      Circuittocircuit:STRING;//  [最小线宽/线距]
      NPTHtopad:STRING;//(是否需要二钻用 ≥7MIL)
      NPTHtocircuit:STRING;
      Lines2:STRING;//最小线宽
      Layer:STRING;
  ...
  TEditBaseForm2 = class(TBaseForm1)
    ...
    ADOQuery3: TADOQuery;
    BitBtn17: TBitBtn;
    qwzpr85_10MySPEC_RKEY: TStringField;
  ...
  private
    ...
    genesisreport2:ARRAY OF Tgenesisreport;
...
begin
 ...
 if (key=40) and        //如果是下箭头如果是最后一条记录
  (qwzpr85_10.RecNo=qwzpr85_10.RecordCount) then Key:=0;
 if key=45 then Key:=0; //insert键
 if (key=46) and (ssCtrl in shift) then Key:=0;//删除记录
  ...
  try
      if FindDATA0050Form2.ShowModal = mrok then
          begin
               ...
               DbEdit4.Field.Value := FindDATA0050Form2.ADOStoredProc1MANU_PART_NUMBER.Value ;//  Adoquery6.Fieldbyname('customer_part_number').asstring;
               Edit18.Text:=FindDATA0050Form2.ADOStoredProc1MANU_PART_DESC.Value ;//   Adoquery6.Fieldbyname('customer_part_desc').asstring;
               ...
               if qwzpr85_10.Locate('unit_ptr',dm.qwzpr85_24rkey.Value,[]) then
                    if qwzpr85_10unit_value.IsNull then
                        ...
                        begin
                            if dm.qwzpr85_24DATA_TYPE.Value = 1 then
                                begin
                                      if qwzpr85_10unit_value.Value = '' then
                                          sto := ' 0 '
                                      else
                                                ...
                                                begin
                                                    ...
                                                    with ADOQuery1 do
                                                        begin
                                                             ...
                                                             EXCEPT
                                                                 CLOSE ;
                                                                 SHOWMESSAGE('公式不正确!'+#13#10+ ' 公式为:'+ s1+#13#10+' 处理后结果:'+s);
                                                                 EXIT ;
                                                             END ;
                                                             if not IsEmpty then
                                                                 begin
...
procedure TEditBaseForm2.BitBtn17Click(Sender: TObject);
var
   mytextfile:textfile;
   s:string;
  ...
  if OpenDialog1.Execute then
      begin
          if OpenDialog1.FileName <>'' then
              begin
                   Open_File_name:=OpenDialog1.FileName;
                   assignfile(mytextfile,Open_File_name );
                   TRY
                       SETLENGTH(genesisreport2,0);
                       I2:= 0;
                       I3 := -1;
                       I4 := -1;
                       I5 := -1;
                       ...
                       while not eof(mytextfile) do
                           begin
                                Readln(mytextfile,s);
                                IF POS('Layer',S)>0 THEN
                                    BEGIN
                                       I2:= I2 + 1 ;
                                       SETLENGTH(genesisreport2,I2);
                                       genesisreport2[I2-1].Layer := S;
                                    END;
                                IF I3 >= 0 THEN
                                    BEGIN
                                        IF I3 = 3 THEN
                                            BEGIN
                                                ...
                                                //SHOWMESSAGE(S);
                                                I := POS('-',S);
                                                //SHOWMESSAGE(COPY(S,0,I-1));
                                                genesisreport2[I2-1].Padtocircuit := TRIM(COPY(S,0,I-1)) ;
                                            END
                                        ELSE
                                            BEGIN
                                                I3 := I3 + 1;
                                            END;
                                    END;
                                IF POS('Pad to circuit',S)>0 THEN
                                    BEGIN
                                        I3 := 0 ;
                                    END;
                                    ...
                                    BEGIN
                                        IF I4 = 3 THEN
                                            BEGIN
                                                ...
                                                I := POS('-',S);
                                                //SHOWMESSAGE(COPY(S,0,I-1));
                                                genesisreport2[I2-1].Circuittocircuit := TRIM(COPY(S,0,I-1)) ;
                                            END
                                        ELSE
                                            BEGIN
                                                I4 := I4 + 1;
                                            END;
                                    END;
                                IF POS('Circuit to circuit',S)>0 THEN
                                    BEGIN
                                        I4 := 0 ;
                                    END;
                                //---------
                                    ...
                                    BEGIN
                                        IF I5 = 3 THEN
                                            BEGIN
                                                ...
                                                I := POS('-',S);
                                                //SHOWMESSAGE(COPY(S,0,I-1));
                                                genesisreport2[I2-1].NPTHtopad := TRIM(COPY(S,0,I-1)) ;
                                            END
                                        ELSE
                                            BEGIN
                                                I5 := I5 + 1;
                                    ...
                                    BEGIN
                                        IF I6 = 3 THEN
                                            BEGIN
                                                ...
                                                I := POS('-',S);
                                                //SHOWMESSAGE(COPY(S,0,I-1));
                                                genesisreport2[I2-1].NPTHtocircuit := TRIM(COPY(S,0,I-1)) ;
                                            END
                                        ELSE
                                            BEGIN
                                                I6 := I6 + 1;
                                            END;
                                    END;
                                IF POS('NPTH to circuit',S)>0 THEN
                                    BEGIN
                                        I6 := 0 ;
                                    END;
                                //-----------
                                    ...
                                    BEGIN
                                        IF I7 = 3 THEN
                                            BEGIN
                                                ...
                                                I := POS('-',S);
                                                //SHOWMESSAGE(COPY(S,0,I-1));
                                                genesisreport2[I2-1].Lines2 := TRIM(COPY(S,0,I-1)) ;
                                            END
                                        ELSE
                                            BEGIN
                                                I7 := I7 + 1;
                       ...
                       {Memo1.Lines.Clear ;
                       FOR I := LOW(genesisreport2) TO HIGH(genesisreport2) DO
                            BEGIN
                                Memo1.Lines.Add(genesisreport2[I].Layer);
                                Memo1.Lines.Add(genesisreport2[I].Padtocircuit);
                                Memo1.Lines.Add(genesisreport2[I].Circuittocircuit );
                                Memo1.Lines.Add(genesisreport2[I].NPTHtopad );
                                Memo1.Lines.Add(genesisreport2[I].NPTHtocircuit);
                            END;  {}

                       if trim(genesisreport2[0].NPTHtopad)<>'' then
                           IF STRTOCURR(genesisreport2[0].NPTHtopad) >= NPTHtopad2 THEN
                               genesisreport2[0].NPTHtopad := 'Y';
                       if trim(genesisreport2[0].NPTHtocircuit)<>'' then
                           IF STRTOCURR(genesisreport2[0].NPTHtocircuit) >= NPTHtopad2 THEN
                               genesisreport2[0].NPTHtopad := 'Y';
                       FOR I := LOW(genesisreport2)+1 TO HIGH(genesisreport2) DO
                           BEGIN
                                IF genesisreport2[0].Padtocircuit <> genesisreport2[I].Padtocircuit THEN
                                    BEGIN
                                        if trim(genesisreport2[I].Padtocircuit)<>'' then
                                            begin
                                                if trim(genesisreport2[0].Padtocircuit) =''  then
                                                    genesisreport2[0].Padtocircuit := genesisreport2[I].Padtocircuit
                                                else
                                                    begin
                                                        B2 := TRUE;
                                                        B2 :=B2 AND (POS('.',genesisreport2[0].Padtocircuit) = 0) ;
                                                        B2 :=B2 AND (POS('.',genesisreport2[I].Padtocircuit) = 0) ;
                                                        IF B2 THEN
                                                            BEGIN
                                                                //SHOWMESSAGE( genesisreport2[I].NPTHtopad);
                                                                IF STRTOINT(genesisreport2[I].Padtocircuit) < STRTOINT(genesisreport2[0].Padtocircuit) THEN
                                                                    genesisreport2[0].Padtocircuit := genesisreport2[I].Padtocircuit;
                                                            END
                                                        ELSE
                                                            BEGIN
                                                                IF STRTOCURR(genesisreport2[I].Padtocircuit) < STRTOCURR(genesisreport2[0].Padtocircuit) THEN
                                                                    genesisreport2[0].Padtocircuit := genesisreport2[I].Padtocircuit;
                                                            END;
                                                    end;
                                            end;
                                    END;
                                if trim(genesisreport2[I].Circuittocircuit)<>'' then
                                    begin
                                          if trim(genesisreport2[0].Circuittocircuit)='' then
                                              genesisreport2[0].Circuittocircuit := genesisreport2[i].Circuittocircuit
                                          else
                                              begin
                                                  IF genesisreport2[0].Circuittocircuit <> genesisreport2[I].Circuittocircuit THEN
                                                      BEGIN
                                                          B2 := TRUE;
                                                          B2 :=B2 AND (POS('.',genesisreport2[0].Circuittocircuit) = 0) ;
                                                          B2 :=B2 AND (POS('.',genesisreport2[I].Circuittocircuit) = 0) ;
                                                          IF B2 THEN
                                                              BEGIN
                                                                  //SHOWMESSAGE( genesisreport2[I].NPTHtopad);
                                                                  IF STRTOINT(genesisreport2[I].Circuittocircuit) < STRTOINT(genesisreport2[0].Circuittocircuit) THEN
                                                                      genesisreport2[0].Circuittocircuit := genesisreport2[I].Circuittocircuit;
                                                              END
                                                          ELSE
                                                              BEGIN
                                                                  IF STRTOCURR(genesisreport2[I].Circuittocircuit) < STRTOCURR(genesisreport2[0].Circuittocircuit) THEN
                                                                      genesisreport2[0].Circuittocircuit := genesisreport2[I].Circuittocircuit;
                                                              END;
                                                      END;
                                              end ;
                                    end;
                                IF genesisreport2[0].NPTHtopad <> 'Y' THEN
                                    BEGIN
                                        if trim(genesisreport2[I].NPTHtopad)<>'' then
                                            IF STRTOCURR(genesisreport2[I].NPTHtopad) >= NPTHtopad2 THEN
                                                genesisreport2[0].NPTHtopad := 'Y';
                                         if trim(genesisreport2[I].NPTHtocircuit)<>'' then
                                            IF STRTOCURR(genesisreport2[I].NPTHtocircuit) >= NPTHtopad2 THEN
                                                genesisreport2[0].NPTHtopad := 'Y';
                                    END;
                                if trim(genesisreport2[i].Lines2)<>'' then
                                    begin
                                        if trim(genesisreport2[0].Lines2)='' then
                                            genesisreport2[0].Lines2 := genesisreport2[i].Lines2
                                        else
                                            begin
                                                IF genesisreport2[0].Lines2 <> genesisreport2[I].Lines2 THEN
                                                    BEGIN
                                                        B2 := TRUE;
                                                        B2 :=B2 AND (POS('.',genesisreport2[0].Lines2) = 0) ;
                                                        B2 :=B2 AND (POS('.',genesisreport2[I].Lines2) = 0) ;
                                                        IF B2 THEN
                                                            BEGIN
                                                                //SHOWMESSAGE( genesisreport2[I].NPTHtopad);
                                                                IF STRTOINT(genesisreport2[I].Lines2) < STRTOINT(genesisreport2[0].Lines2) THEN
                                                                    genesisreport2[0].Lines2 := genesisreport2[I].Lines2;
                                                            END
                                                        ELSE
                                                            BEGIN
                                                                IF STRTOCURR(genesisreport2[I].Lines2) < STRTOCURR(genesisreport2[0].Lines2) THEN
                                                                    genesisreport2[0].Lines2 := genesisreport2[I].Lines2;
                                                            END;
                                                    END;
                                             end;
                      ...
                      IF B2 THEN
                          BEGIN
                              ...
                              if trim(genesisreport2[0].Padtocircuit)<>'' then
                                  if trim(genesisreport2[0].Circuittocircuit)='' then
                                      genesisreport2[0].Circuittocircuit:= genesisreport2[0].Padtocircuit
                                  else
                                      begin
                                          IF STRTOINT(genesisreport2[0].Padtocircuit) < STRTOINT(genesisreport2[0].Circuittocircuit) THEN
                                              genesisreport2[0].Circuittocircuit := genesisreport2[0].Padtocircuit;
                                      end;
                          END;
                       //此处查找参数更新为新的值　

                      { Memo1.Lines.Add(genesisreport2[0].Layer);
                       Memo1.Lines.Add(genesisreport2[0].Padtocircuit);
                       Memo1.Lines.Add(genesisreport2[0].Circuittocircuit );
                       Memo1.Lines.Add(genesisreport2[0].NPTHtopad );
                       {}
                       qwzpr85_10.First ;
                       WHILE NOT qwzpr85_10.Eof DO
                          BEGIN
                              IF qwzpr85_10MySPEC_RKEY.Value ='BA' THEN
                                  BEGIN
                                      //genesisreport2[0].Circuittocircuit
                                      qwzpr85_10.Edit ;
                                      qwzpr85_10UNIT_VALUE.Value :=  genesisreport2[0].Circuittocircuit;
                                      //showmessage(genesisreport2[0].Circuittocircuit);
                                  END
                              ELSE IF qwzpr85_10MySPEC_RKEY.Value ='BB' THEN
                                  BEGIN
                                      //genesisreport2[0].Circuittocircuit
                                      qwzpr85_10.Edit ;
                                      qwzpr85_10UNIT_VALUE.Value :=  genesisreport2[0].Lines2;
                                      //showmessage(genesisreport2[0].Circuittocircuit);
                                  END
                              ELSE IF qwzpr85_10MySPEC_RKEY.Value ='BC' THEN
                                  BEGIN
                                      //genesisreport2[0].Circuittocircuit
                                      qwzpr85_10.Edit ;
                                      qwzpr85_10UNIT_VALUE.Value :=  genesisreport2[0].NPTHtopad ;
                                      //showmessage(genesisreport2[0].Circuittocircuit);
                                  END;

                              //showmessage(qwzpr85_10MySPEC_RKEY.Value);
                              qwzpr85_10.Next ;

                          END; {}
                       {qwzpr85_10.First ;
                       IF qwzpr85_10.Locate('MySPEC_RKEY','BA',[]) THEN
                            showmessage(genesisreport2[0].Layer);
                       {}
...
procedure TEditBaseForm2.DBGridEh2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
 if (key=40) and        //如果是下箭头如果是最后一条记录
  (dm.qwzpr85_23.RecNo=dm.qwzpr85_23.RecordCount) then Key:=0;
 if key=45 then Key:=0; //insert键
 if (key=46) and (ssCtrl in shift) then Key:=0;//删除记录
end;
