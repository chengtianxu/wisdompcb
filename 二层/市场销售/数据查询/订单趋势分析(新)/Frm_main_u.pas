...
uses common,Frm_detail_u,Pick_Item_Single,ConstVar,UTogather;
...
var i:integer;
    select,Lsum,where,group,order:string;
    Ltype:string;
...
begin
 try
   screen.Cursor:=crappstart;
    ...
    where:=' WHERE 1=1  and data0060.exch_rate>0 ';//nd data0060.exch_rate 20180305 add  by tao
  ...
  begin
   ...
   7:
   begin
     select:='select data0025.CPJS as code, null as name,';
     group:='GROUP BY data0025.CPJS ';
     order:='ORDER BY Data0025.CPJS';
   end;
   end;
   
   if (RadioGroup1.ItemIndex<6) or (RadioGroup1.ItemIndex=7) then     //当用户按产品汇总时，增加了字段关联原客户
   select:=select+'sum(data0060.PARTS_ORDERED) Qty,sum(data0060.PARTS_ORDERED*data0025.unit_sq) Area,'+
        'round(sum((data0060.PARTS_ORDERED*data0060.parts_alloc+'+#13+
   'case when data0060.tax_in_price=''Y'' then (Data0060.TOTAL_ADD_L_PRICE/(1+data0060.RUSH_CHARGE*0.01))'+#13+
       'else Data0060.TOTAL_ADD_L_PRICE   end '+#13+
       ')/data0060.exch_rate),2) NotaxMount,'+#13+
       'round(sum(((data0060.PARTS_ORDERED*data0060.parts_alloc)*(1+data0060.RUSH_CHARGE*0.01)+'+#13+
       'case when data0060.tax_in_price=''Y'' then  Data0060.TOTAL_ADD_L_PRICE'+#13+
       'else (Data0060.TOTAL_ADD_L_PRICE*(1+data0060.RUSH_CHARGE*0.01))  end'+#13+
       ')/data0060.exch_rate),2) taxMount'+#13+
   ...
   select:=select+'sum(data0060.PARTS_ORDERED) Qty,sum(data0060.PARTS_ORDERED*data0025.unit_sq) Area,'+
        'round(sum((data0060.PARTS_ORDERED*data0060.parts_alloc+data0060.TOTAL_ADD_L_PRICE)/data0060.exch_rate),2) NotaxMount,'+
        'round(sum((data0060.PARTS_ORDERED*data0060.parts_alloc+data0060.TOTAL_ADD_L_PRICE)*(1+RUSH_CHARGE*0.01)/data0060.exch_rate),2) taxMount,'+
    ...
    Frm_detail.PageControl1.ActivePageIndex:=0;
    Frm_detail.ADOQuery1.close;
    Frm_detail.ADOQuery1.sql.clear;
    Frm_detail.ADOQuery1.sql.text:=select+where+' '+#13+group+' '+#13+order;
//    ShowMessage(Frm_detail.ADOQuery1.sql.text);
  ...
  else if rg1.itemindex=1  then   ///////////汇总///////////////////////////////////////////////////////
  begin
   try
    ...
    Lsum:='sum(data0060.PARTS_ORDERED) 数量,round(sum(data0060.PARTS_ORDERED*data0025.unit_sq),2)面积,'+
        'round(sum((data0060.PARTS_ORDERED*data0060.parts_alloc+'+#13+
   'case when data0060.tax_in_price=''Y'' then (Data0060.TOTAL_ADD_L_PRICE/(1+data0060.RUSH_CHARGE*0.01))'+#13+
       'else Data0060.TOTAL_ADD_L_PRICE   end '+#13+
       ')/data0060.exch_rate),2) 不含税金额,'+#13+
       'round(sum(((data0060.PARTS_ORDERED*data0060.parts_alloc)*(1+data0060.RUSH_CHARGE*0.01)+'+#13+
       'case when data0060.tax_in_price=''Y'' then  Data0060.TOTAL_ADD_L_PRICE'+#13+
       'else (Data0060.TOTAL_ADD_L_PRICE*(1+data0060.RUSH_CHARGE*0.01))  end'+#13+
       ')/data0060.exch_rate),2) 含税金额'+#13+
        'FROM dbo.Data0060 INNER JOIN '+#13+
        '     dbo.Data0025 ON dbo.Data0060.CUST_PART_PTR = dbo.Data0025.RKEY INNER JOIN '+#13+
        '     dbo.Data0010 ON dbo.Data0060.CUSTOMER_PTR = dbo.Data0010.RKEY INNER JOIN '+#13+
        '     dbo.Data0008 ON dbo.Data0025.PROD_CODE_PTR = dbo.Data0008.RKEY INNER JOIN '+#13+
   ...
   case cbb1.itemindex  of
    0:
    begin
     Lstr1:='convert(char(10),data0060.ent_date,120) as 日期';
     Lstr11:='convert(char(10),data0060.ent_date,120)' ;
    end ;
    1:
    begin
     Lstr1:='convert(char(7),data0060.ent_date,120) as 月份';
     Lstr11:='convert(char(7),data0060.ent_date,120)';
    end;
    2:
    begin
     Lstr1:='data0007.PRODUCT_GROUP_NAME as 产品组别' ;
     Lstr11:='data0007.PRODUCT_GROUP_NAME';
    end ;
    3:
    begin
     Lstr1:='data0005.EMPLOYEE_NAME as 业务员' ;
     Lstr11:='data0005.EMPLOYEE_NAME';
    end;
    4:
    begin
     Lstr1:='data0008.PRODUCT_NAME  as 产品类型';
     Lstr11:='data0008.PRODUCT_NAME' ;
    end;
    5:
    begin
     Lstr1:='data0010.ABBR_NAME as 客户';
     Lstr11:='data0010.ABBR_NAME';
    end;
    6:
    begin
     Lstr1:='data0025.MANU_PART_NUMBER as 本厂编号 ';
     Lstr11:='data0025.MANU_PART_NUMBER';
    end;
    7:
    begin
     Lstr1:='data0025.CPJS as 产品阶数 ';
     Lstr11:='data0025.CPJS';
    end;
   end;
    ...
    0:
    begin
     Lstr2:='convert(char(10),data0060.ent_date,120) as 日期';
     Lstr22:='convert(char(10),data0060.ent_date,120)';
    end;
    1:
    begin
     Lstr2:='convert(char(7),data0060.ent_date,120) as 月份';
     Lstr22:='convert(char(7),data0060.ent_date,120)';
    end;
    2:
    begin
     Lstr2:='data0007.PRODUCT_GROUP_NAME as 产品组别' ;
     Lstr22:=' data0007.PRODUCT_GROUP_NAME';
    end;
    3:
    begin
     Lstr2:='data0005.EMPLOYEE_NAME as 业务员' ;
     Lstr22:=' data0005.EMPLOYEE_NAME';
    end;
    4:
    begin
     Lstr2:=' data0008.PRODUCT_NAME  as 产品类型';
     Lstr22:=' data0008.PRODUCT_NAME';
    end;
    5:
    begin
     Lstr2:='data0010.ABBR_NAME as 客户';
     Lstr22:='data0010.ABBR_NAME';
    end;
    6:
    begin
     Lstr2:='data0025.MANU_PART_NUMBER as 本厂编号' ;
     Lstr22:=' data0025.MANU_PART_NUMBER';
    end;
    7:
    begin
     Lstr2:='data0025.CPJS as 产品阶数' ;
     Lstr22:=' data0025.CPJS';
    end;
   end;
