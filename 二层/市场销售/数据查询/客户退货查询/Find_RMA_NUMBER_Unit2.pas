...
type
  Tfind_RMA_Number_Form2 = class(TfindForm1)
    ADOStoredProc1RMA_NUMBER: TStringField;
...
var
  find_RMA_Number_Form2: Tfind_RMA_Number_Form2;

implementation

{$R *.dfm}

procedure Tfind_RMA_Number_Form2.Edit1Change(Sender: TObject);
begin//此事件每次都要改动
  inherited;
  self.EditDataChange(sender,ADOStoredProc1,'RMA_NUMBER');

end;

procedure Tfind_RMA_Number_Form2.Temp_DBGridEh1TitleBtnClick(
...
procedure Tfind_RMA_Number_Form2.Button1Click(Sender: TObject);
begin
  if ADOStoredProc1.RecordCount >0 then
     inherited;
end;

procedure Tfind_RMA_Number_Form2.Temp_DBGridEh1DblClick(Sender: TObject);
begin
  inherited;
  if ADOStoredProc1.RecordCount >0 then
     Button1Click(sender);
