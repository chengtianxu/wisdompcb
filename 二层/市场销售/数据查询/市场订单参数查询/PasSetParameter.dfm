object FrmSetParameter: TFrmSetParameter
  Left = 509
  Top = 180
  BorderStyle = bsDialog
  Caption = #35746#21333#21442#25968#35774#32622
  ClientHeight = 493
  ClientWidth = 408
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -13
  Font.Name = #23435#20307
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object DBGridEh1: TDBGridEh
    Left = 0
    Top = 173
    Width = 408
    Height = 320
    Align = alClient
    DataSource = DataSource1
    FooterColor = clWindow
    FooterFont.Charset = DEFAULT_CHARSET
    FooterFont.Color = clWindowText
    FooterFont.Height = -13
    FooterFont.Name = #23435#20307
    FooterFont.Style = []
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgCancelOnExit]
    ParentShowHint = False
    PopupMenu = PopupMenu1
    ShowHint = False
    TabOrder = 0
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -13
    TitleFont.Name = #23435#20307
    TitleFont.Style = []
    Columns = <
      item
        EditButtons = <>
        FieldName = 'iSort'
        Footers = <>
        Title.Alignment = taCenter
        Title.Caption = #25490#21015#21495
        Width = 40
      end
      item
        EditButtons = <>
        FieldName = 'DEPT_NAME'
        Footers = <>
        Title.Alignment = taCenter
        Title.Caption = #24037#24207
        Width = 90
      end
      item
        EditButtons = <>
        FieldName = 'PARAMETER_NAME'
        Footers = <>
        Title.Alignment = taCenter
        Title.Caption = #21442#25968#21517#31216
        Width = 100
      end
      item
        EditButtons = <>
        FieldName = 'ColName'
        Footers = <>
        Title.Alignment = taCenter
        Title.Caption = #21015#21517#26631#39064
        Width = 100
      end>
  end
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 408
    Height = 41
    Align = alTop
    TabOrder = 1
    object BitBtn5: TBitBtn
      Left = 3
      Top = 4
      Width = 70
      Height = 34
      Caption = #20851#38381
      ModalResult = 2
      TabOrder = 0
      Glyph.Data = {
        
        0000}
      NumGlyphs = 2
    end
    object BitBtn2: TBitBtn
      Left = 73
      Top = 4
      Width = 70
      Height = 34
      Caption = #20445#23384
      TabOrder = 1
      OnClick = BitBtn2Click
      Glyph.Data = {
        
        BF05BF05BF05BF05BF09}
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 41
    Width = 408
    Height = 132
    Align = alTop
    TabOrder = 2
    object Label1: TLabel
      Left = 19
      Top = 75
      Width = 52
      Height = 13
      Caption = #21046#25104#21442#25968
    end
    object Label3: TLabel
      Left = 19
      Top = 47
      Width = 52
      Height = 13
      Caption = #24037#24207#20195#30721
    end
    object labName: TLabel
      Left = 228
      Top = 48
      Width = 7
      Height = 13
    end
    object labUSER_LOGIN_NAME: TLabel
      Left = 229
      Top = 76
      Width = 7
      Height = 13
    end
    object Label2: TLabel
      Left = 19
      Top = 105
      Width = 52
      Height = 13
      Caption = #21015#21517#26631#39064
    end
    object Label4: TLabel
      Left = 19
      Top = 16
      Width = 52
      Height = 13
      Caption = #25490#21015#32534#21495
    end
    object Edit1: TEdit
      Left = 74
      Top = 71
      Width = 126
      Height = 21
      Enabled = False
      TabOrder = 0
    end
    object Edit2: TEdit
      Left = 74
      Top = 42
      Width = 126
      Height = 21
      Enabled = False
      TabOrder = 1
    end
    object BitBtn3: TBitBtn
      Left = 198
      Top = 40
      Width = 25
      Height = 25
      TabOrder = 2
      TabStop = False
      OnClick = BitBtn3Click
      Glyph.Data = {
        
        888888800088800088888888888888888888}
    end
    object BitBtn1: TBitBtn
      Left = 198
      Top = 70
      Width = 25
      Height = 25
      TabOrder = 3
      TabStop = False
      OnClick = BitBtn1Click
      Glyph.Data = {
        
        888888800088800088888888888888888888}
    end
    object Edit3: TEdit
      Left = 74
      Top = 100
      Width = 267
      Height = 21
      TabOrder = 4
    end
    object Edit4: TEdit
      Left = 74
      Top = 13
      Width = 121
      Height = 21
      TabOrder = 5
      OnKeyPress = Edit4KeyPress
    end
  end
  object ads57: TADODataSet
    Connection = DM.ADOConnection1
    CursorType = ctStatic
    Filtered = True
    CommandText = 
      'select data0591.*,data0034.DEPT_CODE,data0034.DEPT_NAME,data0278' +
      '.PARAMETER_NAME,data0278.PARAMETER_DESC'#13#10'from data0591 inner joi' +
      'n data0034'#13#10'on data0591.d34Rkey=data0034.Rkey inner join'#13#10'dbo.da' +
      'ta0278 on data0591.d278Rkey=dbo.data0278.Rkey'#13#10
    Parameters = <>
    Left = 40
    Top = 240
    object ads57iSort: TIntegerField
      FieldName = 'iSort'
    end
    object ads57d34Rkey: TIntegerField
      FieldName = 'd34Rkey'
    end
    object ads57d278Rkey: TIntegerField
      FieldName = 'd278Rkey'
    end
    object ads57ColName: TStringField
      FieldName = 'ColName'
      Size = 30
    end
    object ads57DEPT_CODE: TStringField
      FieldName = 'DEPT_CODE'
      Size = 10
    end
    object ads57DEPT_NAME: TStringField
      FieldName = 'DEPT_NAME'
      Size = 30
    end
    object ads57PARAMETER_NAME: TStringField
      FieldName = 'PARAMETER_NAME'
      Size = 10
    end
    object ads57PARAMETER_DESC: TStringField
      FieldName = 'PARAMETER_DESC'
    end
  end
  object DataSource1: TDataSource
    DataSet = ads57
    Left = 104
    Top = 240
  end
  object PopupMenu1: TPopupMenu
    Left = 208
    Top = 216
    object N1: TMenuItem
      Caption = #21024#38500
      OnClick = N1Click
    end
  end
end
