...
uses
  ...
  ComObj,math, DB, ComCtrls, DBClient,DateUtils;
...
begin
 ...
 { dm.Conn.Close;
  dm.Conn.ConnectionString:='Provider=SQLOLEDB.1;Password=wisdomtopcb1077;Persist Security Info=True;User ID=wzsa;Initial Catalog=sj_v20_live;Data Source=172.16.68.8';
  dm.Conn.Open;}
...
begin
  ...
  begin
    ...
    L_NoPermissionFields:=vararrayOf(['curr_code','exch_rate','part_price','RUSH_CHARGE',
  ...
  dsysDate:=getsystem_date(dm.aqtmp,1);
end;
...
begin
  if DM.DS60.FieldByName('SampleYN').Value = 'Y' then
    ...
    if ((Trunc(dm.DS60.FieldByName('ORIG_SCHED_SHIP_DATE').AsDateTime)-Trunc(dsysDate))<=5) then
//  if (DaysBetween(dsysDate,dm.DS60.FieldByName('ORIG_SCHED_SHIP_DATE').AsDateTime)<=5) then
...
begin
  ...
  begin
    dm.aqTotal.Filter := 'MANU_PART_NUMBER like ''%'+trim(edt1.text)+'%''';
    if not DM.aqTotal.Filtered then DM.aqTotal.Filtered := True;
  end
  else dm.DS60.Filter :='';
end;
