  ...
  while not dm.ADO70.Eof do //+++++++++++++++++++++++++++++++++++++++++++++
   begin
    ...
    while not dm.ADO200.Eof do
     begin
      ...
      dm.ADO200.Post;
      if dm.ADO70.RecNo=dm.ADO70RKEY.Value then dm.ADO200.Next;
     end;
   ...
   if dbgrid1.Visible = true then
    ...
    //   self.update_data0028();  //更新交付排程序
       ...
       if dm.ADO70.RecNo=dm.ADO70RKEY.Value then dm.ADO71.Next;
      end;
    end
   else
    begin
     ...
     while not dm.ADO72.Eof do
      begin
       ...
       dm.ADO72.Post;
       if dm.ADO70.RecNo=dm.ADO70RKEY.Value then dm.ADO72.Next;
      end;
    end;
  ...
  if form9.ShowModal=mrok then
   begin
    ...
    dm.ADO70SUB_TOTAL.AsString :=formatfloat('0.00',
                  dm.ADO70SUB_TOTAL.Value-v_total+
                  dm.ADO71taxprice_total.Value);
...
try
 ...
 if form2.ShowModal = mrok then
  begin
   if strtoint(form2.rkey01.Caption)=dm.ADO70CURRENCY_PTR.Value then
...
begin
 ...
 dbedit7.Field.Value := dbedit7.Field.Value-dm.ADO71taxprice_total.Value;
    //总金额合计
// dm.ADO70DISCOUNT_AMOUNT.Value :=dm.ADO70DISCOUNT_AMOUNT.Value-dm.ADO71price_discount.Value;
    //价格折扣合计
 dm.ADO71.Delete;
end;
...
for i:=1 to strtoint(form2.edit4.Text) do  //多次放记录建立
 begin
  ...
  dm.ADO71avl_flag.Value := form2.po_avlflag.Caption;  //是否需要IQC检查
...
//   dm.ADO70CSI_USER_POINTER.AsString := rkey73; //1系统用户
   ...
   with dm.ADOQuery1 do
    ...
    with dm.ADOQuery1 do
     begin
      active:=false;
      sql.Clear;
      sql.Add('select exch_rate from data0001');
      sql.Add('where rkey='+form2.rkey01.Caption);
      active:=true;
      dm.ADO70EXCHANGE_RATE.Value := fieldvalues['exch_rate'];   //.20货币汇率
     end;
...
procedure tform4.add_0072();     //手工新增杂项采购
var
 i:byte;
 v_sim_total,v_tax_total:single;
begin
...
try
 ...
 if form10.ShowModal=mrok then
  begin
   ...
   dbedit7.Field.Value := dbedit7.Field.Value-v_total+
...
//==============================================================================
 while not dm.ADO70.Eof do  //检查新建PO号与旧号是否有重复
  begin
   {if trim(dm.ADO70fob.Value)='' then
    begin
     messagedlg('第'+inttostr(dm.ADO70.RecNo)+'条请购单号不能为空',mterror,[mbcancel],0);
...
//   dm.ADO70CSI_USER_POINTER.AsString := rkey73;  //1系统用户
   ...
   if (form_req.ADOQuery1FLAG.value = 'S') or (form_req.ADOQuery1FLAG.value = 'T') then
    begin
     dm.ADO70SUPPLIER_POINTER.Value := dm.ADO0069SUPP_PTR.Value;   //3供应商
     dm.ADO70CURRENCY_PTR.Value := dm.ADO0069PO_LINK_PTR.Value;    //5货币
     dm.ADO70EXCHANGE_RATE.Value := dm.ado0069exch_rate.Value;     //.20货币汇率
    ...
    begin
     ...
     dm.ADO70EXCHANGE_RATE.Value := dm.ADO0204exch_rate.Value;  //.20货币汇率
   ...
   with dm.ADOQuery1 do//复制装运地址中的值
    begin
     ...
     dm.ADO70.Post;
     if (dm.ADO70PO_TYPE.Value='S') or (dm.ADO70PO_TYPE.Value='T') then
      dm.ADO71.Filter := 'po_ptr= '+inttostr(dm.ADO70.RecNo)
     else
      dm.ADO72.Filter := 'poptr= '+inttostr(dm.ADO70.RecNo);
      self.change_seed_value(trim(dm.ADO70PO_NUMBER.Value));   //改变data0015中PO编号的初始值
...
begin
 ...
 dm.ado71avl_flag.value := dm.ado0069avl_flag.value;   //是否认可供应商
...
begin
 ...
 if (dm.ADO28tax_flag.Value='N')  //如果交付排程不含税
    and (dm.ADO71PRICE.Value <> dm.ADO28PRICE_1.Value) then
  begin
   dm.ADO28.Edit;
  ...
  if (dm.ADO28tax_flag.Value='Y')
     and (dm.ADO71tax_price.Value <> dm.ADO28PRICE_1.Value) then
   begin
    dm.ADO28.Edit;
    dm.ADO28PRICE_1.Value := dm.ADO71tax_price.Value;
    dm.ADO28.Post;
   end
