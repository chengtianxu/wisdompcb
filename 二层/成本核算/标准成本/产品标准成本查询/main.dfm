object frmMain: TfrmMain
  Left = 289
  Top = 229
  Width = 989
  Height = 442
  Caption = #20135#21697#25104#26412#26597#35810
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -13
  Font.Name = #23435#20307
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  WindowState = wsMaximized
  OnCreate = FormCreate
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 973
    Height = 41
    Align = alTop
    TabOrder = 0
    DesignSize = (
      973
      41)
    object BitBtn1: TBitBtn
      Left = 0
      Top = 2
      Width = 57
      Height = 30
      Hint = #36864#20986#31995#32479
      Caption = #36864#20986
      ParentShowHint = False
      ShowHint = True
      TabOrder = 0
      OnClick = BitBtn1Click
      Glyph.Data = {
        
        0000}
      NumGlyphs = 2
    end
    object BitBtn2: TBitBtn
      Left = 57
      Top = 2
      Width = 57
      Height = 30
      Hint = #23548#20986#25968#25454#21040'Excel'
      Caption = #23548#20986
      ParentShowHint = False
      ShowHint = True
      TabOrder = 1
      OnClick = BitBtn2Click
      Glyph.Data = {
        
        70DA0FFFF0ADA0EEEE0D00000ADADA00000AADADADADADADADAD}
    end
    object BitBtn4: TBitBtn
      Left = 114
      Top = 2
      Width = 57
      Height = 30
      Hint = #37325#26032#23450#20041#26597#35810#26465#20214
      Caption = #26597#35810
      ParentShowHint = False
      ShowHint = True
      TabOrder = 2
      OnClick = BitBtn4Click
      Glyph.Data = {
        
        861D8680775398535355}
    end
    object CheckBox1: TCheckBox
      Left = 829
      Top = 8
      Width = 113
      Height = 17
      Anchors = [akTop, akRight]
      Caption = #21015#23485#33258#21160#35843#33410
      TabOrder = 3
      OnClick = CheckBox1Click
    end
  end
  object StringGrid1: TStringGrid
    Left = 0
    Top = 41
    Width = 973
    Height = 362
    Align = alClient
    ColCount = 9
    FixedCols = 7
    RowCount = 3
    FixedRows = 2
    TabOrder = 1
    OnKeyDown = StringGrid1KeyDown
    ColWidths = (
      88
      90
      56
      71
      63
      82
      64
      68
      64)
  end
  object ADOConnection1: TADOConnection
    CommandTimeout = 200
    ConnectionTimeout = 50
    KeepConnection = False
    LoginPrompt = False
    Provider = 'F:\'#28145#22323#27491#24335'.udl'
    Left = 120
    Top = 136
  end
  object adsMain: TADODataSet
    Connection = ADOConnection1
    LockType = ltReadOnly
    CommandText = 
      'SELECT D25.MANU_PART_NUMBER, D25.MANU_PART_DESC, D10.CUST_CODE, ' +
      #13#10'      D10.ABBR_NAME, D08.PROD_CODE, D08.PRODUCT_NAME, D25.RKEY' +
      #13#10'FROM dbo.Data0025 D25 INNER JOIN'#13#10'      dbo.Data0060 D60 ON D2' +
      '5.RKEY = D60.CUST_PART_PTR INNER JOIN'#13#10'      dbo.Data0064 D64 ON' +
      ' D60.RKEY = D64.SO_PTR INNER JOIN'#13#10'      dbo.Data0008 D08 ON D25' +
      '.PROD_CODE_PTR = D08.RKEY INNER JOIN'#13#10'      dbo.Data0010 D10 ON ' +
      'D25.CUSTOMER_PTR = D10.RKEY'#13#10'GROUP BY D25.MANU_PART_NUMBER, D25.' +
      'MANU_PART_DESC, D10.CUST_CODE, '#13#10'      D10.ABBR_NAME, D08.PROD_C' +
      'ODE, D08.PRODUCT_NAME, D25.RKEY'
    CommandTimeout = 200
    Parameters = <>
    Prepared = True
    Left = 400
    Top = 176
  end
  object adsDept: TADODataSet
    Connection = ADOConnection1
    LockType = ltReadOnly
    CommandText = 
      'select D.*,data0034.dept_code,data0034.dept_name,data0034.seq_nr' +
      ' from ('#13#10'select dept_ptr from data0460 '#13#10'where bom_ptr in (618,8' +
      '95,7151)'#13#10'group by data0460.dept_ptr) D'#13#10'inner join data0034 on ' +
      'D.dept_ptr=data0034.rkey'#13#10'where data0034.is_cost_dept=1'#13#10'order b' +
      'y data0034.seq_nr'#13#10
    CommandTimeout = 200
    Parameters = <>
    Prepared = True
    Left = 664
    Top = 176
  end
  object adsProdCost: TADODataSet
    Connection = ADOConnection1
    LockType = ltReadOnly
    CommandText = 
      'select  Bom_ptr,dept_ptr,IsNUll(MATL_PER_SQFT_1,0) + IsNULL(MATL' +
      '_PER_SQFT_2,0) +IsNULL(Sub_Producing_Cost,0) + IsNULL(Sub_Matl_C' +
      'ost,0) + IsNULL(OVERHEAD_PER_SQFT,0) +IsNULL(OVERHEAD2_PER_SQFT,' +
      '0) + IsNULL(OVERHEAD3_PER_SQFT,0) +'#13#10'IsNULL(Sub_Producing_Cost,0' +
      ')'#13#10'from data0460 '
    CommandTimeout = 200
    Parameters = <>
    Prepared = True
    Left = 248
    Top = 216
  end
  object AQTMP: TADOQuery
    Connection = ADOConnection1
    LockType = ltReadOnly
    CommandTimeout = 200
    Parameters = <>
    Prepared = True
    Left = 360
    Top = 320
  end
  object adsMoreInfo: TADODataSet
    Connection = ADOConnection1
    CursorType = ctStatic
    Parameters = <>
    Left = 520
    Top = 248
  end
end
