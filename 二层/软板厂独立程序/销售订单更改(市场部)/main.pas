...
uses damo,  note, detail, cust_search,common,  
...
begin
  ...
  begin
    ...
    dm.ADO60.Close;
    dm.ADO60.Parameters[0].Value:=strtodate(v_date)-15;
    dm.ADO60.Parameters[1].Value:=strtodate(v_date)+1;
    dm.ADO60.Open;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          sql.add(''''+v_date+''',305,'''+trim(form4.Edit1.Text)+''')');
          ExecSQL;
        end;
      end;
...
//2018-1-15tang更新53成本价格
      ...
      begin
        ...
        begin
         ...
         'update data0053 '+#13+
         'set cost_pcs='+#13+
         'Data0060.PARTS_ALLOC*isnull(Data0060.TAX_2,0)*0.01/Data0060.EXCH_RATE '+#13+
...
//**************************************************************开始更新订单价格明细
      ...
      begin
        if (StrToFloat(Form4.StringGrid3.Cells[1,i])>0) then
        begin
          dm.adoS578.Append;
          dm.adoS578.FieldByName('d579_Ptr').AsString:=Form4.StringGrid3.Cells[2,i];
          dm.adoS578.Fieldvalues['d60_Ptr']:=DM.ADO60RKEY.AsInteger;
          dm.adoS578.FieldByName('Price').AsString:=Form4.StringGrid3.Cells[1,i];
          dm.adoS578.Post;
        end;
      end;
//end tang2013-10-16----------------------
  ...
  begin
    ...
    try
      ...
      begin
        ...
        sql.add(''''+v_date+''',305,'''+trim(form6.Edit3.Text)+''')');
        ExecSQL;
      end;
      ...
      with dm.ADOQuery4 do     //打开业务员子表
      begin
        close;
        sql.Clear;
        sql.Add('select * from data0102');
        sql.Add('where SALES_ORDER_PTR='+dm.ADO60rkey.AsString);
        open;
      end;
      dm.Append60.Open;
      dm.Append102.Open;
      ...
      begin
        ...
        for i:=2 to dm.ADOtemp60.FieldCount-1 do
         ...
         // dm.Append60.FieldValues['PROD_REL']:=2;
         //   dm.ADO60.Edit;
         //   DM.ADO60TO_BE_PLANNED.Value:=DM.ADO60PARTS_ORDERED.AsInteger;
         //  dm.ADO60.Post;
           end
...
//        if (dm.ADO60prod_rel.Value <> '3') then  //从未投产拆分,新订单允许再投
//          dm.append60.FieldValues['TO_BE_PLANNED']:=dm.Append60.FieldValues['PARTS_ORDERED']
//        else
//          dm.append60.FieldValues['TO_BE_PLANNED']:=0;
  ...
  //   dm.ADOtemp60.Close;
       dm.ADOQuery4.First;
       while not dm.ADOQuery4.Eof do   //新增业务员
        begin
         dm.Append102.Append;
         for i:=2 to 4 do
          dm.Append102.FieldValues[dm.Append102.Fields[i].FieldName] :=
                      dm.ADOQuery4.FieldValues[dm.Append102.Fields[i].FieldName];
         dm.Append102.FieldValues['SALES_ORDER_PTR']:=dm.Append60.FieldValues['rkey'];
         dm.Append102.Post;
         dm.ADOQuery4.Next;
        end;
        dm.ADOQuery2.First;
    ...
    // showmessage('订单拆分不成功,请检查订单编号是否重复,或者请与管理员联系!');
    on E: Exception do
     begin
...
//end 20161219tang
  ...
  begin
    ...
    begin
      ...
      if FieldValues['PARTS_SHIPPED']>0 then
      begin
        ShowMessage('此订单已指派，不能修改客户代码/客户订单号');
        Exit;
      end;
    end;
...
for i:=low(v_colwidth) to high(v_colwidth) do
  dbgrid1.Columns[i].Width:=v_colwidth[i]+round((dbgrid1.Width-820)*v_colwidth[i]/800);
end;
...
begin
  ...
  begin
    ...
    sql.Add('where TOTAL_ADD_L_PRICE <> 0');
