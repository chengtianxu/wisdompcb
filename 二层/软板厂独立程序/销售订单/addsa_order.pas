...
uses
  ...
  Dialogs, StdCtrls, Buttons, ComCtrls, Mask, Grids, Menus, DBGrids,
   ActnList, DBCtrls, DB, ADODB, ExtCtrls;
...
type
  TForm2 = class(TForm)
   ...
   // procedure conf_noteClick(Sender: TObject);
    ...
    function find_socongfu(rkey97,rkey50:integer;org_sch_date:string):boolean;
...
begin
 ...
 if not quote_flag then       //不与报价单关联
  begin
   ...
   form3.Edit18.Text:=formatfloat('0.0000000',strtofloat(form3.edit3.Text)*self.set_qty);  //价格set
  end;
...
begin
 ...
 stringgrid1.Cells[6,stringgrid1.RowCount-1]:=form3.edit6.Text;  //优先顺序
 stringgrid1.Cells[7,stringgrid1.RowCount-1]:=form3.maskedit1.Text;
 stringgrid1.Cells[8,stringgrid1.RowCount-1]:=form3.maskedit2.Text;
 ...
 if frm_cust.ShowModal=mrok then
  begin
   ...
   with dm.ADOQuery2 do //货币
    begin
     if active=true then active:=false;
      ...
      edit9.Text:=formatfloat('0.000000',fieldvalues['exch_rate']);
   ...
   with dm.ADOQuery2 do     //查找装运地点;
    ...
    if frm_cust.ADOQuery1.FieldValues['INTERNAL_ECN_COUNT']<>null then   //客户ecn
     label40.Caption:=frm_cust.ADOQuery1.Fieldbyname('INTERNAL_ECN_COUNT').AsString;
    if frm_cust.ADOQuery1.FieldValues['EXTERNAL_ECN_COUNT']<>null then
     label41.Caption:=frm_cust.ADOQuery1.Fieldbyname('EXTERNAL_ECN_COUNT').AsString;
                                              //工程更改指示
...
//  frm_cust.ADOQuery1.Open;
 ...
 if comparetext(trim(edit3.text),trim(dm.ADOQuery3.Fieldbyname('cust_code').AsString))=0 then

  if not dm.ADOQuery3.IsEmpty then
  begin
//   edit3.Font.Color:=clwindowtext;
//   label33.Caption := frm_cust.ADOQuery1.FieldValues['customer_name'];
//   lab_rkey10.Caption := frm_cust.ADOQuery1.Fieldbyname('rkey').asstring;
//   lab_rkey01.Caption := frm_cust.ADOQuery1.fieldbyname('currency_ptr').AsString;
//   label36.Caption := frm_cust.ADOQuery1.fieldbyname('cod_flag').AsString; //货到付款
//   label66.Caption := frm_cust.ADOQuery1.fieldbyname('FED_TAX_ID_NO').AsString; //付款方式
//   credit_rating := frm_cust.ADOQuery1.fieldbyname('credit_rating').AsInteger; //信用等级
//   credit_limit := frm_cust.ADOQuery1.fieldbyname('credit_limit').AsFloat; //信用限额
//
...
//  begin
 ...
 //   end;
      if dm.ADOQuery3.FieldValues['INTERNAL_ECN_COUNT']<>null then//客户ecn信息
       label40.Caption:=dm.ADOQuery3.Fieldbyname('INTERNAL_ECN_COUNT').AsString;
      if dm.ADOQuery3.FieldValues['EXTERNAL_ECN_COUNT']<>null then
       label41.Caption:=dm.ADOQuery3.Fieldbyname('EXTERNAL_ECN_COUNT').AsString;
     ...
     IF (trim(edit5.Text)='') and
      ...
      with dm.ADOQuery2 do
       begin
        ...
        sql.Add('where rkey='+frmcustpart.ADOQuery1prod_route_ptr.AsString);
        active:=true;
...
if edit9.Text = '' then edit9.Text := '0';
 edit9.Text := formatfloat('0.000000',strtofloat(edit9.Text));
end;
...
//tang2013-10-16修改添加订单价格明细------------------
    if (Form3.LabPrice.Tag<>1) then
    begin
      for i := 1 to Form3.StringGrid3.RowCount-2 do
      begin
        if (StrToFloat(Form3.StringGrid3.Cells[1,i])>0) then
        begin
          dm.Ado578.Append;
          dm.Ado578.FieldByName('d579_Ptr').AsString:=Form3.StringGrid3.Cells[2,i];
          dm.Ado578.Fieldvalues['d60_Ptr']:=stringgrid1.RowCount-1;
          dm.Ado578.FieldByName('Price').AsString:=Form3.StringGrid3.Cells[1,i];
          dm.Ado578.Post;
        end;
      end;
    end;
...
//tang2013-10-16添加订单价格明细--
   dm.Ado578.Filter:='d60_Ptr='+inttostr(stringgrid1.Row);//价格明细
   dm.Ado578.First;
   while not dm.Ado578.Eof do
   begin
     Form3.LabPrice.Tag:=0;
     for i :=1 to form3.StringGrid3.RowCount-2 do
     begin
       if (StrToInt(form3.StringGrid3.Cells[2,i])=dm.Ado578.FieldByName('d579_Ptr').AsInteger) then
       begin
         form3.StringGrid3.Cells[1,i]:=FormatFloat('0.0000000',dm.Ado578.FieldValues['Price']);
         break;
       end;
     end;
     dm.Ado578.Next;
...
//end tang2013-10-16--------------
  ...
  if trim(stringgrid1.Cells[1,stringgrid1.Row])<>'' then
   ...
   form3.edit6.Text := stringgrid1.Cells[6,stringgrid1.Row];
  ...
  if form3.ShowModal=mrok then
   begin
    ...
    dm.Ado578.First; //tang2013-10-16添加订单价格明细
    while not dm.Ado578.Eof do dm.Ado578.Delete;
     dm.AQ102.First;
    ...
    begin
      ...
      begin
        ...
        begin
          dm.Ado578.Append;
          dm.Ado578.FieldByName('d579_Ptr').AsString:=Form3.StringGrid3.Cells[2,i];
          dm.Ado578.Fieldvalues['d60_Ptr']:=stringgrid1.Row;
          dm.Ado578.FieldByName('Price').AsString:=Form3.StringGrid3.Cells[1,i];
          dm.Ado578.Post;
        end;
      end;
...
//end tang2013-10-16--------------
  ...
  if trim(form3.edit10.Text)<>'' then
   ...
   stringgrid1.Cells[6,stringgrid1.Row]:=form3.edit6.Text;
...
begin
 ...
 while not dm.AQ62.Eof do dm.AQ62.Delete;
//tang2013-10-16添加订单价格明细--
  dm.Ado578.Filter:='d60_Ptr='+inttostr(stringgrid1.Row);//删除对应的价格明细
  dm.Ado578.First;
  while not dm.Ado578.Eof do dm.Ado578.Delete;
//end tang2013-10-16--------------
...
//tang2013-10-16添加订单价格明细--
  ...
  begin
    dm.Ado578.Edit;
    dm.Ado578.Fieldvalues['d60_Ptr']:=dm.Ado578.FieldValues['d60_Ptr']-1;
    dm.Ado578.Post;
    dm.Ado578.Next;
  end;
//end tang2013-10-16--------------
...
//tang2013-10-16添加订单价格明细---------
        if (Form3.LabPrice.Tag<>1) then
        begin
          for p :=1  to Form3.StringGrid3.RowCount-2 do
          begin
            if (StrToFloat(Form3.StringGrid3.Cells[1,p])>0) then
            begin
              dm.Ado578.Append;
              dm.Ado578.FieldByName('d579_Ptr').AsString:=Form3.StringGrid3.Cells[2,p];
              dm.Ado578.Fieldvalues['d60_Ptr']:=stringgrid1.RowCount-1;
              dm.Ado578.FieldByName('Price').AsString:=Form3.StringGrid3.Cells[1,p];
              dm.Ado578.Post;
            end;
          end;
        end;
...
if (label36.Caption='Y') and (combobox2.ItemIndex=0) and
 ...
 if trim(lab_rkey97.Caption)<>'' then //使用了原有的采购订单号
  begin
   if (self.find_socongfu(strtoint(lab_rkey97.Caption),strtoint(lab_rkey50.Caption),
   stringgrid1.Cells[8,1])) and
   (messagedlg('订单合同号,板号,交期一样有可能重复输入,是否继续?',mtconfirmation,[mbYes,mbNo],0) = mrNo) then
    begin
...
//2017-7-17tang外发状态必须指定外发供应商
  if (radiogroup1.ItemIndex=1) and (rkey23=0) then
  begin
    showmsg('加工形式为外发，必须指定外发供应商！',1);
    exit;
  end;
  if (rkey23>0) and (StrToInt(Trim(Label69.Caption))=0) then
  begin
   ...
   begin
    ...
    for i:=1 to stringgrid1.RowCount-2 do
     begin
      ...
      fieldbyname('ENTERED_BY_EMPL_PTR').AsString := form1.enter_empl_ptr.Caption; //输入人员指针
      fieldbyname('comm_assigned_by_e_p').AsString := form1.enter_empl_ptr.Caption;//拥金指派人员
      ...
      begin
        ...
        fieldbyname('TAX_2').AsString:=Label69.Caption;//委外发入库成本折率%
      end;
      ...
      fieldbyname('P_CODE').AsString := stringgrid1.Cells[6,i];          //优先;
      ...
      if StringGrid1.Cells[25,i]<>'' then //订单预处指针保存并修改213表状态
      begin
       ...
       begin
         Close;
         sql.Clear;
         sql.Add('update data0213 set status=2 where rkey='+stringgrid1.Cells[25,i]);
         ExecSQL;
       end;
      end;
...
//tang2013-10-16添加订单价格明细------------
       dm.Ado578.Filter:='d60_Ptr='+IntToStr(i);
       if dm.Ado578.RecordCount>0 then
       begin
         dm.Ado578.First;
         while not dm.Ado578.Eof do
         begin
           dm.Ado578.Edit;
           dm.Ado578.FieldValues['d60_Ptr']:=fieldvalues['rkey'];
           dm.Ado578.Post;
           if fieldvalues['rkey']=i then dm.Ado578.Next;
         end;
       end;
//end tang2013-10-16------------------------
...
//==============================================================================
 ...
 dm.Ado578.Filter:='';//tang2013-10-16订单价格明细
 try
  dm.AQ62.UpdateBatch();
//  dm.AQ102.UpdateBatch();
  ...
  dm.Ado578.UpdateBatch();//tang2013-10-16订单价格明细
...
begin
 try
  ...
  dm.AQ6011.Filter:='file_pointer = '+inttostr(stringgrid1.row);
  if not dm.AQ6011.IsEmpty then
    edit_note.Memo1.Lines.Add(dm.AQ6011.fieldbyname('MEMO_TEXT').AsString);
  if edit_note.ShowModal=mrok then
   begin
    if not dm.AQ6011.IsEmpty then
     begin
      dm.AQ6011.Edit;
      dm.AQ6011.fieldbyname('MEMO_TEXT').AsString:=edit_note.Memo1.Text;
     end
    else
     if trim(edit_note.Memo1.Text)<>'' then
      begin
       dm.AQ6011.Append;
       dm.AQ6011.FieldValues['file_pointer']:=stringgrid1.Row;
       dm.AQ6011.fieldvalues['source_type']:=60;
       dm.AQ6011.fieldbyname('MEMO_TEXT').AsString:=edit_note.Memo1.Text;
       dm.AQ6011.Post;
      end;
   end;
 ...
 try
  ...
  IF curr_form.showmodal=mrok then
   begin
    ...
    edit9.Text := formatfloat('0.000000',curr_form.ADOQuery1exch_rate.Value);
   end
  else
   edit8.SetFocus;
 finally
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        sql.Add('where cust_part_ptr='+lab_rkey50.Caption);
        SQL.Add(' and  sales_order like '''+Copy(stringgrid1.Cells[0,j],1,Pos('-',stringgrid1.Cells[0,j]))+'%''');
...
if f_supp.ADOQuery1.IsEmpty then
 ...
 if f_supp.ShowModal=mrok then
  begin
   ...
   Label69.Caption:=F_supp.ADOQuery1TAX_2.AsString;
  ...
  begin
   ...
   sql.Text:=
    ...
    'data0015.abbr_name FROM dbo.Data0015'+#13+
     'where WAREHOUSE_CODE='+quotedstr(trim(edit5.Text));
   open;
   if isempty then
    begin
...
begin
 try
  ...
  dm.AQ106011.Filter:='file_pointer = '+inttostr(stringgrid1.row);
  if not dm.AQ106011.IsEmpty then
    edit_note.Memo1.Lines.Add(dm.AQ106011.fieldbyname('MEMO_TEXT').AsString);
  if edit_note.ShowModal=mrok then
   begin
    if not dm.AQ6011.IsEmpty then
     begin
      dm.AQ106011.Edit;
      dm.AQ106011.fieldbyname('MEMO_TEXT').AsString:=edit_note.Memo1.Text;
     end
    else
     if trim(edit_note.Memo1.Text)<>'' then
      begin
...
begin
   ...
   if (Trim(Edit10.Text)='') then
   begin
     ShowMessage('请先确定装运地点或是去设置客户的默认收货地址');
     Exit;
   end;
...
//      SQL.Add('select data0025.*,data0010.CUSTOMER_TYPE from data0025 inner join data0010 on data0025.CUSTOMER_PTR=data0010.rkey where manu_part_number='+quotedstr(form_allbook.ADO213MANU_PART_NUMBER.AsString));
      SQL.text:='select data0213.RKEY, data0213.MANU_PART_NUMBER, data0213.PO_NUMBER, data0213.ANALYSIS_CODE_2, data0213.MANU_PART_DESC, data0213.ORIG_SCHED_SHIP_DATE, data0213.ORIG_REQUEST_DATE, data0213.WAREHOUSE_CODE,'
                     ...
                     +' data0213.CUST_CODE, data0213.orig_customer,data0025.ONHOLD_SALES_FLAG,data0010.CUSTOMER_TYPE,data0025.rkey as Rkey25 '
                     ...
                     +'where data0213.manu_part_number='+quotedstr(form_allbook.ADO213MANU_PART_NUMBER.AsString);
      Open;
     end;
     ...
     if form2.RadioGroup2.ItemIndex = 0 then  //不含税
      begin
       if  (Trim(form2.Edit2.Text)<>'') and (StrToFloat(form2.Edit2.Text)>0)  then
       parts_price:= formatfloat('0.0000000',Form_AllBook.ADO213PART_PRICE.Value/(1+strtofloat(form2.Edit2.Text)*0.01))
       else
       parts_price:= formatfloat('0.0000000',Form_AllBook.ADO213PART_PRICE.Value);
      end
     else
      begin                                           //含税
       parts_price:= formatfloat('0.0000000',Form_AllBook.ADO213PART_PRICE.Value);
     ...
     Form3.MaskEdit1.Text:=Form_AllBook.ADO213ORIG_SCHED_SHIP_DATE.AsString;
     Form3.MaskEdit3.Text:=Form_AllBook.ADO213ORIG_REQUEST_DATE.AsString;
