...
uses mater_rej_search, note,damo, main;
...
//==============================================================================
 with dm.ado95 do
  begin
   close;
   sql.Clear;
   sql.Add('select * from data0095 where rkey is null');
   open;
   append;
   fieldvalues['tran_tp'] := 20;
   fieldvalues['invt_ptr'] := dm.AQ22inventory_ptr.Value;
   fieldvalues['srce_ptr'] := dm.AQ22rkey.Value;
...
//==============================================================================
  with dm.ado9511 do
   begin
    close;
    sql.Clear;
    sql.Add('select * from data0096 where rkey is null');
    open;
    append;
    fieldbyname('rej_ptr').AsString := rkey76.Caption;
    fieldvalues['invt_ptr'] := dm.AQ22inventory_ptr.Value;
    fieldvalues['inv_tran_ptr'] := dm.AQ22rkey.Value;
    fieldvalues['empl_ptr'] := dm.ado95.FieldValues['tran_by'];
    fieldvalues['user_ptr'] := dm.ado95.FieldValues['user_ptr'];
    fieldvalues['tdate'] := dm.ado95.FieldValues['tran_date'];
    fieldbyname('quan_rejd').AsString := edit2.Text;
    fieldvalues['flag'] := '2';//代表是退货给供应商
    fieldvalues['reference_number']:=dm.AQ22BARCODE_ID.Value;
    post;
   end;
//==============================================================================
//在这里就增加一条data0209记录,暂时没有做.
//==============================================================================
  ModalResult := mrok;
end;
