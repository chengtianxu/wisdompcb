...
uses damo, shipmeoth_search, ssadd_search, supplier, edit_misc,
  ...
  edit_misc_po, note, annotation, PR_search, dpment_search;
{$R *.dfm}
 ...
 if (dm.ADO70PO_TYPE.Value = 'S') then  //读取标准采购
  ...
  with dm.ADO72 do   //读取杂项采购
   begin
    ...
    while not eof do
     begin
      v_total:=v_total+dm.ADO72sim_total.Value;
...
if v_new_type=10 then   //如果是编辑状态保存单条记录
 begin
  ...
  dm.ADO70EDIT_BY_EMPL_PTR.Value := form1.v_employee;  //编辑人员
  dm.ADO70EMPLOYEE_POINTER.Value := form1.v_employee; //最后编辑人员也发生改变050125

  if trim(dm.ADO70FOB.Value) = '' then dm.ADO70FOB.AsVariant := null;    //请购编号
  if dm.ADO70STATUS.Value = 5 then self.update_data0078; //如果先前全部已经评审那么再来一次
  if (dm.ADO70STATUS.Value = 3) or (dm.ADO70STATUS.Value = 2) then
   dm.ADO70STATUS.Value:=8;
  ...
  try
   ...
   dm.ADOConnection1.RollbackTrans;
   messagedlg('数据保存失败,请认真检查',mtinformation,[mbcancel],0);
   exit;
  end;
   v_close:=1;   //表示正常退出
   ModalResult:=mrok;
 ...
 if not self.find_po_error() then //如果没有找到错误主要是新增的PO的编号是否重复
  ...
  if v_new_type=9 then //如果是导入数据修改data0068采购请求的状态
   begin                //同时修改请购所对应的预算金额
    dm.asp362.Edit;     //这样预算的使用情况将与采购部门的实际采购为准。
    dm.asp362STATUS.Value := 6;//请购单已处理
    ...
    formatfloat('0.00',dm.asp362.fieldvalues['used_period_'+inttostr(dm.asp362v_month.Value)]+
     (dm.asp362TOTAL.Value-sub_total));
...
begin
 try
  ...
  v_total := dm.ADO71sim_total.Value;
  ...
  if form9.ShowModal=mrok then
   begin
    ...
    dm.ADO71tax_price.Value:=dm.ADO71PRICE.Value;
    edit2.text := formatfloat('0.00',strtofloat(edit2.Text)-v_total+dm.ADO71sim_total.Value);

    edit3.Text := formatfloat('0.00',strtofloat(edit3.Text)-v_tax+dm.ADO71tax_total.Value);

    dbedit7.Field.Value := dbedit7.Field.Value-v_total+dm.ADO71sim_total.Value;
...
begin
 edit2.Text := formatfloat('0.00',strtofloat(edit2.Text)-dm.ADO71sim_total.Value);
 edit3.Text := formatfloat('0.00',strtofloat(edit3.Text)-dm.ADO71tax_total.Value);
 dbedit7.Field.Value := dbedit7.Field.Value-dm.ADO71sim_total.Value;
    //总金额合计
 dm.ADO71.Delete;
end;
...
if (v_new_type=0) or (v_new_type=1) then //如果是新建标准采购或者杂项采购
 ...
 if form_req.ShowModal=mrok then
  begin
//==============================================================================
   if self.find_pr_number() then
    begin//开始...1
     messagedlg('该请购单已经被手工新增了,无法再次生成采购订单!',mtinformation,[mbcancel],0);
     dm.asp362.Active := false;
     dm.asp362.Parameters[1].Value := trim(form_req.ADOQuery1PO_REQ_NUMBER.Value);
     dm.asp362.active := true;
     if not dm.ASP362.IsEmpty then
      begin
       dm.asp362.Edit;     //这样预算的使用情况将与采购部门的实际采购为准。
       dm.asp362STATUS.Value := 6;//请购单已处理
       dm.asp362.Post;
     ...
     while not dm.ADOQuery1.Eof do
      begin
       dm.ADOQuery1.Edit;
       dm.ADOQuery1.FieldValues['PURCHASE_ORDER_TYPE']:=0;
       dm.ADOQuery1.Post;        //林时订单变正式订单
       dm.ADOQuery1.Next;
      end;
     messagedlg('请购单'+trim(form_req.ADOQuery1PO_REQ_NUMBER.Value)+'已被标识为已处理，临时订单已变成正式订单!',mtinformation,[mbok],0);
    end
   else //结束..1
    begin      //开始..2
     tc1.Visible:=true;
...
procedure tform4.add_0071();     //手工新增采购
var
 i:word;
...
for i:=1 to strtoint(form2.edit4.Text) do  //多次放记录建立
 begin
  ...
  dm.ADO71QUAN_ORD.AsString := form2.Edit6.Text;         //采购数量
  dm.ADO71PRICE.Value := strtofloat(form2.Edit13.Text);  //价格(含税)
...
//dm.ADO71avl_flag.Value := form2.po_avlflag.Caption;  //是否通过avl认可
  dm.ADO71tax_price.Value := strtofloat(form2.Edit13.Text); //价格(含税)
  dm.ADO71QUAN_IN_INSP.Value:=dm.ADO71QUAN_ORD.Value;      //请购数量

  dm.ADO71PURCHASE_UNIT_PTR.Value := strtoint(form2.rkey02.Caption);//采购单位
  dm.ADO71WO_PTR.Value := strtoint(form2.req_unitptr.Caption);      //存货单位
  dm.ADO71CONVERSION_FACTOR.Value := strtofloat(form2.Edit15.Text); //转换率
 { if dm.ADO71CONVERSION_FACTOR.Value>0 then
   begin
    dm.ADO71QUAN_IN_INSP.Value:=dm.ADO71QUAN_ORD.Value*dm.ADO71CONVERSION_FACTOR.Value;
    dm.ADO71tax_price.Value:=dm.ADO71PRICE.Value/dm.ADO71CONVERSION_FACTOR.Value;
   end;
  }
  dm.ADO71.Post;

  dm.ADO70SUB_TOTAL.AsString := formatfloat('0.00',
   dm.ADO70SUB_TOTAL.Value+dm.ADO71sim_total.Value);
 ...
 while not dm.ADO71.Eof do
  begin
   v_sim_total :=v_sim_total+dm.ADO71sim_total.Value;
   ...
   if find_supplier(trim(form2.Edit2.Text),strtoint(form2.rkey01.Caption)) then
    begin
     if strtoint(form2.rkey15.Caption)<>dm.ADO70WAREHOUSE_POINTER.Value then
      messagedlg('同一采购定单中,不能有供应商相同而仓库不同的采购!',mtinformation,[mbcancel],0)
     else
      if v_new_type = 0 then
       self.add_0071
      else
...
begin
 with dm.ADO70 do
  begin    //70新增开始
   ...
   dm.ADO70CSI_USER_POINTER.AsString := form1.csi_rkey.caption; //1系统用户
   dm.ADO70EMPLOYEE_POINTER.Value := form1.v_employee;          //2采购人员
...
begin
 for i:=1 to strtoint(form2.edit4.Text) do  //多次放记录建立
  begin
   ...
   dm.ADO72UNIT_PRICE.AsString := form2.Edit13.Text; //含税价格
//   dm.ADO72tax_price.AsString :=form2.Edit13.Text;  //含税价格
   ...
   dm.ADO72.Post;
   dm.ADO70SUB_TOTAL.AsString := formatfloat('0.00',
    dm.ADO70SUB_TOTAL.Value+dm.ADO72sim_total.Value);
  end;
 dm.ADO72.First;
 v_sim_total :=0;
 v_tax_total :=0;
 while not dm.ADO72.Eof do
  begin
   v_sim_total :=v_sim_total+dm.ADO72sim_total.Value;
...
try
 ...
 form2.add_new_type:=1;     //在供应商确定的条件下增加杂项要求
...
begin
 edit2.Text := formatfloat('0.00',strtofloat(edit2.Text)-dm.ADO72sim_total.Value);
 edit3.Text := formatfloat('0.00',strtofloat(edit3.Text)-dm.ADO72tax_total.Value);
 dbedit7.Field.Value := dbedit7.Field.Value-dm.ADO72sim_total.Value;
 dm.ADO72.Delete;
end;
...
try
 ...
 v_total := dm.ADO72sim_total.Value;
 ...
 if form10.ShowModal=mrok then
  begin
   ...
   edit2.text := formatfloat('0.00',strtofloat(edit2.Text)-v_total+dm.ADO72sim_total.Value);
   edit3.Text := formatfloat('0.00',strtofloat(edit3.Text)-v_tax+dm.ADO72tax_total.Value);
   dbedit7.Field.Value := dbedit7.Field.Value-v_total+dm.ADO72sim_total.Value;
   dm.ADO72.Post;
  end;
finally
 form10.free;
...
begin
 ...
 dm.ADO70.First;
//==============================================================================
 while not dm.ADO70.Eof do  //检查新建PO号与旧号是否有重复
  begin
...
begin
 with dm.ADO70 do
  begin                //70新增开始
   ...
   dm.ADO70CSI_USER_POINTER.AsString := form1.csi_rkey.caption;  //1系统用户
   dm.ADO70EMPLOYEE_POINTER.Value := form1.v_employee;  //2员工
   ...
   if (form_req.ADOQuery1FLAG.value = 'S') then
    begin
     ...
     dm.ADO70ANALYSIS_CODE_2.Value:= self.invtory_type;          //物料类别
     tc1.Tabs.Add(trim(dm.ADO0069code.Value));
    end
   else
    begin
     dm.ADO70SUPPLIER_POINTER.Value := dm.ADO0204SUPPLIER_PTR.Value;
   ...
   with dm.ADOQuery1 do//给联络人,电话赋值,付款方法，票据种类，付款地点
    begin
     ...
     sql.Add('CONTACT_NAME_4,CONTACT_NAME_5,CONTACT_NAME_6,');
     sql.Add('discount,discount2,discount_days,discount2_days,payment_terms,');
     sql.Add('PHONE,fax,TAX_ID_NUMBER,ANALYSIS_CODE1,ANALYSIS_CODE2');
     sql.Add('from data0023 where rkey='+dm.ADO70SUPPLIER_POINTER.AsString);
     active :=true;
     dbCombobox1.Clear;
     for i:=1 to 6 do     //联络人
   ...
   with dm.ADOQuery1 do//复制装运地址中的值
    begin
     ...
     dm.ADO70WAREHOUSE_POINTER.value :=form_req.adoquery1WHSE_PTR.Value; //10 仓库
     dm.ADO70STATUS.Value := 8;                            //11  状态未审核
     dm.ADO70PO_DATE.Value := form1.sys_date;               //12  采购定单日期
...
begin
 ...
 dm.ADO71REQ_DATE.Value := dm.ADO0069reply_date.Value;    //需求日期
 dm.ADO71QUAN_ORD.Value := dm.ADO0069QUANTITY.Value;       //采购数量
 dm.ADO71QUAN_IN_INSP.Value:=dm.ADO0069req_quantity.Value; //请购数量
 dm.ADO71PURCHASE_UNIT_PTR.Value := dm.ADO0069UNIT_PTR.Value; //采购单位
 dm.ADO71WO_PTR.Value:= dm.ADO0069req_unit_ptr.Value;         //请购单位
 dm.ADO71PRICE.Value := dm.ADO0069UNIT_PRICE.Value;           //采购价格
 dm.ADO71tax_price.Value := dm.ADO0069tax_price.Value;        //请购价(含税)
 dm.ADO71TAX_2.Value := dm.ADO0069tax.Value;                  //税率

 dm.ADO71DISCOUNT.Value := 0;
...
function tform4.invtory_type():string;
begin
 with dm.ADOQuery2 do
  begin
   close;
   ...
   sql.Add('where data0017.rkey='+dm.ADO0069INVT_PTR.AsString);
   open;
  end;
...
procedure TForm4.update_data0078;
begin
 with dm.asp78 do
  begin
   close;
