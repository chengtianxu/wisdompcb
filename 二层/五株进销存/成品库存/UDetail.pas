...
var
 rkey:integer;
begin
        rkey:=dm.ADOproduct_stockrkey.Value;
        if dm.ADOproduct_stock.RecordCount > 0 then
        begin
            dm.ADOproduct_stock.Close;
            dm.ADOproduct_stock.Open;
            dm.ADOproduct_stock.Locate('rkey',rkey,[]);
        end;
end;

procedure TDetail_Form.BitBtn4Click(Sender: TObject);
begin
    if not dm.ADOproduct_stock.IsEmpty then
...
begin
      ...
      begin        // if DBGridEh1.DataSource.DataSet.FieldByName(Column.FieldName).FieldKind = fkCalculated then  exit ;
                 if (column.Title.SortMarker =smDownEh) or (column.Title.SortMarker =smNoneEh) then
                begin
                   column.Title.SortMarker:=smUpEh;
                   dm.ADOproduct_stock.Sort:=Column.FieldName;
                end
                else
                begin
                   column.Title.SortMarker:=smDownEh;
                    dm.ADOproduct_stock.Sort:=Column.FieldName+' DESC';
                end;
...
begin
  ...
  begin
      DM.ADOproduct_stock.Close ;
      DM.ADOproduct_stock.SQL.Clear;
      DM.ADOproduct_stock.SQL.Text := sdate;
      //ShowMessage(DM.ADOproduct_stock.SQL.Text);
      DM.ADOproduct_stock.Open;
      DM.ADOproduct_stock.Active :=True;
...
begin
       if trim(Edit1.text)<>'' then
        dm.ADOproduct_stock.Filter := PreColumn.FieldName+' like ''%'+trim(edit1.text)+'%'''
        else dm.ADOproduct_stock.filter := '';
end;
