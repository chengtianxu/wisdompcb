...
begin
     ...
     if DM.ADOConnection1.Connected then
      begin
           ...
           DM.ADOTmp1.sql.Text:= ' select distinct stock_date,employee_name from invt_stock ORDER BY stock_date'+#13;
           DM.ADOTmp1.Close ;
           DM.ADOTmp1.Open;
              ...
              try
               dm.ADOConnection1.BeginTrans;
                ...
                begin
                        Close;
                        sql.Clear;
                        sql.Text:= ' CREATE TABLE #invt_stock_tmp(   '+#13+
                                   ' [rkey] [int] IDENTITY(1,1) NOT NULL, '+#13+
                                   ' [INV_PART_NUMBER] [varchar](25) NULL, [INV_NAME] [varchar](100) NULL,[INV_DESCRIPTION] [varchar](200) NULL,	[GROUP_DESC] [varchar](70) NULL,'+#13+
                                   ' [UNIT_NAME] [varchar](20) NULL,'+#13+
                                   ' [quantity] [decimal](10, 2) NULL,'+#13+
                                   ' [LOCATION] [varchar](20) NULL, '+#13+
                                   ' [stock_area] [float] NULL, '+#13+
                                   ' [stock_weight] [nchar](10) NULL, '+#13+
        ...
        try
             ...
             begin
                ...
                begin
                        Close;
                        sql.Clear;
                        sql.Text:= ' select INV_PART_NUMBER '+#13+
                                   '  from #invt_stock_tmp '+#13+
                                                '  where INV_PART_NUMBER in (select   INV_PART_NUMBER  from   #invt_stock_tmp '+#13+
                                                '  group  by   INV_PART_NUMBER  having  count(INV_PART_NUMBER) > 1) '+#13;
                        Close;
                        Open;
                        if not DM.ADOTmp1.IsEmpty then
                        begin
                                   ShowMessage('您导入的数据中可能存在材料编码重复，请修改后重新导入');
                                    try
                                     dm.ADOConnection1.BeginTrans;

                                              Close;
                                              sql.Clear;
                                              sql.Text:= ' drop table #invt_stock_tmp';
                                              ExecSQL;
                                       dm.ADOConnection1.CommitTrans;
                                     except
                                       on E: Exception do
                                ...
                                Close;
                                sql.Clear;
                                sql.Text:= ' select count(distinct stock_date)as num from  #invt_stock_tmp '+#13;
                                Close;
                                Open;
                            ...
                            begin
                                   ShowMessage('您导入的数据中日期有误，请修改后重新导入');
                                    ...
                                    try
                                     dm.ADOConnection1.BeginTrans;
                                      Close;
                                      sql.Clear;
                                      sql.Text:=  ' INSERT INTO invt_stock(INV_PART_NUMBER, INV_NAME, INV_DESCRIPTION, GROUP_DESC, '+#13+
                                                  ' UNIT_NAME, quantity, LOCATION, stock_area, stock_weight, stock_date, employee_name)'+#13+
                                                  '(SELECT  INV_PART_NUMBER, INV_NAME, INV_DESCRIPTION, GROUP_DESC, UNIT_NAME, quantity, '+#13+
                                                  ' LOCATION, stock_area, stock_weight, stock_date, employee_name FROM #invt_stock_tmp)'+#13;
                                      ExecSQL;
                                      dm.ADOConnection1.CommitTrans;
                                     except
                                       on E: Exception do
                                ...
                                Close;
                                sql.Clear;
                                sql.Text:= ' select stock_date,employee_name from invt_stock'+#13;
                                Close;
                                Open;
                                sgrid1.Cells[0,1]:='1(期初)';
...
begin
    try
        ...
        Detail_Form.sdate:=' SELECT     rkey, INV_PART_NUMBER, INV_NAME, INV_DESCRIPTION, GROUP_DESC,'+#13+
                                          ' UNIT_NAME, quantity, LOCATION, stock_area, stock_weight, stock_date,employee_name '+#13+
                                          ' FROM   invt_stock  '+#13+
                      ...
                      begin
                           ...
                           begin
                                  try
                                        ...
                                        DM.ADOTmp1.Close;
                                        DM.ADOTmp1.sql.Clear;
                                        DM.ADOTmp1.sql.Text:= ' DELETE from invt_stock WHERE  ( stock_date =  CONVERT(DATETIME,'+#39+SGrid1.Cells[1,SGrid1.Row]+#39+', 102))'+#13;
                                        DM.ADOTmp1.ExecSQL;
                                        dm.ADOConnection1.CommitTrans;
                                  except
                                     on E: Exception do
