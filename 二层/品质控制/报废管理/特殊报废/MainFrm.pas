...
begin
  ...
  FillZD;
  FWhere := ' WHERE D58.if_stock = 1 and D58.TDATETIME >=getdate() AND D58.TDATETIME <= dateadd(day,1,getdate()) ';
...
begin
  ...
  try
    ...
    begin
      FWhere := 'WHERE D58.if_stock = 1 and D58.TDATETIME BETWEEN ' + QuotedStr(gDateTimeToStr(DateOf(LFrm.dtp1.Date) + TimeOf(LFrm.dtp2.Time)))
...
procedure TfrmMain.ehFrmMainKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
 if (ssAlt in Shift)  and  (Key=86) then
  ShowMessage(dm1.adsFrmMain.CommandText);
end;
