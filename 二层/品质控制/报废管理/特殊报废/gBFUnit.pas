...
//      ShowMessage(LSql);
    ...
    begin
      ...
      //新增58记录
      LErrMsg := 1;
      lsql := 'insert into data0058(if_stock,MRB_NO,WO_PTR,DEPT_PTR,REJECT_PTR,QTY_ORG_REJ,QTY_REJECT,PANELS,STEP,EMPL_PTR,TDATETIME,AUDIT_EMPL_PTR,AUDIT_DATETIME,RESP_DEPT_PTR,REFERENCE,StepNow,warehouse_ptr,FLOW_NO,BOM_PTR,GlobalId,PANEL_ID,coordinate)' +
              ' values(1,' + QuotedStr(LMRB_NO) + ',' + A06RKey + ',' + IntToStr(LSQ.D34Rkey) + ',' +
              AQXDMRKey + ',' + ABFPCSNum + ',' + ABFPCSNum + ',' + ABFPnlNum + ',' + IntToStr(LZR.MIStep) + ',' +
              gUser.User_Ptr + ',getdate(),' + gUser.User_Ptr + ',getdate(),' + IntToStr(LZR.D34Rkey) + ',' +
              QuotedStr(ABakNote) + ',' + IntToStr(LSQ.MIStep) + ',' + ABF15RKey + ',' +
