...
interface
uses
  Windows, ADODB, SysUtils, Classes,DBClient;

var
  gCon: TADOConnection;

//增加新的报废
function NewBF(A06RKey, AZR_34RKey,AZR_Step, ABFPCSNum, ABFPnlNum, AQXDMRKey, ABF15RKey, ABakNote: string;AType:Integer;PANEL_ID: Integer = 0 ;GlobalId: string = '';coordinate: string = '';cds:TclientDataset = nil): Boolean;
//MRB_鉴定报废
  ...
  try
    ...
    begin
      ...
      begin
        ...
        LStepNO:= LQry.FieldByName('WTYPE56').AsString;
    ...
    if AType = 1 then
    begin
      if LStepNO = '' then LStepNO:= '0';     //由于可能不是在线作业单，把返工步骤设置为0
      if LSQ.D34Rkey = 0 then LSQ.D34Rkey:= LZR.D34Rkey;    //由于可能不是在线作业单，不存在申请部门，把责任部门设置成申请部门
...
//      ShowMessage(LSql);
    ...
    begin
      ...
      begin
        ...
        begin
//          ShowMessage(cds.FieldValues['qty'].asstring);
          if (not cds.FieldByName('qty').IsNull) and (cds.FieldByName('qty').AsInteger <> 0) then
          begin

            LSql := 'update data0058IN set used_qty = isnull(used_qty,0) + ' + IntToStr(cds.FieldByName('qty').AsInteger) +
                  ' where rkey = '+ IntToStr(cds.FieldByName('rkey').AsInteger);
            if not SqlExec(LSql) then Exit;
          end;
          cds.Next;
        end;
