...
var
  i:integer;
begin
  if not DM.ADO468.IsEmpty then  i := DM.ADO468rkey.AsInteger ;
  DM.ADO468.Close;
  DM.ADO468.Open;
...
begin
  ...
  FsSearch:=' WHERE (dbo.Data0268.STATUS = 2) AND (data0468.status=0) and (dbo.Data0468.QUAN_ISSUED < dbo.Data0468.QUAN_BOM) AND (dbo.Data0468.invent_or = 0) ';
  DM.ADO468.SQL.Append(FsSearch+' and data0268.DATE>=getdate()-7 ');
  DM.ADO468.Open;
...
begin
  ...
  begin
    ...
    Sqlstr:=FsSearch+' and data0268.date>='''+formatdatetime('YYYY/MM/DD',Frm_QrySet.DateTimePicker1.Date)+
...
begin
  ...
  1: with TFrm_detail.Create(nil) do
   ...
   2: begin
        ...
        try
          Qty:=DM.ADO468quan_alloc.asfloat;
          DM.ADOCon.BeginTrans;
          DM.tmp.Close;
          DM.tmp.SQL.Text:='update data0022 set QUAN_TO_BE_STOCKED=QUAN_TO_BE_STOCKED-B.quan_alloc '+
