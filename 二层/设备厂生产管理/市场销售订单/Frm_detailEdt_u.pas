...
type
  ...
  private
    ...
    Ftax:real;
  public
    class function InitEdt(iDept:integer;Grd:TStringGrid;tax:real):boolean;
  end;
...
class function TFrm_detailEdt.InitEdt(iDept:integer;Grd:TStringGrid;tax:real): boolean;
var i:integer;
    s:string;
begin
  ...
  begin
    s:='';
    for i:=1 to Grd.RowCount-2 do s:=s+Grd.Cells[7,i]+',';   //rkey08 唯一
    s:=s+'0';
    DM.ADO08.Filter:='';
    DM.ADO08.Close;
    DM.ADO08.SQL.Text:='SELECT dbo.Data0008.PROD_CODE, dbo.Data0008.PRODUCT_NAME, dbo.Data0008.PRODUCT_DESC,'+
                       ...
                       'where Data0034.RKEY='+inttostr(iDept)+' and not dbo.Data0008.RKEY in ('+s+') '+
...
begin
  ...
  try
    ...
    begin
      ...
      FGrd.Cells[4,i]:=Edit7.Text;
      FGrd.Cells[6,i]:=Edit8.Text;
      FGrd.Cells[5,i]:=datetostr(DateTimePicker1.date);
      FGrd.Cells[7,i]:=DM.ADO08.Fieldbyname('rkey').AsString;
      FGrd.RowCount:=FGrd.RowCount+1;
      DM.ADO71_1.Append;
      DM.ADO71_1.FieldByName('prod_PTR').AsString:=FGrd.Cells[7,i];
