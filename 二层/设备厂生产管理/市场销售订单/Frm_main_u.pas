...
implementation
 uses common,DM_u,Frm_QrySet_u,Frm_detail_u,UpdatePrice, editPo_noFrm;
{$R *.dfm}
...
begin
  ...
  FsSearch:=' where wzcp0070.type=1 and wzcp0070.sales_date>getdate()-14 ';
  CheckBox1Click(nil);
  FactGrd:=Grd1;
end;
...
procedure TFrm_main.Grd1DrawColumnCell(Sender: TObject; const Rect: TRect;
  DataCol: Integer; Column: TColumnEh; State: TGridDrawState);
begin
{                    case when wzcp0070.status=1 then '未提交'
                     when wzcp0070.status=2 then '待确认'
                     when wzcp0070.status=3 then '已确认'
                     when wzcp0070.status=4 then '已投产'
                     when wzcp0070.status=5 then '已完成'
                     when wzcp0070.status=6 then '被退回'  end status_c}
...
begin
  ...
  begin
    ...
    FsSearch:=' where wzcp0070.type=1 and wzcp0070.sales_date>='''+formatdatetime('YYYY/MM/DD',Frm_QrySet.DateTimePicker1.Date)+
...
begin
  ...
  begin
    Close;
    SQL.Clear;
    SQL.Add('SELECT WZCP0060.status FROM WZCP0063 INNER JOIN WZCP0060 ON WZCP0063.so_ptr = WZCP0060.rkey');
    SQL.Add('WHERE (dbo.WZCP0060.status = 4) and wzcp0063.cp71_ptr='+dm.ADO71rkey.AsString);
    Open;
  end;
...
begin
  ...
  begin
    Close;
    SQL.Clear;
    SQL.text := 'select wzcp0060.rkey from wzcp0060 inner join wzcp0070 on wzcp0060.SALES_REP_PTR =  wzcp0070.rkey '
      + ' where wzcp0070.rkey = ' + dm.ADO70Rkey.AsString;
    Open;
  end;
  if DM.tmp.RecordCount = 0  then
  begin
    with TfrmeditPo_no.Create(nil) do
    try
      if ShowModal = mrOK then
      begin
        btbrush.Click;
