...
uses
  ...
  Dialogs, StdCtrls, Mask, Grids, Buttons, ExtCtrls,
...
implementation
  uses DM_u,Pick_Item_Single,ConstVar,Frm_detailEdt_u,Frm_Auth_u,Frm_Prod;
...
begin
 ...
 // SGrid.Cells[6,0]:='可用库存';
 // SGrid.ColWidths[6]:=70;
 SGrid.Cells[6,0]:='可用库存';
  SGrid.ColWidths[6]:=70;
  ...
  DM.tmp.Open;
  i:=0;
  SetLength(Ffac,DM.tmp.RecordCount);
  ...
  begin
    ...
    EdtSale.Tag := StrToInt(DM.ADO268.FieldByName('user_name').AsString);
    EdtSale.Text := DM.ADO268.FieldByName('ucode').AsString;
    LBSale.Caption := DM.ADO268.FieldByName('uname').AsString;
    ...
    begin
      ...
      SGrid.Cells[6,j] := DM.ADO468.FieldByName('QUAN_ON_HAND').AsString;// 当前库存
...
begin
  ...
  DM.ADO268_1.FieldByName('user_name') .AsInteger := EdtSale.Tag;
...
begin
 try
  ...
  begin
    if EdtDept.tag<>frmPick_Item_Single.adsPick.Fieldbyname('rkey').AsInteger then
    begin
...
begin
   ...
   begin
    if EdtDept.tag<>DM.tmp.Fieldbyname('rkey').AsInteger then
    begin
...
begin
 // SetLength(Ffac,CBFac.ItemIndex);
...
procedure TFrm_detail.SGridKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  {if StrToFloat(sGrid.Cells[2,sGrid.row]) > StrToFloat(sGrid.Cells[6,sGrid.row]) then
    sGrid.Cells[2,sGrid.row] := sGrid.Cells[6,sGrid.row] ;  }
...
begin
 frmPick_Item_Single:=TfrmPick_Item_Single.Create(nil);
  ...
  case Tcomponent(Sender).Tag of
    0:
    begin
     InputVar.Fields := 'EMPL_CODE/申领员代码/200,employee_name/申领员名称/250';
      InputVar.Sqlstr :='SELECT RKEY, EMPL_CODE, employee_name FROM dbo.Data0005 '+
   ...
   if (Fsale <> EdtSale.Text) and (EdtSale.Text <> '') then
     begin
       ...
       if DM.tmp.IsEmpty then
         ...
         begin
           EdtSale.Text := DM.tmp.FieldValues['dept_code'];
           LBSale.Caption := DM.tmp.FieldValues['dept_name'];
