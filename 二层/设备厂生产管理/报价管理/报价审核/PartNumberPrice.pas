...
begin
  ...
  try
    InputVar.Fields := 'inv_part_number/材料编码/120,inv_part_description/材料描述/120,QUAN_ON_HAND/当前库存/120,STD_COST/标准成本/120,';
    InputVar.Sqlstr :='select rkey, inv_part_number, inv_part_description,STD_COST, QUAN_ON_HAND from data0017 ';   //where data0492.tstatus <> 4
    InputVar.AdoConn := DM.ADOConnection1;
    frmPick_Item_Single.InitForm_New(InputVar) ;
    if frmPick_Item_Single.ShowModal=mrok then
    begin
      StringGrid1.Cells[1, StringGrid1.Row] := frmPick_Item_Single.adsPick.FieldValues['inv_part_number'];
      StringGrid1.Cells[3, StringGrid1.Row] := frmPick_Item_Single.adsPick.FieldValues['STD_COST'];
      StringGrid1.Cells[6, StringGrid1.Row] := frmPick_Item_Single.adsPick.FieldValues['rkey'];
...
var
  i, j: Integer;
begin
  if Trim(FrmNumberPrice.StringGrid1.Cells[3,1]) = '' then Exit;
  DM.inventTotalPrice := 0; //对应某个材料类别合计总价

  for i := 0 to StringGrid1.RowCount - 2 do
  begin
    DM.inventTotalPrice := DM.inventTotalPrice + ( strtofloat(StringGrid1.Cells[2, i+1]) * strtofloat(StringGrid1.Cells[3,i+1]) );
  end;
  ...
  begin
    ...
    DM.qry87_1QUAN_BOM.Value := StrToFloat(StringGrid1.Cells[2, i+1]);
    DM.qry87_1STD_COST.Value := StrToFloat(StringGrid1.Cells[3, i+1]);
    if Trim(StringGrid1.Cells[4, i+1]) <> '' then DM.qry87_1VENDOR.Value := StringGrid1.Cells[4, i+1];
    DM.qry87_1INVENT_PTR.Value := StrToInt(StringGrid1.Cells[6, i+1]);
    DM.qry87_1.Post;
  end;
  ModalResult := mrOk;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        StringGrid1.Cells[2, i+1] := FloatToStr(DM.qry87_1.FieldValues['quan_bom']);
        StringGrid1.Cells[3, i+1] := FloatToStr(DM.qry87_1.FieldValues['std_cost']);
        if not dm.qry87_1.FieldByName('vendor').IsNull then
          StringGrid1.Cells[4, i+1] := DM.qry87_1.FieldValues['vendor'];
        StringGrid1.Cells[6, i+1] := IntToStr(DM.qry87_1.FieldValues['invent_ptr']);
  ...
  begin
    ...
    begin
      ...
      begin
        Close;
        SQL.Clear;
        SQL.Text := 'select wzcp0087.*, data0017.rkey rkey17, data0017.inv_part_number from wzcp0087 inner join data0017 on '
