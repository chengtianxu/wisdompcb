...
type
  ...
  private
    ...
    function GetNumber(numbers: Double): string;
  public
    { Public declarations }
    nowdate: string;
  end;
...
begin
  with DM.qrytemp do  //获取最新报价单号及服务器时间
  begin
    Close;
    SQL.Clear;
    ...
    Open;
    edt1.Text := FieldValues['seed_value'];  //报价单号
    nowdate := FieldValues['sysdate'];
    DateTimePicker1.Date := StrToDate(FieldValues['sysdate']);  //报价日期
    DateTimePicker2.Date := StrToDate(FieldValues['sysdate1']); //有效日期
  end;
...
begin
  ...
  try
    ...
    begin
      Close;
      SQL.Clear;
      SQL.Text := 'select WORKHOUSE from data0008 where rkey= ' + IntToStr(BitBtn2.Tag);
      Open;
      if not IsEmpty then
        Edt15.Text := FieldValues['WORKHOUSE'];
    end
  finally
    frmPick_Item_Single.adsPick.Close;
    frmPick_Item_Single.Free ;
...
begin
  ...
  begin
    ...
    edt23.Text := FloatToStr(strtofloat(edt15.Text) * strtofloat(edt17.Text) * strtofloat(edt14.Text))  ;
    edt24.Text := FloatToStr(strtofloat(edt15.Text) * strtofloat(edt25.Text) * strtofloat(edt14.Text))  ;
    edit6.Text := FloatToStr(strtofloat(edt14.Text) * strtofloat(edt19.Text))  ;
    ...
    Edit10.Text := FloatToStr((strtofloat(edt20.Text) + strtofloat(edt21.Text) + strtofloat(edt22.Text)
      + strtofloat(edt15.Text) * strtofloat(edt17.Text)
      + strtofloat(edt15.Text) * strtofloat(edt25.Text) + strtofloat(edt19.Text) + strtofloat(edt26.Text))) ;
  end;
end;
...
begin
  if edt16.Text <> '' then
  begin
    Edit5.text := FloatToStr(strtofloat(Edit10.text) * (1 + strtofloat(edt16.text)/100));
    Edit12.Text := FloatToStr( strtofloat(Edit5.text) * strtofloat(edt14.text));
    //ShowMessage(FloatToStr(strtofloat(Edit8.text) * (1 + strtofloat(edt16.text)/100)))
  end;
end;
...
begin
  ...
  begin
    Edit9.text := GetNumber(StrToFloat(FloatToStr(strtofloat(Edit5.text) * (1 + strtofloat(edt18.text)/100))));
    Edit14.Text := FloatToStr( strtofloat(Edit9.text) * strtofloat(edt14.text));
  end;
end;
...
begin
  ...
  begin
    ...
    Edit17.Text := FloatToStr( strtofloat(Edit15.text) - strtofloat(Edit14.text));
  end;
end;
...
begin
  ...
  if StrToFloat(Trim(edt15.Text)) <= 0 then
  begin
   ShowMessage('工时不能为0！');
   PageControl1.ActivePage := ts2;
   edt15.SetFocus();
  ...
  if StrToFloat(Edit9.Text)<100 then
    quote_taxprice:=StrToFloat(GetNumber(StrToFloat(Edit9.Text)));
  if (StrToFloat(Edit9.Text)>99) and (StrToFloat(Edit9.Text)<1000) then
    quote_taxprice:=StrToFloat(GetNumber(StrToFloat(Edit9.Text)));
  if (StrToFloat(Edit9.Text)>999) and (StrToFloat(Edit9.Text)<10000) then
    quote_taxprice:=StrToFloat(GetNumber(StrToFloat(Edit9.Text)));
  if (StrToFloat(Edit9.Text)>9999) and (StrToFloat(Edit9.Text)<100000) then
    quote_taxprice:=StrToFloat(GetNumber(StrToFloat(Edit9.Text)));
  if StrToFloat(Edit9.Text)>99999 then
    quote_taxprice:=StrToFloat(GetNumber(StrToFloat(Edit9.Text)));
      ...
      begin
        ...
        SQL.Text := Format('insert into wzcp0085([TNUMBER],[customer_ptr],[CUST_PART_PTR],[CUST_CONTACT] '
          ...
          [edt1.text, BitBtn1.Tag, bitbtn2.tag, edt11.Text,
           ...
           StrToFloat(edt7.Text), StrToFloat(edt18.Text), edt6.Tag, StrToFloat(edt14.Text), StrToFloat(Edt20.Text),
           StrToFloat(edt17.Text) * StrToFloat(edt15.Text), StrToFloat(edt25.Text) * StrToFloat(edt15.Text),
           StrToFloat(Edt21.Text), StrToFloat(edt15.Text),
           StrToFloat(edt19.Text), StrToFloat(Edt26.Text), StrToFloat(Edt22.Text), StrToFloat(Edit10.Text),
           StrToFloat(edt16.Text), StrToFloat(Edit5.Text), quote_taxprice , StrToFloat(edt30.Text),
           StrToFloat(edt31.Text), StrToFloat(edt17.Text), StrToFloat(edt25.Text)]) ;
        ExecSQL;
  ...
  begin
    ...
    try
      ...
      //  if StrToDate(nowdate) > DateTimePicker2.Date then FieldByName('audited_status').AsInteger := 2;            //状态
        ...
        FieldByName('MATL_COST').AsFloat := StrToFloat(Edt20.Text);               //材料总成本
        FieldByName('manpower_cost').AsFloat := StrToFloat(edt17.Text)*StrToFloat(edt15.Text);           //人工总成本
        FieldByName('ovhd_cost').AsFloat := StrToFloat(edt25.Text)*StrToFloat(edt15.Text);               //制造总费用
        FieldByName('outward_cost').AsFloat := StrToFloat(edt21.Text);            //外发总成本
        FieldByName('manpower').AsFloat := StrToFloat(edt15.Text);                //工时
        FieldByName('research_cost').AsFloat := StrToFloat(edt19.Text);           //研发总成本
        FieldByName('software_cost').AsFloat := StrToFloat(edt26.Text);           //软件总成本
        FieldByName('manage_cost').AsFloat :=  StrToFloat(edt22.Text);            //管理总费用
        FieldByName('total_cost').AsFloat :=  StrToFloat(Edit10.Text);             //合计总成本
        ...
        FieldByName('ovhd_price').AsFloat := StrToFloat(edt25.Text);              //制造费用每小时价格
        UpdateBatch(arAll);
        DM.ADOConnection1.CommitTrans;
      end;
...
function TFrmAdd.GetNumber(numbers: Double): string;
var
  resultNumber: string;
begin
  numbers := trunc(numbers);
  if Length(FloatToStr(numbers)) < 3 then
    resultNumber := FloatToStr(numbers);
  if Length(FloatToStr(numbers)) = 3 then
    resultNumber := Copy(FloatToStr(numbers), 0 , Length(FloatToStr(numbers)) - 1 ) + '0';
  if Length(FloatToStr(numbers)) = 4 then
    resultNumber := Copy(FloatToStr(numbers), 0 , Length(FloatToStr(numbers)) - 2 ) + '00';
  if Length(FloatToStr(numbers)) = 5 then
    resultNumber := Copy(FloatToStr(numbers), 0 , Length(FloatToStr(numbers)) - 3 ) + '000';
  if Length(FloatToStr(numbers)) > 5 then
    resultNumber := Copy(FloatToStr(numbers), 0 , Length(FloatToStr(numbers)) - 4 ) + '0000';
  Result := resultNumber;
end;
...
begin
  ...
  begin
    StringGrid1.Cells[4, StringGrid1.Row] := FloatToStr(DM.inventTotalPrice);
  end;
  FrmNumberPrice.Free;
end;
...
begin
  ...
  begin
    ...
    edt20.Text := DM.qryCp85.FieldValues['matl_cost'];
    Edit1.Text := FloatToStr(DM.qryCp85.FieldValues['matl_cost']/DM.qryCp85.FieldValues['quantity']);

    edt21.Text := DM.qryCp85.FieldValues['outward_cost'];
    Edit2.Text := FloatToStr(DM.qryCp85.FieldValues['outward_cost']/DM.qryCp85.FieldValues['quantity']);

    edt22.Text := DM.qryCp85.FieldValues['manage_cost'];
    Edit3.Text := FloatToStr(DM.qryCp85.FieldValues['manage_cost']/DM.qryCp85.FieldValues['quantity']);

    edt17.Text := DM.qryCp85.FieldValues['manpower_price'];
    edt23.Text := FloatToStr(DM.qryCp85.FieldValues['manpower_cost']);

    edt25.Text := DM.qryCp85.FieldValues['ovhd_price'];
    Edit6.Text := FloatToStr(DM.qryCp85.FieldValues['ovhd_cost']);

    edt19.Text := DM.qryCp85.FieldValues['research_cost'];
    edt24.Text := FloatToStr(DM.qryCp85.FieldValues['research_cost']/DM.qryCp85.FieldValues['quantity']);

    edt26.Text := DM.qryCp85.FieldValues['software_cost'];
    Edit7.Text := FloatToStr(DM.qryCp85.FieldValues['software_cost']/DM.qryCp85.FieldValues['quantity']);
    ...
    Edit12.Text := FloatToStr(StrToFloat(Edit5.text) * StrToFloat(edt14.Text) );
    Edit14.Text := FloatToStr(StrToFloat(Edit9.text) * StrToFloat(edt14.Text) );
    Edit15.Text := FloatToStr(StrToFloat(edt30.text) * StrToFloat(edt14.Text) );
    Edit17.Text := FloatToStr(StrToFloat(Edit15.text) - StrToFloat(edit14.Text) );
...
procedure TFrmAdd.edt22Change(Sender: TObject);
begin
  Edit15.Text := FloatToStr(strtofloat(edt30.Text) * strtofloat(edt14.Text));
  BitBtn4.Click;
end;
...
begin
  common.Export_Grid_to_Excel(StringGrid1, '材料大类价格');
end;

procedure TFrmAdd.EXCEL1Click(Sender: TObject);
begin
  if DBGrid1.DataSource.DataSet.RecordCount>0 then
    Export_dbGrid_to_Excel(DBGrid1,'材料明细类型');
end;
